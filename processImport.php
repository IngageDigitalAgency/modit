...
class importCheck extends Common {
    ...
    function checkCountry($name) {
		return $this->fetchScalarTest('select id from countries where (country = "%s" or country_code = "%1$s") and deleted = 0', $name);
	}
    ...
    function checkProvince($c_id, $name) {
		return $this->fetchScalarTest('select id from provinces where country_id = %d and (province = "%2$s" or province_code = "%2$s")', $c_id, $name);
	}
    ...
    function buildAddress($type, $row, $v, $flds, &$valid) {
		...
		if ($flds[$type."_address_id"]["mapTo"] != "" && $v[$flds[$type."_address_id"]["mapTo"]] > 0) {
			if (!($addr = $this->fetchSingleTest("select * from addresses where id = %d and ownertype = 'member' and ownerid = %d", $v[$flds[$type."_address_id"]["mapTo"]], $this->getUserInfo("id")))) {
				$valid = false;
			...
			if (!$c_id = $this->checkCountry($this->getFieldData($flds[$type."_country"],$v))) {
				...
				$addr["country_code"] = $this->fetchScalarTest("select country_code from countries where id = %d", $c_id);
				...
				if (!$p_id = $this->checkProvince($c_id, $this->getFieldData($flds[$type."_province"],$v))) {
					...
					$addr["province_code"] = $this->fetchScalarTest("select province_code from provinces where id = %d", $p_id);
...
if (!($valid = $form->validate())) {
    ...
    function getWeightCode($row, $data, $flds, &$valid ) {
		...
		if (!$c_id = $this->fetchScalarTest("select id from code_lookups where type='weights' and code = '%s'", $cd)) {
			$valid = false;
    ...
    function getDimensionCode($row, $data, $flds, &$valid ) {
		...
		if (!$c_id = $this->fetchScalarTest("select id from code_lookups where type='dimensions' and code = '%s'", $cd)) {
			$valid = false;
    ...
    function getPackaging($row, $data, $flds, &$valid ) {
		if (!$grp = $this->fetchSingleTest("select p.* 
    ...
    function getServiceType($row, $data, $flds, &$valid ) {
		$junction = $this->fetchSingleTest("select * from members_by_folder where member_id = %d limit 1", $this->getUserInfo("id"));
		if ($mbr_prod = $this->fetchSingleTest("select cpo.* from product p, custom_member_product_options cpo where cpo.member_id = %d and cpo.isgroup = 0 and p.id = cpo.product_id and p.name = '%s'", $junction["id"], $data[$flds["service"]["mapTo"]])) {
			$type = $mbr_prod["id"];
		}
		else if ($grp_prod = $this->fetchSingleTest("select cpo.* from product p, custom_member_product_options cpo where cpo.member_id = %d and cpo.isgroup = 1 and p.id = cpo.product_id and p.name = '%s'", $junction["folder_id"], $data[$flds["service"]["mapTo"]])) {
			$type = $grp_prod["id"];
		}
		else {
			$valid = false;
    ...
    function doImport() {
		...
		foreach($data as $k=>$v) {
			if ($k==1) continue 1;
			if ($chk = $this->fetchScalarTest("select abort from order_import where id = %d", $this->getImportId())) {
			...
			if (strlen($pu_driver) > 0) {
				if ($pu_id = $this->fetchScalarTest("select d.id from drivers d, members m where d.deleted = 0 and  d.enabled = 1 and d.member_id = m.id and m.enabled and m.deleted = false and m.company = '%s'", $pu_driver)) {
			...
			if (strlen($del_driver) > 0) {
				if ($del_id = $this->fetchScalarTest("select d.id from drivers d, members m where d.deleted = 0 and  d.enabled = 1 and d.member_id = m.id and m.enabled and m.deleted = false and m.company = '%s'", $del_driver)) {
			...
			if ($valid) {
				...
				$prod = $this->fetchSingleTest("select p.* from product p, custom_member_product_options cpo where cpo.id = %d and p.id = cpo.product_id", $_POST["serviceType"]);
				if (!(is_array($fromZone) && is_array($toZone))) {
					...
					if (!$prod["is_fedex"]) {
						...
						$_POST["wt"] = $this->fetchSingleTest("select * from code_lookups where id = %d",$_POST["custom_weight_code"]);
						$_POST["sz"] = $this->fetchSingleTest("select * from code_lookups where id = %d",$_POST["custom_dimension_code"]);
...
$recs = $chk->fetchAllTest("select * from order_import where processed = 0");
foreach($recs as $k=>$v) {
