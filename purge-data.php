...
class Purge extends Common {
    ...
    public function doIt($dt) {
		...
		$orders = array_merge(array(0),$this->fetchScalarAllTest("select id from orders where order_date < '%s' and id not in (%s) and (order_status & %d) = 0 and (order_status & %d or order_status & %d)", $dt, implode(", ", $recurring), STATUS_RECURRING, STATUS_SHIPPED, STATUS_CANCELLED));
