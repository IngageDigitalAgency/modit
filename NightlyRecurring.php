...
*/
    ...
    function processRecurring() {
		...
		$su = $c->fetchSingleTest("select * from members where custom_super_user = 1 and enabled = 1 and deleted = 0");
		$c->logMeIn($su["username"], $su["password"], 0, $su["id"]);
		$_SESSION["mgmt"]["user"] = $_SESSION["user"]["info"];	// set up super user 
		unset($_SESSION["cart"]);

		if (!($log = $c->fetchSingleTest("select * from order_processing where bill_date = '%s'", date("Y-m-d")))) {
			echo sprintf("Closed for the day %s\r\n", date("d-M-Y"));
			exit;
		}
		else $log_id = $log["id"];
		$all_to_bill = $c->fetchAllTest('select b.*, o.authorization_transaction 
		...
		foreach($all_to_bill as $k=>$src) {
			$src_order = $c->fetchSingleTest("select * from orders where id = %d", $src["original_id"]);
			$user = $c->fetchSingleTest("select * from members where id = %d", $src_order["member_id"]);
		...
		$c->logMessage(__FUNCTION__, sprintf("*** post logmein session [%s]", print_r($_SESSION,true)), 1);
			$src_service = $c->fetchSingleTest("select p.*, ol.line_id from product p, order_lines ol where ol.order_id = %d and p.id = ol.product_id and ol.custom_package = 'S' and ol.deleted = 0", $src_order["id"]);
			$src_packages = $c->fetchAllTest("select ol.* from order_lines ol where ol.order_id = %d and ol.custom_package = 'P' and ol.deleted = 0", $src_order["id"]);
			$src_contracted = $c->fetchSingleTest("select * from order_lines where order_id = %d and product_id = %d and deleted = 0", $src_order["id"], CONTRACTED_RATE);
			$c->logMessage(__FUNCTION__, sprintf("src_packages [%s]", print_r($src_packages,true)), 1);
			$src_extras = $c->fetchScalarAllTest("select ol.product_id from order_lines ol, product p where ol.order_id = %d and ol.custom_package = 'A' and p.id = ol.product_id and p.custom_special_requirement and ol.deleted = 0", $src_order["id"]);
			...
			foreach($src_packages as $k=>$v) {
				...
				$src_product[$k]["dimensions"] = $c->fetchAllTest("select quantity,weight,height,width,depth from order_lines_dimensions od where od.order_id = %d and od.line_id = %d", $src_order["id"], $v["line_id"]);
			...
			$pickup = $c->fetchSingleTest("select * from addresses where ownertype='order' and ownerid=%d and addresstype=%d", $src_order["id"], ADDRESS_PICKUP);
			$delivery = $c->fetchSingleTest("select * from addresses where ownertype='order' and ownerid=%d and addresstype=%d", $src_order["id"], ADDRESS_DELIVERY);
			if ($src_order["custom_recurring_pu_time"] !="00:00:00") {
				...
				$dt = $c->fetchScalarTest("select scheduled_date from custom_delivery where order_id = %d and service_type='P'", $src["authorization_transaction"]);
		...
		//
			...
			$group = $c->fetchSingleTest("select * from members_by_folder where member_id = %d", $src_order["member_id"]);
			$g_product = $c->fetchSingleTest("select * from custom_member_product_options where member_id = %d and isgroup = 1 and product_id = %d", $group["folder_id"], $src_service["id"]);
			$m_product = $c->fetchSingleTest("select * from custom_member_product_options where member_id = %d and isgroup = 0 and product_id = %d", $group["id"], $src_service["id"]);
			if (!is_array($g_product)) {
				...
				$_SESSION["cart"]["header"]["pickupInstructions"] = $c->fetchScalarTest("select instructions from custom_delivery where order_id=%d and service_type='P'",$src["authorization_transaction"]);
				$_SESSION["cart"]["header"]["deliveryInstructions"] = $c->fetchScalarTest("select instructions from custom_delivery where order_id=%d and service_type='D'",$src["authorization_transaction"]);
				$_SESSION["cart"]["header"]["custom_weight_code"] = $src_order["custom_weight_code"];
				$_SESSION["cart"]["header"]["custom_dimension_code"] = $src_order["custom_dimension_code"];
			...
			//
				$allowedZones = $c->fetchScalarAllTest("select zone_id from zones_by_folder z, members m where m.id = %d and z.folder_id = m.custom_zones", $c->getUserInfo("id"));
			
				$fromZone = $c->fetchSingleTest("select zf.*, f.downtown from zone_fsa zf, fsa f where f.fsa='%s' and zf.fsa_id = f.id and zone_id in (%s)",
									strtoupper(substr($_SESSION["cart"]["addresses"]["pickup"]["postalcode"],0,3)), is_array($allowedZones) ? implode(", ", $allowedZones) : 0);
				$toZone = $c->fetchSingleTest("select zf.*, f.downtown from zone_fsa zf, fsa f where f.fsa='%s' and zf.fsa_id = f.id and zone_id in (%s)",
				...
				if ($src_service["is_fedex"]==1) {
					$quote["wt"] = $c->fetchSingleTest("select * from code_lookups where id = %d",$weight_cd);
					$quote["sz"] = $c->fetchSingleTest("select * from code_lookups where id = %d",$dimension_cd);
				...
				$quote["pickupInstructions"] = $c->fetchScalarTest("select instructions from custom_delivery where order_id = %d and service_type='P'",$src["authorization_transaction"]);
				$quote["deliveryInstructions"] = $c->fetchScalarTest("select instructions from custom_delivery where order_id = %d and service_type='P'",$src["authorization_transaction"]);
	...
	$c->logMessage(__FUNCTION__,sprintf("pre-postSaleProcessing session is [%s] src_order [%s]", print_r($_SESSION,true), print_r($src_order,true)),1);
					...
					//
					$reschedule = $c->fetchAllTest("select * from order_billing where original_id = %d and billed = 0 order by period_number", $src["original_id"]);
					$dt = $src["billing_date"];
					$period = 0;
					...
					$period = $c->fetchSingleTest("select * from order_billing where original_id = %d order by period_number desc", $src["original_id"]);
