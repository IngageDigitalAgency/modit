...
class PHPMailer {
 ...
 /**
  ...
  public function Send() {
    try {
		$this->myCommon->logMessage(__FUNCTION__, sprintf("call PreSend"),1 );

      if(!$this->PreSend()) return false;
		$this->myCommon->logMessage(__FUNCTION__, sprintf("return from PreSend"),1 );
		$r = $this->PostSend();
		$this->myCommon->logMessage(__FUNCTION__, sprintf("return from PostSend"),1 );

      return $r;	//$this->PostSend();
  ...
  protected function MailSend($header, $body) {
    ...
    if ($this->Sender != '' and !ini_get('safe_mode')) {
      ...
      if ($this->SingleTo === true && count($toArr) > 1) {
		...
		$this->myCommon->logMessage(__FUNCTION__, sprintf(" subject [%s] body [%s] header [%s] params [%s]", $this->EncodeHeader($this->SecureHeader($this->Subject)), $body, print_r($header,true), print_r($params,true)),1 );
  ...
  protected function SmtpSend($header, $body) {
    ...
    foreach($this->bcc as $bcc) {
      if (!$this->smtp->Recipient($bcc[0])) {
		...
		$this->myCommon->logMessage(__FUNCTION__, sprintf(" subject [%s] body [%s] this [%s]", $this->Subject, $body, print_r($this,true)),1 );
  ...
  public function SmtpConnect() {
  	$this->myCommon->logMessage(__FUNCTION__, sprintf( "start" ), 1);
