...
/**
 * Class for batch order processing
 */
class Batch extends Common {
    ...
    function copyOrder($id, $pmtInfo, &$newId) {
		...
		$hdr = $this->fetchSingleTest('select * from orders where id = %d',$id);
		$dtls = $this->fetchAllTest('select * from order_lines where order_id = %d and deleted = 0 order by id',$id);
		$dtl_ids = $this->fetchScalarAllTest('select line_id from order_lines where order_id = %d and deleted = 0',$id);
		$taxes = $this->fetchAllTest('select * from order_taxes where order_id = %d and (line_id = 0 or line_id in (%s)) order by id',$id,implode(",",$dtl_ids));
		$addresses = $this->fetchAllTest('select * from addresses where ownertype = "order" and ownerid = %d order by id',$id);
		unset($hdr['id']);
		...
		foreach($recs as $key=>$order) {
			...
			switch($billingType) {
				...
				case "PayFlow":
					if (strpos($order["authorization_type"],"PayPal") !== false && $order["baid"] == "") {
						$o_order = $this->fetchSingleTest('select * from orders where id = %d',$order['ref_transaction']);
						if ($o_order["nags"] > 3) {
					...
					if ($result['RESULT'] != 0) {
						...
						if ($order['attempts'] >= 5) {
							...
							$this->sendEmail('order-cancelled',$this->fetchSingleTest('select * from orders where id = %d',$order['original_id']),true,'Credit Card Declined');
							$c = new Custom(0,array());
			...
			if ($valid) {
				...
				if ($this->copyOrder($order['original_id'],$result,$newId)) {
					...
					$stmt->bindParams(array_merge(array(str_repeat('s',count($upd))),array_values($upd)));
					$stmt->execute();
					$code = $this->fetchSingleTest('select * from code_lookups where type="recurrenceType" and code = "%s"',$order['recurring_type']);
					$terms = explode("|",$code["extra"]);
					if ($terms[2] == "1") {
						//
						//	an unending renewal
						//
						$b = $this->fetchSingleTest('select * from order_billing where original_id = %d order by billing_date desc limit 1',$order['original_id']);
						...
						$b_new["billing_date"] = $c->calcPickup($dt,$this->fetchSingleTest("select * from order_lines where order_id = %d and custom_package = 'S'", $order["original_id"]));
					...
					//
					$period = explode("|",$this->fetchScalarTest('select * from code_lookups where type="recurrenceType" and code = "%s"',$order["recurring_type"]));
					$next_dt = date("Y-m-d");
					//
					//	Only reschedule 1 order - weekends can shift 1 delivery but shouldn't affect subsequent
					//
					$to_bill = $this->fetchAllTest("select * from order_billing where original_id = %d and billed = 0 order by period_number limit 1",$order["original_id"]);
					foreach($to_bill as $k1=>$v1) {
						...
						$next_dt = $c->calcPickup($dt,$this->fetchSingleTest("select * from order_lines where order_id = %d and custom_package = 'S'", $order["original_id"]));
    ...
    function sendEmail($form, $order, $override = false, $title = 'Expiring Authorization') {
		...
		$member = $this->fetchSingleTest('select * from members where id = %d',$order['member_id']);
		$body = new Forms();
    ...
    function test($order_id) {
		...
		$snoopy->httpmethod = 'POST';
		$snoopy->curl_path = $parms['curl_path'];
		$order = $this->fetchSingleTest('select * from orders where id = %d',$order_id);
		$query['ORIGID'] = $order['authorization_transaction'];
		$query['AMT'] = $order['total'];
		$snoopy->submit($parms['auth'],$query);
	...
	function sendProcessingReport() {
		$alerts = $this->fetchAllTest("select * from order_processing_details where processing_id = %d",$this->m_processing["id"]);
		$module = $this->fetchSingleTest("select * from fetemplates where id = %d",$this->getOption("adminLog"));
		$dtl = new Forms();
    ...
    function preRecurringOrder($orig_order, $o_id) {
		...
		$_SESSION["cart"]["products"] = $this->fetchAllTest("select p.*, ol.price, ol.quantity, ol.value, ol.custom_package from order_lines ol, product p where ol.order_id = %d and p.id = ol.product_id", $o_id);
		//$pu = $this->fetchSingleTest("select * from custom_delivery where order_id = %d and service_type='P'", $orig_order["ref_transaction"]);
		$order = $this->fetchSingleTest("select * from orders where id = %d", $orig_order["original_id"]);

		if ($order["custom_recurring_pu_time"]=="00:00:00") 
			$order["custom_recurring_pu_time"] = $this->fetchScalarTest("select scheduled_date from custom_delivery where order_id = %d and service_type='P'",$orig_order["ref_transaction"]);
		...
		$_SESSION["quote"] = array(
			"custom_recurring_pu_driver"=>$order["custom_recurring_pu_driver"] > 0 ? $this->fetchScalarTest("select d.id from drivers d, members m where m.id = d.member_id and m.deleted = 0 and m.enabled = 1 and d.deleted = 0 and d.enabled = 1 and d.id = %d",$order["custom_recurring_pu_driver"]) : 0,
			"custom_recurring_del_driver"=>$order["custom_recurring_del_driver"] > 0 ? $this->fetchScalarTest("select d.id from drivers d, members m where m.id = d.member_id and m.deleted = 0 and m.enabled = 1 and d.deleted = 0 and d.enabled = 1 and d.id = %d",$order["custom_recurring_del_driver"]) : 0,
			"pickupInstructions"=>$this->fetchScalarTest("select instructions from custom_delivery where order_id = %d and service_type='P'", $orig_order["ref_transaction"]),
			"deliveryInstructions"=>$this->fetchScalarTest("select instructions from custom_delivery where order_id = %d and service_type='D'", $orig_order["ref_transaction"])
		);
