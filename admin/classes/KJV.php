...
/**
 * Handles KJV common functionality
 */
class KJV extends Common {
    ...
    public function getPrice($products, $extras, $pickup, $shipto, $c_id, $weight_code, $dimension_code ) {
		...
		$weightConversion = $this->fetchScalarTest("select extra from code_lookups where id = %d",$weight_code);
		$dimensionConversion = $this->fetchScalarTest("select extra from code_lookups where id = %d",$dimension_code);
		...
		$group = $this->fetchSingleTest("select f.* from members_folders f, members_by_folder m where m.member_id = %d and f.id = m.folder_id limit 1",$this->getUserInfo("id", true));
		$member = $this->fetchSingleTest("select * from members where id = %d",$this->getUserInfo("id", true));
		$product = $this->fetchSingleTest("select p.* from product p, custom_member_product_options c where c.id = %d and p.id = c.product_id",$c_id);
		$p_id = $product["id"];
		$g_override = $this->fetchSingleTest("select * from custom_member_product_options where member_id = %d and isgroup = 1 and product_id = %d",$group["id"],$product["id"]);
		...
		$m_override = $this->fetchSingleTest("select * from custom_member_product_options c, members_by_folder mf where mf.member_id = %d and mf.folder_id = %d and c.member_id = mf.id and c.isgroup = 0 and c.product_id = %d",$member["id"],$group["id"],$product["id"]);
		...
		//
		$allowedZones = $this->fetchScalarAllTest("select zone_id from zones_by_folder z where folder_id = %d", $member["custom_zones"]);
				
		$fromZone = $this->fetchSingleTest("select zf.*, f.downtown from zone_fsa zf, fsa f  where f.fsa='%s' and zf.fsa_id = f.id and zf.zone_id in (%s) and zf.enabled = 1",strtoupper(substr($pickup["postalcode"],0,3)), is_array($allowedZones) && count($allowedZones) > 0 ? implode(", ",$allowedZones) : 0);
		$toZone = $this->fetchSingleTest("select zf.*, f.downtown from zone_fsa zf, fsa f where f.fsa='%s' and zf.fsa_id = f.id and zf.zone_id in (%s) and zf.enabled = 1",strtoupper(substr($shipto["postalcode"],0,3)), is_array($allowedZones) && count($allowedZones) > 0 ? implode(", ",$allowedZones) : 0 );
		...
		if ($by_pc = $this->fetchSingleTest('select * from custom_product_by_pc where member_product_option_id = %d and ((from_postal_code = "%s" and to_postal_code = "%s") or (from_postal_code = "%3$s" and to_postal_code = "%2$s"))', $c_id, $from_pc, $to_pc)) {
		...
		$specials = $this->fetchScalarAllTest("select id from product where deleted = 0 and enabled = 1 and published = 1 and custom_special_requirement = 1 and id in (%s)", implode(", ",array_merge(array(0),array_values($extras))));
		if ((!$product["custom_same_day"]) && count($specials)==0) {
			$pickupDriver = 0;	//$this->fetchSingleTest("select d.*, v.name as vehicleType from fsa f, delivery_zones_fsa dzf, delivery_zones dz, drivers d, vehicles v where f.fsa = '%s' and dzf.fsa_id = f.id and dz.id = dzf.delivery_zone_id and d.id = dz.driver_id and v.id = d.vehicle_id and v.max_weight >= %s and (disabled_as_of = '0000-00-00 00:00:00' or date(disabled_as_of < current_date()) order by max_weight limit 1",substr($pickup["postalcode"],0,3),$weightToShip));
			$deliveryDriver = $this->fetchSingleTest("select d.*, v.name as vehicleType from fsa f, delivery_zones_fsa dzf, delivery_zones dz, drivers d, vehicles v where f.fsa = '%s' and dzf.fsa_id = f.id and dz.id = dzf.delivery_zone_id and d.id = dz.driver_id and v.id = d.vehicle_id and v.max_weight >= %s order by max_weight limit 1",substr($shipto["postalcode"],0,3,$weightToShip));
		...
		if (array_key_exists("custom_recurring_pu_driver",$_REQUEST) && $_REQUEST["custom_recurring_pu_driver"] > 0) {
			$quote["pickup_driver"] = $this->fetchSingleTest("select * from drivers where id = %d",$_REQUEST["custom_recurring_pu_driver"]);
		...
		if ($this->checkArray("quote:custom_recurring_pu_driver",$_SESSION) && $_SESSION["quote"]["custom_recurring_pu_driver"] > 0) {
			$quote["pickup_driver"] = $this->fetchSingleTest("select * from drivers where id = %d",$_SESSION["quote"]["custom_recurring_pu_driver"]);
		...
		if (array_key_exists("custom_recurring_del_driver",$_REQUEST) && $_REQUEST["custom_recurring_del_driver"] > 0) {
			$quote["delivery_driver"] = $this->fetchSingleTest("select * from drivers where id = %d",$_REQUEST["custom_recurring_del_driver"]);
		...
		if ($this->checkArray("quote:custom_recurring_del_driver",$_SESSION) && $_SESSION["quote"]["custom_recurring_del_driver"] > 0) {
			$quote["delivery_driver"] = $this->fetchSingleTest("select * from drivers where id = %d",$_SESSION["quote"]["custom_recurring_del_driver"]);
		...
		if ($quote["custom_declared_value"] > 0) {
			$tmp = $this->fetchSingleTest("select * from product where id = %d",INSURANCE);
		...
		$weight_pricing = $this->fetchAllTest("select * from product_pricing where product_id = %d order by min_quantity",$group["custom_weight"]);
		...
		if (!(is_array($fromZone) && is_array($toZone))) {
			...
			if (!$product["custom_same_day"]) {
				...
				//
				$quote["delivery_driver"] = $this->fetchSingleTest("select d.*, v.name as vehicleType from drivers d, vehicles v where d.id = %d and v.id = d.vehicle_id", FEDEX_DRIVER);
				$quote["pickup_driver"] = $quote["delivery_driver"];
				//
				//	p/u fsa is valid - kjv pickup, fedex delivery
				//
				if ($checkPickupDriver = $this->fetchSingleTest("select d.*, v.name as vehicleType from fsa f, delivery_zones_fsa dzf, delivery_zones dz, drivers d, vehicles v where f.fsa = '%s' and dzf.fsa_id = f.id and dz.id = dzf.delivery_zone_id and d.id = dz.driver_id and v.id = d.vehicle_id and v.max_weight >= %s order by max_weight limit 1",substr($pickup["postalcode"],0,3,$weightToShip))) {
					$quote["pickup_driver"] = array("id"=>0);	// as per Victor - do not assign a p/u driver if in a serviced FSA
					if ($checkDeliveryDriver = $this->fetchSingleTest("select d.*, v.name as vehicleType from fsa f, delivery_zones_fsa dzf, delivery_zones dz, drivers d, vehicles v where f.fsa = '%s' and dzf.fsa_id = f.id and dz.id = dzf.delivery_zone_id and d.id = dz.driver_id and v.id = d.vehicle_id and v.max_weight >= %s order by max_weight limit 1",substr($shipto["postalcode"],0,3,$weightToShip))) {
						$quote["delivery_driver"] = $checkDeliveryDriver;
					}
				}
				$quote["wt"] = $this->fetchSingleTest("select * from code_lookups where id = %d",$weight_code);
				$quote["sz"] = $this->fetchSingleTest("select * from code_lookups where id = %d",$dimension_code);
				...
				$cart["products"] = array();
				foreach($products as $key=>$p) {
					$tmp = $this->fetchSingleTest("select * from product where id = %d",$p["product_id"]);
					$key = $tmp["id"]."|0|0|0";
				...
				foreach($cart["custom"]["ourRates"] as $key=>$value) {
					...
					if ($c_id == $value["member_id"] || $c_id == $value["group_id"]) {
						$tmp = $this->fetchSingleTest("select p.* from product p, custom_member_product_options cpo where cpo.id = %d and p.id = cpo.product_id",$c_id);
						$k = $tmp["id"]."|0|0|0";
						...
						foreach($value["surcharges"] as $sk=>$svalue) {
							$tmp = $this->fetchSingleTest("select * from product p where id = %d",$svalue["product_id"]);
							$kk = $tmp["id"]."|0|0|0";
...
*/
				...
				//
				$oda = $this->fetchSingleTest("select sum(custom_minimum_charge) as charge, id from product where code in ('%s','%s') and enabled=1 and published=1 and deleted=0", str_replace(" ", "", $cart["addresses"]["pickup"]["postalcode"]), str_replace(" ", "", $cart["addresses"]["shipping"]["postalcode"]));
				if (!is_array($oda) || $oda["charge"] < 0.01) {
					$oda = $this->fetchSingleTest("select sum(custom_minimum_charge) as charge, id from product where code in ('%s','%s') and enabled=1 and published=1 and deleted=0", substr(str_replace(" ", "", $cart["addresses"]["pickup"]["postalcode"]),0,3), substr(str_replace(" ", "", $cart["addresses"]["shipping"]["postalcode"]),0,3));
				}
				if (is_array($oda) && (float)$oda["charge"] > .01) {
					$tmp = $this->fetchSingleTest("select * from product where id = %d",$oda["id"]);
					$key = $tmp["id"]."|0|0|0";
					$cart = $this->initPriceLine($cart,$key,$tmp);
				...
				foreach($extras as $key=>$rec) {
					if ($rec > 0) {
						$tmp = product::formatData($this->fetchSingleTest("select * from product where id = %d",$rec));
						$key = $tmp["id"]."|0|0|0";
						$cart = $this->initPriceLine($cart,$key,$tmp);
						$cart['products'][$key]['url'] = $this->getUrl('product',$tmp['id'],$tmp);
						$cart['products'][$key]['line_id'] = count($cart['products']);
				...
				//$fuel = $this->fetchSingleTest("select * from product_pricing where product_id = %d order by min_quantity limit 1",$group["custom_fuel"]);
...
//				if ($member["custom_fuel_exempt"] == 1) {
	$this->logMessage(__FUNCTION__, sprintf("fuel exempt check member [%s] pc [%s]", print_r($m_override,true), print_r($by_pc,true)),1);
				...
				if ($quote["fuelCharge"] >= .01) {
					$tmp = $this->fetchSingleTest("select * from product where id = %d",$group["custom_fuel"]);
					$key = $tmp["id"]."|0|0|0";
					$cart = $this->initPriceLine($cart,$key,$tmp);
					$cart['products'][$key]['url'] = $this->getUrl('product',$tmp['id'],$tmp);
					$cart['products'][$key]['line_id'] = count($cart['products']);
		...
		$from = $this->fetchSingleTest("select bf.* from zone_folders zf, zones_by_folder bf, zones z, zone_fsa zfsa, fsa where zf.id = %d and bf.folder_id = zf.id and z.id = bf.zone_id and zfsa.zone_id = z.id and fsa.fsa = '%s' and zfsa.fsa_id = fsa.id and zfsa.enabled = 1",
				$member["custom_zones"], strtoupper(substr($pickup["postalcode"],0,3)));
		$to = $this->fetchSingleTest("select bf.* from zone_folders zf, zones_by_folder bf, zones z, zone_fsa zfsa, fsa where zf.id = %d and bf.folder_id = zf.id and z.id = bf.zone_id and zfsa.zone_id = z.id and fsa.fsa = '%s' and zfsa.fsa_id = fsa.id and zfsa.enabled = 1",
				$member["custom_zones"], strtoupper(substr($shipto["postalcode"],0,3)));
		$interZone = false;
		if (is_array($from) && is_array($to))
			$interZone = $this->fetchSingleTest("select * from zone_to_zone where (zone_from=%d and zone_to=%d) or (zone_from = %d and zone_to = %d)",
...
$this->logMessage(__FUNCTION__, sprintf("///-2 tmpQuote [%s]", print_r($tmpQuote,true)),1);
		...
		if (array_key_exists("pickup_driver",$quote) && array_key_exists("delivery_driver",$quote)) {
			$p_fc = $this->fetchSingleTest("select v.* from vehicles v, drivers d where d.id = %d and v.id = d.vehicle_id",$quote["pickup_driver"]["id"]);
			$d_fc = $this->fetchSingleTest("select v.* from vehicles v, drivers d where d.id = %d and v.id = d.vehicle_id",$quote["delivery_driver"]["id"]);
...
//				if ($member["custom_fuel_exempt"] == 1) {
	$this->logMessage(__FUNCTION__, sprintf("fuel exempt check mbr [%s] pc [%s] group [%s]", print_r($m_override,true), print_r($by_pc,true), print_r($g_override,true)),1);
		...
		if ($quote["weightCharge"] != 0) {
			$tmp = $this->fetchSingleTest("select * from product where id = %d",$group["custom_weight"]);
			$key = $tmp["id"]."|0|0|0";
			$cart = $this->initPriceLine($cart,$key,$tmp);
			$cart['products'][$key]['url'] = $this->getUrl('product',$tmp['id'],$tmp);
			$cart['products'][$key]['line_id'] = count($cart['products']);
		...
		if ($quote["zoneCharge"] != 0) {
			$tmp = $this->fetchSingleTest("select * from product where id = %d",ZONE_ID);
			$key = $tmp["id"]."|0|0|0";
			$cart = $this->initPriceLine($cart,$key,$tmp);
			$cart['products'][$key]['url'] = $this->getUrl('product',$tmp['id'],$tmp);
			$cart['products'][$key]['line_id'] = count($cart['products']);
		...
		if ($quote["kmCharge"] != 0) {
			$tmp = $this->fetchSingleTest("select * from product where id = %d",$kmProduct);
			$key = $tmp["id"]."|0|0|0";
			$cart = $this->initPriceLine($cart,$key,$tmp);
			$cart['products'][$key]['url'] = $this->getUrl('product',$tmp['id'],$tmp);
			$cart['products'][$key]['line_id'] = count($cart['products']);
		...
		if ($quote["downtownCharge"] != 0) {
			$tmp = $this->fetchSingleTest("select * from product where id = %d",$quote["downtownChargeType"]);
			$key = $tmp["id"]."|0|0|0";
			$cart = $this->initPriceLine($cart,$key,$tmp);
			$cart['products'][$key]['url'] = $this->getUrl('product',$tmp['id'],$tmp);
			$cart['products'][$key]['line_id'] = count($cart['products']);
...
//
		...
		if ($contractedRate >= .01) {
			...
			$tmp = $this->fetchSingleTest("select * from product where id = %d",CONTRACTED_RATE);
			$key = $tmp["id"]."|0|0|0";
			$cart = $this->initPriceLine($cart,$key,$tmp);
			$cart["products"][$key]['url'] = $this->getUrl('product',$tmp['id'],$tmp);
			$cart["products"][$key]['line_id'] = count($cart['products']);
		...
		foreach($cart["products"] as $k=>$v) {
			if ($v["custom_package"] == "P") {
				if ($chg = $this->fetchSingleTest("select * from member_package_charges where member_id = %d and product_id = %d", $this->getUserInfo("id", true, $v["id"]))) {
					$qty = $v["quantity"] - $chg["free"];
					if ($qty > 0) {
						$tmp = $this->fetchSingleTest("select * from product where id = %d",ADDITIONAL_PIECE_CHARGES);
						//
    ...
    public function calcDelivery($pickup, $product) {
		...
		$p = $this->fetchSingleTest("select * from product where id = %d",$product["id"]);
		...
		if ($p["custom_same_day"] == 0) {
			$dow = date("w",$dt);
			$i = 0;
			$tmp = $this->fetchSingleTest("select * from order_processing where bill_date = '%s'", date('Y-m-d',$dt));
			while($i < 10 && (false == $this->fetchSingleTest("select * from order_processing where bill_date = '%s'", date('Y-m-d',$dt)) || ($p["custom_availability"] & pow(2,$dow)) == 0)) {
    ...
    public function insertOrder($cart, &$orderid) {
		...
		$stmt->bindParams(array('iididddddddssisiidsssddd', 
			...
			date(DATE_ATOM), date(DATE_ATOM), random_int(1, 1000000), $cart['header']['discount_type'],
...
*/
    ...
    function getWalkingDistance($addresses) {
		$src = sprintf("%s, %s %s %s %s", $addresses["pickup"]["line1"], 
			$addresses["pickup"]["city"], $this->fetchScalarTest("select province_code from provinces where id = %d", $addresses["pickup"]["province_id"]), $addresses["pickup"]["postalcode"], $this->fetchScalarTest("select country from countries where id = %d", $addresses["pickup"]["country_id"]));
		$dest = sprintf("%s, %s %s %s %s", $addresses["shipping"]["line1"],
			$addresses["shipping"]["city"], $this->fetchScalarTest("select province_code from provinces where id = %d", $addresses["shipping"]["province_id"]), $addresses["shipping"]["postalcode"], $this->fetchScalarTest("select country from countries where id = %d", $addresses["shipping"]["country_id"]));
...
*/
    ...
    function getKmRate(&$quote, &$r, &$kmProduct, $group, $product, $member, $distance, $pickup, $shipto, $m_override, $g_override, $in_zone) {
		...
		$src_zone = $this->fetchSingleTest("select z.title, zbf.id
	...
	substr($pickup["postalcode"],0,3));
		...
		$dest_zone = $this->fetchSingleTest("select z.title, zbf.id
	from zones z, zone_fsa zfsa, fsa, zones_by_folder zbf
	where zbf.folder_id = %d and z.id = zbf.zone_id and zfsa.zone_id = z.id and fsa.fsa = '%s' and zfsa.fsa_id = fsa.id and zfsa.enabled = 1",$_SESSION["user"]["info"]["custom_zones"],
	substr($shipto["postalcode"],0,3));
		...
		if (is_array($src_zone) && is_array($dest_zone)) {
			...
			if ($z_to_z = $this->fetchSingleTest("select * from zone_to_zone where (zone_from = %d and zone_to = %d) or (zone_to = %d and zone_from = %d)",$src_zone["id"],$dest_zone["id"],$src_zone["id"],$dest_zone["id"])) {			
				if ($zone_override = $this->fetchSingleTest("select * from zone_to_zone_caps where zone_to_zone_id = %d and product_id = %d",$z_to_z["id"],$product["id"])) {
					$kmOverride = true;
