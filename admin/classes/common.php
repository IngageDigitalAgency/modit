...
/**
 * Contains all the common functionality shared between backend and frontend
 */
class Common {
    ...
    function getConfigVar($name, $type='config') {
		if ($data = $this->fetchSingleTest("select * from config where name = '%s' and type = '%s'",$name,$type)) {
			switch($data['field_type']) {
				case 'address':
					$address = $this->fetchSingleTest('select * from addresses where id = %d',$data['value']);
    ...
    function getConfigData($name) {
		$data = $this->fetchSingleTest("select * from config where name = '%s' and type = 'config'",$name);
    ...
    function getGlobalVar($name) {
		if (array_key_exists($name,$GLOBALS))
			...
			return $this->fetchScalarTest("select value from config where name = '%s' and type = 'global'",$name);
	}
    ...
    protected function nodeList($root, $type ) {
		...
		if ($root == 0) {
			...
			if (!$node = $this->fetchSingleTest("select * from %s where %s = %d",$tbl,$fld,$root)) {
		...
		$nodes = $this->fetchAllTest("select %s from %s where left_id >= %d and right_id <= %d order by left_id",$fld, $tbl, $node["left_id"], $node["right_id"]);
    ...
    function nodeSelect($root, $type, $indent = 1, $required = false, $inclusive = false, $description = "-none-", $where = null) {
		...
		if ($root != 0) {
			if (!$node = $this->fetchSingleTest("select * from %s where %s = %d %s",$tbl,$fld,$root,$where)) {
				$body = sprintf("A folder [%d] has been deleted from [%s] that is still in use.\nDetails: %s", $root, $tbl, print_r($this,true));
	...
	function fetchSingleTest() {
		$args = func_get_args();				
		$squery = $args[0];
		$h = $GLOBALS['globals']->getConnection();
		if (count($args) > 1) {				
			$squerymatch = preg_match_all("/(?<=\%)\w/i", $squery, $matches);				
			$squery = preg_replace("/\%\w/", "?" , $squery);				
			array_shift($args);
			$possibleformats = array('%%','%b','%c','%d','%e','%E','%u','%f','%F','%g','%G','%o','%s','%x','%X');
			$squery = str_replace($possibleformats, "?", $squery);
			$squery = str_replace(array("'?'", '"?"'), '?', $squery);
			$checkparameterizetable = strpos($squery, 'from ?');
            if ($checkparameterizetable !== false) {				
				$checktablepostion = substr_count($squery, '?', 0,$checkparameterizetable);				
				$squery = str_replace("from ?",'from '.$args[$checktablepostion],$squery);
				unset($args[$checktablepostion]);
            }			
			$stmt = $h->prepare($squery);
			if(!$stmt){
				echo "Fetach Single Prepare failed: (". $h->errno.") ".$h->error." ".$squery."<br>";
			}
		...
		if ($status) {
			...
			$this->logDBMessage('fetchSingle', sprintf("Error: sql[%s] Msg[%s]", $squery, $h->error), 1);		
			return array();
		}		
	}
	...
	function fetchAllTest() {
		...
		if (count($args) > 1 && $connection == false) {				
			$squerymatch = preg_match_all("/(?<=\%)\w/i", $squery, $matches);				
			$squery = preg_replace("/\%\w/", "?" , $squery);				
			array_shift($args);
			$possibleformats = array('%%','%b','%c','%d','%e','%E','%u','%f','%F','%g','%G','%o','%s','%x','%X');
			...
			$stmt = $h->prepare($squery);
			if(!$stmt){
				echo "Fetach All Prepare failed: (". $h->errno.") ".$h->error." ".$squery."<br>";
			}
			$stmt->bind_param(str_repeat('s', count($args)), ...$args);
        }else{						
			$stmt = $h->prepare($squery);
		...
		if ($status) {
			...
			$this->logDBMessage('fetchAll', sprintf("Error: sql[%s] Msg[%s]", $squery, $h->error), 1);
			return array();
		}
	}

	function fetchScalarTest() {
		$args = func_get_args();				
		$squery = $a = $args[0];
		$h = $GLOBALS['globals']->getConnection();
		if (count($args) > 1) {				
			...
			$checkparameterizetableTwo = strpos($squery, '? t where');			
            if ($checkparameterizetableTwo !== false) {				
				$checktablepostionTwo = substr_count($squery, '?', 0,$checkparameterizetableTwo+1);								
				$squery = str_replace("? t where",$args[$checktablepostionTwo].' t where',$squery);
				unset($args[$checktablepostionTwo]);
            }		
			$stmt = $h->prepare($squery);	
			if(!$stmt){
				echo "Fetch Scalar Prepare failed: (". $h->errno.") ".$h->error." ".$squery."<br>";
			}		
            if (count($args) > 0) {     				
                $stmt->bind_param(str_repeat('s', count($args)), ...$args);
            }			
		...
		if ($status) {
			$result = $stmt->get_result();
			if ($tmp = $result->fetch_row()){
				$result -> free_result();return $tmp[0];
			}
			else return null;
		}else{	
			$this->logDBMessage('fetchScalar', sprintf("Error: sql[%s] Msg[%s]", $squery, $h->error), 1);		
			return array();
		}
	}
	
	function fetchScalarAllTest() {
		$args = func_get_args();
		$squery = $a = $args[0];
		$h = $GLOBALS['globals']->getConnection();		
		$checkinfunction = strpos($squery, 'in ("%s")');		
        if ($checkinfunction !== false) {
			unset($args[0]);
			$squery = sprintf($squery, ...$args);			
			$args = array();
        }
		if (count($args) > 1) {			
			$squerymatch = preg_match_all("/(?<=\%)\w/i", $squery, $matches);				
			...
			$stmt = $h->prepare($squery);
			if(!$stmt){
				echo "Fetch Scalar All Prepare failed: (". $h->errno.") ".$h->error." ".$squery."<br>";
			}
            if (count($args) > 0) {				
                $stmt->bind_param(str_repeat('s', count($args)), ...$args);
            }
		...
		if ($status) {			
			...
			$this->logDBMessage('fetchScalarAll', sprintf("Error: sql[%s] Msg[%s]", $squery, $h->error), 1);		
			return array();
		}		
	}

	function fetchOptionsTest($sql, $h = null) {
		$args = func_get_args();				
		$squery = $args[0];
		$connection = false;
        if (isset($GLOBALS['globals'])) {
		...
		if (count($args) > 1 && $connection == false) {				
			...
			$stmt = $h->prepare($squery);
			if(!$stmt){
				echo "Fetch Options Prepare failed: (". $h->errno.") ".$h->error." ".$squery."<br>";
			}
			$stmt->bind_param(str_repeat('s', count($args)), ...$args);
        }else{						
			$stmt = $h->prepare($squery);
		...
		if ($status) {
			...
			$this->logDBMessage('fetchOptions', sprintf("Error: sql[%s] Msg[%s]", $squery, $h->error), 1);		
			return array();
		}		
	}	
    ...
    function getUrl($class, $id, $data = array()) {
		...
		switch($class) {
			...
			case 'eventday':
				...
				break;
			case 'page':
				$data = $this->fetchSingleTest('select * from content where id = %d',$id);
			case 'menu':
				switch($data['type']) {
					case 'page':
						...
						break;
					case 'internallink':
						$p = $this->fetchSingleTest('select * from content where id = %d',$data['internal_link']);
			...
			case 'image':
				...
				break;
			case 'lease':
				if (!($l = $this->fetchSingleTest("select mk.name as make, ml.model from leases l, lease_make mk, lease_model ml where l.id = %d and mk.id = l.make and ml.id = l.model",$data["id"])))
    ...
    function logMeIn($username, $password, $rememberMe = false, $bypassId = 0 ) {
		...
		if ($user = $this->fetchSingleTest($sql)) {
			if ($this->hasOption('enforce_membership') && array_key_exists('folder_id',$this->m_module) && $this->m_module['folder_id'] > 0) {
				...
				//
				if (!$this->fetchSingleTest('select 1 from members_by_folder where member_id = %d and folder_id = %d',$user['id'],$this->m_module['folder_id'])) {
		...
		if (is_array($user) && count($user) > 0) {
			...
			if (array_key_exists('cart',$_SESSION)) {
				...
				if ($tmp = $this->fetchSingleTest('select * from addresses where deleted = 0 and ownertype = "member" and ownerid = %d and addresstype = %d',$_SESSION['user']['info']['id'],ADDRESS_PICKUP))
    ...
    function formatOrder($data) {
		$data['itemCount'] = $this->fetchScalarTest('select sum(quantity) from order_lines where order_id = %d and deleted = 0 and custom_package = "P"',$data['id']);
		...
		$status = $this->fetchAllTest('select * from code_lookups where type="orderStatus" order by sort');
		$tmp = array();
		...
		$data['formattedShipVia'] = $this->fetchScalarTest('select value from code_lookups where type="ship_via" and id=%d',$data['ship_via']);
    ...
    function formatOrderLine($data) {
		...
		if ($data['options_id'] != 0) {
			$data['optionsName'] = $this->fetchScalarTest('select teaser from product_options where id = %d',$data['options_id']);
		}
		else $data['optionsName'] = '';
		if ($data['color'] != 0) {
			$data['colorName'] = $this->fetchScalarTest('select value from code_lookups c where c.id = %d',$data['color']);
		}	
		else $data['colorName'] = '';
		if ($data['size'] != 0) {
			$data['sizeName'] = $this->fetchScalarTest('select value from code_lookups c where c.id = %d',$data['size']);
		}
		else $data['sizeName'] = '';
		if ($data['recurring_period'] != 0) {
			$data['recurringInfo'] = $this->fetchSingleTest('select * from product_recurring where id = %d',$data['recurring_period']);
		...
		$wt = $this->fetchSingleTest("select l.code, sum(od.weight*od.quantity) as weight from code_lookups l, order_lines_dimensions od, orders o where od.line_id = %d and od.order_id = %d and o.id = od.order_id and l.id = o.custom_weight_code", $data["line_id"], $data["order_id"]);
    ...
    function getClassId($className) {
		...
		return $this->fetchScalarTest('select id from modules where classname = "%s"',$className);
	}
...
and t.id = (select max(t1.id) from templates t1 where template_id = t.template_id)', $class, $id );
		$templates = $this->fetchAllTest($sql);
    ...
    function processUploadedFiles($allowedTypes, &$return, &$messages) {
		...
		foreach($_FILES as $key=>$file) {
			...
			if (strlen($file['name']) > 0) {
				...
				$types = $this->fetchScalarAllTest($sql);
				...
				$return[$key] = array('name'=>'/'.$name,'type'=>$this->fetchScalarTest('select code from code_lookups where type="MimeUploadTypes" and extra like "%%%s%%"',$file['type']));
				if (defined('FRONTEND')) 
					$name = './'.$name;
				else
					$name = '../'.$name;
    ...
    function setQueryOptions($queryString) {
		...
		$source = $this->fetchAllTest('select * from search_keywords where keyword in ("%s")',implode('","',$keywords));
...
#			return money_format(GLOBAL_DEFAULT_CURRENCY_FORMAT,$value);
    ...
    public static function geoCode($address, &$lat, &$long ) {
		...
		$url = urlencode(sprintf("%s, %s, %s %s",$address['line1'],$address['city'],$c->fetchScalarTest('select province_code from provinces where id = %d',$address['province_id']),$address['postalcode']));
		$s = new Snoopy();
					...
					foreach($tmp["address_components"] as $k=>$v) {
						...
						foreach($v["types"] as $sk=>$sv) {
							if ($sv == "administrative_area_level_1") {
								...
								//	Province validation
								//
								if (array_key_exists("short_name",$v) & $prov = $c->fetchSingleTest("select * from provinces where province_code = '%s'", $v["short_name"])) {
									if ($address["province_id"] != $prov["id"]) {
										$valid = false;
										$c->addError(sprintf("Province Mismatch [%s] vs [%s]", $v["short_name"], $c->fetchScalarTest("select province_code from provinces where id = %d", $address["province_id"])));
									}
								}
							}
							if ($sv == "postal_code") {
    ...
    function calc_driver_allocations($o_id) {
		...
		if (!$pu_driver = $this->fetchSingleTest("select cd.id as del_id, d.* from custom_delivery cd, drivers d where cd.service_type='P' and cd.order_id = %d and d.id = cd.driver_id",$o_id))
			$pu_driver = array("id"=>0,"vehicle_id"=>0,"del_id"=>0);
		if (!$del_driver = $this->fetchSingleTest("select cd.id as del_id, d.* from custom_delivery cd, drivers d where cd.service_type='D' and cd.order_id = %d and d.id = cd.driver_id",$o_id))
			$del_driver = array("id"=>0,"vehicle_id"=>0,"del_id"=>0);
		$prod = $this->fetchSingleTest("select p.* from order_lines ol, product p where ol.order_id = %d and p.id = ol.product_id and custom_package = 'S' and ol.deleted = 0", $o_id);
		if ($prod["is_fedex"]) {
    ...
    function calculateCommissions($o_id) {
		$this->logMessage(__FUNCTION__,sprintf("calc order %d",$o_id),1);
		//$value = $this->fetchScalarTest("select sum(o.value) from order_lines o, product p where o.order_id = %d and p.id = o.product_id and p.custom_commission = 1",$o_id);
		//$this->execute(sprintf("update orders set custom_commissionable_amt = %s where id = %s",(float)$value,$o_id));
		if (!$actions = $this->fetchAllTest("select cd.*, d.commission, d.after_hours_commission, v.fuel_charge from custom_delivery cd, drivers d, vehicles v where cd.order_id = %d and d.id = cd.driver_id and v.id = d.vehicle_id",$o_id))
			return;
		$group = $this->fetchSingleTest("select f.* from members_folders f, members_by_folder m, orders o where o.id = %d and m.member_id = o.member_id and f.id = m.folder_id limit 1",$o_id);
		$member = $this->fetchSingleTest("select m.* from members m, orders o where o.id = %d and m.id = o.member_id",$o_id);
		$recs = $this->fetchAllTest("select o.* from order_lines o where o.order_id = %d and o.product_id = %d and o.deleted = 0",$o_id,$group["custom_fuel"]);
		$recalc = false;
		$fuelCharge = 0;
		...
		if ((array_key_exists(0,$actions) && $actions[0]["fuel_charge"] == 1) || (array_key_exists(1,$actions) && $actions[1]["fuel_charge"] == 1)) {
			...
			//
			$fuelExempt = 0;
			if ($g_override = $this->fetchSingleTest("select mpo.* from custom_member_product_options mpo, order_lines ol where ol.order_id = %d and ol.custom_package = 'S' and ol.deleted = 0 and mpo.member_id = %d and mpo.product_id = ol.product_id and mpo.isgroup = 1", $o_id, $group["id"])) {
				$fuelExempt = $g_override["fuel_exempt"];
				$this->logMessage(__FUNCTION__,sprintf("group fuel exempt [%s]", print_r($g_override,true)),1);
			}
			if ($m_override = $this->fetchSingleTest("select mpo.* from custom_member_product_options mpo, members_by_folder mbf, order_lines ol where ol.order_id = %d and ol.custom_package = 'S' and ol.deleted = 0 and mbf.member_id = %d and mpo.member_id = mbf.id and mpo.product_id = ol.product_id and mpo.isgroup = 0", $o_id, $member["id"])) {
				if ($m_override["fuel_exempt"] == 1) {
					...
					$pc = $this->fetchScalarAllTest('select replace(ucase(postalcode)," ","") from addresses where ownertype="order" and ownerid = %d and addresstype in (%d,%d)', $o_id, ADDRESS_PICKUP, ADDRESS_DELIVERY );					
					if ($pc_override = $this->fetchSingleTest('select * from custom_product_by_pc where ((from_postal_code = "%s" and to_postal_code = "%s") or (from_postal_code = "%2$s" and to_postal_code = "%1$s")) and member_product_option_id = %d', $pc[0], $pc[1], $m_override["id"])) {
			...
			//
			$from_pc = strtoupper(str_replace(" ","",$this->fetchScalarTest("select postalcode from addresses where ownertype='order' and ownerid = %d and addresstype = %d", $o_id, ADDRESS_PICKUP)));
			$to_pc = strtoupper(str_replace(" ","",$this->fetchScalarTest("select postalcode from addresses where ownertype='order' and ownerid = %d and addresstype = %d", $o_id, ADDRESS_DELIVERY)));			
			if (is_array($m_override) && $by_pc = $this->fetchSingleTest('select * from custom_product_by_pc where member_product_option_id = %d and ((from_postal_code = "%s" and to_postal_code = "%s") or (from_postal_code = "%3$s" and to_postal_code = "%2$s"))', $m_override["id"], $from_pc, $to_pc)) {
				...
				if (is_array($g_override)) {					
					if ($by_pc = $this->fetchSingleTest('select * from custom_product_by_pc where member_product_option_id = %d and ((from_postal_code = "%s" and to_postal_code = "%s") or (from_postal_code = "%3$s" and to_postal_code = "%2$s"))', $g_override["id"], $from_pc, $to_pc)) {
			...
			if ($fuelExempt) {
				...
				if (!$this->fetchSingleTest("select * from order_lines where order_id = %d and product_id = %d and deleted = 0", $o_id, $group["custom_fuel"])) {
					$recalc = true;
					$line_id = $this->fetchScalarTest("select max(line_id) from order_lines where order_id = %d", $o_id);
					$value = $this->fetchScalarTest("select coalesce(sum(value),0) from order_lines ol, product p where ol.order_id = %d and ol.deleted = 0 and ol.product_id != %d and p.id = ol.product_id and p.custom_commission = 1 and p.custom_has_fuel_surcharge = 1", $o_id, $group["custom_fuel"]);

					$fuel = $this->fetchSingleTest("select * from product where id = %d", $group["custom_fuel"]);
					$fuelCharge = max(0,round(($fuel["custom_minimum_charge"] + $group["custom_fuel_override"] + $member["custom_fuel_override"])/100,2));

					$tmp = $this->fetchScalarTest("select custom_fuel_rate from orders where id = %d", $o_id);
					...
					$fuelCharge = $this->fetchScalarTest("select price from order_lines where order_id = %d and product_id = %d and deleted = 0", $o_id, $group["custom_fuel"]);
				}
			}
		}
		else {
		...
		foreach($actions as $key=>$rec) {
			...
			$recs = $this->fetchAllTest("select o.*, p.custom_driver_split, p.custom_biker_split2, p.is_fedex, p.custom_split_allocation, p.custom_has_fuel_surcharge from order_lines o, product p where o.deleted = 0 and o.order_id = %d and p.id = o.product_id and p.custom_commission = 1 and (o.custom_package in ('S','P') or (o.custom_package = 'A' and p.custom_split_allocation in ('B','%s')))",$o_id, $rec["service_type"]);
			$fuel = 0;
			foreach($recs as $k=>$v) {
			...
			//
			if ($fuelCharge >= 0.01) {
				$driver = $this->fetchSingleTest("select d.*, v.fuel_charge from drivers d, vehicles v where d.id = %d and v.id = d.vehicle_id", $rec["driver_id"]);
				$value = $this->fetchScalarTest("select coalesce(sum(value),0) from order_lines ol, product p where ol.order_id = %d and ol.deleted = 0 and ol.product_id != %d and p.id = ol.product_id and p.custom_commission = 1 and p.custom_has_fuel_surcharge = 1", $o_id, $group["custom_fuel"]);
    ...
    function calcDriverCommission($d_id) {
		...
		//
		$r = $this->fetchSingleTest("select * from custom_delivery where id = %d",$d_id);
		$d = $this->fetchSingleTest("select * from drivers where id = %d",$r["driver_id"]);
		$o = $this->fetchSingleTest("select * from orders where id = %d",$r["order_id"]);
		$f = $this->fetchSingleTest("select ol.* 
...
$r["order_id"], $o["member_id"]);
    ...
    function checkDowntown($m_id, $quote, $product, $fromZone, $toZone, $pickupDriver, $deliveryDriver ) {
		...
		elseif (is_array($fromZone) && is_array($toZone) && array_key_exists("downtown", $fromZone) && $fromZone["downtown"] == 0 && array_key_exists("downtown", $toZone) && $toZone["downtown"] == 0) {
			...
			if (is_array($fromZone) && $fromZone["downtown"] == 1 || is_array($toZone) && $toZone["downtown"] == 1) {
				if ($this->fetchScalarTest("select custom_downtown_surcharge from members where id = %d", $m_id) == 0) {
					$this->logMessage(__FUNCTION__,sprintf("no surcharge for this member"),1);
				}
				else {
					if (!$grp_override = $this->fetchScalarTest("select downtown_surcharge from custom_member_product_options cpo, members_by_folder mf where mf.member_id = %d and cpo.member_id = mf.folder_id and cpo.isgroup = 1 and cpo.product_id = %d", $m_id, $product["id"])) $grp_override = 0;
					if (!$mbr_override = $this->fetchScalarTest("select downtown_surcharge from custom_member_product_options cpo, members_by_folder mbf where mbf.member_id = %d and cpo.member_id = mbf.id and cpo.isgroup = 0 and cpo.product_id = %d", $m_id, $product["id"])) $mbr_override = 0;
    ...
    function recalcDowntown($o_id) {
		$order = $this->fetchSingleTest("select * from orders where id = %d", $o_id);
		$customer = $this->fetchSingleTest("select * from members where id = %d", $order["member_id"]);
		if ($customer["custom_downtown_surcharge"] == 0) {			
			if ($car = $this->fetchSingleTest("select * from order_lines where order_id = %d and product_id in(%d,%d) and deleted = 0", $o_id, DOWNTOWN_PU_ID, DOWNTOWN_DEL_ID)) {
				$this->execute(sprintf("delete from order_lines where order_id = %d and id = %d", $o_id, $car["id"]));
				$this->initRecalc($o_id);
			}
			return;
		}
		$addresses = $this->fetchAllTest("select a.*, l.extra from addresses a left join code_lookups l on a.addresstype = l.id where ownertype='order' and ownerid = %d",$o_id);
		foreach($addresses as $k=>$v) {
			switch($v["addresstype"]) {
			case ADDRESS_PICKUP:
				$fromZone = $this->fetchSingleTest("select zf.*, f.downtown from zone_fsa zf, fsa f where f.fsa='%s' and zf.fsa_id = f.id",strtoupper(substr($v["postalcode"],0,3)));
				break;
			case ADDRESS_DELIVERY:
				$toZone = $this->fetchSingleTest("select zf.*, f.downtown from zone_fsa zf, fsa f where f.fsa='%s' and zf.fsa_id = f.id",strtoupper(substr($v["postalcode"],0,3)));
				break;
			default:
				break;
			}
		}
		$pu_driver = $this->fetchSingleTest("select d.*, v.name as vehicleType from drivers d, custom_delivery cd, vehicles v where cd.order_id = %d and service_type='P' and d.id = cd.driver_id and v.id = d.vehicle_id", $o_id);
		$del_driver = $this->fetchSingleTest("select d.*, v.name as vehicleType from drivers d, custom_delivery cd, vehicles v where cd.order_id = %d and service_type='D' and d.id = cd.driver_id and v.id = d.vehicle_id", $o_id);
		$product = $this->fetchSingleTest("select p.* from product p, order_lines o where order_id = %d and custom_package='S' and p.id = o.product_id and o.deleted = 0", $o_id);
		...
		$c = $this->fetchSingleTest("select * from order_lines where order_id = %d and product_id = %d and deleted = 0", $o_id, $quote["downtownChargeType"]);
		...
		if ($quote["downtownCharge"] < .01) {
			...
			if (!($c = $this->fetchSingleTest("select * from order_lines where order_id = %d and product_id = %d and deleted = 0", $o_id, $quote["downtownChargeType"]))) {
				$e = new Ecom();
				$tmp = $e->initCart();
				$tmp["header"]["id"] = $o_id;

				if (!$address = $this->fetchSingleTest('select * from addresses where ownerid = %d and ownertype = "order" and addresstype=12',$o_id))
				...
				$line = array("product_id"=>$quote["downtownChargeType"],"quantity"=>1,"price"=>$tmp["header"]["value"],
					"order_id"=>$o_id, "line_id"=>$this->fetchScalarTest("select max(line_id) from order_lines where order_id = %d", $o_id)+1,
    ...
    function initRecalc($o_id) {
		...
		$tmp = Ecom::initCart();
		$products = $this->fetchAllTest('select p.*, options_id, color, size, l.price, l.quantity, l.value, l.shipping, l.discount_value, l.recurring_discount_value, l.total, l.line_id, l.taxes, l.product_id from product p, order_lines l where l.order_id = %d and l.deleted = 0 and p.id = l.product_id',$o_id);
		foreach($products as $k=>$v) {
			$key = sprintf("%d|%d|%d|%d", $v["id"], $v["options_id"], $v["color"], $v["size"]);
			$tmp["products"][$key] = $v;
			$tmp["products"][$key]["taxdata"] = $this->fetchAllTest("select * from order_taxes where order_id = %d and line_id = %d", $o_id, $v["line_id"]);
		}
$this->logMessage(__FUNCTION__, sprintf("*** initRecalc tax check [%s]", print_r($tmp,true)), 1);
		$tmp['taxes'] = $this->fetchAllTest('select * from order_taxes ot where ot.order_id = %d and ot.line_id in (select o.line_id from order_lines o where o.order_id = ot.order_id and o.deleted = 0)',$o_id);
		if (!$address = $this->fetchSingleTest('select * from addresses where ownerid = %d and ownertype = "order" and tax_address=1',$o_id))
			$address = array('id'=>0,'province_id'=>0,'country_id'=>0);
		$tmp['addresses']['shipping'] = $address;
//		if (!$address = $this->fetchSingleTest('select * from addresses where ownerid = %d and ownertype = "order" and tax_address=0',$o_id))
//			$address = array('id'=>0,'province_id'=>0,'country_id'=>0);
//		$tmp['addresses']['billing'] = $address;
		$tmp['header'] = $this->fetchSingleTest("select * from orders where id = %d", $o_id);
    ...
    function sendText($d_id, $form, $fields) {
		...
		if (!$delivery = $this->fetchSingleTest("select cd.*, d.member_id, m.firstname, m.lastname from custom_delivery cd, drivers d, members m where cd.id = %d and d.id = cd.driver_id and m.id = d.member_id", $d_id)) {
		...
		if ($address = $this->fetchSingleTest("select * from addresses where ownertype='member' and ownerid = %d and phone1 != '' and deleted = 0 and addresstype = %d", $delivery["member_id"], ADDRESS_COMPANY)) {

			$body = new Forms();
			$body->init($form);
			$flds = $body->buildForm($fields);
			$delivery["address"] = Address::formatData($this->fetchSingleTest("select * from addresses where ownertype='order' and ownerid = %s and addresstype=%d",
    ...
    function sendAlert($d_id, $form, $fields) {
		$this->logMessage(__FUNCTION__,sprintf("d_id [%s] form [%s] fields [%s]", $d_id, print_r($form,true), print_r($fields,true)), 1);
		if (!$delivery = $this->fetchSingleTest("select cd.* from custom_delivery cd where cd.id = %d", $d_id)) {
			$this->logMessage(__FUNCTION__,sprintf("no assigned driver"),1);
			return;
		}
		$body = new Forms();
    ...
    function fedExRates($cart, $quote) {
		...
		$allowedZones = $this->fetchScalarAllTest("select zone_id from zones_by_folder z, members m where m.id = %d and z.folder_id = m.custom_zones", $this->getUserInfo("id",true));
...
*/
		...
		if (is_array($fromZone)) {
			...
			$request['RequestedShipment']['Recipient'] = array(
				'Address'=> array(
					...
					'CountryCode' => $this->fetchScalarTest('select country_code from countries where id = %d',$cart['addresses']['shipping']['country_id']),
					'Residential' => $cart['addresses']['shipping']['residential']
				)
			);
			if ($province = $this->fetchScalarTest('select province_code from provinces where id=%d',$cart['addresses']['shipping']['province_id']))
		...
		foreach($quote["prod"] as $k=>$p) {
			if ($xxx = $this->fetchSingleTest("select * from product where id = %d", $p["product_id"]))
			...
			if (!($response->HighestSeverity == 'FAILURE' || $response->HighestSeverity == 'ERROR')){
				...
				if (array_key_exists("custom",$cart) && array_key_exists("rates",$cart["custom"]) && count($cart["custom"]["rates"]) > 0) {
					foreach($cart["custom"]["rates"] as $k=>$r) {
						if ($c = $this->fetchSingleTest("select * from code_lookups where type='ship_via' and code = '%s'", $k)) {
							$e = explode("|",$c["extra"]);
							if ($e[1] > 0) {
								$p = $this->fetchSingleTest("select p.* from product p, product_by_folder pf, custom_member_product_options cpo, members_by_folder mf where p.deleted = 0 and p.enabled = 1 and p.id = %d and pf.product_id = p.id and pf.folder_id = %d and cpo.product_id = p.id and mf.member_id = %d and cpo.isgroup = 1 and cpo.member_id = mf.folder_id",
										$e[1], FEDEX_PRODUCTS, $this->getUserInfo("id",TRUE)); 
								$s = $this->fetchSingleTest("select cpo.* from product p, product_by_folder pf, custom_member_product_options cpo, members_by_folder mf where p.deleted = 0 and p.enabled = 1 and p.id = %d and pf.product_id = p.id and pf.folder_id = %d and cpo.product_id = p.id and cpo.member_id = mf.id and mf.member_id = %d and cpo.isgroup = 0",
										$e[1], FEDEX_PRODUCTS, $this->getUserInfo("id",TRUE));
								$g = $this->fetchSingleTest("select cpo.* from product p, product_by_folder pf, custom_member_product_options cpo, members_by_folder mf where p.deleted = 0 and p.enabled = 1 and p.id = %d and pf.product_id = p.id and pf.folder_id = %d and cpo.product_id = p.id and mf.member_id = %d and cpo.isgroup = 1 and cpo.member_id = mf.folder_id",
										$e[1], FEDEX_PRODUCTS, $this->getUserInfo("id",TRUE)); 
								if (is_array($p)) {
...
//				'net'=>$rate->ShipmentRateDetail->TotalNetFedExCharge->Amount,
    ...
    function parseFedexSurcharge($v, $tmp, $cart) {
		if ($sc = $this->fetchSingleTest("select * from code_lookups where type='ship_via' and code = '%s'", $v->SurchargeType)) {
			$this->logMessage(__FUNCTION__, sprintf("found a surcharge [%s] from [%s]", print_r($sc,true), print_r($v,true)), 2);
			$cd = explode("|",$sc["extra"]);
			if ($surcharge = $this->fetchSingleTest("select * from product where id = %d and deleted = 0 and published = 1 and enabled = 1", $cd[1])) {
				//
    ...
    function byKmOverride($product ) {
		...
		if (!$junction = $this->fetchSingleTest("select * from members_by_folder where member_id = %d limit 1", $m_id)) return $default;
		$grp = $this->fetchSingleTest("select * from custom_member_product_options where isgroup = 1 and member_id = %d and product_id = %d", $junction["folder_id"], $product["id"]);
		$mbr = $this->fetchSingleTest("select * from custom_member_product_options where isgroup = 0 and member_id = %d and product_id = %d", $junction["id"], $product["id"]);
...
/**
 * Create prepared statements to write safe SQL queries
 */
class preparedStatement extends Common {
	...
	function fetchSingle($sql=null,$handle=null) {
		if (!is_bool($this->m_statement)) {
			if (!$this->m_statement->execute()) {
				$this->logDBMessage('fetchSingle', sprintf("Error: sql[%s] Msg[%s]", $sql, $this->m_handle->error), 1);
				return false;
			}
			else {
				$result = $this->m_statement->get_result();				
				return $result;
			}
		} else {
			$this->logDBMessage('fetchSingle', 'Error: invalid statement handle', 1);
			return false;
		}
	}
...
/**
 * Common ajax functionality for backend and frontend
 */
...
/**
 * Common inventory functionality
 */
class Inventory extends Common {
    ...
    static function updateInventory($inventory_id, $quantity, $reference_id, $comments ) {
		...
		$inv = $c->fetchSingleTest('select * from product_inventory where id = %d',$inventory_id);
		//$c->beginTransaction();
...
class Address extends Common {
    ...
    static function formatData($data) {
		...
		if (array_key_exists('province_id',$data) && $p = $c->fetchSingleTest('select * from provinces where id = %d',$data['province_id'])) {
		...
		if ($ctry = $c->fetchSingleTest('select * from countries where id = %d',$data['country_id'])) {
		...
		if (array_key_exists("addresstype",$data))
			$data["code"] = $c->fetchSingleTest("select * from code_lookups where id = %d", $data["addresstype"]);
    ...
    static function formatAddress($id, $formatType = null) {
		...
		$return = '';
		$addr = array();
		if ($address = $c->fetchSingleTest('select * from addresses where id = %d',$id)) {
			if (strlen($address['province_id']) > 0 && $prov = $c->fetchSingleTest('select * from provinces where id = %d',$address['province_id'])) {
			...
			if (strlen($address['country_id']) > 0 && $country = $c->fetchSingleTest('select * from countries where id = %d',$address['country_id'])) {
...
class Ecom extends Common {
    ...
    static function initCart($passedId = null) {
		...
		if (array_key_exists('user',$_SESSION)) {
			//if ($tmp = $c->fetchSingleTest('select * from addresses where deleted = 0 and ownertype = "member" and ownerid = %d and addresstype=%d',$passedId,ADDRESS_PICKUP))
			//	$cart['addresses']['pickup'] = $tmp;
			//if ($tmp = $c->fetchSingleTest('select * from addresses where deleted = 0 and ownertype = "member" and ownerid = %d and addresstype=%d',$passedId,ADDRESS_DELIVERY))
			//	$cart['addresses']['shipping'] = $tmp;
			//if ($tmp = $c->fetchSingleTest('select * from addresses where deleted = 0 and ownertype = "member" and ownerid = %d and addresstype=%d',$passedId,ADDRESS_BILLING))
    ...
    function updateLine($cart, $key, $p_id, $opt, $color, $size, $qty, $r_prd, $r_qty, $msg) {
		...
		if (!array_key_exists($key,$cart['products'])) {
			$prod = $this->fetchSingleTest('select * from product where id = %d',$p_id);
		...
		$invAmt = $this->fetchSingleTest("select * from product_inventory where product_id = %d and options_id = %d and color = %d and size = %d and (start_date = '0000-00-00' or start_date <= CURDATE()) and (end_date = '0000-00-00' or end_date >= CURDATE())",$p_id,$opt,$color,$size);
		...
		if ($opt > 0)
			$cart['products'][$key]['qty_multiplier'] = $this->fetchScalarTest('select qty_multiplier from product_options where id = %d',$opt);
		else
		...
		if ($r_prd > 0 && $r_qty > 0) {
			...
			$tmp = $this->fetchSingleTest('select * from product_recurring where id = %d',$r_prd);
    ...
    function updateCart($cart=array(), $toUpdate = null) {
		...
		if (array_key_exists('currency_id',$request)) {
			if (!array_key_exists('currency_id',$cart['header']) || $cart['header']['currency_id'] != $request['currency_id']) {
				...
				$tmp = $this->fetchSingleTest('select e.*, l.extra from exchange_rate e, code_lookups l where l.id = %d and e.currency_id = l.id order by effective_date desc limit 1',$request['currency_id']);
			...
			if (array_key_exists('quantity',$request)) {
				foreach($request['quantity'] as $key=>$value) {
					if ($value == 0 || !array_key_exists($key,$cart["products"])) {
						...
						$invAmt = $this->fetchSingleTest("select i.* from product_inventory i where product_id = %d and options_id = %d and color = %d and size = %d and (start_date = '0000-00-00' or start_date <= CURDATE()) and (end_date = '0000-00-00' or end_date >= CURDATE())",$tmpProd,$tmpOption,$tmpColor,$tmpSize);
						if (is_array($invAmt) && count($invAmt) > 0) {
			...
			if (array_key_exists('options_id',$request)) {
				foreach($request['options_id'] as $key=>$value) {
					if (array_key_exists($key,$cart['products']) && $value > 0) {
						...
						$cart['products'][$key]['qty_multiplier'] = $this->fetchScalarTest('select qty_multiplier from product_options where id = %d',$value);
					}
				}
			}
		...
		if (array_key_exists('discount_code',$request) && strlen($request['discount_code']) > 0) {			
			if (!$code = $this->fetchSingleTest('select * from coupons where code = "%s" and enabled = 1 and published = 1 and (start_date = "0000-00-00 00:00:00" or start_date <= now()) and (end_date = "0000-00-00 00:00:00" or end_date >= now())',$request['discount_code'])) {
				...
				$valid = true;
				$header = true;
				if ($products = $this->fetchScalarAllTest('select owner_id from relations where owner_type = "product" and related_type = "coupon" and related_id = %d',$code['id'])) {
					...
					if ($folders = $this->fetchScalarAllTest('select owner_id from relations where owner_type = "productfolder" and related_type = "coupon" and related_id = %d',$code['id'])) {
						$valid = false;
						...
						foreach($folders as $folder) {
							foreach($cart['products'] as $key=>$line) {
								if ($member = $this->fetchSingleTest('select 1 from product_by_folder where folder_id = %d and product_id = %d',$folder['owner_id'],$line['product_id'])) {
									$valid = true;
									$cart['products'][$key]['coupon_id'] = $code['id'];
							    $cart['products'][$key]['discount_rate'] = $code['amount'];
    							$cart['products'][$key]['discount_type'] = $code['percent_or_dollar'];
		...
		foreach($cart['products'] as $key=>$item) {
			...
			$item['points'] = 0;
			if ($pricing = $this->fetchSingleTest($sql)) {
			...
			if ($item['options_id'] != 0) {
				if ($options = $this->fetchSingleTest('select * from product_options where product_id = %d and id = %d',$item['product_id'],$item['options_id'])) {
			...
			if ($item['recurring_period'] > 0 && $item['coupon_id'] == 0) {
				$tmp = $this->fetchSingleTest('select * from product_recurring where id = %d',$item['recurring_period']);
		...
		if ($cart['header']['coupon_id'] > 0) {			
			$code = $this->fetchSingleTest('select * from coupons where id = %d and enabled = 1 and published = 1 and (start_date = "0000-00-00 00:00:00" or start_date <= now()) and (end_date = "0000-00-00 00:00:00" or end_date >= now())',$cart['header']['coupon_id']);
    ...
    static function updateAbandoned($cart) {
		...
		$tmp = $c->fetchAllTest('select id,line_id from cart_lines where order_id=%d',$cart['abandoned']['id']);
		$cart['abandoned']['products'] = array();
    ...
    static function lineValue($data) {
		...
		$tmp = $data;
		//if (!$coupon = $c->fetchSingleTest('select * from coupons where id = %d',$data['coupon_id']))
		...
		$taxamt = 0;
		$product = $c->fetchSingleTest('select * from product where id = %d',$tmp['product_id']);
		$address = array();
		if (array_key_exists('order_id',$tmp)) {
			//if ($c->fetchScalarTest("select count(id) from addresses where country_id > 1 and ownertype='order' and ownerid=%d", $tmp["order_id"]) > 0)
			//	$address = array();
			//else
				$address = $c->fetchSingleTest('select * from addresses where ownerid = %d and ownertype = "order" and addresstype = %d',$tmp['order_id'], ADDRESS_DELIVERY);
		}
		else {
...
*/
		...
		if (count($address) > 0) {
			...
			$taxes = $c->fetchAllTest('select * from taxes where province_id = %d and deleted = 0 and 1=1',$prov);
			$p_tmp = $product; 
...
if (!array_key_exists("total",$line)) $c->logMessage(__FUNCTION__,sprintf("missing taxes for [%s]", print_r($line,true)), 1);
		...
		if ($order['header']['points_redeemed'] > 0) {
			$prices = $c->fetchAllTest("select * from product_pricing pr, product p where p.code = '%s' and pr.product_id = p.id order by pr.min_quantity",REDEMPTION_PRODUCT);
			$amt = 0;
		...
		if (array_key_exists('coupon_id',$order['header']))
			$coupon = $c->fetchSingleTest('select * from coupons where id = %d',$order['header']['coupon_id']);
		else
			$coupon = array('shipping_only'=>0);
		$taxable_goods = array();
		...
		if (array_key_exists('id',$order['header']))
			$address = $c->fetchSingleTest('select * from addresses where ownerid = %d and ownertype = "order" and addresstype=%d',$order['header']['id'],ADDRESS_DELIVERY);
		else {
		...
		if (count($address) > 0 ) {
			$prov = $address['province_id'];
			$taxes = $c->fetchAllTest('select * from taxes where province_id = %d and deleted = 0 and 2=2',$prov);
			foreach($taxes as $tax) {
    ...
    static function getPricing($prod_id, $qty, $order_date) {
		...
		if ($pricing = $c->fetchSingleTest('select * from product_pricing where product_id = %d and min_quantity <= %d and max_quantity >= %d',$prod_id,$qty,$qty)) {
			$prod = $c->fetchSingleTest('select * from product where id = %s',$prod_id);
    ...
    static function recalcOrderFromDB($order_id ) {
		...
		foreach($c->fetchAllTest("select * from order_lines ol where ol.order_id = %d and ol.deleted = 0 order by line_id", $order_id) as $k=>$v) {
			$key = sprintf("%d|%d|%d|%d", $v["product_id"], $v["options_id"], $v["color"], $v["size"]);
			$cart["products"][$key] = $v;
			$cart["products"][$key]["taxdata"] = $c->fetchAllTest("select * from order_taxes where order_id = %d and line_id = %d", $order_id, $v["line_id"]);
			$cart["products"][$key]["dimensions"] = $c->fetchAllTest("select * from order_lines_dimensions where order_id = %d and line_id = %d", $order_id, $v["line_id"]);
			$cart["products"][$key] = Ecom::lineValue($cart["products"][$key]);
		}
		$cart["header"] = $c->fetchSingleTest("select * from orders where id = %s", $order_id);
		foreach($c->fetchAllTest("select * from addresses where ownertype='order' and ownerid = %d", $order_id) as $k=>$v) {
...
/**
 * Common image resize functionality
 */
