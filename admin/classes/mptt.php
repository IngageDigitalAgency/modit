...
class mptt extends Common {
	...
	function add($parent, $child_num = 0, $misc_data = false) {
		...
		if($parent != 0)
		{
			...
			if (!($parent = $this->fetchSingleTest($sql)))
				return false;
		}
		else
		{
	...
	function get_children($left_id, $right_id, $level) {
		...
		$children = $this->fetchAllTest($sql);
		return $children;
	}
    ...
    function fetchChildren($id) {
		$data = $this->fetchSingleTest('select * from %s where %s = %d', $this->table, $this->pkey, $id);
