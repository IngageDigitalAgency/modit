...
/**
 * Class to modify an html element
 */
...
/**
 * Manages tags for html elements
 */
...
/**
 * Class for formatted tag fields
 */
    ...
    public function show($data = null) {
		if (is_null($data)) {
			if ($this->hasAttribute("sql")) {
				$data = $this->fetchScalarTest($this->getAttribute('sql'));
...
/**
 * Error Class for reformatted tags
 */
...
class select extends input  {
    ...
    function show($data = null) {
		...
		if (count($this->m_options) == 0) {
			...
			if ($this->hasAttribute('sql')) {
				$tmp = $this->fetchOptionsTest($this->getAttribute('sql'));
			...
			if ($this->hasAttribute('optionslist')) {
				...
				if ($tmp['table'] == 'content' && $this->hasAttribute('addContentClass')) {
					foreach($nodes as $key=>$value) {
						$classes[$key] = $this->fetchScalarTest('select type from %s where id = %d',$tmp['table'],$key);
					}
				}
...
class countrySelect extends select {
    ...
    function show($data = null) {
		if (count($this->m_options) == 0) {
			if ($this->hasAttribute('sql')) {
				$this->addOptions($this->fetchOptionsTest($this->getAttribute('sql')));
			}
			else {
				if (is_null($data)) {
					$data = $this->fetchScalarTest('select id from countries where deleted = 0 order by sort limit 1');
				}
				$this->addOptions($this->fetchOptionsTest('select id,country from countries where deleted = 0 order by sort'));
			}
		}
		return parent::show($data);
	}
...
/**
 * Prepare provinceSelect element
 */
    ...
    function show($data = null) {
		if (count($this->m_options) == 0) {
			if (!$this->hasAttribute('country_id')) {
				$this->addAttribute('country_id',$this->fetchScalarTest('select id from countries where deleted = 0 order by sort limit 1'));
			...
			if ($this->hasAttribute('sql')) {
				...
				$this->addOptions($this->fetchOptionsTest($sql));
			}
		}
...
/**
 * Prepare unordered list element
 */
...
/**
 * Prepare product option group element
 */
class productOptGroup extends HtmlElement {
    ...
    function subOptGroup($fldr, $data) {
		...
		if ($this->hasAttribute("where")) {
			$prods = $this->fetchAllTest("select p.* from product p, product_by_folder pf where pf.folder_id = %d and p.id = pf.product_id and %s", $fldr["id"], $this->getAttribute("where"));
		}
		else {
			$prods = $this->fetchAllTest("select p.* from product p, product_by_folder pf where pf.folder_id = %d and p.id = pf.product_id", $fldr["id"]);
		}
