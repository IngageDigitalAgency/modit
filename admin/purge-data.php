...
class Purge extends Common {
    ...
    public function doIt($dt) {
		$recurring = array_merge(array(0),$this->fetchScalarAllTest("select cast(o1.authorization_transaction as integer) from orders o1 where o1.order_status & %d and cast(o1.authorization_transaction as integer) > 0 and order_date < '%s'", STATUS_RECURRING, $dt));
		$orders = array_merge(array(0),$this->fetchScalarAllTest("select id from orders where order_date < '%s' and id not in (%s) and (order_status & %d) = 0 and (order_status & %d or order_status & %d or order_status & %d)", $dt, implode(", ", $recurring), STATUS_RECURRING, STATUS_SHIPPED, STATUS_CANCELLED, STATUS_INCOMPLETE));
		$del_ids = array_merge(array(0),$this->fetchScalarAllTest("select id from custom_delivery where order_id in (%s)", implode(", ", $orders)));
