...
/**
 * Common newsletter functionality
 */
class Newsletter extends Common {
     ...
     */
    function process() {
		$this->logMessage('process','start processing',1,true,true);
echo "test";
		$safety = 0;
		while ($safety < 2 && $batches = $this->fetchSingleTest('select b.*, n.page_id, n.title, n.from_name, n.from_email, n.customized, n.teaser, n.description, c.search_title, 1 as isMailing from newsletter_batch b, newsletter n, content c where b.newsletter_id = n.id and c.id = n.page_id and started != "0000-00-00 00:00:00" and completed = "0000-00-00 00:00:00" and aborted = "0000-00-00 00:00:00" order by id')) {
			...
			$userList = $this->fetchScalarAllTest('select subscribers_id from newsletter_by_subscribers where newsletter_id = %d',$batches['newsletter_id']);
			if (count($userList) == 0)
			...
			$page = $this->fetchSingleTest('select * from content where id = %d',$batches['page_id']);
			...
			if ($snoopy->status == 200) {
				...
				while ((!$killIt) && $list = $this->fetchAllTest('select b.*, s.firstname, s.lastname, s.email from newsletter_batch_subscriber b, subscriber s where batch_id = %d and sent = "0000-00-00 00:00:00" and s.id = b.subscriber_id order by random_id limit 50',$batches['id'])) {
					...
					foreach($list as $subscriber) {
						...
						if ($status = $mailer->Send()) {
							$this->logMessage('process',sprintf('mailed to %s %s <%s>',$subscriber['firstname'],$subscriber['lastname'],$subscriber['email']),3,true);
						}
						else {
							$this->logMessage('process',sprintf('mail failed to %s %s <%s> mailer [%s]',$subscriber['firstname'],$subscriber['lastname'],$subscriber['email'], print_r($mailer,true)),3,true,true);
						}
						$this->execute(sprintf('update newsletter_batch_subscriber set sent=now(), success=%d where id=%s',$status?1:0,$subscriber['id']));
						$aborted = $this->fetchScalarTest('select aborted from newsletter_batch where id = %d',$batches['id']);
