...
$path = str_replace("/home/vhosts","",$path);
$dbg = array();
$dbg[] = sprintf("path is now [%s] childPath [%s]", $path, $childPath);
		if ($this->fileSystems) {
			$this->createFileSystems();

			// Is a http URL then ask each file systems URL resolver for a file
			if (preg_match('/^https?:\/\//', $path)) {
$dbg[] = sprintf("found https in %s", $path);
				$file = null;
			...
			if ($childPath) {
$dbg[] = sprintf("found childpath in %s^%s", $path, $childPath);
...
$dbg[] = sprintf("found {num} in %s^%s", $path, print_r($matches,true));
...
$dbg[] = sprintf("matches: %s", print_r($matches,true));

			foreach ($this->fileSystems as $fileSystem) {
				$tmp = str_replace("/home/vhosts","",$fileSystem->getRootPath());
				$tmp1 = str_replace("//","/",$tmp);
$dbg[] = sprintf("test path [%s] scheme [%s] [%s] rootPath [%s vs %s vs %s]", $path, print_r($scheme,true), $fileSystem->getScheme(), $fileSystem->getRootPath(),$tmp, $tmp1);

//			throw new MOXMAN_Exception(
//				sprintf("^^^ trace [%s] ", print_r($dbg,true)) ,
...
//			);
				...
				if (!$scheme || $fileSystem->getScheme() === $scheme) {
					...
					if (strpos($path, $rootPrefix) === 0) {
						...
						$dbg[] = sprintf("did not get to root path: [%s] rootPrefix: [%s]", $path, $rootPrefix);
					}
				}
			}
		}
	else {
		$dbg[] = PHP_EOL."no filesSystems";
		}
		// Could not resolve path no file system returned a file
		throw new MOXMAN_Exception(sprintf("Could not resolve path: %s^%s^%s^%s^%s", implode(PHP_EOL,$dbg), $path, $childPath, $rootPrefix, print_r($matches,true)));
	}
