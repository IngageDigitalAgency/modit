...
/**
 * Main Class for website frontend functionality
 */
class Frontend extends Common {
    ...
    public function __construct($init = false) {
		...
		$modules = $this->fetchAllTest('select * from modules where enabled = 1 and frontend = 1');
		if ($init) {
...
#		}
		...
		if (DEFINED('FRONTEND')) {
			...
			//if (array_key_exists('REDIRECT_URL',$_SERVER)) {
				$defunct = $this->fetchAllTest('select * from traffic_301');
				...
				$ignored = $this->fetchAllTest('select * from traffic_ignored_urls');
		...
		if (array_key_exists('module',$_REQUEST)) {
			switch($_REQUEST['module']) {
				...
				case 'page':
					...
					if (!$page = $this->fetchSingleTest($sql)) {						
						if ($link = $this->fetchSingleTest("select * from content where enabled = 1 and published = 1 and type = 'internallink' and (search_title = '%s' or seo_url = '%s')",$name,$name)) {
				...
				case 'advert':
					...
					$data = $this->fetchSingleTest('select * from advert where id = %d',$_REQUEST['advert_id']);
...
union (select * from news_folders where id = %d and template_id > 0) order by level desc limit 1",$_REQUEST['f_id'],$_REQUEST['f_id']);
						if ($folder = $this->fetchSingleTest($tmpsql)) {
						...
						if ($folder = $this->fetchSingleTest('select f.* from news_folders f, news_by_folder i where i.article_id = %d and f.id = i.folder_id order by rand() limit 1',array_key_exists('news_id',$_REQUEST) ? $_REQUEST['news_id'] : 0)) {
				...
				case 'category':
					...
					if (array_key_exists('cat_id',$_REQUEST) && $_REQUEST['cat_id'] > 0) {
						$cat = $this->fetchScalarTest("select template_id from product_folders where id = %d",$_REQUEST["cat_id"]);
						if ( $cat > 0) {
...
union (select * from product_folders where id = %d and details_id > 0) order by level desc limit 1",$_REQUEST['f_id'],$_REQUEST['f_id']);
						...
						if ($folder = $this->fetchSingleTest('select f.* from product_folders f, product_by_folder i where i.product_id = %d and f.id = i.folder_id order by rand() limit 1',array_key_exists('prod_id',$_REQUEST) ? $_REQUEST['prod_id'] : 0)) {
...
union (select * from members_folders where id = %d and template_id > 0) order by level desc limit 1",$_REQUEST['f_id'],$_REQUEST['f_id']);
						...
						if ($folder = $this->fetchSingleTest('select f.* from members_folders f, events_by_folder i where i.event_id = %d and f.id = i.folder_id order by rand() limit 1',array_key_exists('event_id',$_REQUEST) ? $_REQUEST['event_id'] : 0)) {
...
union (select * from store_folders where id = %d and template_id > 0) order by level desc limit 1",$_REQUEST['storecat'],$_REQUEST['storecat']);
						...
						if ($folder = $this->fetchSingleTest('select f.* from store_folders f, stores_by_folder i where i.store_id = %d and f.id = i.folder_id order by rand() limit 1',array_key_exists('store_id',$_REQUEST) ? $_REQUEST['store_id'] : 0)) {
...
//		if (array_key_exists('secure',$page)) {
    ...
    protected function isLoggedIn($admin = false) {
		...
		elseif (array_key_exists('user',$_SESSION) && is_array($_SESSION['user']) && array_key_exists('info',$_SESSION['user'])) {
			$user = $this->fetchSingleTest('select * from members where id = %d and deleted = 0 and enabled = 1 and (expires = "0000-00-00" || expires > curdate())',$_SESSION['user']['info']['id']);
    ...
    function getFileList() {
		...
		if ((int)$class > 0) {
			$class = $this->fetchScalarTest('select classname from modules where id = %d and enabled = 1 and frontend = 1',$class);
		}
		$form = new Forms();
    ...
    private function getFiles($dir, $level) {
		...
		$files = array(""=>"");
		if (is_dir($dir) && $dh = opendir(realpath($dir))) {
    ...
    function renderTemplate($data, $fromEditMode = false) {
		...
		if (!defined('FRONTEND')) {
			...
			if ($saving) {
				...
				//
				$template = $this->fetchSingleTest('select * from templates where id = %d',$_REQUEST['t_id']);
				$old_id = $template['id'];
				unset($template['id']);
				//
				//	i would normally expect this to be $tempalte['version']+1 but not necessarily
				//
				$version = $this->fetchScalarTest('select max(version) from templates where template_id = (select template_id from templates where id = %d)',$_REQUEST['t_id']);
				...
				if ($status) {
					...
					//
					$modules = $this->fetchAllTest('select * from modules_by_page where page_type = "T" and page_id = %d',$old_id);
				...
				if ($status && $overwrite) {
					$modules = $this->fetchAllTest('select * from modules_by_page where page_type = "T" and page_id = '.$t_id);
					...
					foreach($pages as $page) {
						...
						unset($tmp['id']);
						$tmp['version'] = $this->fetchScalarTest('select max(version) from pages where content_id = %d',$page['content_id'])+1;	//$tmp['version']+1;
				...
				if ($status && $changesOnly) {
					...
					foreach($pages as $page) {
						...
						$stmt->execute();
						$p_id = $stmt->insertId();
						$modules = $this->fetchAllTest('select * from modules_by_page where page_id = %d and page_type = "P"',$page['id']);
						$session = $_SESSION['changeModule']['T_'.$old_id];
						//
						...
						foreach($modules as $module) {
							...
							if(array_key_exists($module['module_name'],$session)) {
								if ((array_key_exists('fetemplate_id',$session[$module['module_name']]) && $session[$module['module_name']]['fetemplate_id'] != 0) || $session[$module['module_name']]['content_area'] != 0) {
									$module = $this->fetchSingleTest('select * from modules_by_page where page_type = "T" and page_id = %d and module_name = "%s"',$t_id,$module['module_name']);
									$module['page_type'] = "P";
									$module['page_id'] = $p_id;
									unset($module['id']);
									$stmt = $this->prepare(sprintf('insert into modules_by_page(%s) values(%s)',implode(',',array_keys($module)),str_repeat("?,",count($module)-1)."?"));
						...
						//
						$modules = $this->fetchScalarAllTest('select module_name from modules_by_page where page_type="P" and page_id = %d',$p_id);
						...
						foreach($session as $key=>$module) {
							...
							if ((array_key_exists('fetemplate_id',$module) && $module['fetemplate_id'] > 0) || $module['content_area'] != 0) {
								if (strpos($modules,'~'.$module['module_name'].'~') === false) {
									...
									$module = $this->fetchSingleTest('select * from modules_by_page where page_type = "T" and page_id = "%s" and module_name = "%s"',$t_id,$module['module_name']);
									$module['page_type'] = 'P';
									$module['page_id'] = $p_id;
									unset($module['id']);
									$stmt = $this->prepare(sprintf('insert into modules_by_page(%s) values(%s)',implode(',',array_keys($module)),str_repeat("?,",count($module)-1)."?"));
				...
				if ($status) {
					...
					$_REQUEST['t_id'] = $t_id;
					$data = $this->fetchSingleTest('select * from templates where id = %d',$_REQUEST['t_id']);
		...
		foreach($matches[0] as $key=>$match) {
			...
			if ((!defined('FRONTEND')) && 
				...
				if (array_key_exists('fetemplate_id',$module)) {
					$module['classname'] = $this->fetchScalarTest('select classname from modules m, fetemplates t where t.id = %d and m.id = t.module_id',$module['fetemplate_id']);
					$module['module_function'] = $this->fetchScalarTest('select module_function from fetemplates t where t.id = %d',$module['fetemplate_id']);
				}
				else {
					$module['classname'] = '';
		...
		if (array_key_exists('module',$_REQUEST)) {
			...
			case 'news':
				if (array_key_exists('news_id',$_REQUEST)) {
					if ($article = $this->fetchSingleTest("select * from news where deleted = 0 and enabled = 1 and published = 1 and id = %d",$_REQUEST["news_id"])) {
						$f_id = 0;
						if (array_key_exists("f_id",$_REQUEST)) $f_id = $_REQUEST["f_id"];
						if (array_key_exists("folder_id",$_REQUEST)) $f_id = $_REQUEST["folder_id"];
						if (!$folder = $this->fetchSingleTest("select * from news_folders where id = %d",$f_id)) {
			...
			case 'blogcat':
				if (array_key_exists("blogcat",$_REQUEST)) {
					if ($folder = $this->fetchSingleTest("select * from blog_folders where id = %d",$_REQUEST['blogcat'])) {
						$p = new blog(0);
			...
			case 'category':
				if (array_key_exists("cat_id",$_REQUEST)) {
					if ($folder = $this->fetchSingleTest("select * from product_folders where id = %d",$_REQUEST['cat_id'])) {
						$p = new product(0);
						$form->addData(array("folder"=>$p->formatFolder($folder)));
					}
				}
			...
			case "product":
				if (array_key_exists('prod_id',$_REQUEST)) {
					if ($product = $this->fetchSingleTest("select * from product where deleted = 0 and enabled = 1 and published = 1 and id = %d",$_REQUEST["prod_id"])) {
						$f_id = 0;
						if (array_key_exists("cat_id",$_REQUEST)) $f_id = $_REQUEST["cat_id"];
						if (array_key_exists("f_id",$_REQUEST)) $f_id = $_REQUEST["f_id"];
						if (array_key_exists("folder_id",$_REQUEST)) $f_id = $_REQUEST["folder_id"];
						if (!$folder = $this->fetchSingleTest("select * from product_folders where id = %d",$f_id)) {
							$folder = array("id"=>0,"title"=>"","level"=>0,"left_id"=>0,"right_id"=>0);
						}
						$p = new product(0);
			...
			case "blog":
				if (array_key_exists('blog_id',$_REQUEST)) {
					if ($blog = $this->fetchSingleTest("select * from blog where deleted = 0 and enabled = 1 and published = 1 and id = %d",$_REQUEST["blog_id"])) {
						$f_id = 0;
						if (array_key_exists("blog_cat",$_REQUEST)) $f_id = $_REQUEST["blog_cat"];
						if (array_key_exists("f_id",$_REQUEST)) $f_id = $_REQUEST["f_id"];
						if (array_key_exists("folder_id",$_REQUEST)) $f_id = $_REQUEST["folder_id"];
						if (!$folder = $this->fetchSingleTest("select * from blog_folders where id = %d",$f_id)) {
							$folder = array("id"=>0,"title"=>"","level"=>0,"left_id"=>0,"right_id"=>0);
						}
						$p = new blog(0);
			...
			case 'storecat':
				if (array_key_exists("folder_id",$_REQUEST)) {
					if ($folder = $this->fetchSingleTest("select * from store_folders where id = %d",$_REQUEST['folder_id'])) {
						$p = new stores(0);
						$form->addData(array("folder"=>$p->formatFolder($folder)));
					}
				}
			...
			case "store":
				if (array_key_exists('store_id',$_REQUEST)) {
					if ($store = $this->fetchSingleTest("select * from stores where deleted = 0 and enabled = 1 and published = 1 and id = %d",$_REQUEST["store_id"])) {
						$f_id = 0;
						if (array_key_exists("store_cat",$_REQUEST)) $f_id = $_REQUEST["store_cat"];
						if (array_key_exists("f_id",$_REQUEST)) $f_id = $_REQUEST["f_id"];
						if (array_key_exists("folder_id",$_REQUEST)) $f_id = $_REQUEST["folder_id"];
						if (!$folder = $this->fetchSingleTest("select * from store_folders where id = %d",$f_id)) {
							$folder = array("id"=>0,"title"=>"","level"=>0,"left_id"=>0,"right_id"=>0);
						}
						$p = new stores(0);
			...
			case 'member':
				if (array_key_exists("folder_id",$_REQUEST)) {
					if ($folder = $this->fetchSingleTest("select * from members_folders where id = %d",$_REQUEST['folder_id'])) {
						$p = new calendar(0);
						$form->addData(array("folder"=>$p->formatFolder($folder)));
					}
				}
			...
			case "event":
				if (array_key_exists('event_id',$_REQUEST)) {
					if ($event = $this->fetchSingleTest("select * from events where deleted = 0 and enabled = 1 and published = 1 and id = %d",$_REQUEST["event_id"])) {
						$f_id = 0;
						if (array_key_exists("event_cat",$_REQUEST)) $f_id = $_REQUEST["event_cat"];
						if (array_key_exists("f_id",$_REQUEST)) $f_id = $_REQUEST["f_id"];
						if (array_key_exists("folder_id",$_REQUEST)) $f_id = $_REQUEST["folder_id"];
						if (!$folder = $this->fetchSingleTest("select * from members_folders where id = %d",$f_id)) {
							$folder = array("id"=>0,"title"=>"","level"=>0,"left_id"=>0,"right_id"=>0);
						}
						$p = new calendar(0);
    ...
    function renderPage($data, $fromEditMode = false) {
		...
		if (!defined('FRONTEND') && $fromEditMode == true) {
			...
			if ($updating) {
				...
				$this->beginTransaction();
				$template = $this->fetchSingleTest('select * from pages where id = %d',$_REQUEST['p_id']);
				$old_id = $template['id'];
				unset($template['id']);
				//
				//	i would normally expect this to be $template['version']+1 but not necessarily
				//
				$version = $this->fetchScalarTest('select max(version) from pages where content_id = %d',$template['content_id']);
				$template['version'] = $version+1;
				$template['created'] = date(DATE_ATOM);
				$insert = array();
				foreach($template as $fld=>$value) {
			...
			if ($saving) {
				...
				if ($status) {
					...
					//	grab the current modules & compare to the session ones
					//
					$modules = $this->fetchAllTest('select * from modules_by_page where page_type = "P" and page_id = %d',$old_id);
			...
			if ($deleting) {
				$status = true;
				$content_id = $this->fetchScalarTest('select content_id from pages where id = %d',$_REQUEST['p_id']);
				$versions = $this->fetchScalarTest('select count(0) from pages where content_id = %d',$content_id);
				$this->logMessage(__FUNCTION__,sprintf("delete content_id [%d] versions [%d]",$content_id,$versions),3);
				if ($versions == 1) {
					$links = $this->fetchScalarAllTest('select title from content where internal_link = %d',$content_id);
					...
					if (count($links) > 0) {
						...
						$children = $this->fetchScalarTest('select count(0) from content c1, content c2 where c1.id = %d and c2.left_id > c1.left_id and c2.right_id < c1.right_id and c2.level > c1.level',$content_id);
						if ($children > 0) {
    ...
    function processQueryData($data) {
		...
		if ($search = $this->fetchSingleTest('select * from search_groups where id = %d',$this->m_searchWinner)) {
		...
		if ($modules = $this->fetchAllTest($sql)) {
    ...
    function renderModule() {
		...
		if (array_key_exists($data['module_name'],$_SESSION['changeModule'][$key])) {
			...
			if (!($module = $this->fetchSingleTest($sql))) {
		...
		if (class_exists($module['classname'])) {
			...
			if ($module['content_area']) {
				if ((!defined('FRONTEND')) && array_key_exists($key,$_SESSION['changeModule'])) {
					...
					$html = $module['content'];	//$this->fetchScalarTest('select content from pages where id = %d',$data['page_id']);
				}
				//
				//	embed the tinymce editor within a normal change module structure
				//
    ...
    function changeContent() {
		...
		if ($_REQUEST['pageContent']) {
			$data = $this->fetchSingleTest('select * from pages where id = %d',$_REQUEST['page_id']);
			$form->addData($data);
			...
			if (array_key_exists('id',$_REQUEST) && $data = $this->fetchSingleTest('select * from modules_by_page where id = %d',$_REQUEST['id'])) {
    ...
    function changeModule($data = array()) {
		...
		if ($this->isAjax()) {
			...
			if (($_REQUEST['reset'] == 1 && array_key_exists($tmp,$_SESSION['changeModule'][$session_key])) || array_key_exists($tmp,$_SESSION['changeModule'][$session_key])) {
				...
				if (!($data = $this->fetchSingleTest($sql))) {
					//
			...
			if (array_key_exists('changeModule',$_REQUEST) && $_REQUEST['changeModule'] == 2) {
				...
				if (array_key_exists('query_linked',$data) && $data['query_linked'] != 0) {
					...
					//
					$ct = $this->fetchScalarTest('select count(0) from modules_by_page where page_id = %d and query_linked != 0 and id != %d',$_REQUEST['page_id'],$_REQUEST['id']);
		...
		$placement = $this->fetchScalarAllTest($sql);
    ...
    function buildFolderList($classId, $required=false) {
		...
		if ($className = $this->fetchScalarTest('select classname from modules where id = %d and frontend = 1',$classId)) {
			switch($className) {
				case 'newsletter':
    ...
    protected function getPagination(&$sql, $module, &$recordCount, &$pageNum = 0) {
		...
		if ($module['rows'] > 0) {
			...
			if ($this->hasOption('pagination')) {
				...
				$this->logMessage(__FUNCTION__,sprintf("looking for m_dir this [%s] module [%s]",print_r($this,true),print_r($module,true)),4);
...
/**
 * Handles ajax functionality on the frontend
 */
