...
/**
 * Newsletter feature for frontend
 */
class newsletter extends Frontend {
    ...
    function signup() {
		...
		if ($module["folder_id"] > 0) {
			if ($f = $this->fetchSingleTest("select * from newsletter_folders where id = %d",$module["folder_id"])) {
		...
		if (count($_POST) > 0 && array_key_exists('newsletterSignup',$_POST)) {
			$inner->addData($_POST);
			if ($inner->validate()) {
				$id = (int)$this->fetchScalarTest('select id from subscriber where email = "%s" and deleted = 0 and enabled = 1',$_POST['email']);
				$valid = true;
				...
				if ($module['folder_id'] != 0 && $valid) {
					$f_id = (int)$this->fetchScalarTest('select id from subscriber_by_folder where folder_id = %d and subscriber_id = %d',$module['folder_id'],$id);
					if ($f_id == 0) {
    ...
    function unsubscribe() {
		...
		if ($rec = $this->fetchSingleTest("select s.*, s.id as sub_id from subscriber s, newsletter_batch_subscriber b where b.batch_id=%d and b.subscriber_id=%d and b.random_id=%d and s.id = b.subscriber_id", $n, $s, $r)) {
			$inner->addData($rec);
		}
    ...
    function tracker() {
		...
		$nl = $this->fetchSingleTest('select * from newsletter n, newsletter_batch b where b.id = %d and n.id = b.newsletter_id',array_key_exists('nl_id',$_REQUEST ? $_REQUEST['nl_id'] : 0));
		switch($mode) {
		case 'view':
		...
		case 'unsub':
			if (array_key_exists('sub_id',$_REQUEST) && array_key_exists('rand',$_REQUEST)) {
				$this->execute(sprintf('update newsletter_batch_subscriber set unsubscribe = unsubscribe+1 where batch_id = %d and subscriber_id = %d and random_id = %d',
					$_REQUEST['nl_id'],$_REQUEST['sub_id'],$_REQUEST['rand']));
				if ($nl['testing'] == 0) {
					if ($id = $this->fetchScalarTest('select subscriber_id from newsletter_batch_subscriber where batch_id = %d and subscriber_id = %d and random_id = %d',
								$_REQUEST['nl_id'],$_REQUEST['sub_id'],$_REQUEST['rand']))
			...
			if ($nl['unsubscribe'] > 0) {
				$p = $this->fetchSingleTest('select * from content where id = %d',$nl['unsubscribe']);
				$url = sprintf('%s?n=%s&s=%d&r=%d',$this->getUrl('menu',$nl['unsubscribe'],$p),$_REQUEST['nl_id'],$_REQUEST['sub_id'],$_REQUEST['rand']);
			}
			else $url = '/';
			ob_clean();
			$allowedUrls = sprintf('%s?n=%s&s=%d&r=%d',$this->getUrl('menu',$nl['unsubscribe'],$p),$_REQUEST['nl_id'],$_REQUEST['sub_id'],$_REQUEST['rand']);
            if (strpos($allowedUrls, $url) !== false) {
                header('Location: '.$url);
            } else {
				header("Location: /");
			}
    ...
    function subscriberInfo() {
		...
		if (array_key_exists('n',$_REQUEST) && array_key_exists('s',$_REQUEST) && array_key_exists('r',$_REQUEST)) {
			$subscriber = $this->fetchSingleTest('select s.* from subscriber s, newsletter_batch_subscriber b where b.batch_id = %d and b.subscriber_id = %d and b.random_id = %d and s.id = b.subscriber_id',
    ...
    function latest() {
		...
		$outer = new Forms();
		if ($module['folder_id'] > 0) {
			$folder = $this->fetchSingleTest('select * from newsletter_folders where id = %d',$module['folder_id']);
			$outer->addData($this->formatFolder($folder));
			$rec = $this->fetchAllTest('select * from newsletter n, newsletter_by_folder nf where nf.folder_id = %d and n.id = nf.newsletter_id %s limit %d',$folder['id'],$module['sort_by'] != '' ? 'order by '.$module['sort_by'] : '',$module['records']);
		}
		else {
			$rec = $this->fetchSingleTest('select * from newsletter where id = %d',$module['nl_id']);
		}
		$outer->init($this->m_dir.$module['outer_html']);
		$flds = $this->config->getFields($module['configuration']);
		$inner = new Forms();
