...
/**
 * Feeds feature for frontend section
 */
class feeds extends Frontend {
    ...
    function showFeed() {
		...
		if ($module['parm2'] > 0)
			$this->checkFeed($feed,$module['parm2']);
		else
			$this->checkFeed($feed);
		$return = array();
    ...
    function facebookPosts() {
		...
		try {
			$info = $this->fetchSingleTest("select * from feeds where source='%s' order by created desc limit 1",$module["parm1"]);
			if (!$info || date("Y-m-d H:i:s",strtotime(sprintf("%s + %d minutes",$info["created"],$module["parm2"]))) < date("Y-m-d H:i:s")) {
				...
				if (!is_array($info)) {
					...
					$info = array();
					$info["data"] = json_decode($this->fetchScalarTest("select value from feeds where source='%s' order by created desc limit 1",$module["parm1"]),true);
				}
				else {
			...
			if (is_array($info)) {
				...
				foreach($slice as $key=>$value) {
					...
					$this->logMessage('facebookPosts',sprintf('info [%s]',print_r($value,true)),4);
					$inner->reset();
    ...
    function formatTweet($data) {
		$this->logMessage(__FUNCTION__,sprintf('incoming [%s]',print_r($data,true)),4);
		...
		$this->logMessage(__FUNCTION__,sprintf('outgoing [%s]',print_r($data,true)),3);
		return $data;
	}
    ...
    private function checkInstagramFeed($module, $parms) {
		...
		if ($rec = $this->fetchSingleTest($sql)) {
			...
			$this->getInstagram($this->getOption("feedname"),$module,$parms);
		return $this->fetchSingleTest("select * from feeds where source='%s' order by created desc limit 1",$this->getOption("feedname"));
	}
