...
class gallery extends Frontend {
    ...
    public function images() {
		...
		if ($module['folder_id'] != 0) {
			$folder = $this->fetchSingleTest('select * from gallery_folders where id = %d',$module['folder_id']);
    ...
    function listing() {
		...
		$sql = $this->buildSql($module);
		if ($module['folder_id'] != 0)
			$root = $this->fetchSingleTest('select * from gallery_folders where id = %d',$module['folder_id']);
		else
			$root = array('left_id'=>0,'right_id'=>999999,'level'=>0,'id'=>0);
		if ($this->hasOption('typeSelect')) {
			$folders = $this->fetchAllTest('select * from gallery_folders where left_id > %d and right_id < %d and level > %d order by left_id',$root['left_id'],$root['right_id'],$root['level']);
			$form = new Forms();
			$form->init($this->m_dir.$module['inner_html']);
			$result = array();
    ...
    private function buildUL($root, $module, $root_level) {
		...
		$level = $this->fetchAllTest('select * from gallery_folders where level = %d and left_id > %d and right_id < %d and enabled = 1 order by left_id',$root['level']+1,$root['left_id'],$root['right_id']);
		$form = new Forms();
		$form->init($this->m_dir.$module['inner_html']);
		$menu = array();
		$seq = 0;
    ...
    function buildDiv($root, $module, $root_level) {
		...
		$level = $this->fetchAllTest('select * from gallery_folders where level = %d and left_id > %d and right_id < %d and enabled = 1  order by %s limit %d',$root['level']+1,$root['left_id'],$root['right_id'], $sort, $limit);
		$outer = new Forms();
		//
    ...
    function upload() {
		...
		if (count($_REQUEST) > 0 && array_key_exists('mediaUpload',$_REQUEST)) {
			...
			if ($status) {
				...
				$ffilename = '.'.$file['name'];
				$ttmp = '.'.$tmp;
				$status = $status && rename(realpath($ffilename),realpath($ttmp));
				...
				if ($status) {
					$data = $this->fetchSingleTest('select * from gallery_images where id = %d',$id);
    ...
    function single() {
		...
		if ($images = $this->fetchAllTest($sql)) {
			$inner = new Forms();
			$inner->init($this->m_dir.$module['inner_html']);
			$result = array();
		...
		if ($module['folder_id'] > 0 && $folder = $this->fetchSingleTest('select * from gallery_folders where id = %d',$module['folder_id']))
			$outer->addData($this->formatFolder($folder));
		return $outer->show();
	}
    ...
    function memberOf() {
		...
		$outer->init($this->m_dir.$module['outer_html']);
		if ($this->image_id > 0) {
			$folders = $this->fetchAllTest("select * from gallery_folders f where f.id in (select folder_id from gallery_images_by_folder where image_id = %d) and f.enabled = 1 order by left_id",$this->image_id);
		}
		elseif ($module["folder_id"] > 0) {
			$root = $this->fetchSingleTest("select * from gallery_folders where id = %d",$module["folder_id"]);
			$folders = $this->fetchAllTest("select * from gallery_folders f where left_id > %d and right_id < %d and enabled = 1 order by left_id",$root["left_id"],$root["right_id"]);
		}
		else $folders = array();
		$recs = array();
		$inner = new Forms();
