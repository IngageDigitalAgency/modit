...
class artist extends Frontend {
    ...
    function profile() {
		...
		$outer = new Forms();
		$outer->init($this->m_dir.$module['outer_html']);
		if ($module["folder_id"] > 0 && $folder = $this->fetchSingleTest('select * from members_folders where id = %d',$module['folder_id'])) {
			$profile["folder"] = $this->formatFolder($folder);
			$outer->addData($profile["folder"]);
		}
		$inner = new Forms();
		if ($profile = $this->fetchSingleTest('select * from members where id = %d and enabled = 1 and deleted = 0',$profile_id)) {
			$inner->init($this->m_dir.$module['inner_html']);
    ...
    function socialLinks() {
		...
		$inner = new Forms();
		if ($profile = $this->fetchSingleTest('select * from members where id = %d',$profile_id)) {
			$inner->init($this->m_dir.$module['inner_html']);
			$inner->addData($this->formatData($profile));
		}
		$recs = $this->fetchAllTest("select s.*, c.extra from members_social s, code_lookups c where member_id = %d and enabled = 1 and deleted = 0 and c.type='social_media' and c.code = s.social_type", $profile_id);
		$return = array();
    ...
    function portfolios() {
		...
		$inner = new Forms();
		$inner->init($this->m_dir.$module['inner_html']);
		$recs = $this->fetchAllTest("select * from portfolio where member_id = %d and enabled = 1 and deleted = 0 order by %s limit %d", $profile_id, strlen($module["sort_by"] > 0 ? $module["sort_by"]:"seq", $module["records"]));
		$return = array();
		foreach($recs as $key=>$value) {
    ...
    function portfolio() {
		...
		$outer = new Forms();
		$outer->init($this->m_dir.$module['outer_html']);
		if ($portfolio = $this->fetchSingleTest('select * from portfolio where id = %d',$portfolio_id))
			$outer->addData($this->formatPortfolio($portfolio));
		$inner = new Forms();
		$inner->init($this->m_dir.$module['inner_html']);
		$recs = $this->fetchAllTest("select * from portfolio_item where portfolio_id = %d and enabled = 1 and deleted = 0 order by %s limit %d", $portfolio_id, strlen($module["sort_by"] > 0 ? $module["sort_by"]:"seq", $module["records"]));
		$return = array();
		foreach($recs as $key=>$value) {
