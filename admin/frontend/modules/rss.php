...
class rss extends Frontend {
    ...
    public function formatData($data, $folder = array()) {
		...
		if ($video = $this->fetchSingleTest('select * from videos where owner_type="rss" and owner_id = %d', $data['id']))
    ...
    function listing() {
		...
		if ($module['folder_id'] != 0)
			$root = $this->fetchSingleTest('select * from rss_folders where id = %d',$module['folder_id']);
		else
			$root = array('left_id'=>0,'right_id'=>999999,'level'=>0,'id'=>0);
		if ($this->hasOption('typeSelect')) {
			$folders = $this->fetchAllTest('select * from rss_folders where left_id > %d and right_id < %d and level > %d order by left_id',$root['left_id'],$root['right_id'],$root['level']);
			$form = new Forms();
    ...
    private function buildUL($root, $module, $root_level) {
		...
		$level = $this->fetchAllTest('select * from rss_folders where level = %d and left_id > %d and right_id < %d and enabled = 1 order by left_id',$root['level']+1,$root['left_id'],$root['right_id']);
		$form = new Forms();
		$form->init($this->m_dir.$module['inner_html']);
		$menu = array();
		$seq = 0;
    ...
    public function items() {
		...
		foreach($records as $rec) {
			...
			if ($this->m_module['folder_id'] != 0)
				$fldr = $this->fetchSingleTest('select * from rss_folders where id = %d',$this->m_module['folder_id']);
			else {
			...
			$rec["author"] = $this->fetchSingleTest("select * from users where id = %d",$rec["author_id"]);
		...
		if ($this->m_module['folder_id'] != 0) {
			if ($fldr = $this->fetchSingleTest('select * from rss_folders where id = %d',$this->m_module['folder_id']))
				$outer->addData($this->formatFolder($fldr));
		}
		else {
			if ($fldr = $this->fetchSingleTest('select * from rss_folders where id in (select folder_id from rss_by_folder where article_id in (%s)) order by rand() limit 1',implode(',',$ids)))
				$outer->addData($this->formatFolder($fldr));
		}
    ...
    function details($articleId = null) {
		...
		if ($this->m_module['folder_id'] != 0) {
			...
			$fldr = $this->fetchSingleTest('select * from rss_folders where id = (select folder_id from rss_by_folder where article_id = %d order by rand() limit 1)',$articleId);
		}
		$frm = new Forms();
		$frm->init($this->m_dir.$module['inner_html']);
    ...
    function rssSearchForm() {
		...
		if ($module["folder_id"] > 0) {
			$fldr = $this->fetchSingleTest("select * from rss_folders where id = %d", $module["folder_id"]);
			$outer->addData($this->formatFolder($fldr));
		}
    ...
    function rssSearchResults() {
		...
		if ($module["include_subfolders"] != 0) {
			$fldrList = $this->fetchScalarAllTest("select id from rss_folders where left_id >= %d and right_id <= %d and enabled = 1", $fldr["left_id"], $fldr["right_id"]);
		}
		else {
		...
		if (array_key_exists(__FUNCTION__,$_REQUEST) && $_REQUEST[__FUNCTION__] > 0) {
			...
			for($x = 0; $x < count($searchPhrase); $x++) {
				$rss[$x] = $this->fetchAllTest('select * from rss where enabled = 1 and published = 1 and (title like "%%%s%%" or teaser like "%%%s%%" or body like "%%%s%%")',
