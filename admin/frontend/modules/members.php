...
class members extends Frontend {
		...
		if (count($_POST) > 0 && array_key_exists('signupForm',$_POST)) {
			...
			if ($valid = $outer->validate()) {
				...
				if ($valid && array_key_exists('username',$flds)) {
					$email = $this->fetchScalar(sprintf('select count(*) from members where deleted = 0 and username = "%s"',$outer->getData('username')));
					if ($email > 0) {
						$valid = false;
    ...
    function memberOf() {
		...
		if ($f = $this->fetchSingle(sprintf("select * from members_folders where id = %d", $module["folder_id"]))) {
			$outer->addData($this->formatFolder($f));
			if ($m = $this->fetchSingle(sprintf("select * from members_by_folder where folder_id = %d and member_id = %d", $f["id"], $this->getUserInfo("id")))) {
				$inner->addData($m);
			}
			else {
				$inner->init($this->m_dir.$module['parm1']);
