...
class product extends Frontend {
    ...
    function getModule() {
		...
		if ($this->hasOption('buyUrl') && array_key_exists('addToCart',$_REQUEST)) {
			...
			$allowedUrls = $this->getOptions();
			$url = $this->getOption('buyUrl');
    ...
    private function buildSql($module, $addLimit = false) {
		...
		if ($module['folder_id'] > 0) {
			if ($module['include_subfolders'] != 0) {
				...
				$tmp = array_merge(array(0),$this->fetchScalarAllTest($sql));
		...
		if ($this->hasOption('unique')) {
			...
			$test = $this->fetchScalarAllTest($tmp);
    ...
    /**
     * @param $data
     * @param bool $getPricing
     * @return mixed
     * @throws phpmailerException
     */
    static function formatData($data, $getPricing = true) {
		$c = new Common();
		...
		$data['url'] = $c->getUrl('product',$data['id'],$data);
		$data['href'] = sprintf('<a href="%s">',$data['url']);
		$data['href_end'] = '</a>';
		if ($getPricing && $pricing = $c->fetchSingleTest('select * from product_pricing where product_id = %d order by min_quantity limit 1',$data['id'])) {
			$data['onSale'] = 0;
			...
			$data['regularPriceFormatted'] = $c->my_money_format($data['regularPrice']);
			if ($data['sale_startdate'] <= date('Y-m-d H:i:s') && $data['sale_enddate'] > date('Y-m-d H:i:s')) {
				...
				$data['saleDiscountFormatted'] = $c->my_money_format($data['saleDiscount']);
		...
		$opts = $c->fetchAllTest('select * from product_options where deleted = 0 and product_id = %d order by sequence',$data['id']);
		$frm = new Forms();
		$select = new select();
		$select->addAttributes(array('name'=>'options_id','value'=>array_key_exists('options_id',$data) ? $data['options_id'] : 0));
		foreach($opts as $key=>$opt) {
			if ($data['onSale'])
				$select->addOption($opt['id'],sprintf('%s : %s',$opt['teaser'],$c->my_money_format(round($opt['qty_multiplier']*($opt['sale_price']+$data['price']),2))));
			else
				$select->addOption($opt['id'],sprintf('%s : %s',$opt['teaser'],$c->my_money_format($opt['qty_multiplier']*(round($opt['price']+$data['price'],2)))));
		}
		...
		foreach($opts as $key=>$opt) {
			...
			if ($optKey == $opt['id'])
				$radio->addAttribute('checked','checked');
			$data['optionsRadio'.$key] = sprintf(sprintf('<span class="radio">%s</span>&nbsp;<span class="teaser">%s</span>&nbsp;<span class="price">%s</span>',$radio->show(),$opt['teaser'],$c->my_money_format(round($opt['price']+$data['price'],2))));
			$data['optionsRadio'][] = sprintf(sprintf('<span class="radio">%s</span>&nbsp;<span class="teaser">%s</span>&nbsp;<span class="price">%s</span>',$radio->show(),$opt['teaser'],$c->my_money_format(round($opt['price']+$data['price'],2))));
		}
		$data['price_fmt'] = $c->my_money_format($data['price']);
		...
		$data['extendedPriceFormatted'] = $c->my_money_format($data['extendedPrice']);
		if (array_key_exists('color',$data)) {
			$data['colorName'] = $c->fetchScalarTest('select value from code_lookups where id = %d',$data['color']);
		}
		if (array_key_exists('size',$data)) {
			$data['sizeName'] = $c->fetchScalarTest('select value from code_lookups where id = %d',$data['size']);
		}
		if (array_key_exists('shipping',$data))
			$data['shipping_fmt'] = $c->my_money_format(round($data['shipping'],2));
		if ($data['status'] > 0 && $status = $c->fetchSingleTest('select * from code_lookups where type="product_status" and id=%d',$data['status'])) {
			$status['state'] = $c->depairOptions($status['extra']);
			$data['inventoryState'] = $status;
		}
		else {
			$data['inventoryState'] = array('state'=>array('inventoryStatus'=>1));
		}
		$invAmt = $c->fetchSingleTest("select * from product_inventory where product_id = %d and options_id = 0 and color = 0 and size = 0 and (start_date = '0000-00-00' or start_date <= CURDATE()) and (end_date = '0000-00-00' or end_date >= CURDATE())",$data["id"]);
		$data["availability"] = -1;
		if (is_array($invAmt) && count($invAmt) > 0) {
			if ($invAmt["quantity"] <= 0) {
				$status = $c->fetchSingleTest('select * from code_lookups where type="product_status" and extra like "%%inventoryState:%d%%"',$data['status'],PRODUCT_OUT_OF_STOCK);
				$status['state'] = $c->depairOptions($status['extra']);
		...
		if ($video = $c->fetchSingleTest('select * from videos where owner_type="product" and owner_id = %d', $data['id'])) {
			$data['video'] = $video;
		}
		$reviews = $c->fetchSingleTest("select count(product_id) as ct, avg(rating) as avg from product_reviews where product_id = %d and approved = 1 group by product_id",$data["id"]);
		if (is_array($reviews)) {
		...
		$data["manufacturer"] = $c->fetchSingleTest("select * from code_lookups where id = %d",$data["manufacturer_id"]);
		$c->logMessage('formatData',sprintf('return data [%s]',print_r($data,true)),3);
		return $data;
	}
    ...
    function formatFolder($data, $module = array()) {
		...
		$tmp = $data;
		$ct = 0;
		while(($tmp = $this->fetchSingleTest('select * from product_folders where level = %d and left_id <= %d and right_id >= %d',$tmp['level']-1,$tmp['left_id'],$tmp['right_id'])) && $ct < 10) {
			$level += 1;
    ...
    function products() {
		...
		$products = $this->fetchAllTest($sql);
		...
		foreach($products as $key=>$product) {
			...
			if ($product['folder_id'] > 0 && $fldr = $this->fetchSingleTest('select * from product_folders where id = %d and enabled = 1',$product['folder_id'])) {
		...
		if ($module['folder_id'] != 0) {
			if ($fldr = $this->fetchSingleTest('select * from product_folders where id = %d',$module['folder_id']))
    ...
    function listing() {
		...
		if ($module['folder_id'] != 0)
			$root = $this->fetchSingleTest('select * from product_folders where id = %d',$module['folder_id']);
		else
    ...
    private function buildOpt($root, $module, $root_level) {
		...
		$folders = $this->fetchAllTest('select * from product_folders where left_id > %d and right_id < %d and level = %d order by left_id',$root['left_id'],$root['right_id'],$root['level']+1);
		$form = new Forms();
		...
		foreach($folders as $key=>$folder) {
			if ($folder['internal_link'] != 0) {
				...
				$folder = $this->fetchSingleTest($sql);
			}
    ...
    private function buildUL($root, $module, $root_level) {
		...
		if (array_key_exists('internal_link',$root) && $root['internal_link'] > 0) {
			...
			$root = $this->fetchSingleTest('select * from product_folders where id = %d',$root['internal_link']);
		}
		$level = $this->fetchAllTest('select * from product_folders where level = %d and left_id > %d and right_id < %d and enabled = 1 order by left_id',$root['level']+1,$root['left_id'],$root['right_id']);
		$form = new Forms();
		$form->init($this->m_dir.$module['inner_html']);
		$menu = array();
		$seq = 0;
    ...
    function details() {
		...
		if (array_key_exists('prod_id',$_REQUEST) || $this->hasOption('showAlways')) {
			if ($module['folder_id'] != 0) {
				if ($fldr = $this->fetchSingleTest('select * from product_folders where enabled = 1 and id = %d',$module['folder_id'])) {
					$outer->addData($this->formatFolder($fldr));
				}
				...
				if ($fldr = $this->fetchSingleTest($sql)) {
					$outer->addData($this->formatFolder($fldr));
    ...
    function cartInfo() {
		...
		foreach($cart['products'] as $key=>$product) {
			...
			if (!$this->hasOption('displayOnly')) {
				...
				$product['options_id'] = $this->fetchScalarTest('select teaser from product_options where id = %d',$product['options_id']);
				$product['color'] = $this->fetchScalarTest('select value from code_lookups where id = %d',$product['color']);
				$product['size'] = $this->fetchScalarTest('select value from code_lookups where id = %d',$product['size']);
				$product['recurring_period'] = $this->fetchScalarTest('select teaser from product_recurring r where r.id = %d',$product['recurring_period']);
				$form = array();
			}
		...
		if ($cart['header']['redemption_value'] != 0) {
			$totals = new Forms();
			$totals->init($this->m_dir.$templates['total']);
			$totals->addTag('description','Points Redeemed:');
			$totals->addTag('total',$cart['header']['formattedRedemptionValue']);
			$tot[] = $totals->show();
		}
		...
		if ($cart['header']['points_collected'] > 0) {
			$totals->addTag('description','Points Collected:');
			$totals->addTag('total',number_format($cart['header']['points_collected']));
			$tot[] = $totals->show();
		}

		if ($cart['header']['points_redeemed'] > 0) {
			$totals->addTag('description','Points Redeemed:');
			$totals->addTag('total',number_format($cart['header']['points_redeemed']));
			$tot[] = $totals->show();
		}

		$header->addTag('total',$cart['header']['formattedTotal']);
		$header->addTag('quantity',$qty);
    ...
    function shippingAddress() {
		...
		if ((!array_key_exists('shipping',$_SESSION['cart']['addresses']))  || count($_SESSION['cart']['addresses']['shipping']) == 0)
			if ($this->isLoggedIn())
				$_SESSION['cart']['addresses']['shipping'] = $this->fetchSingleTest('select * from addresses where ownertype = "member" and ownerid = %d and tax_address = 1',$_SESSION['user']['info']['id']);
		...
		if (count($_POST) > 0 && array_key_exists('shippingAddressForm',$_POST)) {
			if (array_key_exists('saveAddress',$_POST)) {
				...
				if ($outer->validate()) {
					...
					$outer = new Forms();
					$outer->init($this->m_dir.$module['outer_html']);
					$_SESSION['cart']['addresses']['shipping'] = array_merge($this->fetchSingleTest('select * from addresses where ownertype="member" and id = %d',$_POST['addressId']),$values);
					...
					$allowedUrls = $this->getOptions();
					$url = $this->getOption('recalcOrder');
					if (in_array($url, $allowedUrls, true)) {						
						header('Location: '.$url);
					}
					else
    ...
    function billingAddress() {
		...
		if ((!array_key_exists('billing',$_SESSION['cart']['addresses'])) || count($_SESSION['cart']['addresses']['billing']) == 0)
			if (array_key_exists('member_id',$_SESSION['cart']['header'])) {
				if ($tmp = $this->fetchSingleTest('select * from addresses where ownertype = "member" and ownerid = %d and tax_address = 0',$_SESSION['cart']['header']['member_id']))
...
//
//	Note: This is being heavily modified for KJV only. The only active functionality to be considered is the PayFlow logic 
//	- moving the authorization results to the new table order_authorization
//	- June 2018
		...
		if (count($_REQUEST) > 0) {
			if ($this->hasOption('moneris') && array_key_exists('isMoneris',$_REQUEST)) {
				if (array_key_exists('response_order_id',$_POST)) {
					...
					if ($_POST['response_code'] == '027') {
						$tmp['order_status'] = $this->fetchScalarTest('select code from code_lookups where type="orderStatus" order by sort limit 1');
					}
			...
			if ($this->hasOption('2checkout') && array_key_exists('is2checkout',$_REQUEST)) {
				...
				$tmp = array(
					...
					'authorization_info'=>implode("\n",$return),
					'order_status'=>$this->fetchScalarTest('select code from code_lookups where type="orderStatus" order by sort limit 1')
				);
			...
			if ($this->hasOption('beanstream') && array_key_exists('ref1',$_REQUEST) && $_REQUEST['ref1'] == 'isBeanstream') {
				...
				if ($_REQUEST['messageId'] == 1) {
					...
					$tmp = array(
						...
						'authorization_info'=>implode("\n",$return),
						'order_status'=>$this->fetchScalarTest('select code from code_lookups where type="orderStatus" and sort = 4')
					);
			...
			if ($this->hasOption('payflow')) {
				if (array_key_exists('SECURETOKEN',$_REQUEST)) {
					$order_id = $_SESSION['cart']['header']['order_id'];
					$this->beginTransaction();
					$valid = true;
					if ($reauthorizing) {
						...
						//
						$expiring = $this->fetchSingleTest('select o2.* from orders o1, orders o2 where o1.id = %d and o2.id = o1.authorization_transaction',$order_id);
						$original = $this->fetchSingleTest('select * from orders where id = %d',$expiring['authorization_transaction']);
						$this->logMessage(__FUNCTION__,sprintf('expiring [%s] original [%s]',print_r($expiring,true),print_r($original,true)),1);
					}
					$success = $this->fetchScalarTest("select extra from code_lookups where type='payflowResponse' and code='success'");
					$tmp = array();
						...
						if (array_key_exists("AVSADDR",$_REQUEST) && array_key_exists("AVSZIP",$_REQUEST)) {
							...
							if (strpos($parms["AVS"],sprintf("|%s%s|",$_REQUEST["AVSADDR"],$_REQUEST["AVSZIP"]))===false) {
								...
								$tmp = array('authorization_amount'=>0,
									'authorization_partner'=>'PayFlow',
									'authorization_code'=>"AVSADDR",
									'authorization_type'=>$_REQUEST['TRXTYPE'],
									'authorization_message'=>sprintf("%s %s", $_REQUEST['RESPMSG'], array_key_exists("PREFPSMSG", $_REQUEST ) ? $_REQUEST['PREFPSMSG'] : ""),
					...
					if (array_key_exists('RESULT',$_REQUEST) && strpos($success,"|".$_REQUEST['RESULT']."|") !== false) {
						...
						if (array_key_exists("PPREF",$_REQUEST)) {
							...
							//
							$tmp = array('authorization_amount'=>$_REQUEST['AMT'],
								'authorization_partner'=>'PayPal via PayFlow',
								'authorization_type'=>$_REQUEST['TRXTYPE'],
								'authorization_message'=>sprintf("%s %s", $_REQUEST['RESPMSG'], array_key_exists("PREFPSMSG", $_REQUEST ) ? $_REQUEST['PREFPSMSG'] : ""),
					...
					if ($reauthorizing) {
						$stmt = $this->prepare(sprintf('update orders set %s where id = %d',implode('=?,',array_keys($tmp1)).'=?',$original['id']));
					}
					else {
						$stmt = $this->prepare(sprintf('update orders set %s where id = %d',implode('=?,',array_keys($tmp1)).'=?',$order_id));
					}
					$stmt->bindParams(array_merge(array(str_repeat('s',count($tmp1))),array_values($tmp1)));
					$valid &= $stmt->execute();
					$tmp["authorization_date"] = date(DATE_ATOM);
					//$tmp["authorization_type"] = $_REQUEST['TRXTYPE'];
					$stmt = $this->prepare(sprintf("insert into order_authorization(%s) values(?%s)", implode(", ", array_keys($tmp)), str_repeat(", ?", count($tmp)-1)));
					$stmt->bindParams(array_merge(array(str_repeat("s",count($tmp))), array_values($tmp)));
					$valid &= $stmt->execute();
					if (array_key_exists('RESULT',$_REQUEST) && strpos($success,"|".$_REQUEST['RESULT']."|") !== false) {
						if ($valid) {
							...
							if (array_key_exists("PPREF",$_REQUEST)) {
								...
								if ($ba = $this->fetchSingleTest("select * from orders where authorization_transaction = '%d' and order_status & %d = %d limit 1",
									...
									if (!$this->hasOption("billingAgreement")) {
										...
										$fn = $this->fetchSingleTest('select t.id as fetemplate_id, t.module_function, m.classname from fetemplates t, modules m where t.id = %d and m.id = t.module_id',$this->getOption('billingAgreement'));
					...
					if (count($_POST) > 0 && array_key_exists("HOSTCODE",$_POST)) {
						$this->logMessage(__FUNCTION__,sprintf("an odd payflow response [%s] from [%s]", print_r($_POST,true), print_r($_SESSION,true)),1,true);
						$outer->addFormError(sprintf("We received an unexpected response: %s The Web Master has been notified", $_POST["RESPMSG"]));
					}
				}
			}
		...
		if (defined('FRONTEND') && (!array_key_exists('order_id',$cart['header']) || $cart['header']['order_id'] == 0)) {
			...
			foreach($cart['products'] as $key=>$product) {
				if ($product['recurring_qty'] > 0) {
					$cd = $this->fetchSingleTest('select * from product_recurring where id = %d',$product['recurring_period']);
		...
		$cart['addresses']['shipping']['province'] = $this->fetchScalarTest($sql);
		$cart['addresses']['shipping']['country'] = $this->fetchScalarTest('select country from countries where id = %d',$cart['addresses']['shipping']['country_id']);
		if ($this->checkArray("addresses:billing",$cart) && count($cart['addresses']['billing']) > 0) {
			$cart['addresses']['billing']['province'] = $this->fetchScalarTest('select province_code from provinces where id = %d',$cart['addresses']['billing']['province_id']);
			$cart['addresses']['billing']['country'] = $this->fetchScalarTest('select country from countries where id = %d',$cart['addresses']['billing']['country_id']);
		}
		$f = new Forms();
		...
		if ($this->hasOption('payflow')) {
			...
			if ($reauthorizing || $this->hasOption("authorize_only")) $trxtype="A";
			$formvars = array(
		...
		$tmp = array();
		if ($this->checkArray("addresses:billing", $cart)) {
     ...
     * @throws Exception
     */
    public function createOrder($cart, &$orderid) {
		$kjv = new KJV();
		return $kjv->insertOrder($cart,$orderid);
	}
     ...
     * @throws Exception
     */
    public function createOrderDNU($cart, &$orderid) {
		...
		$stmt->bindParams(array('iididddddddssisiidsssddd', 
			...
			date(DATE_ATOM), date(DATE_ATOM), random_int(0,99), $cart['header']['discount_type'],
			...
			$reauthorizing ? sprintf('Reauthorizing Order #%d',$this->fetchScalarTest('select authorization_transaction from orders where id = %d',$cart['reauthorize']['o_id'])) : "",
    ...
    function orderDetails($o_id = null) {
		...
		if (!$header = $this->fetchSingleTest('select o.* from orders o, members m where o.id = %d and o.member_id = m.id and (m.id = %d or m.custom_parent_org = %2$d)',$o_id,$user))
			return $outer->show();
		$header["notes_formatted"] = nl2br($header["notes"]);
		$return = array();
		$inner = new Forms();
		$inner->init($this->m_dir.$module['inner_html']);
		$hasRecalc = $this->fetchScalarTest("select id from order_lines where order_id = %d and product_id = %d and deleted = 0", $header['id'], FEDEX_RECALC);
		$outer->setData("hasRecalc", $hasRecalc > 0);
		$lines = $this->fetchAllTest('select l.*, p.*, l.id as oline_id from order_lines l, product p where l.order_id = %d and l.deleted = 0 and p.id = l.product_id order by line_id',$header['id']);
		foreach($lines as $key=>$line) {
			$inner->reset();
			$inner->setData("hasRecalc", $hasRecalc > 0);
		...
		if ($auth = $this->fetchSingleTest("select * from order_authorization where order_id = %d and authorization_type = 'D' and authorization_success=1", $o_id)) {
			$inner->setData('description','Paid');
			$inner->setData('amount',$this->my_money_format($auth["authorization_amount"]));
			$return[] = $inner->show();
			$inner->setData('description','Balance');
			$inner->setData('amount',$this->my_money_format($header["total"] - $auth["authorization_amount"]));
			$return[] = $inner->show();
		}

		$outer->addTag('lines',implode('',$return),false);
		$addresses = $this->fetchAllTest('select a.*, p.province, c.country from addresses a left join provinces p on p.id = a.province_id, countries c, code_lookups l where ownertype="order" and ownerid = %d and c.id = a.country_id and l.id = addresstype order by l.sort',$header['id']);
		$inner->init($this->m_dir.$module['parm1']);
		$return = array();
$member = $this->fetchSingleTest("select * from members where id = %d",$header["member_id"]);
		foreach($addresses as $key=>$address) {
			$inner->reset();
			$inner->setData('addressType',$address['tax_address'] == 1 ? 'Shipping' : 'Billing');
			if ($action = $this->fetchSingleTest("select cd.*, m.firstname,m.lastname from custom_delivery cd left join drivers d on d.id = cd.driver_id left join members m on m.id = d.member_id where cd.order_id = %d and service_type='%s'", $o_id, $address["addresstype"] == ADDRESS_PICKUP ? "P":"D")) {
				$action["has_signature"] = strlen($action["signature"]) > 0;
				$action["sig_image"] = base64_encode($action["signature"]);
				$address["action"] = $action;
			}
			$inner->addData(Address::formatData($address));
			if ($address["addresstype"] != ADDRESS_BILLING || $member["custom_on_account"] == 0 ) {
				$return[] = $inner->show();
			}
		}
...
select related_id from relations where owner_type = "product" and related_type = "product" and owner_id = %d
    ...
    function productOptions() {
		...
		$outer->init($this->m_dir.$module['outer_html']);
		if (array_key_exists('prod_id',$_REQUEST) || $this->hasOption('showAlways')) {
			if ($prod = $this->fetchSingleTest('select * from product where id = %d and deleted = 0 and enabled = 1',$_REQUEST['prod_id'])) {
    ...
    function folderRelations() {
		...
		$fn = $this->fetchSingleTest('select t.id as fetemplate_id, t.module_function, m.classname from fetemplates t, modules m where t.id = %d and m.id = t.module_id',$this->getOption('templateId'));
		$tmp = $this->m_module;
    ...
    private function galleryFolder($module, $folder) {
		...
		if ($folders = $this->fetchSingleTest('select * from relations where owner_type = "productfolder" and owner_id = %d and related_type = "galleryfolder"',$folder)) {
    ...
    function sizeOrColor() {
		...
		$inner = new Forms();
		$inner->init($this->m_dir.$module['inner_html']);
		if ($prod = $this->fetchSingleTest('select * from product where deleted = 0 and enabled = 1 and published = 1 and id = %d',$this->m_productId)) {
			foreach($_REQUEST as $key=>$value) {
				if (!array_key_exists($key,$prod)) $prod[$key] = $value;
			}
			if (array_key_exists('product_quantity',$_REQUEST)) {
				$pricing = $this->fetchSingleTest('select * from product_pricing where product_id = %d and min_quantity <= %d and max_quantity > %d order by min_quantity limit 1',$prod['id']);
			}
			else
				$pricing = $this->fetchSingleTest('select * from product_pricing where product_id = %d order by min_quantity limit 1',$prod['id']);
			$outer->addData($this->formatData($prod));
			$options = array();
			if (array_key_exists('color',$_REQUEST)) {
				$data = $this->fetchAllTest('
...
order by ls.sort, ls.value',$prod['id'],$_REQUEST['color'],$_REQUEST['color']);
			...
			elseif (array_key_exists('size',$_REQUEST)) {
				$data = $this->fetchAllTest('select o.*, l1.value as colorName, l2.value as sizeName from product_options o, code_lookups l1, code_lookups l2 where o.product_id = %d and o.size = %d and l1.id = o.color and l2.id = o.size and o.deleted = 0',$prod['id'],$_REQUEST['size']);
			}
			else {
				$data = $this->fetchAllTest('select o.*, l1.value as colorName, l2.value as sizeName from product_options o, code_lookups l1, code_lookups l2 where o.product_id = %d and l1.id = o.color and l2.id = o.size and o.deleted = 0',$prod['id']);
			}
		...
		if (count($_POST) > 0) {
			...
			if (array_key_exists('item_name',$_POST)) {
				...
				if (count($matches) > 0) {
					...
					$order = $this->fetchSingleTest($sql);
    ...
    private function news($module) {
		...
		if ($folders = $this->fetchSingleTest('select * from relations where owner_type = "news" and related_id = %d and related_type = "newsfolder"',$this->m_productId)) {
			$module['folder_id'] = $folders['related_id'];
		}
		else {
			$folders = array();
		...
		if ($items = $this->fetchScalarAllTest('select owner_id as news_id from relations where owner_type = "news" and related_id = %d and related_type = "product"',$this->m_productId))
    ...
    private function blog($module) {
		...
		if ($folders = $this->fetchSingleTest('select * from relations where owner_type = "blogfolder" and related_id = %d and related_type = "product"',$p_id)) {
			$module['folder_id'] = $folders['related_id'];
		}
		else {
			$folders = array();
		...
		if ($items = $this->fetchScalarAllTest('select owner_id as blog_id from relations where owner_type = "blog" and related_id = %d and related_type = "product"',$p_id))
    ...
    function pricing() {
		...
		if ($prod = $this->fetchSingleTest('select * from product where deleted = 0 and enabled = 1 and published = 1 and id = %d',$this->m_productId)) {
			...
			$outer->addData($prod);
			$prices = $this->fetchAllTest('select * from product_pricing where product_id = %d order by min_quantity',$prod['id']);
			$lines = array();
    ...
    function initRecurring($order_id) {
		...
		foreach($_SESSION['recurring_items'] as $recKey=>$recurCart) {
			$term = $this->fetchSingleTest('select * from code_lookups l where l.id = %d and l.type="recurringBilling"',$recKey);
			...
			foreach($recurCart['products'] as $prodKey=>$product) {
				if (!$discount = $this->fetchSingleTest('select * from product_recurring where id = %d and product_id = %d',$product['recurring_period'],$product['id'])) {
	...
	function activateRecurring($order_id) {
		$info = $this->fetchSingleTest('select * from orders where id = %d',$order_id);
		$recurring = $this->fetchAllTest('select * from orders where member_id = %d and authorization_transaction = "%d" and order_status & %d = %d',
    ...
    function reauthorize() {
		...
		$outer->init($this->m_dir.$module['outer_html']);
		$status = true;
		if (!($order = $this->fetchSingleTest('select * from orders where id = %d and member_id = %d and random = %d',$o_id,$this->getUserInfo('id',$r_id)))) {
			$status = false;
		...
		if (count($_POST) > 0 && array_key_exists('reauthorize',$_POST)) {
			$hdr = $this->fetchSingleTest('select * from orders where id = %d',$o_id);
			$dtls = $this->fetchAllTest('select p.*, l.product_id, l.line_id, l.value, l.shipping, l.total, l.qty_multiplier, l.coupon_id, l.discount_rate, l.discount_value, l.shipping_only, l.discount_type, l.recurring_period, l.recurring_discount_rate, l.recurring_discount_value, l.recurring_shipping_only, l.recurring_discount_type, l.quantity, l.options_id, l.recurring_qty, l.price, l.price as regularPrice, l.recurring_period, l.color, l.size, 0 as inventory_id from order_lines l, product p where l.order_id = %d and p.id = l.product_id order by l.id',$o_id);
			$addresses = $this->fetchAllTest('select * from addresses where ownertype = "order" and ownerid = %d order by id',$o_id);
			if ($exchange = $this->fetchSingleTest('select * from exchange_rate where currency_id = %d and effective_date <= "%s" order by effective_date desc limit 1',$hdr['currency_id'],date("Y-m-d")))
			...
			foreach($dtls as $key=>$product) {
				...
				$taxes = $this->fetchAllTest('select * from order_taxes where order_id = %d and line_id = %d',$o_id,$product['line_id']);
    ...
    private function gallery($module) {
		$this->logMessage(__FUNCTION__,sprintf('parms [%s]',print_r($module,true)),2);
		if ($folders = $this->fetchSingleTest('select * from relations where owner_type = "product" and owner_id = %d and related_type = "galleryfolder"',$this->m_productId)) {
			$module['folder_id'] = $folders['related_id'];
		}
		else {
			$folders = array();
    ...
    function billingAgreement() {
		...
		if (array_key_exists("o_id",$_REQUEST) && array_key_exists("r",$_REQUEST)) {
			$o_id = $this->fetchScalarTest("select id from orders where id = %d and member_id = %d and random = %d",
				$_REQUEST['o_id'], $this->getUserInfo("id"), $_REQUEST['r']);
			if ($o_id > 0) {
				$o_id = $this->fetchScalarTest("select * from orders where authorization_transaction = '%d' and order_status & %d = %d",
		...
		if (count($_REQUEST) > 0 && array_key_exists('token',$_REQUEST)) {
			...
			$this->logMessage(__FUNCTION__,sprintf("get status returned [%s] from [%s]",print_r($result,true),print_r($snoopy,true)),2);

			//
			//	curl not supported with new ssl, back to Snoopy
			//
			//$tmp = tempnam("/tmp","ppd");
			//$data = array();
			...
			if (!(array_key_exists("RESULT",$result) && array_key_exists("PAYERID",$result))) {
				$this->logMessage(__FUNCTION__,sprintf("something went wrong getting the billing agreement result [%s]", print_r($result,true)),1,true);
				...
				if ($result['RESULT'] == 0 && array_key_exists('PAYERID',$result)) {
					...
					$data = implode("&",$data);					
					$arg = sprintf("-k -D \"%s\" -H \"User-Agent: Snoopy v1.2.4\" -H \"Host: %s:443\" -H \"Accept: image/gif, image/x-xbitmap, image/jpeg, image/pjpeg, */*\" -H \"Content-type: application/x-www-form-urlencoded\" -H \"Content-length: %d\" -d \"%s\" %s",
						$tmp, str_replace("https://","",$parms['auth']), strlen($data), $data, $parms['auth']);
						$arg2 = sprintf("-k -D \"%s\" -H \"User-Agent: Snoopy v1.2.4\" -H \"Host: %s:443\" -H \"Accept: image/gif, image/x-xbitmap, image/jpeg, image/pjpeg, */*\" -H \"Content-type: application/x-www-form-urlencoded\" -H \"Content-length: %d\" -d \"%s\" %s",
						$tmp, str_replace("https://","",$parms['auth']), strlen($data), $data, $parms['auth']);			
					$allowed = array($arg,$arg2);
					if (in_array($arg, $allowed, true)) {						
						$exec = "/usr/bin/curl ".escapeshellarg($arg);
						exec($exec,$results,$return);
					}					
					$ba = array();
					...
					if (array_key_exists("RESULT",$ba) && $ba["RESULT"] == 0) {
						$orig_id = $this->fetchScalarTest("select o1.authorization_transaction from orders o1 where o1.id = %s",$o_id);
				...
				if (array_key_exists('RESULT', $result) && $result['RESULT'] != 0) {
					//
    ...
    function generatePayPalToken($o_id) {
		...
		$parms = $GLOBALS['payflow'];
		$snoopy = new Snoopy();
		$order = $this->fetchSingleTest("select o.*, c.value as recurringDesc from orders o, code_lookups c where o.id = %d and c.id = o.recurring_period",$o_id);
		$lines = $this->fetchAllTest("select l.*, p.name from order_lines l, product p where l.order_id = %d and p.id = l.product_id",$o_id);
		$query = array(
			'PARTNER'=>$parms['partner'],
			'VENDOR'=>$parms['vendor'],
			'PWD'=>$parms['pwd'],
    ...
    function reviews() {
		...
		$outer->setModule($module);
		$outer->init($this->m_dir.$module["outer_html"]);
		$product = $this->fetchSingleTest("select * from product where id = %d",$this->m_productId);
    ...
    function writeaReview() {
		...
		$outer->setModule($module);
		$outer->init($this->m_dir.$module["outer_html"]);
		if ($product = $this->fetchSingleTest("select * from product where id = %d",array_key_exists("product_id",$_REQUEST ? $_REQUEST["product_id"] : $this->m_productId)))
			$product = $this->formatData($product);
		...
		if (count($_POST) > 0 && array_key_exists("writeaReview",$_POST)) {
			...
			//
			$inner->setData('r_secret',random_int(0,99));
			...
			if ($valid) {
				...
				if ($stmt->execute()) {
					...
					if ($this->hasOption("email")) {
						...
						$body = new Forms();
						$html = $this->fetchSingleTest('select * from htmlForms where class = %d and type = "%s"',$this->getClassId('product',$this->getOption('email')));
    ...
    function commentApproval() {
		...
		if ($review = $this->fetchSingleTest("select * from product_reviews where id = %d and rand = %d",$comment,$rand)) {
    ...
    function getInventory() {
		...
		$invAmt = $this->fetchSingleTest("select i.* from product_inventory i where product_id = %d and options_id = %d and color = %d and size = %d and (start_date = '0000-00-00' or start_date <= CURDATE()) and (end_date = '0000-00-00' or end_date >= CURDATE()) and deleted = 0",$prod,$option,$color,$size);
		if (is_array($invAmt) && count($invAmt) > 0) {
			$inner->init($this->m_dir.$module["inner_html"]);
    ...
    function buildAssembly() {
		...
		if ($f_id > 0) {
			$folder = $this->fetchSingleTest("select * from product_folders where id = %d", $f_id);
			$prod["folder"] = self::formatFolder($folder);
		}
		$outer->addData(self::formatData($prod));
		$products = $this->fetchAllTest("select p.*, pp.quantity as multiplier from product p, product_by_package pp where p.id = pp.subproduct_id and pp.product_id = %d order by pp.sequence", $p_id);
		$recs = array();
    ...
    function redeemPoints() {
		...
		$flds = $this->config->getFields($module["configuration"]);
		$inner->buildForm($flds);
		$pts = (int)$this->fetchScalarTest("select sum(points) from members_points where member_id = %d", $this->getUserInfo("id"));
		$total_pts = $pts;
		$outer->setData("points",$pts);
		$inner->setData("points",$pts);
		$prices = $this->fetchAllTest("select * from product_pricing pr, product p where p.code = '%s' and pr.product_id = p.id order by pr.min_quantity",REDEMPTION_PRODUCT);
		$amt = 0;
		...
		if (array_key_exists(__FUNCTION__,$_POST)) {
			$inner->addData($_POST);
			if ($inner->validate()) {
				$this->logMessage(__FUNCTION__,sprintf("made it past validation"),1);
				$to_redeem = min($_POST["to_redeem"], $total_pts);
				$e = new Ecom();
				$cart = $e->getCart();
				$cart['header']['points_redeemed'] = $to_redeem;
				$cart['header']['redemption_value'] = -min($net,$cart['header']['value']);
				$cart = $e->updateCart($cart);
				$_SESSION['cart'] = $cart;
				$this->logMessage(__FUNCTION__,sprintf("updated cart [%s]", print_r($cart,true)), 1);
				if (strlen($module["parm1"]) > 0) {
					$inner->init($this->m_dir.$module["parm1"]);
				}
			}
		}
		$outer->setData("amount",$amt);
		$inner->setData("amount",$amt);
		$outer->addTag("form",$inner->show(),false);
		return $outer->show();
	}
