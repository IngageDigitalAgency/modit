...
//
			...
			if ($this->checkArray(sprintf("cart:products:%d|0|0|0:id", DRY_ICE), $_SESSION)) {
				if ($dtl = $this->fetchSingle(sprintf("select * from order_lines where order_id = %d and product_id = %d", $orderId, DRY_ICE))) {
...
*/
	...
	function setRecurring($orderId,$cart) {
		...
		$status = true;
		$o = $this->fetchSingle(sprintf("select * from orders where id = %d", $orderId));
		unset($o["id"]);
		...
		$newId = $this->insertId();
		$d = $this->fetchAll(sprintf("select * from order_lines where order_id = %d and deleted = 0 order by line_id", $orderId));
		...
		$d = $this->fetchAll(sprintf("select * from order_lines_dimensions where order_id = %d", $orderId));
		foreach($d as $key=>$line) {
			unset($line["id"]);
			$line["order_id"] = $newId;
		...
		$d = $this->fetchAll(sprintf("select * from addresses where ownertype='order' and ownerid = %d", $orderId));
		foreach($d as $key=>$line) {
			unset($line["id"]);
			$line["ownerid"] = $newId;
...
/*
		$deliveries = $this->fetchAll(sprintf("select * from custom_delivery where order_id = %d", $orderId));
...
*/
	...
	function setNextDate($origId,$orderId) {
		...
		$billed = $this->fetchSingle(sprintf("select * from order_billing where original_id = %d order by period_number desc", $orderId));
		...
		if (is_array($billed)) {
			...
			$pu = $this->fetchSingle(sprintf("select * from custom_delivery where order_id = %d and service_type='P'",$origId));
		...
		$p = $this->fetchSingle(sprintf("select * from order_lines where order_id = %d and custom_package = 'S' and deleted = 0", $orderId));
		...
		$status = $status && $stmt->execute();
		$ct = $this->fetchScalar(sprintf("select count(0) from order_billing where original_id = %d and billed = 0", $orderId));
		if ($ct < 10) {
	...
	function pickupAddresses() {
		...
		if (array_key_exists("a_id",$_REQUEST)) {
			...
			if ($addresses = $this->fetchAll(sprintf("select * from addresses where ownertype='member' and ownerid = %d and id = %d order by company, line1",$member,$_REQUEST["a_id"])))
			...
			$addresses = $this->fetchAll(sprintf("select * from addresses where ownertype='member' and ownerid = %d and address_book=1 and deleted = 0 and addresstype != %d order by company, line1",$member, ADDRESS_COMPANY));
		$result = array();
		...
		if (array_key_exists("pickupAddressForm",$_REQUEST) && array_key_exists("saveAddress",$_REQUEST)) {
			...
			if ($inner->validate()) {
				...
				$values["addresstype"] = ADDRESS_PICKUP;	//$this->fetchScalar("select id from code_lookups where type='memberAddressTypes' order by sort desc limit 1");
				if ($_REQUEST["a_id"] == 0) {
	...
	function shippingAddresses() {
		...
		if (array_key_exists("shippingAddressForm",$_REQUEST) && array_key_exists("saveAddress",$_REQUEST)) {
			...
			if ($inner->validate()) {
				...
				$values["ownerid"] = $member;
				$values["ownertype"] = "member";
				$values["addresstype"] = ADDRESS_DELIVERY;	//$this->fetchScalar("select id from code_lookups where type='memberAddressTypes' order by sort asc limit 1");
				if ($_REQUEST["a_id"] == 0) {
					$stmt = $this->prepare(sprintf("insert into addresses(%s) values(?%s)",implode(", ",array_keys($values)),str_repeat(", ?",count($values)-1)));
				}
				else {
	...
	function setAddressType() {
		...
		$_SESSION["cart"]["addresses"][$_REQUEST["a_type"]] = $this->fetchSingle(sprintf("select * from addresses where id = %d",$_REQUEST["a_id"]));
	...
	public function checkSubsidiary() {
		...
		if (array_key_exists("checkSubsidiary",$_REQUEST)) {
			if ($_REQUEST["c_id"] > 0) {
				if (!array_key_exists("mgmt",$_SESSION)) $_SESSION["mgmt"]["user"] = $this->fetchSingle(sprintf("select * from members where id = %d", $this->getUserInfo("id")));
				if ($company = $this->fetchSingle(sprintf("select * from members where id = %d and (custom_parent_org = %d || %d = 1) and id not in (select member_id from drivers)", $_REQUEST["c_id"], $_SESSION["mgmt"]["user"]["id"],$_SESSION["mgmt"]["user"]["custom_super_user"]))) {
				...
				if (array_key_exists("mgmt",$_SESSION)) {
					if ($company = $this->fetchSingle(sprintf("select * from members where id = %d", $_SESSION["mgmt"]["user"]["id"]))) {
						if ($this->logMeIn($company["username"],$company["password"],0,$company["id"])) {
							$_SESSION["cart"] = Ecom::initCart();
							if (strlen($module["parm1"]) > 0)
								$outer->init($this->m_dir.$module["parm1"]);
		...
		if ($test) {
			//$companies = $this->fetchAll(sprintf("select * from members where deleted = 0 and enabled = 1 and custom_on_account = 1 and id not in (select member_id from drivers) order by company, lastname"));
			$companies = $this->fetchAll(sprintf("select * from members where deleted = 0 and enabled = 1 and id not in (select member_id from drivers) order by company, lastname"));
		}
		else {
			if (array_key_exists("mgmt",$_SESSION) && array_key_exists("user",$_SESSION["mgmt"]) && $_SESSION["mgmt"]["user"]["id"] > 0) {
				$companies = $this->fetchAll(sprintf("select * from members where (custom_parent_org = %d || %d = 1) and deleted = 0 and enabled = 1 and id not in (select member_id from drivers) order by company, lastname",$_SESSION["mgmt"]["user"]["id"],$_SESSION["mgmt"]["user"]["custom_super_user"]));
			}
			else {
				$companies = $this->fetchAll(sprintf("select * from members where custom_parent_org = %d and deleted = 0 and enabled = 1 order by company, lastname",$this->getUserInfo("id")));
			}
		}
		if (array_key_exists("mgmt",$_SESSION)) {
			$ho = $this->fetchSingle(sprintf("select * from members where (id=%d || id=%d)",$_SESSION["mgmt"]["user"]["id"], $_SESSION["mgmt"]["user"]["custom_super_user"] ));
			$outer->addData($ho);
		}
		else {
	...
	function selectService() {
		...
		if (!$group = $this->fetchSingle(sprintf("select mf.* from members_folders mf, members_by_folder mb where mb.member_id = %d and mf.id = mb.folder_id",$m_id)))
			$group = array("id"=>0, "custom_additional_services"=>0);
		$g_id = $group["id"];	//$this->fetchScalar(sprintf("select folder_id from members_by_folder where member_id = %d",$m_id));
		...
		$g_id = $group["id"];	//$this->fetchScalar(sprintf("select folder_id from members_by_folder where member_id = %d",$m_id));
//		$m_services = $this->fetchAll(sprintf("select c.id, p.name, c.product_id, p.custom_minimum_charge, p.custom_km_mincharge from product p, custom_member_product_options c, members_by_folder mf where p.id = c.product_id and c.member_id = mf.id and mf.member_id = %d and mf.folder_id = %d and isgroup = 0 and is_fedex = 0 and p.deleted = 0 and p.published = 1 and p.enabled = 1 and custom_availability & %3\$d = %3\$d and unavailable = 0",$m_id,$g_id, 2**$pu_dow));
		$super = $this->checkArray("mgmt:user:custom_super_user",$_SESSION) && $_SESSION["mgmt"]["user"]["custom_super_user"] == 1;
		$m_temp = $this->fetchAll(sprintf("select 0 as total, c.*, p.name, p.custom_minimum_charge, p.custom_km_mincharge from product p, custom_member_product_options c, members_by_folder mf where p.id = c.product_id and c.member_id = mf.id and mf.member_id = %d and mf.folder_id = %d and isgroup = 0 and is_fedex = 0 and p.deleted = 0 and p.published = 1 and p.enabled = 1 and custom_availability & %3\$d = %3\$d and unavailable = 0 and (admin_only = 0 or %4\$d = 1)",$m_id,$g_id, 2**$pu_dow, $super));
		$tmp = array(0);
		foreach($m_temp as $key=>$value) {
			$tmp[] = $value["product_id"];
		}
		if (!$m_remove = $this->fetchScalarAll(sprintf("select c.product_id from product p, custom_member_product_options c, members_by_folder mf where p.id = c.product_id and c.member_id = mf.id and mf.member_id = %d and mf.folder_id = %d and isgroup = 0 and is_fedex = 0 and p.deleted = 0 and p.published = 1 and p.enabled = 1 and unavailable = 1",$m_id,$g_id)))
			$m_remove = array(0=>0);
//		$g_services = $this->fetchAll(sprintf("select c.id, p.name, c.product_id, p.custom_minimum_charge, p.custom_km_mincharge from product p, custom_member_product_options c where p.id = c.product_id and c.member_id = %d and isgroup = 1 and p.id not in (%s) and is_fedex = 0 and p.deleted = 0 and p.published = 1 and p.enabled = 1 and c.product_id not in (%s) and custom_availability & %4\$d = %4\$d order by p.custom_minimum_charge",$g_id, implode(",",array_merge(array(0),$tmp)), implode(", ", $m_remove), 2**$pu_dow));
		$g_temp = $this->fetchAll(sprintf("select 0 as total, c.*, p.name, p.custom_minimum_charge, p.custom_km_mincharge from product p, custom_member_product_options c where p.id = c.product_id and c.member_id = %d and isgroup = 1 and p.id not in (%s) and is_fedex = 0 and p.deleted = 0 and p.published = 1 and p.enabled = 1 and custom_availability & %3\$d = %3\$d and p.id not in (%4\$s) and (admin_only = 0 or %5\$d = 1) order by p.custom_minimum_charge",$g_id, implode(",",array_merge(array(0),$tmp)), 2**$pu_dow, implode(", ", $m_remove), $super));
		...
		if (array_key_exists(__FUNCTION__,$_REQUEST)) {
			...
			if ($valid) {
				if (array_key_exists("serviceType",$_REQUEST) && $this->checkArray("cart:header:inzone",$_SESSION) && $_SESSION["cart"]["header"]["inzone"] == 1) {
					if ($product = $this->fetchSingle(sprintf("select p.*, c.cutoff_time from product p, custom_member_product_options c where c.id = %d and p.id = c.product_id",$_REQUEST["serviceType"]))) {
...
//
						if ($isClosed = $this->fetchSingle(sprintf("select * from office_schedule where office_date = '%s'", date("Y-m-d",$pu_time)))) {
...
//
	...
	private function getProducts(&$inner,&$outer,$module,$i_flds,$services,$fromCart) {
		...
		if (array_key_exists("prod",$_REQUEST)) {
			foreach($_REQUEST["prod"] as $key=>$rec) {
				$inner->reset();
				$rec["product"] = $p->formatData($this->fetchSingle(sprintf("select * from product where id = %d",$rec["product_id"])));
				$rec["row"] = $key;
				$inner->addData($rec);
	...
	private function getExtras(&$inner,&$outer,$module) {
		...
		if (array_key_exists("extras",$_REQUEST)) {
			foreach($_REQUEST["extras"] as $key=>$rec) {
				if ($rec > 0) {
					$inner->reset();
					$inner->addData($p->formatData($this->fetchSingle(sprintf("select * from product where id = %d",$rec))));
	...
	function selectServiceRow($module) {
		...
		$inner->getField("product_id")->setOptions($this->fetchOptions(sprintf("select p.id, p.name from product p, product_by_folder pf, members_folders m, members_by_folder mf where mf.member_id = %d and m.id = mf.folder_id and pf.folder_id = m.custom_package_types and p.id = pf.product_id and p.enabled = 1 and p.deleted = 0 and p.id not in (%s) order by p.name",$this->getUserInfo("id"),$exists)));
	...
	function selectServiceDimensions($module = null) {
		...
		if (array_key_exists("product",$module) && array_key_exists("dimensions",$module["product"]) && count($module["product"]["dimensions"]) > 0) {
			...
			if (array_key_exists("addPkg",$_REQUEST)) {
				...
				$o = $this->fetchOptions(sprintf("select p.id, p.name from product p, product_by_folder pf, members_folders m, members_by_folder mf where mf.member_id = %d and m.id = mf.folder_id and pf.folder_id = m.custom_package_types and p.id = pf.product_id and p.enabled = 1 and p.deleted = 0 and p.id not in (%s) order by p.name",$this->getUserInfo("id"),$exists));
				$p = 0;
				foreach($o as $key=>$value) {
					if ($p==0) $p=$key;
				}
				$p = $this->fetchSingle(sprintf("select * from product where id = %d",$p));
				$inner->addData(array(
					"ht"=>$p["height"],
					"wd"=>$p["width"],
					"dp"=>$p["depth"],
	...
	function KJVService() {
		...
		$quote["wt"] = $this->fetchSingle(sprintf("select * from code_lookups where id = %d",$quote["custom_weight_code"]));
		$quote["sz"] = $this->fetchSingle(sprintf("select * from code_lookups where id = %d",$quote["custom_dimension_code"]));
		$outer->addData($quote);
		$flds = $this->config->getFields($module["configuration"]);
		$group = $this->fetchSingle(sprintf("select mf.* from members_folders mf, members_by_folder mb where mb.member_id = %d and mf.id = mb.folder_id",$m_id));
		$g_id = $group["id"];	//$this->fetchScalar(sprintf("select folder_id from members_by_folder where member_id = %d",$m_id));
		$super = $this->checkArray("mgmt:user:custom_super_user",$_SESSION) && $_SESSION["mgmt"]["user"]["custom_super_user"] == 1;
		$m_services = $this->fetchAll(sprintf("select c.id, p.name, c.product_id from product p, custom_member_product_options c, members_by_folder mf where p.id = c.product_id and c.member_id = mf.id and mf.member_id = %d and mf.folder_id = %d and isgroup = 0 and p.enabled = 1 and p.published = 1 and unavailable = 0 and (admin_only = 0 or %d = 1) order by p.name",$m_id,$g_id, $super));
		$tmp = array(0);
		...
		$g_services = $this->fetchAll(sprintf("select c.id, p.name, c.product_id from product p, custom_member_product_options c where p.id = c.product_id and c.member_id = %d and isgroup = 1 and p.id not in (%s) and p.id not in (%s) and (admin_only = 0 or %d = 1)",$g_id, implode(",",array_merge(array(0),$tmp)), implode(", ", $m_remove), $super));
		$services = array_merge($m_services, $g_services);
		$result = array();
...
//
	...
	function FedExService() {
		...
		if (!array_key_exists("rates",$quote) || count($quote["rates"]) == 0) {
			...
			if (array_key_exists("custom",$cart) && array_key_exists("rates",$cart["custom"]) && count($cart["custom"]["rates"]) > 0) {
				foreach($cart["custom"]["rates"] as $k=>$r) {
					if ($s = $this->fetchSingle(sprintf("select * from product where code='%s' and enabled = 1 and deleted = 0",$k))) {
		...
		if (array_key_exists(__FUNCTION__,$_REQUEST)) {
			...
			$cart['products'] = array();
			$cart = $this->initPriceLine($cart,$key,$this->fetchSingle(sprintf("select * from product where id = %d",$_REQUEST["serviceType"])));
	...
	function calcPickup($pickup,$product) {
		$p = $this->fetchSingle(sprintf("select * from product where id = %d",$product["product_id"]));
		$dt = strtotime($pickup);
		$dow = date("w",$dt);
		$i = 0;
	...
	function myAddresses() {
		...
		$addresses = $this->fetchAll(sprintf("select * from addresses where ownertype='member' and ownerid = %d and deleted = 0 and address_book = 1 and addresstype != %d order by company, lastname, firstname",$this->getUserInfo("id"), ADDRESS_COMPANY));
		$recs = array();
		$ct = 0;
	...
	function addressEditing() {
		...
		$inner->init($this->m_dir.$module['inner_html']);
		$flds = $inner->buildForm($this->config->getFields($module['configuration']));
		if (!($data = $this->fetchSingle(sprintf("select * from addresses where id = %d and ownerid = %d",$a_id,$this->getUserInfo("id")))))
			$data = array("id"=>0);
		$inner->addData($data);
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__,$_POST)) {
			...
			if ($valid) {
				...
				if ((!array_key_exists("addresstype",$values)) || $values["addresstype"] == 0)
					$values["addresstype"] = ADDRESS_PICKUP;	//$this->fetchScalar(sprintf("select id from code_lookups where type='memberAddressTypes' and extra = 1"));		
	...
	function driverSchedule() {
		...
		if (array_key_exists("lat", $_REQUEST) && abs($_REQUEST["lat"]) > .01 &&
			...
			if ($pos = $this->fetchSingle(sprintf("select * from driver_location where member_id = %d order by datetime desc limit 1", $this->getUserInfo("id")))) {
...
order by driver_sequence ", STATUS_CANCELLED, $this->getUserInfo("id"),date("Y-m-d"), date("Y-m-d", strtotime("today + 1 day")), ACK_ACKNOWLEDGE, $module["where_clause"]);
		...
		foreach($recs as $key=>$rec) {
			if ($address = $this->fetchSingle(sprintf("select * from addresses where ownertype='order' and ownerid = %d and addresstype = '%s'",
										$rec["order_id"],$rec["service_type"] == "P" ? ADDRESS_PICKUP : ADDRESS_DELIVERY)))
				$rec["address"] = Address::formatData($address);
			//$pkgs = $this->fetchAll(sprintf("select l.*, p.name from order_lines l, product p where l.order_id = %d and l.is_product = 1 and p.id = l.product_id",$rec["order_id"]));
			...
			$extras = $this->fetchScalarAll(sprintf("select p.name from product p, order_lines ol where ol.order_id = %d and ol.deleted = 0 and p.id = ol.product_id and ol.custom_package = 'A' and p.custom_special_requirement order by ol.line_id", $rec["order_id"]));
			$rec["extras"] = implode(", ", $extras);
			$pkgs = $this->fetchScalarAll(sprintf("select concat(sum(ol.quantity),' @ ',p.name) as pkgs from product p, order_lines ol where ol.order_id = %d and ol.deleted = 0 and p.id = ol.product_id and ol.custom_package = 'P' group by p.name", $rec["order_id"]));
	...
	function pickupInfo() {
		...
		if ($rec = $this->fetchSingle(sprintf("select d.*, p.name, c1.code as weight_type, c2.code as dimension_type, l.quantity as quantity, o.order_status, CURRENT_TIMESTAMP() as actual_date from orders o, custom_delivery d, order_lines l, product p, code_lookups c1, code_lookups c2, drivers dr where dr.member_id = %d and d.driver_id = dr.id and d.id = %d and l.order_id = d.order_id and l.custom_package = 'S' and p.id = l.product_id and c1.id = custom_weight_code and c2.id = custom_dimension_code and o.id = d.order_id and l.deleted = 0 %s order by driver_sequence limit %d", $this->getUserInfo("id"), $c_id, $module["where_clause"], $module["records"]))) {
			$rec["address"] = Address::formatData($this->fetchSingle(sprintf("select * from addresses where ownerid = %d and ownertype = 'order' and addresstype=%d", $rec["order_id"], ADDRESS_PICKUP )));
			$pkgs = $this->fetchAll(sprintf("select od.*, p.name from order_lines ol, order_lines_dimensions od, product p where ol.deleted = 0 and ol.order_id = %d and ol.custom_package='P' and p.id = ol.product_id and od.order_id = ol.order_id and od.line_id = ol.line_id and ol.deleted = 0 order by ol.line_id, od.id", $rec["order_id"]));
			$p = array();
			if ($delivery = $this->fetchSingle(sprintf("select * from custom_delivery where order_id = %d and service_type = 'D'",$rec["order_id"]))) {
				$delivery["address"] = Address::formatData($this->fetchSingle(sprintf("select * from addresses where ownertype='order' and ownerid = %d and addresstype = %d", $rec["order_id"], ADDRESS_DELIVERY)));
			...
			$pkgs = $this->fetchAll(sprintf("select ol.*, p.name from order_lines ol, product p, product_by_folder pf where ol.order_id = %d and ol.custom_package='A' and p.id = ol.product_id and pf.folder_id = %d and pf.product_id = p.id and ol.deleted = 0 order by ol.line_id", $rec["order_id"],DELIVERY_SURCHARGES));
			$p = array();
			$inner->init($this->m_dir.$module["parm1"]);
			foreach($pkgs as $key=>$tmp) {
				$inner->addData($tmp);
		...
		if (array_key_exists(__FUNCTION__,$_REQUEST)) {
			...
			if ($outer->validate()) {
				...
				if ($this->hasOption("email")) {
					if ($conf = $this->fetchSingle(sprintf("select o.custom_pickup_email, m.* from orders o, members m where o.id = %d and m.id = o.member_id", $rec["order_id"]))) {
						if ($conf["custom_pickup_notification"] || strlen($conf["custom_pickup_email"]) > 0 || strlen($conf["custom_pickup_emails"]) > 0) {
							...
							$html = $this->fetchSingle($sql);
							$body->setHTML($html['html']);
							if (!$order = $this->fetchSingle(sprintf('select o.*, cd.delivery_name, m.firstname, m.lastname, m.email from orders o, members m, custom_delivery cd where o.id = %d and m.id = o.member_id and cd.order_id = o.id and cd.service_type = "D"',$orderId)))
								$this->logMessage(__FUNCTION__,sprintf('cannot locate order #[%d]',$orderId),1,true);
							if ($module = $this->fetchSingle(sprintf('select t.*, m.classname, t.id as fetemplate_id from fetemplates t, modules m where t.id = %d and m.id = t.module_id',$this->getOption('email')))) {
	...
	function validatePickup($data,$obj,$dataKey,$req,&$errors,&$messages) {
		...
		if ($dataKey == "delivery_name") {
			$o = $this->fetchSingle(sprintf("select * from orders where id = %d", $data["order_id"]));
	...
	function editPackageLine() {
		...
		$c_id = array_key_exists("c_id",$_REQUEST) ? $_REQUEST["c_id"] : 0;
		if ($l_id == 0)
			$rec = $this->fetchSingle(sprintf("select o.member_id, o.id as order_id, l1.value as weight_code, l2.value as dimension_code, d.service_type 
...
and l1.id = o.custom_weight_code and l2.id = o.custom_dimension_code and ol.deleted = 0", $c_id, $this->getUserInfo("id"), $o_id, $l_id));
		...
		if (count($_REQUEST) > 0 && array_key_exists(__FUNCTION__,$_REQUEST)) {
			if (array_key_exists("delete",$_REQUEST) && $_REQUEST["delete"] == 1) {
				$this->updateNotes($rec["order_id"], sprintf("Deleting package type %s, %d @ %.2f x %.2f x %.2f %.2f%s", 
					$this->fetchScalar(sprintf("select p.name from product p, order_lines ol, order_lines_dimensions old where old.id = %d and ol.order_id = old.order_id and ol.line_id = old.line_id and p.id = ol.product_id", $rec["l_id"])),
				...
				if (($ct = $this->fetchScalar(sprintf("select count(0) from order_lines_dimensions where id = %d and order_id = %d",$_REQUEST["l_id"],$_REQUEST["o_id"]))) == 0) {
				...
				if ($valid) {
					...
					if ($l_id == 0) {
						...
						$this->beginTransaction();
						$this->execute(sprintf("update orders set order_status = order_status | %d where id = %d", STATUS_NEEDS_APPROVAL | STATUS_DRIVER_CHANGED, $rec["order_id"]));
						if ($ol = $this->fetchSingle(sprintf("select order_id, line_id from order_lines where order_id = %d and product_id = %d and deleted = 0",$o_id,$_REQUEST["product_id"]))) {
							$l_id = $ol["line_id"];
						}
						else {
							$l_id = $this->fetchScalar(sprintf("select max(line_id) from order_lines o where o.order_id = %d",$o_id)) + 1;
							$ol = array(
						...
						if ($valid) {
							...
							//$outer = new Forms();
							$_REQUEST["name"] = $this->fetchScalar(sprintf("select name from product where id = %d",$_REQUEST["product_id"]));
							//$outer->addData($_REQUEST);
							$flds = $outer->buildForm($flds);
			...
			//
			$wt = $this->fetchSingle(sprintf("select sum(weight) as weight, l.* from order_lines_dimensions old, code_lookups l, orders o where o.id = %d and old.order_id = o.id and l.id = o.custom_weight_code", $rec["order_id"]));
			$wt["weight"] = $wt["weight"] * $wt["extra"];
			$grp = $this->fetchSingle(sprintf("select mf.*, mbf.id as junction from members_folders mf, members_by_folder mbf where mbf.member_id = %d and mf.id = mbf.folder_id limit 1", $rec["member_id"]));
			$mbr = $this->fetchSingle(sprintf("select * from members where id = %d", $rec["member_id"]));
			$free_weight = max($grp["custom_free_weight"],$mbr["custom_free_weight"]);
			$charge_weight = round($wt["weight"] - $free_weight,0);
			$curr_charge = $this->fetchSingle(sprintf("select * from order_lines where order_id = %d and product_id = %d and deleted = 0", $rec["order_id"], $grp["custom_weight"]));
			...
			$line = $this->fetchSingle(sprintf("select * from product where id = %d", $grp["custom_weight"]));
			$price = $this->fetchSingle(sprintf("select * from product_pricing where product_id = %d order by max_quantity desc limit 1", $line["id"]));
			...
			if ($charge_weight > 0.1) {
				...
				if (is_array($curr_charge)) {
					...
					$line["line_id"] = $this->fetchScalar(sprintf("select max(line_id) from order_lines where order_id = %d", $rec["order_id"])) + 1;
	...
	function editServiceLine() {
		...
		$flds = $this->getFields($module["configuration"]);
		if ($l_id == 0) {
			$rec = $this->fetchSingle(sprintf("select o.member_id, o.id as order_id, d.service_type from custom_delivery d, orders o, drivers dr where dr.member_id = %d and d.driver_id = dr.id and d.id = %d and o.id = d.order_id", $this->getUserInfo("id"), $c_id));
			...
			$rec = $this->fetchSingle(sprintf("select ol.*, d.service_type from custom_delivery d, order_lines ol, drivers dr where dr.member_id = %d and d.driver_id = dr.id and ol.order_id = d.order_id and ol.line_id = %d and d.id = %d and ol.deleted = 0", $this->getUserInfo("id"), $l_id, $c_id));
		...
		if (count($_REQUEST) > 0 && array_key_exists(__FUNCTION__,$_REQUEST)) {
			if (array_key_exists("delete",$_REQUEST) && $_REQUEST["delete"] == 1) {
				$this->updateNotes($rec["order_id"], sprintf("Deleting product %s", $this->fetchScalar(sprintf("select p.name from product p, order_lines ol where ol.order_id = %d and ol.line_id = %d and p.id = ol.product_id", $rec["order_id"], $_REQUEST["l_id"]))));
				$this->beginTransaction();
				$this->execute(sprintf("update orders set order_status = order_status | %d where id = %d", STATUS_NEEDS_APPROVAL | STATUS_DRIVER_CHANGED, $rec["order_id"]));
			...
			if ($valid) {
				...
				$product = $this->fetchSingle(sprintf("select * from product where id = %d", $outer->getData("product_id")));
				...
				if ($l_id == 0) {
					...
					$l_id = $this->fetchScalar(sprintf("select max(line_id) from order_lines ol where ol.order_id = %d",$o_id)) + 1;
					...
					$this->updateNotes($rec["order_id"], sprintf("Updating product %s, Quantity: %d", $this->fetchScalar(sprintf("select p.name from product p, order_lines ol where ol.order_id = %d and ol.line_id = %d and p.id = ol.product_id", $rec["order_id"], $_REQUEST["l_id"])), $outer->getData("quantity")));
				...
				if ($valid) {
					$line = $this->fetchSingle(sprintf("select * from order_lines ol where order_id = %d and line_id = %d",$outer->getData("o_id"), $l_id));
					$line = Ecom::lineValue($line);
	...
	function pickupComments() {
		...
		if (defined("FRONTEND") && $this->getUserInfo("id") == 0)
				return "";
		$o_id = array_key_exists("o_id",$_REQUEST) ? $_REQUEST["o_id"] : 0;
		$rec = $this->fetchSingle(sprintf("select d.* from custom_delivery d where d.driver_id = %d and d.id = %d", $this->getUserInfo("id"), $o_id));
		$outer = new Forms();
		$outer->init($this->m_dir.$module['outer_html']);
		$outer->setModule($module);
	...
	function deliveryInfo() {
		...
		if ($rec = $this->fetchSingle(sprintf("select d.*, p.name, c1.code as weight_type, c2.code as dimension_type, l.quantity as quantity from orders o, custom_delivery d, order_lines l, product p, code_lookups c1, code_lookups c2, drivers dr where dr.member_id = %d and d.driver_id = dr.id and d.id = %d and l.order_id = d.order_id and l.custom_package = 'S' and p.id = l.product_id and c1.id = custom_weight_code and c2.id = custom_dimension_code and o.id = d.order_id and l.deleted = 0 %s order by driver_sequence limit %d", $this->getUserInfo("id"), $c_id, $module["where_clause"], $module["records"]))) {
			$rec["address"] = Address::formatData($this->fetchSingle(sprintf("select * from addresses where ownerid = %d and ownertype = 'order' and addresstype=%d", $rec["order_id"], ADDRESS_PICKUP )));
			$rec["pu_action"] = $this->fetchSingle(sprintf("select * from custom_delivery where order_id = %d and service_type='P'", $rec["order_id"]));
			$outer->addData($rec);
			...
			if (array_key_exists(__FUNCTION__,$_POST) && $_POST[__FUNCTION__] > 0) {
				...
				if ($valid) {
					...
					$stmt->bindParams(array_merge( array(str_repeat("s", count($upd))), array_values($upd) ));
					$old_notes = $this->fetchScalar(sprintf("select comments from custom_delivery where id = %d", $c_id));
					if ($stmt->execute()) {
			...
			$pkgs = $this->fetchAll(sprintf("select od.*, p.name from order_lines ol, order_lines_dimensions od, product p where ol.order_id = %d and ol.custom_package='P' and p.id = ol.product_id and od.order_id = ol.order_id and od.line_id = ol.line_id and ol.deleted = 0 order by ol.line_id, od.id", $rec["order_id"]));
			$p = array();
			foreach($pkgs as $key=>$tmp) {
				$tmp["o_id"] = $c_id;
				$inner->addData($tmp);
			...
			$outer->addTag("packages",implode("",$p),false);
			$pkgs = $this->fetchAll(sprintf("select ol.*, p.name from order_lines ol, product p, product_by_folder pf where ol.order_id = %d and ol.custom_package='A' and p.id = ol.product_id and pf.folder_id = %d and pf.product_id = p.id and custom_split_allocation in ('B','D') and ol.deleted = 0 order by ol.line_id", $rec["order_id"],DELIVERY_SURCHARGES));
			$p = array();
			$inner->init($this->m_dir.$module["parm1"]);
			foreach($pkgs as $key=>$tmp) {
				$inner->addData($tmp);
	...
	function captureSignature() {
		...
		if (count($_REQUEST) > 0 && array_key_exists("captureSignature",$_REQUEST)) {
			...
			if ($im != null) {
				$fn = sprintf("./images/signature-%s.png",rand(1000,10000));
...
*/
				...
				if ($this->fetchScalar(sprintf("select coalesce(count(order_id),0) from order_authorization where order_id = %d and authorization_type='A' and authorization_success = 1", $rec["order_id"])) > 0) {
				...
				if ($this->hasOption("email") && ($rec["custom_delivery_notification"] != 0 || strlen($rec["custom_email_confirmation"]) > 0)) {
					...
					$html = $this->fetchSingle($sql);
					$body->setHTML($html['html']);
					if (!$order = $this->fetchSingle(sprintf('select o.*, cd.delivery_name, m.firstname, m.lastname, m.email, m.custom_delivery_emails from orders o, members m, custom_delivery cd where o.id = %d and m.id = o.member_id and cd.order_id = o.id and cd.service_type = "D"',$orderId)))
						$this->logMessage(__FUNCTION__,sprintf('cannot locate order #[%d]',$orderId),1,true);
					if ($module = $this->fetchSingle(sprintf('select t.*, m.classname, t.id as fetemplate_id from fetemplates t, modules m where t.id = %d and m.id = t.module_id',$this->getOption('email')))) {
						$p = new product($module['id'],$module);
	...
	function orderHistory() {
		...
		$group = $this->fetchSingle(sprintf("select mf.* from members_folders mf, members_by_folder mb where mb.member_id = %d and mf.id = mb.folder_id",$m_id));
		$g_id = $group["id"];
		$m_services = $this->fetchAll(sprintf("select c.id, p.name, c.product_id from product p, custom_member_product_options c, members_by_folder mf where p.id = c.product_id and c.member_id = mf.id and mf.member_id = %d and mf.folder_id = %d and isgroup = 0",$m_id,$g_id));
		$tmp = array(0);
		foreach($m_services as $key=>$value) {
			$tmp[] = $value["product_id"];
		}
		$g_services = $this->fetchAll(sprintf("select c.id, p.name, c.product_id from product p, custom_member_product_options c where p.id = c.product_id and c.member_id = %d and isgroup = 1 and p.id not in (%s)",$g_id, implode(",",array_merge(array(0),$tmp))));
		$services = array_merge($m_services, $g_services);
		$tmp = array("0"=>"-Any-");
		...
		if ($outer->getData("export") > 0) {
			if (!($outer->getData("c_id")==0 && $orgs = $this->fetchScalarAll(sprintf("select id from members where custom_parent_org = %d", $this->getUserInfo("id")))))
...
implode(",",$orgs), $sqlPlus);
			...
			$recs = array();
			$flds = array();
			$o_status = $this->fetchAll(sprintf('select * from code_lookups where type="orderStatus" order by sort'));
...
$this->logMessage(__FUNCTION__,sprintf("recs [%s] orders [%s]", print_r($recs,true), print_r($orders,true)),1);
			...
			$totals = $this->fetchSingle(sprintf("select coalesce(sum(total),0) as g_total, coalesce(sum(authorization_amount),0) as g_authorization_amount, count(1) as g_ct %s %s", $sql, $sqlPlus));
			...
			foreach($orders as $key=>$value) {
				...
				$inner->reset();
				$value["serviceType"] = $this->fetchScalar(sprintf("select p.name from product p, order_lines ol where ol.order_id = %d and ol.custom_package = 'S' and p.id = ol.product_id and ol.deleted = 0", $value["id"]));
				$value["early"] = 0;
				if ($pickup = $this->fetchSingle(sprintf("select * from custom_delivery where order_id = %d and service_type='P'", $value["id"]))) {
					$pickup["address"] = $this->fetchSingle(sprintf("select * from addresses where ownerid = %d and ownertype='order' and addresstype=%d", $value["id"], ADDRESS_PICKUP));
					$value["pickup"] = $pickup;
				}
				if ($delivery = $this->fetchSingle(sprintf("select * from custom_delivery where order_id = %d and service_type='D'", $value["id"]))) {
					$delivery["address"] = $this->fetchSingle(sprintf("select * from addresses where ownerid = %d and ownertype='order' and addresstype=%d", $value["id"], ADDRESS_DELIVERY));
	...
	function getDimensions() {
		...
		$outer->init($this->m_dir.$module['outer_html']);
		$flds = $this->config->getFields($module['configuration']);
		$p = $this->fetchSingle(sprintf("select * from product where id = %d", array_key_exists("p_id",$_REQUEST) ? $_REQUEST["p_id"] : 0));
		$outer->addData($p);
		return $outer->show();
	}
	...
	function postSignup($data = array()) {
		...
		if ((!array_key_exists("custom_zones",$data)) || $data["custom_zones"] == 0) {
			$rec = $this->fetchSingle(sprintf("select * from members where id = %d", $data["id"]));
		...
		$data["address"] = Address::formatData($this->fetchSingle(sprintf("select * from addresses where ownertype='member' and ownerid=%d", $data["id"])));
	...
	function orderPayments() {
		...
		$o_id = array_key_exists("o_id",$_REQUEST) ? $_REQUEST["o_id"] : 0;
		$pmts = $this->fetchAll(sprintf("select d.*, p.reference_number, p.reference_date from order_payment p, order_payment_detail d where d.order_id = %d and p.id = d.payment_id", $o_id));
		$result = array();
	...
	function getSignature() {
		...
		$inner = new Forms();
		$inner->init($this->m_dir.$module["inner_html"]);
		$rec = $this->fetchSingle(sprintf("select * from custom_delivery where id = %d", array_key_exists("c_id", $_REQUEST) ? $_REQUEST["c_id"] : 0));
		$order = $this->fetchSingle(sprintf("select * from orders where id = %d", $rec["order_id"]));
	...
	function waybill() {
		...
		$o_id = array_key_exists("o_id",$_REQUEST) ? $_REQUEST["o_id"] : 0;
		if ($o_id == 0) return;
		if ($order = $this->fetchSingle(sprintf('select o.*, c1.code as wtCode, c2.code as dimCode from orders o, code_lookups c1, code_lookups c2, members m 
where o.id = %d and m.id = o.member_id and (m.id = %d or m.custom_parent_org = %2$d or m.id in (select m1.id from members m1 where custom_parent_org = %d)) and c1.id = o.custom_weight_code and c2.id = o.custom_dimension_code', $o_id, 
			...
			$pu = $this->fetchSingle(sprintf("select * from custom_delivery where order_id = %d and service_type = 'P'",$o_id));
			$de = $this->fetchSingle(sprintf("select * from custom_delivery where order_id = %d and service_type = 'D'",$o_id));
			$pu["address"] = Address::formatData($this->fetchSingle(sprintf("select * from addresses where ownerid = %d and ownertype = 'order' and addresstype = %d",$o_id,ADDRESS_PICKUP)));
			$de["address"] = Address::formatData($this->fetchSingle(sprintf("select * from addresses where ownerid = %d and ownertype = 'order' and addresstype = %d",$o_id,ADDRESS_DELIVERY)));
			$pu["driver"] = $this->fetchSingle(sprintf("select m.* from drivers d, members m where d.id = %d and m.id = d.member_id", $pu["driver_id"]));
			$de["driver"] = $this->fetchSingle(sprintf("select m.* from drivers d, members m where d.id = %d and m.id = d.member_id", $de["driver_id"]));
			$de["has_signature"] = strlen($de["signature"]) > 0;
			$de["sig_image"] = base64_encode($de["signature"]);
			$order["pickup"] = $pu;
			$order["delivery"] = $de;
			$service = $this->fetchSingle(sprintf("select ol.*, p.name from order_lines ol, product p where ol.order_id = %d and ol.custom_package='S' and p.id = ol.product_id and ol.deleted = 0",$o_id));
			$packages = $this->fetchAll(sprintf("select ol.*, p.name, ol.product_id, d.quantity as qty, d.height, d.width, d.depth, d.weight, d.quantity*d.weight as tWeight 
...
order by ol.line_id", $o_id, FEDEX_RECALC ));
			$additional = $this->fetchAll(sprintf("select ol.*, p.name from order_lines ol, product p where ol.order_id = %d and ol.custom_package='A' and p.id = ol.product_id and ol.deleted = 0",$o_id));
...
where id in (select id from (select c.id from custom_delivery c, order_lines ol, product p where ol.order_id = c.order_id and p.id = ol.product_id and p.custom_same_day = 1 and ol.custom_package = 'S' and ol.deleted = 0) as ids);");
			$recs = $this->fetchAll(sprintf("select order_id, cast(rand() as unsigned) as r from order_lines ol, product p where p.id = ol.product_id and p.custom_same_day = 0 and ol.custom_package = 'S' and ol.deleted = 0"));
			foreach($recs as $rec) {
				if ($rec["r"] == 0) {
	...
	function driverSearch() {
		...
		if (array_key_exists(__FUNCTION__,$_REQUEST)) {
			...
			$cur = array_merge(array(0),$this->fetchScalarAll($sql));
	...
	function dispatching() {
		...
		foreach($data as $rec) {
			if ($rec["pickup"] = $this->fetchSingle(sprintf("select *, date(scheduled_date) as sd, curdate() as td from custom_delivery where order_id = %d and service_type='P'",$rec["order_id"]))) {
				...
				if ($et->d <= 0)
					...
					$rec["pickup"]["et"] = 999;
				if (!$rec["pickup"]["driver"] = $this->fetchSingle(sprintf("select v.name as vehicle, m.* from members m, drivers d, custom_delivery c, vehicles v where c.order_id = %d and service_type='P' and d.id = c.driver_id and m.id = d.member_id and v.id = d.vehicle_id", $rec["order_id"])))
					$rec["pickup"]["driver"] = array("id"=>0,"firstname"=>"","lastname"=>"","vehicle"=>"0");
			}
			if ($rec["delivery"] = $this->fetchSingle(sprintf("select *, date(scheduled_date) as sd, curdate() as td from custom_delivery where order_id = %d and service_type='D'",$rec["order_id"]))) {
				$d = new DateTime($rec["delivery"]["scheduled_date"]);
				$et = $d->diff(new DateTime(date(DATE_ATOM)));
				$rec["delivery"]["et"] = (60*$et->h + $et->i)*($et->invert == 0 ? -1:1);
				if (!$rec["delivery"]["driver"] = $this->fetchSingle(sprintf("select v.name as vehicle, m.* from members m, drivers d, custom_delivery c, vehicles v where c.order_id = %d and service_type='D' and d.id = c.driver_id and m.id = d.member_id and v.id = d.vehicle_id", $rec["order_id"])))
					$rec["delivery"]["driver"] = array("id"=>0,"firstname"=>"","lastname"=>"","vehicle"=>"0");
			}
			$rec["pickup"]["address"] = $this->fetchSingle(sprintf("select * from addresses where ownerid=%d and ownertype='order' and addresstype=%d", $rec["order_id"], ADDRESS_PICKUP));
			$rec["delivery"]["address"] = $this->fetchSingle(sprintf("select * from addresses where ownerid=%d and ownertype='order' and addresstype=%d", $rec["order_id"], ADDRESS_DELIVERY));
			$rec["services"] = implode(", ", $this->fetchScalarAll(sprintf("select p.name from product p, order_lines ol where ol.order_id = %d and ol.custom_package = 'A' and ol.deleted = 0 and p.id = ol.product_id and p.custom_special_requirement = 1", $rec["order_id"])));
			$rec["billto"] = $this->fetchSingle(sprintf("select * from addresses where ownerid = %d and ownertype = 'member' and addresstype = %d and deleted = 0", $rec["member_id"], ADDRESS_COMPANY));
			$packages = $this->fetchAll(sprintf("select sum(old.quantity) as qty, sum(old.weight) as weight, p.name, c.code, c1.code as dim_units, old.height, old.width, old.depth from order_lines ol, order_lines_dimensions old, product p, orders o, code_lookups c, code_lookups c1 where ol.order_id = %d and ol.custom_package='P' and p.id = ol.product_id and old.order_id = ol.order_id and old.line_id = ol.line_id and o.id = ol.order_id and c.id = o.custom_weight_code and c1.id = o.custom_dimension_code group by old.line_id", $rec["order_id"]));
			$tmp = array();
...
"q"=>"o.order_status_processing"
		...
		$data = $this->fetchAll($sql);
		foreach($data as $rec) {
			if ($rec["pickup"] = $this->fetchSingle(sprintf("select *, CURDATE() as td, DATE(actual_date) as ad, DATE(scheduled_date) as sd from custom_delivery where id=%d",$rec["pickup"]))) {
				$d = new DateTime($rec["pickup"]["scheduled_date"]);
				$et = $d->diff(new DateTime(date(DATE_ATOM)));
				$rec["pickup"]["et"] = (60*$et->h + $et->i)*($et->invert == 0 ? -1:1);
				if (!($rec["pickup"]["driver"] = $this->fetchSingle(sprintf("select v.name as vehicle, m.* from members m, drivers d, custom_delivery c, vehicles v where c.order_id = %d and service_type='P' and d.id = c.driver_id and m.id = d.member_id and v.id = d.vehicle_id", $rec["order_id"]))))
			...
			if ($rec["delivery"] = $this->fetchSingle(sprintf("select *, CURDATE() as td, DATE(actual_date) as ad, DATE(scheduled_date) as sd from custom_delivery where id=%d",$rec["delivery"]))) {
				$d = new DateTime($rec["delivery"]["scheduled_date"]);
				$et = $d->diff(new DateTime(date(DATE_ATOM)));
				$rec["delivery"]["et"] = (60*$et->h + $et->i)*($et->invert == 0 ? -1:1);
				if (!($rec["delivery"]["driver"] = $this->fetchSingle(sprintf("select v.name as vehicle, m.* from members m, drivers d, custom_delivery c, vehicles v where c.order_id = %d and service_type='D' and d.id = c.driver_id and m.id = d.member_id and v.id = d.vehicle_id", $rec["order_id"]))))
...
where %s", STATUS_NEEDS_APPROVAL, implode(" and ",array_values($srch)));
		...
		$data = $this->fetchAll($sql);
		foreach($data as $rec) {
			if ($rec["delivery"] = $this->fetchSingle(sprintf("select *, CURDATE() as td, DATE(actual_date) as ad, DATE(scheduled_date) as sd from custom_delivery where order_id = %d and service_type='D'",$rec["order_id"]))) {
				$d = new DateTime($rec["delivery"]["scheduled_date"]);
				$et = $d->diff(new DateTime(date(DATE_ATOM)));
				$rec["delivery"]["et"] = (60*$et->h + $et->i)*($et->invert == 0 ? -1:1);
				if (!$rec["delivery"]["driver"] = $this->fetchSingle(sprintf("select v.name as vehicle, m.* from members m, drivers d, custom_delivery c, vehicles v where c.order_id = %d and service_type='D' and d.id = c.driver_id and m.id = d.member_id and v.id = d.vehicle_id", $rec["order_id"])))
	...
	function getBillingType($orderId) {
		...
		switch($o["authorization_type"]) {
			...
			case "PayFlow":
				...
				break;
			case "Other":
				$c = $this->fetchSingle(sprintf("select * from members where id = %d", $o["member_id"]));
		...
		if (count($data["prod"]) == 0) {
			...
			if ($wt < .1) {
				...
				if ($data["optType"]=="S") {
					...
					$group = $this->fetchSingle(sprintf("select mf.* from members_folders mf, members_by_folder mb where mb.member_id = %d and mf.id = mb.folder_id",$m_id));
					$g_id = $group["id"];
					$m_services = $this->fetchAll(sprintf("select c.id, c.id as serviceType, p.name, c.product_id, p.custom_minimum_charge from product p, custom_member_product_options c, members_by_folder mf where p.id = c.product_id and c.member_id = mf.id and mf.member_id = %d and mf.folder_id = %d and isgroup = 0 and custom_same_day = 1 and custom_availability & %3\$d = %3\$d and unavailable = 0 and (admin_only = 0 or %4\$d = 1)",$m_id,$g_id, 2**$pu_dow, $super));
					$tmp = array(0);
					foreach($m_services as $key=>$value) {
						$tmp[] = $value["product_id"];
					}

					if (!$m_remove = $this->fetchScalarAll(sprintf("select c.product_id from product p, custom_member_product_options c, members_by_folder mf where p.id = c.product_id and c.member_id = mf.id and mf.member_id = %d and mf.folder_id = %d and isgroup = 0 and custom_same_day = 1 and unavailable = 1",$m_id,$g_id)))
						$m_remove = array("0"=>"0");
					$g_normal = $this->fetchAll(sprintf("select c.id, c.id as serviceType, p.name, c.product_id, p.custom_minimum_charge from product p, custom_member_product_options c where p.id = c.product_id and c.member_id = %d and isgroup = 1 and p.id not in (%s) and custom_same_day = 1 and custom_availability & %3\$d = %3\$d and p.id not in (%4\$s) and (admin_only = 0 or %5\$d = 1) and c.out_of_zone_alternate = 0",$g_id, implode(",",array_merge(array(0),$tmp)), 2**$pu_dow, implode(", ", $m_remove), $super));
					$g_prod = $this->fetchScalarAll(sprintf("select c.out_of_zone_alternate from product p, custom_member_product_options c where p.id = c.product_id and c.member_id = %d and isgroup = 1 and p.id not in (%s) and custom_same_day = 1 and custom_availability & %3\$d = %3\$d and p.id not in (%4\$s) and (admin_only = 0 or %5\$d = 1) and c.out_of_zone_alternate > 0",$g_id, implode(",",array_merge(array(0),$tmp)), 2**$pu_dow, implode(", ", $m_remove), $super));
					$g_abnormal = $this->fetchAll(sprintf("select c.id, c.id as serviceType, p.name, c.product_id, p.custom_minimum_charge from product p, custom_member_product_options c where p.id = c.product_id and c.member_id = %d and isgroup = 1 and p.id not in (%s) and custom_same_day = 1 and custom_availability & %3\$d = %3\$d and p.id not in (%4\$s) and c.product_id in (%5\$s)",$g_id, implode(",",array_merge(array(0),$tmp)), 2**$pu_dow, implode(", ", $m_remove), implode(",",array_merge(array(0),$g_prod))));
					...
					//$g_services = $this->fetchAll(sprintf("select c.id, c.id as serviceType, p.name, c.product_id, p.custom_minimum_charge from product p, custom_member_product_options c where p.id = c.product_id and c.member_id = %d and isgroup = 1 and p.id not in (%s) and custom_same_day = 1 and custom_availability & %3\$d = %3\$d and p.id not in (%4\$s) and (admin_only = 0 or %5\$d = 1)",$g_id, implode(",",array_merge(array(0),$tmp)), 2**$pu_dow, implode(", ", $m_remove), $super));
					$services = array();
...
//					if ($this->checkArray("addresses:pickup:id",$cart) && $this->checkArray("addresses:shipping:id", $cart) && $this->checkArray("quote:prod", $_SESSION)) {
	...
	function postLogin($user) {
		$cart = Ecom::getCart();
		$cart["addresses"] = array('shipping'=>array('id'=>0,'country_id'=>0,'province_id'=>0), 'pickup'=>array('id'=>0,'country_id'=>0,'province_id'=>0), 'billing'=>$this->fetchSingle(sprintf("select * from addresses where ownertype='member' and ownerid = %d and addresstype=%d", $this->getUserInfo("id"), ADDRESS_BILLING)));
		$_SESSION["cart"] = $cart;
	}
	...
	function messageing() {
		...
		$inner->init($this->m_dir.$module["inner_html"]);
		$flds = $inner->buildForm($this->getFields($module["configuration"]));
		$recs = $this->fetchAll(sprintf("select cd.*, p.name, ol.custom_package, timediff(CURRENT_TIMESTAMP(),ack_requested) as delay from custom_delivery cd, drivers d, members m, order_lines ol, product p 
...
$this->getUserInfo("id"), ACK_REQUEST, date("Y-m-d"), date("Y-m-d", strtotime("today + 1 day")), STATUS_CANCELLED , STATUS_CANCELLED ));
	...
	function acknowledgement() {
		...
		$flds = $outer->buildForm($this->getFields($module["configuration"]));
		$a_id = array_key_exists("a_id",$_REQUEST) ? $_REQUEST["a_id"] : 0;
		if ($rec = $this->fetchSingle(sprintf("select cd.*, p.code, p.name from custom_delivery cd, drivers d, product p, order_lines ol where cd.id = %d and d.id = cd.driver_id and d.member_id = %d and ol.order_id = cd.order_id and ol.custom_package='S' and p.id = ol.product_id and ol.deleted = 0", $a_id, $this->getUserInfo("id")))) {
			$address = $this->fetchSingle(sprintf("select * from addresses where ownerid = %d and ownertype='order' and addresstype=%d", $rec["order_id"], $rec["service_type"] == "P" ? ADDRESS_PICKUP : ADDRESS_DELIVERY));
			$now = new DateTime();
...
date(cd.scheduled_date) >= '%s' and ol.custom_package = 'S' and ol.deleted = 0 order by scheduled_date", date("Y-m-d"));
		...
		foreach($data as $rec) {
			$inner->reset();
			$rec["address"] = Address::formatData($this->fetchSingle(sprintf("select * from addresses where ownertype='order' and ownerid=%d and addresstype = %d", $rec["order_id"], $rec["service_type"] == "P" ? ADDRESS_PICKUP : ADDRESS_DELIVERY)));
			if ($services = $this->fetchScalarAll(sprintf("select name from product p, order_lines ol where ol.order_id = %d and ol.custom_package = 'A' and p.id = ol.product_id and p.custom_special_requirement = 1 and ol.deleted = 0", $rec["order_id"])))
				$rec["services"] = implode(", ",$services);
			$rec["driver"] = $this->fetchSingle(sprintf("select * from members m, drivers d where d.id = %d and m.id = d.member_id", $rec["driver_id"]));
			$inner->addData($rec);
			$recs[] = $inner->show();
		}
		$outer->addTag("deliveries", implode("", $recs), false);
	...
	function invoiceDetails() {
		...
		$flds = $inner->buildForm($this->config->getFields($module['configuration']."Row"));
		$outer->addData($_REQUEST);
		if ($inv = $this->fetchSingle(sprintf('select qb.* from qb_export qb, members m where qb.id = %d and m.id = qb.member_id and (m.id = %2$d or m.custom_parent_org = %2$d)', array_key_exists("i_id",$_REQUEST) ? $_REQUEST["i_id"] : 0,$this->getUserInfo("id")))) {
			$outer->addData($inv);
	...
	function invoicePDF( $saveAsFile = false, $filename = "" ) {
		...
		//$inv = $this->fetchSingle(sprintf("select *, invoice_amount+tax_amount as i_total, invoice_amount + tax_amount - paid_amount as balance from qb_export where id = %d and member_id = %d", $i_id,$this->getUserInfo("id")));
		if ($r_id == 0) {
			$inv = $this->fetchSingle(sprintf('select e.*, sum(o.total) as i_total, sum(o.total) - sum(coalesce(a.authorization_amount,0)) as balance 
...
where o.deleted = 0 and e.id = %d and rand_id = %d and m.id = e.member_id and d.qb_export_id = e.id and o.id = d.order_id and o.order_status & %3$d = 0', $i_id, $r_id, STATUS_CANCELLED));
		...
		$subtotal = 0;
		foreach($recs as $k=>$v) {
			if (!$address = $this->fetchSingle(sprintf("select * from addresses where ownertype='order' and ownerid = %d and addresstype=%d", $v["order_id"], ADDRESS_PICKUP )))
			...
			if (!$address = $this->fetchSingle(sprintf("select * from addresses where ownertype='order' and ownerid = %d and addresstype=%d", $v["order_id"], ADDRESS_DELIVERY )))
			...
			$v["pu"] = $this->fetchSingle(sprintf("select * from custom_delivery where order_id = %d and service_type = 'P'", $v["order_id"]));
			$v["del"] = $this->fetchSingle(sprintf("select * from custom_delivery where order_id = %d and service_type = 'D'", $v["order_id"]));
			$v["order"] = $this->fetchSingle(sprintf("select o.*, o.total - o.authorization_amount as balance, coalesce(a.authorization_amount,0) as paid_amount from orders o left join order_authorization a on a.order_id = o.id and a.authorization_type='D' and a.authorization_success = 1 where o.id = %d", $v["order_id"]));

			$inner->addData($v);
			$dtls = $this->fetchAll(sprintf("select ol.*, p.name, p.subtitle from order_lines ol, product p where order_id = %d and p.id = ol.product_id and ol.deleted = 0 and custom_package != 'P' and (value > .01 or custom_package = 'S') order by line_id", $v["order_id"]));
			$dtl = array();
			foreach($dtls as $sk=>$sv) {
				if ($sv["product_id"] == FEDEX_RECALC) {
					$sv["dims"] = $this->fetchSingle(sprintf("select code as wt_code, sum(weight) as wt, sum(quantity) as qty from order_lines_dimensions old, code_lookups cl where old.order_id = %d and old.line_id = %d and cl.id = %d", $sv["order_id"], $sv["line_id"], $v["order"]["custom_weight_code"]));
				}
				$details->addData($sv);
				$dtl[] = $details->show();
			}
		...
		$inv["member"] = $this->fetchSingle(sprintf("select * from members m where m.id = %d", $inv["member_id"]));
		if (!$address = $this->fetchSingle(sprintf("select a.* from addresses a, code_lookups cl where a.ownertype='member' and a.ownerid = %d and cl.id = a.addresstype and cl.id = %d and a.deleted = 0 order by a.id limit 1", $inv["member_id"], ADDRESS_COMPANY)))
...
*/
	...
	function ytdStats() {
		...
		$ct = $this->fetchScalar(sprintf("select count(0) from orders where member_id = %d and order_status & %d != 0 and year(order_date) = %d", $id, STATUS_PROCESSING | STATUS_SHIPPED, date("Y")));
		$completed = $this->fetchScalar(sprintf("select count(o.id) from orders o, custom_delivery d where o.member_id = %d and d.completed = 1 and d.order_id = o.id and d.service_type='D'", $id));
		$late = $this->fetchScalar(sprintf("select count(order_id) from orders o, custom_delivery d where o.member_id = %d and d.completed = 1 and d.order_id = o.id and d.actual_date < d.scheduled_date + INTERVAL 15 MINUTE and d.service_type='D'", $id));
		$percent = (int) ($completed > 0 ? $late/$completed*100 : 0);
		$cost = $this->fetchScalar(sprintf("select sum(total) from orders where member_id = %d and order_status & %d != 0 and year(order_date) = %d", $id, STATUS_PROCESSING | STATUS_SHIPPED, date("Y")));
		$most_used = $this->fetchScalarAll(sprintf("select p.name as name, count(p.id) as ct from orders o, product p, order_lines ol 
...
order by count(p.id) desc limit 1", $id, STATUS_PROCESSING | STATUS_SHIPPED, date("Y")));
	...
	function mobileDetails() {
		...
		$outer->setModule($module);
		$c_id = array_key_exists("c_id",$_REQUEST) ? $_REQUEST["c_id"] : 0;
		if ($action = $this->fetchSingle(sprintf("select c.*, o.member_id from custom_delivery c, orders o where c.id = %d and o.id = c.order_id", $c_id))) {
			if ($addr = $this->fetchSingle(sprintf("select a.*, cd.scheduled_date from addresses a, custom_delivery cd where a.ownertype='order' and a.ownerid = %d and a.addresstype='%s' and cd.order_id = a.ownerid and cd.service_type='P'", $action["order_id"], ADDRESS_PICKUP)))
				$action["pickup"] = Address::formatData($addr);
			else $action["pickup"] = array();
			if ($addr = $this->fetchSingle(sprintf("select a.*, cd.scheduled_date from addresses a, custom_delivery cd where a.ownertype='order' and a.ownerid = %d and a.addresstype='%s' and cd.order_id = a.ownerid and cd.service_type='D'", $action["order_id"], ADDRESS_DELIVERY)))
			...
			if ($action["action"]["ownerid"] != $action["member_id"]) {
				$action["client"] = $this->fetchSingle(sprintf("select m.*, a.phone1, a.email from members m left join addresses a on a.ownertype='member' and a.ownerid = m.id and a.addresstype = %d where m.id = %d", ADDRESS_COMPANY, $action["member_id"]));
				$action["showClient"] = $action["client"]["company"] != $action["action"]["company"];
			}
			if ($action["service_type"] == "D") {
				$action["pu_action"] = $this->fetchSingle(sprintf("select * from custom_delivery where order_id = %d and service_type='P'", $action["order_id"]));
			}
	...
	function customOrderHistory() {
		...
		foreach($orders as $key=>$order) {
			$order["pu_address"] = Address::formatData($this->fetchSingle(sprintf("select * from addresses where ownertype='order' and ownerid=%d and addresstype=%d limit 1", $order["id"], ADDRESS_PICKUP)));
			$order["del_address"] = Address::formatData($this->fetchSingle(sprintf("select * from addresses where ownertype='order' and ownerid=%d and addresstype=%d limit 1", $order["id"], ADDRESS_DELIVERY)));
			$order["service"] = $this->fetchSingle(sprintf("select * from product p, order_lines od where od.order_id = %d and p.id = od.product_id and od.custom_package='S' and od.deleted = 0", $order["id"]));
			$inner->reset();
	...
	function simulateDriver() {
		...
		$outer->init($this->m_dir.$module['outer_html']);
		$flds = $outer->buildForm($this->config->getFields($module['configuration']));
		if ($rec = $this->fetchSingle(sprintf("select * from members where deleted = 0 and enabled = 1 and id = %d", $this->getUserInfo("id"))))
			$outer->addData($rec);
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__,$_POST)) {
			$outer->addData($_POST);
			if ($outer->validate()) {
				if ($rec = $this->fetchSingle(sprintf("select * from members where id = %d and enabled = 1 and deleted = 0", $outer->getData("member_id")))) {
		...
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__,$_POST)) {
			...
			if ($outer->validate()) {
				...
				if (array_key_exists("summary",$_REQUEST) && $_REQUEST["summary"] == 1) {
					...
					$rows = array();
					foreach($recs as $k=>$v) {
						$v["pickup"] = $this->fetchSingle(sprintf("select cd.*, m.firstname, m.lastname from custom_delivery cd, drivers d, members m where cd.id = %d and d.id = cd.driver_id and m.id = d.member_id", $v["pu_id"]));
						$v["pickup"]["address"] = Address::formatData($this->fetchSingle(sprintf("select * from addresses where ownertype='order' and ownerid = %d and addresstype=%d", $v["id"], ADDRESS_PICKUP)));
						$v["delivery"] = $this->fetchSingle(sprintf("select cd.*, m.firstname, m.lastname from custom_delivery cd, drivers d, members m where cd.id = %d and d.id = cd.driver_id and m.id = d.member_id", $v["del_id"]));
						$v["delivery"]["address"] = Address::formatData($this->fetchSingle(sprintf("select * from addresses where ownertype='order' and ownerid = %d and addresstype=%d", $v["id"], ADDRESS_DELIVERY)));
						$v["tax_codes"] = implode(", ", $this->fetchScalarAll(sprintf("select t.name from order_taxes ot, taxes t where t.id = ot.tax_id and ot.order_id = %d and ot.line_id = 0 and ot.tax_amount >= .01", $v["id"])));
						$inner->addData($v);
						$rows[] = $inner->show();
					}
				}
	...
	function fedexStatus() {
		...
		foreach($recs as $k=>$v) {
			...
			$v["pickup"] = $this->fetchSingle(sprintf("select * from custom_delivery where id = %d", $v["pu_id"]));
			$v["pickup"]["address"] = Address::formatData($this->fetchSingle(sprintf("select * from addresses where ownertype='order' and ownerid = %d and addresstype = %d", $v["order_id"], ADDRESS_PICKUP )));
			$v["pickup"]["driver"] = $this->fetchSingle(sprintf("select m.*, d.third_party from members m, drivers d where d.id = %d and m.id = d.member_id", $v["pickup"]["driver_id"]));
			$v["delivery"] = $this->fetchSingle(sprintf("select * from custom_delivery where id = %d", $v["del_id"]));
			$v["delivery"]["address"] = Address::formatData($this->fetchSingle(sprintf("select * from addresses where ownertype='order' and ownerid = %d and addresstype = %d", $v["order_id"], ADDRESS_DELIVERY )));
			$v["delivery"]["driver"] = $this->fetchSingle(sprintf("select m.*, d.third_party from members m, drivers d where d.id = %d and m.id = d.member_id", $v["delivery"]["driver_id"]));
			$v["service"] = $this->fetchSingle(sprintf("select p.* from product p, order_lines ol where ol.order_id = %d and ol.custom_package = 'S' and p.id = ol.product_id", $v["order_id"]));
			$packages = $this->fetchAll(sprintf("select p.name, old.*, c.code, c.value from product p, order_lines ol, order_lines_dimensions old, orders o, code_lookups c where o.id = %d and ol.order_id = o.id and ol.custom_package = 'P' and p.id = ol.product_id and ol.deleted = 0 and old.order_id = ol.order_id and old.line_id = ol.line_id and c.id = o.custom_weight_code", $v["order_id"]));
			$p_rows = array();
 	...
 	function invoicing() {
		...
		$flds = $this->config->getFields($module['configuration']);
		$flds["invoice_id"]["value"] = $this->fetchScalar(sprintf("select max(qb_invoice_id) from qb_export")) + 1;
		$flds = $outer->buildForm($flds);
		$rows = array();
		...
		if (count($_REQUEST) > 0 && array_key_exists(__FUNCTION__,$_REQUEST)) {
			...
			if ($valid) {
				...
				$ct = $this->fetchScalar(sprintf("select count(0) from qb_export where qb_invoice_id >= %d", $outer->getData("invoice_id")));
				if ($ct > 0) {
					$valid = false;
	...
	private function getInvoiceReport($flds, $outer, $inner, $consolidated) {
		...
		foreach($recs as $k=>$v) {
			...
			unset($tmp["e"]);
			$c = $this->fetchSingle(sprintf("select min(qb.qb_invoice_id) as qb_invoice_id, m.company, qb.member_id, count(o.id) as order_count from qb_export qb, qb_export_dtl qbd, orders o, members m where %s group by qb.consolidate_id", implode(" and ",$tmp)));
			$consolidated->addData($c);
			$consolidated->addData($v);
			//$rows[] = $consolidated->show();
			$d = $this->fetchAll(sprintf("select qb.qb_invoice_id, m.company, qb.member_id, count(o.id) as order_count from qb_export qb, qb_export_dtl qbd, orders o, members m where %s group by qb.qb_invoice_id", implode(" and ",$tmp)));
			$r_tmp = array();
			...
			foreach($d as $sk=>$sv) {
				...
				$o_ids = $this->fetchScalarAll($o_sql);
				$hdr = $this->fetchSingle(sprintf("select round(sum(IF(ol.taxes > 0,ol.total,0)),2) as taxable, sum(IF(ol.taxes < 0.01,ol.total,0)) as nontaxable, o.authorization_amount from orders o, order_lines ol where ol.order_id = o.id and ol.deleted = 0 and o.id in (%s)",implode(",",$o_ids)));
				$taxes = $this->fetchScalar(sprintf("select round(sum(o.taxes),2) as taxes from orders o where o.id in (%s)", implode(",",$o_ids)));
					...
					switch($v["name"]) {
						...
						case "member_id":
							if (strlen($d = $outer->getData($v["name"])) > 0) {
								$tmp = $this->fetchScalarAll(sprintf("select m1.id from members m1, members m2 where m1.custom_parent_org = %d and m2.id = m1.custom_parent_org and m2.custom_consolidate_invoices > 0", $d));
								if (count($tmp) > 0) {
	...
	private function orderNeedsRecalc( $o_id ) {
		$o = $this->fetchSingle(sprintf("select * from orders where id = %d", $o_id));
		$lines = $this->fetchAll(sprintf("select * from order_lines where order_id = %d and deleted = 0", $o_id));
		$taxes = $this->fetchAll(sprintf("select * from order_taxes where order_id = %d and line_id > 0", $o_id));
	...
	private function createInvoices($form) {
		...
		$invoices = $this->fetchAll(sprintf("select distinct invoice_id as invoice_id, invoice_date as invoice_date, consolidate_id from `inv-temp` where invoice_id not in (select qb_invoice_id from qb_export) order by consolidate_id"));
		$this->logMessage(__FUNCTION__,sprintf("invoices [%s]", print_r($invoices,true)), 3);
		foreach($invoices as $k=>$invoice) {
			$orders = $this->fetchAll(sprintf("select order_id, member_id, custom_parent_org from `inv-temp` i, members m where m.id = i.member_id and i.invoice_id = %d", $invoice["invoice_id"]));
			$this->beginTransaction();
			...
			foreach($orders as $sk=>$order) {
				...
				//$hdr = $this->fetchSingle(sprintf("select * from orders where id = %d", $order["order_id"]));
				$hdr = $this->fetchSingle(sprintf("select sum(IF(ol.taxes > 0,ol.total,0)) as taxable, sum(IF(ol.taxes < 0.01,ol.total,0)) as nontaxable, sum(ol.taxes) as taxes, o.authorization_amount from orders o, order_lines ol where ol.order_id = o.id and ol.deleted = 0 and o.id = %d group by o.id",$order["order_id"]));
				...
				if ($this->fetchScalar(sprintf("select coalesce(count(order_id),0) from order_authorization where order_id = %d", $order["order_id"])) == 0)
			...
			if ($valid) {
				...
				$member = $this->fetchSingle(sprintf("select * from members where id = %d", $orders[0]["member_id"]));
				...
				$html = $this->fetchSingle($sql);
				$body->setHTML($html['html']);
				$inv_rec = $this->fetchSingle(sprintf("select *, invoice_amount+tax_amount as invoice_total  from qb_export where id = %d",$inv_id));
		...
		if (is_array($invoices) && count($invoices) > 0) {
			$consolidated = $this->fetchScalarAll(sprintf("select m2.id from qb_export qb, members m1, members m2 where m1.id = qb.member_id and invoice_date = '%s' and m2.id = m1.custom_parent_org and m2.custom_consolidate_invoices > 0 group by m2.id", $invoices[0]["invoice_date"]));
			foreach($consolidated as $k=>$v) {
				$member = $this->fetchSingle(sprintf("select m.* from members m where id = %d", $v));
				$this->m_module = array();
				$this->config = new Custom(0);
				$this->logMeIn($member["username"],$member["password"],0,$member["id"]);
				...
				$html = $this->fetchSingle($sql);
				$body->setHTML($html['html']);
				$inv_rec = $this->fetchSingle(sprintf("select *, sum(invoice_amount+tax_amount) as invoice_total from qb_export where consolidate_id = %d and invoice_date = '%s'",$v, $invoices[0]["invoice_date"]));
				$inv_rec["user"] = $_SESSION["user"]["info"];
				if (!$address = $this->fetchSingle(sprintf("select a.* from addresses a, code_lookups c where a.ownertype='member' and a.ownerid = %d and a.addresstype = c.id and c.id = %d and c.type = 'memberAddressType' and a.deleted = 0", $inv_rec["user"]["id"], ADDRESS_COMPANY))) {
					$address = array("id"=>0,"country_id"=>0,"province_id"=>0);
					$this->logMessage(__FUNCTION__,sprintf("No member address for company [%s] id [%d]", $member["company"], $inv_rec["user"]["id"]),1,true,false);
				}
				$inv_rec["address"] = Address::formatData($address);
	...
	function orderAuthorizations() {
		...
		$rows = array();
		$o_id = array_key_exists("o_id",$_REQUEST) ? $_REQUEST["o_id"] : 0;
		$auth = $this->fetchAll(sprintf("select * from order_authorization where order_id = %d order by id", $o_id));
		$rows = array();
		foreach($auth as $k=>$v) {
			$inner->addData($v);
			$rows[] = $inner->show();
	...
	function modifiedByDriver() {
		...
		$rows = array();
		$o_id = array_key_exists("o_id",$_REQUEST) ? $_REQUEST["o_id"] : 0;
		$auth = $this->fetchAll(sprintf("select distinct o.* from orders o, custom_delivery cd where o.order_status & %d != 0 and cd.order_id = o.id and ((cd.completed = 0 and date(cd.scheduled_date) = curdate()) or (cd.completed = 1 and date(cd.actual_date) = curdate())) order by o.id", STATUS_NEEDS_APPROVAL | STATUS_DRIVER_CHANGED));
		$rows = array();
		foreach($auth as $k=>$v) {
			$v["order_status"] = $v["order_status"] & ~(STATUS_PROCESSING | STATUS_SHIPPED);
			$v = $this->formatOrder($v);
			$v["customer"] = $this->fetchSingle(sprintf("select * from members where id = %d", $v["member_id"]));
			$v["notes_br"] = nl2br($v["notes"]);
			$v["pickup"] = $this->fetchSingle(sprintf("select cd.*, m.firstname, m.lastname from custom_delivery cd left join drivers d on d.id = cd.driver_id left join members m on m.id = d.member_id where order_id=%d and service_type ='P'", $v["id"]));
			$v["pickup"]["address"] = Address::formatData($this->fetchSingle(sprintf("select * from addresses where ownertype='order' and ownerid = %d and addresstype = %d", $v["id"], ADDRESS_PICKUP)));
			$v["delivery"] = $this->fetchSingle(sprintf("select cd.*, m.firstname, m.lastname from custom_delivery cd left join drivers d on d.id = cd.driver_id left join members m on m.id = d.member_id where order_id=%d and service_type ='P'", $v["id"]));
			$v["delivery"]["address"] = Address::formatData($this->fetchSingle(sprintf("select * from addresses where ownertype='order' and ownerid = %d and addresstype = %d", $v["id"], ADDRESS_DELIVERY)));
	...
	function subsidiaryInvoices() {
		...
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__,$_POST)) {
			...
			if ($outer->validate()) {
				...
				$recs = $this->fetchAll(sprintf("select qb.id, qb_invoice_id, count(o.id) as ct, sum(o.total) as total, m.company 
...
group by qb.qb_invoice_id order by company", $outer->getData("invoice_date"), $this->getUserInfo("id")));
	...
	function consolidatedPDF( $saveAsFile = false, $filename = "" ) {
		...
		$r_id = array_key_exists("r_id",$_REQUEST) ? $_REQUEST["r_id"] : 0;
		if ($r_id == 0) {
			$member = $this->fetchSingle(sprintf("select m.* from members m where id = %d", $this->getUserInfo("id")));
		}
		else {
			$member = $this->fetchSingle(sprintf("select m.* from members m, qb_export i where i.rand_id = %d and i.invoice_date = '%s' and m.id = i.consolidate_id", $r_id, $d_id));
		}
		if (!is_array($member)) return "";
		$member["address"] = ($addr = $this->fetchSingle(sprintf("select a.* from addresses a, code_lookups l where a.ownerid = %d and a.ownertype='member' and a.addresstype = l.id and l.type='memberAddressTypes' and l.id = %d and a.deleted = 0", $member["id"], ADDRESS_COMPANY))) ? Address::formatData($addr) : array();
...
*/
	...
	function emailInvoices( $request, $module, $form ) {
		...
		if (is_array($emails)) {
			foreach($emails as $k=>$v) {
				if ($v==1) {
					$member = $this->fetchSingle(sprintf("select m.* from members m, qb_export i where i.id = %d and m.id = i.member_id", $k));
					$this->m_module = array();
					$this->config = new Custom(0);
					//$this->logMeIn($member["username"],$member["password"],0,$member["id"]);
					...
					$i_module = $this->fetchSingle($sql);
					$class = new Custom($i_module['id'],$i_module);
					$class->config = new Custom(0);
					$_REQUEST["i_id"] = $k;
					$invoice = $this->fetchSingle(sprintf("select * from qb_export where id = %d", $k));
		...
		if (array_key_exists("send_consolidated",$request) && $request["send_consolidated"] == 1) {
			...
			$class = new Custom($i_module['id'],$i_module);
			$class->config = new Custom(0);
			$invoice = $this->fetchSingle(sprintf("select * from qb_export where invoice_date = '%s'", $form->getData("invoice_date")));
...
*/
	...
	function incompletes() {
		...
		$inner->init($this->m_dir.$module['inner_html']);
		$flds = $inner->buildForm($this->config->getFields($module['configuration']."Row"));
		$recs = $this->fetchAll(sprintf("select o.id, a.id, ol.id, ol.custom_package, p.id, m.*, a.phone1, a.email, o.id, p.name, p.custom_color_coding 
	...
	function driverQueue() {
		...
		$outer->validate();
		$drivers = $this->fetchAll(sprintf("select m.company, m.firstname, m.lastname, cd.*, o.id, a.postalcode, a.line1, a.city, p.custom_same_day, p.custom_color_coding from drivers d, members m, custom_delivery cd, orders o, order_lines ol, product p, addresses a where %s order by m.company, cd.driver_sequence", implode(" and ",$opts)));
		$recs = array();
		$d_id = 0;
		$ct = 0;
		$addresses = array();
		foreach($drivers as $k=>$v) {
			...
			//$addresses = $this->fetchAll(sprintf("select m.firstname, m.lastname, cd.*, o.id, a.line1, a.city from drivers d, members m, custom_delivery cd, orders o, order_lines ol, product p, addresses a where %s order by m.lastname, cd.driver_sequence", implode(" and ",$opts)));
	...
	function isOfficeClosed() {
		...
		$flds = $outer->buildForm($this->config->getFields($module['configuration']));
		$outer->setModule($module);
		if (!$stat = $this->fetchSingle(sprintf("select os.*, cl.extra as msg from office_schedule os left join code_lookups cl on cl.code = os.stock_msg and cl.type = 'closed_messages' where office_date = '%s'", date("Y-m-d")))) {
	...
	function getAQuote() {
		...
		$g_services = $this->fetchAll(sprintf("select c.id, p.name, c.product_id from product p, custom_member_product_options c where p.id = c.product_id and c.member_id = %d and isgroup = 1 and p.id not in (%s) and is_fedex = 0 and p.deleted = 0 and p.published = 1 and p.enabled = 1",$g_id, implode(",",array_merge(array(0),$tmp))));
		$services = array_merge($m_services, $g_services);
		$result = array();
		foreach($services as $key=>$rec) {
			$result[$rec["id"]] = $rec["name"];
	...
	function idleDrivers() {
		...
		$inner = new Forms();
		$inner->init($this->m_dir.$module['inner_html']);
		$d = $this->fetchAll(sprintf("select m.company, m.firstname, m.lastname from members m, drivers d 
where d.deleted = 0 and d.enabled and m.id = d.member_id and d.id not in (
	...
	function driverLocations() {
		...
		if (array_key_exists("driver_id",$_REQUEST) && (int)$_REQUEST["driver_id"] > 0) {
			$recs = $this->fetchAll(sprintf("select dl.member_id, dl.datetime, dl.latitude, dl.longitude, m.company, m.firstname, m.lastname, cd.order_id, timediff(now(),datetime) as elapsed
...
where m.id = dl.member_id and date(datetime) = current_date() and dl.member_id = %d", $_REQUEST["driver_id"]));
			...
			$recs = $this->fetchAll(sprintf("select d.vehicle_id, dl.member_id, dl.datetime, dl.latitude, dl.longitude, m.company, m.firstname, m.lastname, cd.order_id, timediff(now(),datetime) as elapsed 
...
where d.member_id = m.id and m.id = dl.member_id and date(datetime) = current_date() and datetime = (
	...
	private function calcFreeItems($o_id) {
		...
		$p = $this->fetchAll(sprintf("select ol.product_id, sum(old.quantity) as qty from order_lines ol, order_lines_dimensions old where ol.order_id = %d and ol.custom_package = 'P' and old.order_id = ol.order_id and old.line_id = ol.line_id group by ol.product_id", $o_id));
		$charges = 0;
		$member = $this->fetchSingle(sprintf("select m.* from members m, orders o where o.id = %d and m.id = o.member_id", $o_id));
		$total = array();
		foreach($p as $k=>$v) {
			if ($fee = $this->fetchSingle(sprintf("select * from member_package_charges where member_id = %d and product_id = %d", $member["id"], $v["product_id"]))) {
				if ($v["qty"] - $fee["free"] > 0)
	...
	function showDriverRoute() {
		...
		if (array_key_exists("d_id", $_REQUEST) && $_REQUEST["d_id"] > 0) {
			$_REQUEST["driver"] = $this->fetchSingle(sprintf("select m.* from members m, drivers d where m.id = d.member_id and d.id = %d", $_REQUEST["d_id"]));
			$outer->addData($_REQUEST);
			$outer->validate();
			if ($route = $this->fetchSingle( sprintf("select * from custom_delivery_route where driver_id = %d and delivery_date = '%s'", $outer->getData("d_id"), date("Y-m-d")))) {
				...
				foreach($addr as  $key=>$value) {
					if ($value > 0) {
						$address = $this->fetchSingle(sprintf("select a.*, cd.order_id, cd.actual_date, p.custom_same_day from addresses a, custom_delivery cd, product p, order_lines ol where cd.id = %d and a.ownerid = cd.order_id and a.ownertype='order' and addresstype=if(cd.service_type='P',%d,%d) and ol.order_id = cd.order_id and ol.custom_package = 'S' and p.id = ol.product_id",$value,ADDRESS_PICKUP,ADDRESS_DELIVERY));
	...
	function optionalFieldCheck($data,$obj,$key,$req,&$errors,&$messages) {
		$opts = $this->fetchAll(sprintf("select cv.* from code_lookups cv, member_optional_fields mof where mof.member_id = %d and cv.id = mof.optional_id", $this->getUserInfo("id")));
		$form = new Forms();
