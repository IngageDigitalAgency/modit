...
$this->logMessage(__FUNCTION__,sprintf("recs [%s] orders [%s]", print_r($recs,true), print_r($orders,true)),1);
			...
			$totals = $this->fetchSingleTest("select coalesce(sum(total),0) as g_total, coalesce(sum(authorization_amount),0) as g_authorization_amount, count(1) as g_ct %s %s", $sql, $sqlPlus);
    ...
    function waybill() {
		...
		$o_id = array_key_exists("o_id",$_REQUEST) ? $_REQUEST["o_id"] : 0;
		if ($o_id == 0) return;
		if ($order = $this->fetchSingleTest('select o.*, c1.code as wtCode, c2.code as dimCode from orders o, code_lookups c1, code_lookups c2, members m 
...
date(cd.scheduled_date) >= '%s' and ol.custom_package = 'S' and ol.deleted = 0 order by scheduled_date", date("Y-m-d"));
    ...
    function invoicePDF($saveAsFile = false, $filename = "" ) {
		...
		if ($r_id == 0) {
			$inv = $this->fetchSingleTest('select e.*, sum(o.total) as i_total, sum(o.total) - sum(coalesce(a.authorization_amount,0)) as balance 
...
*/
    ...
    function incompletes() {
		...
		$inner->init($this->m_dir.$module['inner_html']);
		$flds = $inner->buildForm($this->config->getFields($module['configuration']."Row"));
		$recs = $this->fetchAllTest("select o.id, a.id, ol.id, ol.custom_package, p.id, m.*, a.phone1, a.email, o.id, p.name, p.custom_color_coding 
