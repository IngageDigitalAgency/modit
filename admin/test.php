...
$src = $c->fetchSingleTest("select * from order_billing where billed = 1 order by billing_date limit 1");

$src_order = $c->fetchSingleTest("select * from orders where id = %d", $src["original_id"]);
$src_service = $c->fetchSingleTest("select * from product p, order_lines o where ol.order_id = %d and p.id = ol.product_id and ol.custom_package = 'S'", $src_order["id"]);
$src_packages = $c->fetchSingleTest("select ol.* from order_lines o where ol.order_id = %d and ol.custom_package = 'P'", $src_order["id"]);
$src_extras = $c->fetchScalarAllTest("select ol.product_id from order_lines o where ol.order_id = %d and ol.custom_package = 'A'", $src_order["id"]);
