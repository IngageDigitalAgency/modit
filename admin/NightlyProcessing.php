...
//require_once("classes/tcpdf/tcpdf.php");//updating to latest version using php packagemanger ,so it will load automatically
...
class Processing extends common {
    ...
    function setCommissions() {
		$ids = $this->fetchScalarAllTest("select distinct(order_id) from custom_delivery where service_type='P' and completed = 1 and percent_of_delivery = 0 and order_id in (select order_id from custom_delivery where service_type='D' and completed = 1 and percent_of_delivery = 0)");
		$this->logMessage(__FUNCTION__,sprintf("orders to be allocated [%s]",implode(",",$ids)),1);
		foreach($ids as $key=>$orderId) {
			$pickup = $this->fetchSingleTest("select c.*, d.commission, v.fuel_charge, o.custom_commissionable_amt from custom_delivery c, drivers d, vehicles v, orders o where c.order_id = %d and c.service_type='P' and d.id = c.driver_id and v.id = d.vehicle_id and o.id = c.order_id",$orderId);
			$delivery = $this->fetchSingleTest("select c.*, d.commission, v.fuel_charge from custom_delivery c, drivers d, vehicles v where c.order_id = %d and c.service_type='D' and d.id = c.driver_id and v.id = d.vehicle_id",$orderId);
			$product = $this->fetchSingleTest("select * from product where id = %d",$pickup["delivery_type"]);
			//
...
if ($rec = $b->fetchSingleTest('select * from order_processing where bill_date <= curdate() and started = "0000-00-00 00:00:00" order by bill_date limit 1')) {
	$b->setProcessing($rec);
