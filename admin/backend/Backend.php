...
#		}
	...
	function updateRelations($postArg, $ownerId, $ownerType, $relatedType, $relatedTable, $isOwner = true, $hasDeleted = true) {
		...
		if ($isOwner) {
			...
			//	insert new items
			//			
			$new = $this->fetchScalarAllTest(sprintf('select id from %s where %s and id in (%s) and id not in (select related_id from relations where owner_id = %d and owner_type="%s" and related_id in (%s) and related_type = "%s")', 
			...
			//	insert new items
			//
			$sql = sprintf('select id from %s where %s and id in (%s) and id not in (select owner_id from relations where related_id = %d and related_type="%s" and owner_id in (%s) and owner_type = "%s")', 
			$relatedTable, $hasDeleted?'deleted = 0':'1=1', implode(',',$destItems), $ownerId, $ownerType, implode(',',$destItems),$relatedType);
