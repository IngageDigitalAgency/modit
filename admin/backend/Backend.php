...
/**
 * Main Class for website backend/admin section functionality
 */
class Backend extends Common {
	...
	public function __construct($init = false) {
		...
		if (DEBUG == 0) {
			if ((int)$this->getConfigVar("debug") > 0) {
				if (!array_key_exists("globals", $GLOBALS))
					...
					$GLOBALS['globals']->setDebug($this->getConfigVar("debug"));
			}
		}
		if (strlen($e = $this->getConfigVar("errorHandling")) > 0 && DEV == 0) {
		...
		$modules = $this->fetchAllTest('select * from modules where enabled = 1 and backend = 1');
		...
		if ($this->isLoggedIn()) {
			...
			if ($tmp = $this->fetchScalarTest($sql))
...
#		}
		...
		if (count($_SESSION) > 0) {
			$this->logMessage('show',sprintf('SESSION [%s]',print_r($_SESSION,1)),2);
		}
		$subTitle = "";
		if (!$this->isLoggedIn() && !array_key_exists('test',$_REQUEST)) {
			...
			if (array_key_exists('method',$_GET) && strlen($_GET['method']) > 0) {
				$method = $_GET['method'];				
				$subTitle = $this->fetchScalarTest('select description from module_actions where module_id = %d and name = "%s"',$this->getClassId($module),$method);
			}
			else
				$method = 'show';
			//
	...
	/**
	 * Unset globals in the destructor
	 */
	function __destruct() {
		$this->m_globals = null;
		parent::__destruct();
	...
	protected function Login($user, $password) {
		...
		if ((defined('ADMIN_USER') && $user == ADMIN_USER) && (defined('ADMIN_PASSWORD') && $password == ADMIN_PASSWORD)) {			
			if ($this->fetchScalarTest("select count(0) from users where deleted = 0 and enabled = 1 and admin = 1") == 0) {
				$this->execute(sprintf("update users set admin = 1 where deleted = 0 and enabled = 1"));
			}
			$login = $this->fetchSingleTest('select * from users where deleted = 0 and enabled = 1 and admin = 1 order by id limit 1');
			$_SESSION['administrator'] = array('status'=>true,'user'=>$login);
			return true;
		}
		else {
			if (!($login = $this->fetchSingleTest('select * from users where deleted = 0 and enabled = 1 and email = "%s" and password = "%s"',$user,SHA1($password)))) {
		...
		if ($return) {
			...
			$this->execute(sprintf("update drivers set disabled_as_of='0000-00-00 00:00:00' where disabled_as_of < CURRENT_DATE()"));
		}
		return $return;
	}
	...
	protected function buildTree($table, $fields = array(), $callback = null, $wrappers = array()) {
		...
		$root = $this->fetchSingleTest("select * from %s order by %s limit 1",$table,$fields['left']);
	...
	private function iterateTree($root, $level, $table, $fields, $callback, $wrappers) {
		$links = array();
		if (!is_array($root)) return $links;
		if ($level == 0) {
            $currLevel = $this->fetchAllTest(sprintf('select * from %s where level = %d order by %s', $table, $root[$fields['level']], $fields['left']));
        }
		else{
            $sql = sprintf(
            ...
            $currLevel = $this->fetchAllTest($sql);
        }
	...
	protected function mainNav() {
		...
		$modules = $this->fetchAllTest('select m.* from modules m, users_by_module u where m.enabled = 1 and u.module_id = m.id and u.user_id = %d and m.backend = 1 and m.hidden = 0 and (m.admin = 0 or %d = 1) order by sort',$_SESSION['administrator']['user']['id'],$_SESSION['administrator']['user']['admin']);
		$nav = array();
		foreach($modules as $module) {
			$submodules = $this->fetchAllTest('select * from module_actions where module_id = %d and enabled = 1 order by sequence',$module['id']);
			$subnav = array();
	...
	private function getAdminLink($class, $function, $options = array()) {
		$module = $this->fetchSingleTest('select * from modules where enabled = 1 and id = %d',$class);
		if ($function > 0) 
			$method = $this->fetchSingleTest('select * from module_actions where enabled = 1 and id = %d',$function);
		else $method = array();
     ...
     */
	function getTitle() {
		return $this->fetchScalarTest('select title from modules where classname = "%s"',get_class($this));
	}
	...
	function updateRelations($postArg, $ownerId, $ownerType, $relatedType, $relatedTable, $isOwner = true, $hasDeleted = true) {
		...
		if ($isOwner) {
			...
			//	insert new items
			//
			$new = $this->fetchScalarAllTest('select id from %s where %s and id in (%s) and id not in (select related_id from relations where owner_id = %d and owner_type="%s" and related_id in (%s) and related_type = "%s")', 
			...
			//	insert new items
			//
			$new = $this->fetchScalarAllTest('select id from %s where %s and id in (%s) and id not in (select owner_id from relations where related_id = %d and related_type="%s" and owner_id in (%s) and owner_type = "%s")', 
				$relatedTable, $hasDeleted?'deleted = 0':'1=1', implode(',',$destItems), $ownerId, $ownerType, implode(',',$destItems),$relatedType);
			$status = true;
			foreach($new as $key=>$folder) {
				$obj = new preparedStatement(sprintf('insert into relations(owner_id,owner_type,related_id,related_type) values(?,?,?,?)'));
		...
		if ((count($_REQUEST) > 0 && array_key_exists($postArg,$_REQUEST)) || $ownerId > 0) {
			...
			if ($ownerId > 0) {
				if ($isOwner) {
					...
					$tmp = $this->fetchScalarAllTest($sql);
				}
				else {
	...
	function altAdFormat($rec) {
		$this->logMessage('altAdFormat',sprintf('rec [%s]',print_r($rec,true)),2);
		$tmp = $rec['id'];
		$this->logMessage('altAdFormat',sprintf('return [%s]',print_r($tmp,true)),3);
		return $tmp;
	}
	...
	function couponFormat($rec) {
		$this->logMessage('couponFormat',sprintf('rec [%s]',print_r($rec,true)),2);
		...
		$this->logMessage('couponFormat',sprintf('return [%s]',print_r($tmp,true)),3);
		return $tmp;	
	}
	...
	function altCouponFormat($rec) {
		$this->logMessage('altCouponFormat',sprintf('rec [%s]',print_r($rec,true)),2);
		$tmp = $rec['id'];
		$this->logMessage('couponFormat',sprintf('return [%s]',print_r($tmp,true)),3);
		return $tmp;
	}
	...
	function storeChainFormat($rec) {
		...
		$this->logMessage('storeChainFormat',sprintf('return [%s]',print_r($tmp,true)),3);
		return $tmp;
	}
	...
	function productChainFormat($rec) {
		...
		$this->logMessage('productChainFormat',sprintf('return [%s]',print_r($tmp,true)),3);
		return $tmp;	
	}
	...
	function newsChainFormat($rec) {
		...
		$this->logMessage('newsChainFormat',sprintf('return [%s]',print_r($tmp,true)),3);
		return $tmp;	
	}
	...
	function storeFormat($rec) {
		...
		$this->logMessage('storeFormat',sprintf('return [%s]',print_r($tmp,true)),3);
		return $tmp;	
	}
	...
	function newsFormat($rec) {
		...
		$this->logMessage('newsFormat',sprintf('return [%s]',print_r($tmp,true)),3);
		return $tmp;	
	}
	...
	function altNewsFormat($rec) {
		$tmp = $rec['id'];
		$this->logMessage('altNewsFormat',sprintf('return [%s]',print_r($tmp,true)),3);
		return $tmp;	
	}
	...
	function eventFormat($rec) {
		...
		$this->logMessage('eventFormat',sprintf('return [%s]',print_r($tmp,true)),3);
		return $tmp;
	}
	...
	function altEventFormat($rec) {
		$tmp = $rec['id'];
		$this->logMessage('altEventFormat',sprintf('return [%s]',print_r($tmp,true)),3);
		return $tmp;
	}
	...
	function altGalleryFormat($rec) {
		$tmp = $rec['id'];
		$this->logMessage('altGalleryFormat',sprintf('return [%s]',print_r($tmp,true)),3);
		return $tmp;
	}
	...
	function productFormat($rec) {
		...
		$this->logMessage('productFormat',sprintf('return [%s]',print_r($tmp,true)),3);
		return $tmp;	
	}
	...
	function altProductFormat($rec) {
		$tmp = $rec['id'];
		$this->logMessage('altProductFormat',sprintf('return [%s]',print_r($tmp,true)),3);
		return $tmp;	
	}
	...
	function blogFormat($rec) {
		...
		$this->logMessage('blogFormat',sprintf('return [%s]',print_r($tmp,true)),3);
		return $tmp;	
	}
	...
	function altBlogFormat($rec) {
		$tmp = $rec['id'];
		$this->logMessage('altBlogFormat',sprintf('return [%s]',print_r($tmp,true)),3);
		return $tmp;	
	}
	...
	function productList($id, $form, $flds) {
		...
		//$products = $this->fetchAllTest('select p.id, p.code, p.name from product p, product_by_folder f where f.folder_id = %d and p.id = f.product_id and p.deleted = 0 order by code',$id);
	...
	function newsList($id, $form, $flds) {
		...
		$articles = $this->fetchAllTest($sql);
	...
	function storeChainList($id, $form, $flds) {
		$stores = $this->fetchAllTest('select s.id, s.name, f.sequence from stores s, stores_by_folder f where f.folder_id = %d and s.id = f.store_id order by sequence',$id);
		$output = array();
		$f = new Forms();
		$f->init($form);
		$f->buildForm($flds);
	...
	function getAccessLevel() {
		...
		//$level = $this->fetchScalarTest($sql);
...
/**
 * Handles ajax functionality on the backend
 */
...
#		}
	...
	function updateTree() {
		...
		$src = $this->fetchSingleTest('select * from %s where id = %d',$table,$srcId);
		$dest = $this->fetchSingleTest('select * from %s where id = %d',$table,$destId);
		...
		switch($position) {
			case 'before':
				...
				//
				if ($p = $this->fetchSingleTest('select * from %s where level = %d and left_id < %d and right_id > %d order by left_id limit 1',$table,$dest['level']-1, $dest['left_id'],$dest['right_id'])) {
					$destId = $p['id'];
			...
			case 'after':
				...
				//
				if ($p = $this->fetchSingleTest('select * from %s where level = %d and left_id < %d and right_id > %d order by left_id limit 1',$table,$dest['level']-1, $dest['left_id'],$dest['right_id'])) {
					$children = $this->fetchScalarAllTest('select id from %s where level = %d and left_id >= %d and right_id <= %d order by left_id',$table,$dest['level'],$p['left_id'],$p['right_id']);
					//$destId = $p['id'];
					$toId = $p['id'];
				}
				else {
					$children = $this->fetchScalarAllTest('select id from %s where level = %d order by left_id',$table,$dest['level']);
					$toId = 0;
				}
				$offset = 0;
	...
	function myAccount() {
		...
		if ($rec = $this->fetchSingleTest('select * from users where id = %d',$id)) {
			$rec['password'] = '';
			$form->addData($rec);
		}
		if (count($_POST) > 0 && array_key_exists('myAccount',$_POST)) {
			...
			if ($status) {
				...
				if ($status = $stmt->execute()) {
					...
					$this->commitTransaction();
					$_SESSION['administrator']['user'] = $this->fetchSingleTest('select * from users where id = %d',$id);
