...
class gallery extends Backend {
    ...
    function showPageProperties($fromMain = false) {
		...
		$frmFlds['destNews']['options'] = $this->fetchOptionsTest(sprintf('select id as code, title as value from news where deleted = 0 and enabled = 1 and published = 1 and id in (%s)',implode(',',array_merge(array(0),$data['destNews']))));
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d', $this->m_tree, $p_id)) {
			...
			if ($this->m_pagination > 0) {
				...
				$count = $this->fetchScalarTest('select count(n.id) from '.$this->m_content.' n where n.id in (select f.image_id from '.$this->m_junction.' f where f.folder_id = %d)',  $_REQUEST['p_id']);
