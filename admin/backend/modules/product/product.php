...
class product extends Backend {
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from '.$this->m_content.' n where n.id in (select f.product_id from '.$this->m_junction.' f where f.folder_id = %d) and n.deleted = 0', $_REQUEST['p_id']);
    ...
    function addContent($fromMain = false) {
		...
		if (!(array_key_exists('p_id',$_REQUEST) && $_REQUEST['p_id'] > 0 && $data = $this->fetchSingleTest(sprintf('select * from %s where id = %d', $this->m_content, $_REQUEST['p_id'])))) {
		...
		if (count($_REQUEST) > 0 && array_key_exists('destFolders',$_REQUEST) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest(sprintf('select folder_id from %s where product_id = %d', $this->m_junction, $data['id']));
		...
		if ((count($_REQUEST) > 0 && array_key_exists('destSearch',$_REQUEST)) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest(sprintf('select folder_id from product_by_search_group where product_id = %d', $data['id']));
		...
		if (count($_REQUEST) > 0 && array_key_exists('destProducts',$_REQUEST) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest(sprintf('select id from product_by_package where product_id = %d order by sequence',$data['id']));
				$ids = array_merge($ids,$tmp);
			}
			if (count($ids) > 0) {
				$ids = implode(',',$ids);
				$source = $this->fetchAllTest(sprintf('select k.* from product_by_package k where k.id in (%s) order by sequence', $ids));
				$rows = array();
		...
		$source = $this->fetchAllTest(sprintf('select k.* from product_by_package k where k.subproduct_id = %d', $data['id']));
		$rows = array();
		foreach($source as $key=>$assembly) {
		...
		$frmFields['destCoupons']['options'] = $this->fetchOptionsTest(sprintf('select id, code from coupons where id in (%s)',implode(',',array_merge(array(0),$data['destCoupons']))));
		...
		$frmFields['destEvents']['options'] = $this->fetchOptionsTest(sprintf('select id as code,name as value from events where id in (%s)',implode(',',array_merge(array(0),$data['destEvents']))));
		...
		$frmFields['destStores']['options'] = $this->fetchOptionsTest(sprintf('select id, name from stores where id in (%s)',implode(',',array_merge(array(0),$data['destStores']))));
		...
		$frmFields['relatedDestProducts']['options'] = $this->fetchOptionsTest(sprintf('select id, concat(code," - ",name) from product where id in (%s)',implode(',',array_merge(array(0),$data['relatedDestProducts']))));
		...
		$frmFields['relatedFromDestProducts']['options'] = $this->fetchOptionsTest(sprintf('select id, concat(code," - ",name) from product where id in (%s)',implode(',',array_merge(array(0),$data['relatedFromDestProducts']))));
		...
		$frmFields['destNews']['options'] = $this->fetchOptionsTest(sprintf('select id as code,title as value from news where id in (%s)',implode(',',array_merge(array(0),$data['destNews']))));
		...
		$frmFields['destBlogs']['options'] = $this->fetchOptionsTest(sprintf('select id as code,title as value from blog where id in (%s)',implode(',',array_merge(array(0),$data['destBlogs']))));
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($status) {
				...
				if ($status = $stmt->execute()) {
					...
					$new = $this->fetchScalarAllTest(sprintf('select id from %s where id in (%s) and id not in (select folder_id from %s where product_id = %d and folder_id in (%s))',
					...
					$new = $this->fetchScalarAllTest(sprintf('select id from search_groups where id in (%s) and id not in (select folder_id from product_by_search_group where product_id = %d and folder_id in (%s))',
					...
					if (array_key_exists('inventory',$_POST) && $status) {
						if (array_key_exists('delete',$_POST['inventory'])) {
							foreach($_POST['inventory']['delete'] as $key=>$value) {
								...
								$rec = $this->fetchSingleTest(sprintf('select * from product_inventory where id=%d',$value));
