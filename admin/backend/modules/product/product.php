...
class product extends Backend {
    ...
    function showPageProperties($fromMain = false) {
		...
		//
		//$frmFlds['destCoupons']['options'] = $this->fetchOptionsTest('select id as code,concat(code," - ",name) as value from coupons where deleted = 0 and enabled = 1 and published = 1 order by code'));
		...
		$tmp = $this->fetchAllTest('select id, code from coupons where id in (%s)',implode(',',array_merge(array(0),$data['destCoupons'])));
		...
		//
		//$frmFlds['destStores']['options'] = $this->fetchOptionsTest('select id as code,name as value from stores where deleted = 0 and enabled = 1 and published = 1 order by code');
		...
		$tmp = $this->fetchAllTest('select id, name from stores where id in (%s)',implode(',',array_merge(array(0),$data['destStores'])));
		foreach($tmp as $key=>$value) {
		...
		$tmp = $this->fetchAllTest('select id, code from product where id in (%s)',implode(',',array_merge(array(0),$data['destRelatedProducts'])));
		foreach($tmp as $key=>$value) {
			$frmFlds['destRelatedProducts']['options'][$value['id']] = $value['code'];
		}
		$this->logMessage('showPageProperties',sprintf('destRelatedProducts [%s] field [%s] tmp [%s]',print_r($data,true),print_r($frmFlds['destRelatedProducts'],true),print_r($tmp,true)),1);
		...
		$tmp = $this->fetchAllTest('select id, title from blog where id in (%s)',implode(',',array_merge(array(0),$data['destBlogs'])));
		foreach($tmp as $key=>$value) {
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from %s n where n.id in (select f.product_id from %s f where f.folder_id = %d) and n.deleted = 0', $this->m_content, $this->m_junction, $_REQUEST['p_id']);
		...
		if (count($_POST) > 0 && array_key_exists('showSearchForm',$_POST)) {
			...
			if ($form->validate()) {
				...
				if (count($srch) > 0) {
					...
					$form->setData('pager',$perPage);
					$count = $this->fetchScalarTest('select count(n.id) from %s n where 1=1 and %s', $this->m_content, implode(' and ',$srch));
    ...
    function addContent($fromMain = false) {
		...
		if (!(array_key_exists('p_id',$_REQUEST) && $_REQUEST['p_id'] > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d', $this->m_content, $_REQUEST['p_id']))) {
			$data = array('id'=>0,'published'=>true,'enabled'=>true,'physical_product'=>true,'tax_exemptions'=>'||','image1'=>'','image2'=>'','image3'=>'','image4'=>'','attachment_protected'=>0,'custom_availability'=>0); 
		}
		...
		if (count($_REQUEST) > 0 && array_key_exists('destFolders',$_REQUEST) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest('select folder_id from %s where product_id = %d', $this->m_junction, $data['id']);
		...
		if ((count($_REQUEST) > 0 && array_key_exists('destSearch',$_REQUEST)) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest('select folder_id from product_by_search_group where product_id = %d', $data['id']);
		...
		if (count($_REQUEST) > 0 && array_key_exists('destProducts',$_REQUEST) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest('select id from product_by_package where product_id = %d order by sequence',$data['id']);
				$ids = array_merge($ids,$tmp);
			}
			if (count($ids) > 0) {
				$ids = implode(',',$ids);
				$source = $this->fetchAllTest('select k.* from product_by_package k where k.id in (%s) order by sequence', $ids);
				$rows = array();
		...
		$source = $this->fetchAllTest('select k.* from product_by_package k where k.subproduct_id = %d', $data['id']);
		$rows = array();
		foreach($source as $key=>$assembly) {
		...
		$frmFields['destCoupons']['options'] = $this->fetchOptionsTest('select id, code from coupons where id in (%s)',implode(',',array_merge(array(0),$data['destCoupons'])));
		...
		$frmFields['destEvents']['options'] = $this->fetchOptionsTest('select id as code,name as value from events where id in (%s)',implode(',',array_merge(array(0),$data['destEvents'])));
		...
		$frmFields['destStores']['options'] = $this->fetchOptionsTest('select id, name from stores where id in (%s)',implode(',',array_merge(array(0),$data['destStores'])));
		...
		$frmFields['relatedDestProducts']['options'] = $this->fetchOptionsTest('select id, concat(code," - ",name) from product where id in (%s)',implode(',',array_merge(array(0),$data['relatedDestProducts'])));
		...
		$frmFields['relatedFromDestProducts']['options'] = $this->fetchOptionsTest('select id, concat(code," - ",name) from product where id in (%s)',implode(',',array_merge(array(0),$data['relatedFromDestProducts'])));
		...
		$frmFields['destNews']['options'] = $this->fetchOptionsTest('select id as code,title as value from news where id in (%s)',implode(',',array_merge(array(0),$data['destNews'])));
		...
		$frmFields['destBlogs']['options'] = $this->fetchOptionsTest('select id as code,title as value from blog where id in (%s)',implode(',',array_merge(array(0),$data['destBlogs'])));

		$frmFields['destBlogFolders']['options'] = $this->nodeSelect(0,'blog_folders',2,false);
		$data['destBlogFolders'] = $this->loadRelations('destBlogFolders',$this,"altBlogFormat",$data['id'],'productfolder','blogfolder','blog_folders','',false);
		$this->logMessage(__FUNCTION__,sprintf('destBlogFolders [%s] [%s] frmFields [%s]',print_r($frmFields['destBlogFolders'],true),print_r($data['destBlogFolders'],true),print_r($frmFields,true)),1);
		...
		$this->logMessage(__FUNCTION__,sprintf('destGalleryFolders [%s] frmFields [%s]',print_r($data['destGalleryFolders'],true),print_r($frmFields,true)),1);

		//
		//	build the tax exemptions
		//
		$exemptions = array();
		$taxes = $this->fetchAllTest('select distinct name from taxes where deleted = 0 order by name');
		...
		if (method_exists($customFields,'productDisplay')) {
			$custom = $customFields->productDisplay($data,$form);
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($status) {
				...
				if ($status = $stmt->execute()) {
					...
					$new = $this->fetchScalarAllTest('select id from %s where id in (%s) and id not in (select folder_id from %s where product_id = %d and folder_id in (%s))',
					...
					$new = $this->fetchScalarAllTest('select id from search_groups where id in (%s) and id not in (select folder_id from product_by_search_group where product_id = %d and folder_id in (%s))',
					...
					if (array_key_exists('inventory',$_POST) && $status) {
						if (array_key_exists('delete',$_POST['inventory'])) {
							foreach($_POST['inventory']['delete'] as $key=>$value) {
								...
								$rec = $this->fetchSingleTest('select * from product_inventory where id=%d',$value);
    ...
    function moveArticle() {
		...
		if ($_REQUEST['type'] == 'tree') {
			...
			if ($folder = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$dest)) {
				$curr = $this->fetchScalarTest('select product_id from %s where id = %d',$this->m_junction,$src);
				$status = true;
				if (array_key_exists('move',$_REQUEST)) {
					...
					if ($status = $this->execute(sprintf('delete from %s where id = %d', $this->m_junction, $src))) {
						if (!$this->fetchSingleTest('select * from %s where product_id = %d and folder_id = %d',$this->m_junction,$curr,$dest)) {
							$obj = new preparedStatement(sprintf('insert into %s(product_id,folder_id) values(?,?)',$this->m_junction));
			...
			if ($form->validate()) {
				...
				switch($type) {
					...
					case 'one':
						...
						$ct = $this->fetchScalarTest('select count(0) from %s where product_id = %d',$this->m_junction,$_REQUEST['p_id']);
						if ($ct == 0)
							$this->execute(sprintf('update %s set deleted = 1 where id = %d',$this->m_content,$_REQUEST['p_id']));
						break;
					default:
    ...
    function deleteContent() {
		if (array_key_exists('p_id',$_REQUEST)) {
			...
			$ct = $this->fetchScalarTest('select count(0) from %s t1, %s t2 where t2.id = %d and t1.left_id > t2.left_id and t1.right_id < t2.right_id and t1.level > t2.level',$this->m_tree, $this->m_tree, $_REQUEST['p_id']);
			if ($ct > 0) {
    ...
    function assemblyRow($rowId = null, $owner = true) {
		...
		if ((count($_REQUEST) > 0 && array_key_exists('assemblyRow',$_REQUEST)) || $rowId > 0) {
			...
			if ($owner) 
				$rec = $this->fetchSingleTest('select a.*, p.name, p.code, a.subproduct_id as edit_id from product_by_package a, product p where a.id = %d and p.id = a.subproduct_id',$id);
			else
				$rec = $this->fetchSingleTest('select a.*, p.name, p.code, a.product_id as edit_id from product_by_package a, product p where a.id = %d and p.id = a.product_id',$id);
			$frm->addData($rec);
			if ($rowId == null)
			...
			$rec = $this->fetchSingleTest('select 0 as id, p.name, p.code, p.id as subproduct_id, 0 as sequence, 0 as quantity from product p where p.id = %d and deleted = 0',$_REQUEST['addRow']);
			$frm->addData($rec);
			return $this->ajaxReturn(array('status'=>'true','html'=>$frm->show()));
		}
		else
    ...
    function assembledFromRow_dnu($rowId = null) {
		...
		if ((count($_REQUEST) > 0 && array_key_exists('assembledFromRow',$_REQUEST)) || $rowId > 0) {
			...
			$rec = $this->fetchSingleTest('select a.*, p.name, p.code from product_by_package a, product p where a.id = %d and p.id = a.product_id',$id);
			$frm->addData($rec);
			if ($rowId == null)
				return $this->ajaxReturn(array('status'=>'true','html'=>$frm->show()));
			else
    ...
    function showInventory() {
		...
		$count = $this->fetchScalarTest('select count(*) from product_inventory where product_id = %d and deleted = 0',$id);
		$pagination = $this->pagination($count, $perPage, $pageNum, 
			array('prev'=>$this->M_DIR.'forms/paginationPrev.html','next'=>$this->M_DIR.'forms/paginationNext.html',
				'pages'=>$this->M_DIR.'forms/paginationPage.html', 'wrapper'=>$this->M_DIR.'forms/paginationWrapper.html'),
		...
		$form->addTag('pagination',$pagination,false);
		$form->addTag('i_id',$id);
		$inv = $this->fetchAllTest('select p.* from product_inventory p where p.product_id = %d and p.deleted = 0 order by start_date desc,id limit %d,%d',$id,$start,$perPage);
		$temp = array();
		foreach($inv as $rec) {
    ...
    function inventoryRow($rowId = null) {
		...
		if ((count($_REQUEST) > 0 && array_key_exists('inventoryRow',$_REQUEST)) || $rowId > 0) {
			...
			if ($rec = $this->fetchSingleTest('select p.* , l1.value as color_name, l2.value as size_name, o.teaser from product_inventory p left join code_lookups l1 on l1.id = p.color left join code_lookups l2 on l2.id = p.size left join product_options o on o.id = p.options_id where p.id = %d and p.deleted = 0',$id)) {
    ...
    function inventoryAudit() {
		if (array_key_exists('i_id',$_REQUEST) && $prod = $this->fetchSingleTest('select p.*, i.id as inventory_id from product p, product_inventory i where i.id = %d and p.id = i.product_id',$_REQUEST['i_id'])) {
			if (array_key_exists('pagenum',$_REQUEST)) 
				$pageNum = $_REQUEST['pagenum'];
			else
				$pageNum = 1;	// no 0 based calcs
			...
			$perPage = $this->m_perrow;
			if (array_key_exists('pager',$_REQUEST)) $perPage = $_REQUEST['pager'];
			$count = $this->fetchScalarTest('select count(i.id) from product_inventory_audit i where i.inventory_id = %d', $_REQUEST['i_id']);
			$pagination = $this->pagination($count, $perPage, $pageNum, 
				array('prev'=>$this->M_DIR.'forms/paginationPrev.html','next'=>$this->M_DIR.'forms/paginationNext.html',
					'pages'=>$this->M_DIR.'forms/paginationPage.html', 'wrapper'=>$this->M_DIR.'forms/paginationWrapper.html'),
			...
			$data = $this->fetchAllTest('select * from product_inventory_audit i where i.inventory_id = %d order by id desc limit %d,%d', $_REQUEST['i_id'],$start,$perPage);
			$output = array();
			foreach($data as $rec) {
				$subForm->addData($rec);
				$output[] = $subForm->show();
    ...
    function editInventory() {
		if (!(array_key_exists('i_id',$_REQUEST) && $data = $this->fetchSingleTest('select * from product_inventory where id = %d',$_REQUEST['i_id']))) {
			...
			if ($form->validate()) {
				...
				if (array_key_exists('delete',$_POST)) {
					if ($id > 0) {
						...
						$rec = $this->fetchSingleTest('select * from product_inventory where id=%d',$id);
		...
		if (count($_POST) > 0 && array_key_exists('inventorySearchForm',$_POST)) {
			...
			if ($form->validate()) {
				...
				if (count($srch) > 0) {
					...
					$perPage = $this->m_perrow;
					if (array_key_exists('pager',$_REQUEST)) $perPage = $_REQUEST['pager'];					
					$count = $this->fetchScalarTest('select count(i.id), p.code, p.name from product_inventory i, product p where p.id = i.product_id and %s', implode(' and ',$srch));
					$pageNum = max(1,min($pageNum, (floor(($count-1)/$perPage)+1)));
					$form->setData('pagenum', $pageNum);
					$pagination = $this->pagination($count, $perPage, $pageNum,
							array('prev'=>$this->M_DIR.'forms/paginationPrev.html','next'=>$this->M_DIR.'forms/paginationNext.html',
     ...
     * @return array|false|Forms|mixed|string|string[]
     * @throws phpmailerException
     */
    function pricingRow($passedId = null, $data = null, $returnForm = false) {
		...
		if (!is_array($data))	// pass an optional array of populated data
			if (!($data = $this->fetchSingleTest('select p.* from product_pricing p where id = %d',$id)))
    ...
    function moduleStatus($fromMain = 0) {
		if (array_key_exists('formData',$_SESSION) && array_key_exists('productSearchForm', $_SESSION['formData'])) {
			...
			$ct = $this->fetchScalarTest('select count(0) from %s where (enabled = 0 or published = 0) and deleted = 0',$this->m_content);
			if ($ct == 0) {
    ...
    function editOption() {
		...
		$p_id = $_REQUEST['p_id'];
		$o_id = $_REQUEST['o_id'];
		if (!($data = $this->fetchSingleTest('select * from product_options where id = %d and product_id = %d',$o_id,$p_id))) {
		...
		$data['colors'] = $this->fetchScalarAllTest('select type_id from product_options_info, code_lookups c where c.id = type_id and options_id = %d and options_type="color" order by sort,value',$data['id']);
		$data['sizes'] = $this->fetchScalarAllTest('select type_id from product_options_info, code_lookups c where c.id = type_id and options_id = %d and options_type="size" order by sort,value',$data['id']);
		$frmFields = $form->buildForm($frmFields);
		$form->addData($data);
		...
		if (count($_POST) > 0 && array_key_exists('editOption',$_POST)) {
			...
			if ($form->validate()) {
				...
				if ($status) {
					...
					$new = $this->fetchAllTest('select * from code_lookups where type="color" and id in (%s) and id not in (select type_id from product_options_info where options_id = %d and options_type = "color")',implode(',',$colors),$data['id']);					
					...
					$new = $this->fetchAllTest('select * from code_lookups where type="size" and id in (%s) and id not in (select type_id from product_options_info where options_id = %d and options_type = "size")',implode(',',$sizes),$data['id']);
    ...
    function loadOptions($owner = null) {
		if (array_key_exists('o_id',$_REQUEST) && is_null($owner)) {
			$options = $this->fetchAllTest('select o.* from product_options o where o.product_id = %d and deleted = 0 order by o.sequence',$_REQUEST['o_id']);
		}
		else
			$options = $this->fetchAllTest('select o.* from product_options o where o.product_id = %d and deleted = 0 order by o.sequence',$owner);
		$form = new Forms();
    ...
    function loadPricing($owner = null) {
		if (array_key_exists('o_id',$_REQUEST) && is_null($owner)) {
			$recs = $this->fetchAllTest('select * from product_pricing where product_id = %d order by min_quantity',$_REQUEST['o_id']);
		}
		else
			$recs = $this->fetchAllTest('select * from product_pricing where product_id = %d order by min_quantity',$owner);
		$form = new Forms();
    ...
    function editPricing() {
		...
		$p_id = $_REQUEST['p_id'];
		$o_id = $_REQUEST['o_id'];
		if (!($data = $this->fetchSingleTest('select * from product_pricing where id = %d and product_id = %d',$o_id,$p_id))) {
			$data = array('id'=>0,'product_id'=>$p_id);
		}
		$form = new Forms();
    ...
    function resize($original, $size) {
		...
		if ($folder != "") {
			$this->logMessage(__FUNCTION__,sprintf("folder test [%s] exists test [%s] exists [%s]", $folder, $tmp.$folder, file_exists($tmp.$folder)),1);
     ...
     */
    function resizeAll() {
		$records = $this->fetchAllTest('select * from product');
    ...
    function editRecurring() {
		...
		$p_id = $_REQUEST['p_id'];
		$o_id = $_REQUEST['o_id'];
		if (!($data = $this->fetchSingleTest('select * from product_recurring where id = %d and product_id = %d',$o_id,$p_id))) {
			$data = array('id'=>0,'product_id'=>$p_id);
		}
		$form = new Forms();
    ...
    function moveRecurring() {
		...
		$src = $this->fetchSingleTest('select * from product_recurring where id = %d',$src);
		...
		elseif ($src['product_id'] != $dest['product_id']) {
			...
			if ($this->execute($sql)) {
				$items = $this->fetchAllTest('select * from product_recurring where product_id = %d order by sequence',$src['product_id']);
				$seq = 10;
    ...
    function getVideo($b_id = 0, $fromPost) {
		...
		if ($fromPost)
			...
			if (!$data = $this->fetchSingleTest('select * from videos where owner_id = %d and owner_type = "product"', $b_id))
				$data = array('id'=>0);
    ...
    function saveVideo($id) {
		...
		if ($data = $this->fetchSingleTest('select * from videos where owner_type="product" and owner_id = %d', $form->getData('v_id'))) {
    ...
    function moveOption() {
		...
		$src = $this->fetchSingleTest('select * from product_options where id = %d',$src);
    ...
    function resequenceOptions($p_id) {
		$recs = $this->fetchAllTest('select * from product_options where product_id = %d order by sequence',$p_id);
		$seq = 10;
		foreach($recs as $rec) {
    ...
    function comments() {
		...
		$count = $this->fetchScalarTest('select count(id) from product_reviews where product_id = %d', $p_id);
		$pagination = $this->pagination($count, $perPage, $pagenum, 
			array('prev'=>$this->M_DIR.'forms/paginationPrev.html','next'=>$this->M_DIR.'forms/paginationNext.html',
			'pages'=>$this->M_DIR.'forms/paginationPage.html', 'wrapper'=>$this->M_DIR.'forms/paginationWrapper.html'),
		...
		$comments = $this->fetchAllTest($sql);
		$inner = new Forms();
    ...
    function editComment($byAjax = true) {
		...
		$comment = $this->fetchSingleTest("select * from product_reviews where id = %d",$c_id);
    ...
    function deleteComment() {
		...
		$c_id = array_key_exists("c_id",$_REQUEST) ? $_REQUEST["c_id"] : 0;
		if ($comment = $this->fetchSingleTest("select * from product_reviews where id = %d",$c_id)) {
			$outer->addData($comment);
    ...
    function inventoryColorSize() {
		...
		$s->addOptions($this->fetchOptionsTest("select id,value from code_lookups where type='%s' and id in (select type_id from product_options_info where options_id = %d and options_type='%s') order by sort,value",$sz,$o_id,$sz));
