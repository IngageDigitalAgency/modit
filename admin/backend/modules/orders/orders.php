...
class orders extends Backend {
    ...
    function showPageContent($fromMain = false) {
		...
		$form = new Forms();
		if ($o_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_content,$o_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from %s n where deleted = 0', $this->m_content);
			...
			$status = $this->fetchAllTest('select * from code_lookups where type="orderStatus" order by sort, code');
		...
		if (count($_POST) > 0 && array_key_exists('showSearchForm',$_POST)) {
			...
			if ($form->validate()) {
				...
				if (count($srch) > 0) {
					...
					$perPage = $this->m_perrow;
					if (array_key_exists('pager',$_REQUEST)) $perPage = $_REQUEST['pager'];
					$count = $this->fetchScalarTest('select count(o.id) from %s o,members m where m.id = o.member_id and %s', $this->m_content, implode(' and ',$srch));
    ...
    function addContent($fromMain = false) {
		...
		if (!(array_key_exists('o_id',$_REQUEST) && $_REQUEST['o_id'] > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d', $this->m_content, $_REQUEST['o_id']))) {
		...
		$data['mgmt'] = $this->fetchSingleTest("select * from members where id = %d", $data["login_id"] );
		$data['currency_code'] = $this->fetchScalarTest("select value from code_lookups where id = %d",$data['currency_id']);		
		$lines = $this->fetchAllTest('select o.*, IF(o.custom_package="A","Additional Charge",IF(o.custom_package="S","Service","Package")) as l_type, p.code, p.name, c.name as coupon_name, po.teaser, p.is_fedex from order_lines o left join coupons c on c.id = o.coupon_id left join product_options po on o.options_id = po.id, product p where o.order_id = %d and p.id = o.product_id and o.deleted = 0 order by line_id',$data['id']);
		$details = array();
		$dtlForm = new Forms();
		...
		foreach($lines as $line) {
			...
			$dims = array();
			$subrecs = $this->fetchAllTest("select old.*, l1.code as wt_unit, l2.code as dim_unit from order_lines_dimensions old, orders o, code_lookups l1, code_lookups l2 where order_id = %d and line_id = %d and o.id = old.order_id and l1.id = o.custom_weight_code and l2.id = custom_dimension_code", $line["order_id"], $line["line_id"]);
		...
		$data["isFedex"] = $this->fetchScalarTest("select p.is_fedex from product p, order_lines ol where ol.order_id = %d and ol.custom_package = 'S' and p.id = ol.product_id and ol.deleted = 0", $data["id"]);
		$status = $this->fetchAllTest('select * from code_lookups where type="orderStatus" order by sort, code');
		$tmp = array();
		...
		$split = $this->fetchScalarTest("select sum(percent_of_delivery) from custom_delivery where order_id = %d",$_REQUEST['o_id']);
		if ($split == 0) 
			...
			if ($status) {
				if ($_POST['tempEdit'] == 1) {
					switch($_POST['fldName']) {
						case 'recurring_period':
							if ($rp = $this->fetchSingleTest('select * from product_recurring where id = %d',$_POST['recurring_period'])) {
							...
							break;
						case 'coupon_id':
							if ($cp = $this->fetchSingleTest('select * from coupons where id = %d',$_POST['coupon_id'])) {
					...
					$tmp['products'] = $this->fetchAllTest('select * from order_lines where order_id = %d and deleted = 0',$data['id']);

					if (!$address = $this->fetchSingleTest('select * from addresses where ownerid = %d and ownertype = "order" and tax_address=1',$data['id']))
						$address = array('id'=>0,'province_id'=>0,'country_id'=>0);
					$tmp['addresses']['shipping'] = $address;
					if (!$address = $this->fetchSingleTest('select * from addresses where ownerid = %d and ownertype = "order" and tax_address=0',$data['id']))
						$address = array('id'=>0,'province_id'=>0,'country_id'=>0);
				...
				if ($status = $stmt->execute()) {
					...
					if ($status) {
						...
						if ($data['id'] == 0) 
							$form->addTag('id',$id);
						$data = $this->fetchSingleTest('select * from %s where id = %d', $this->m_content, $id);
						$data['discount_dollar'] = '';
						$data['discount_percent'] = '';
						switch ($data['discount_type']) {
							case 'P':
						...
						if (array_key_exists('submitEmail',$_POST) && $_POST['submitEmail'] == 1) {
							...
							$body->setHTML($html);
							$order = $this->fetchSingleTest('select o.*, m.firstname, m.lastname, m.email from orders o, members m where o.id = %d and m.id = o.member_id',$id);
    ...
    function deleteArticle() {
		if (array_key_exists('j_id',$_REQUEST)) {
			$id = $_REQUEST['j_id'];
			$curr = $this->fetchScalarTest('select * from %s where id = %d',$this->m_content,$id);
     ...
     */
    function edit() {
		if (array_key_exists('e_id',$_REQUEST) && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_content,$_REQUEST['e_id'])) {
			$f = new Forms();
    ...
    function editLine() {
		...
		if (!(array_key_exists('l_id',$_REQUEST) && $data = $this->fetchSingleTest('select * from order_lines where id = %d',$_REQUEST['l_id']))) {
		...
		$order = $this->fetchSingleTest('select * from orders where id = %d',$_REQUEST['order_id']);
			...
			if ($_POST['tempEdit']) {
				...
				switch($_POST['fldName']) {
					case 'recurring_period':
						...
						break;
					case 'options_id':
						$option = $this->fetchSingleTest('select * from product_options where id = %d',$_POST['options_id']);
					...
					case 'product_id':
						...
						if ($p = Ecom::getPricing($_POST['product_id'],$_POST['quantity'],$_POST['order_date'])) {
							...
							if ($form->getField('options_id')->hasOptions()) {
								...
								if ($option = $this->fetchSingleTest('select * from product_options where id = %d',$_POST['options_id'])) {
									$_POST['price'] += $option['price'];
									$_POST['shipping'] += $option['shipping'];
								}
							}
				...
				if ($form->validate()) {
					$status = true;
					if ($form->getData("custom_package") == "S" && 0 != $this->fetchScalarTest("select count(0) from order_lines where deleted = 0 and order_id = %d and custom_package = 'S' and line_id != %d", $form->getData("order_id"), $form->getData("line_id"))) {
					...
					$prod = $this->fetchSingleTest('select * from product where id = %d',$values['product_id']);
					...
					if ($data['id'] != 0) {
						...
						$seq = $this->fetchScalarTest('select max(line_id) from order_lines where order_id = %d',$values['order_id']);
					...
					if ($data['id'] > 0) {
						...
						//
						$oline = $this->fetchSingleTest('select * from order_lines where id = %d',$data['id']);
					...
					if ($status) {
						if ($data['id'] == 0) {
							...
							if ($form->getData("product_id") == FEDEX_RECALC) {
								$nDim = $this->fetchAllTest("select old.* from order_lines_dimensions old, order_lines ol where ol.order_id = %d and ol.custom_package ='P' and old.order_id = ol.order_id and old.line_id = ol.line_id and ol.deleted = 0 order by id", $form->getData("order_id"));
								$pType = $this->fetchScalarTest("select product_id from order_lines where order_id = %d and custom_package = 'P'", $form->getData("order_id"));
    ...
    function loadAddresses($passed_id = null) {
		if ($passed_id == null) {
			...
			$o_id = $passed_id;
		$addresses = $this->fetchAllTest('select a.*, c.value as addressType from addresses a, code_lookups c where ownertype = "order" and ownerid = %d and deleted = 0 and c.id = a.addressType',$o_id);
    ...
    function editAddress() {
		...
		if (!($data = $this->fetchSingleTest('select a.* from addresses a where a.id = %d and a.ownertype = "order" and a.ownerid = %d',$a_id,$o_id))) {
			$data = array('id'=>0,'ownertype'=>'order','ownerid'=>$o_id);
			$addresses = array();
		}
		else 
			$addresses = $this->fetchAllTest('select * from addresses where ownertype = "order" and ownerid = %d and deleted = 0',$o_id);
		$form = new Forms();
		...
		if (count($_POST) > 0 && array_key_exists('editAddress',$_POST)) {
			...
			if ($form->validate()) {
				...
				if ($data['id'] == 0) {
					$flds['tax_address'] = (int)$this->fetchScalarTest('select extra from code_lookups where id = %d',$form->getData('addresstype'));
    ...
    function moduleStatus($fromMain = 0) {
		if (array_key_exists('formData',$_SESSION) && array_key_exists('orderSearchForm', $_SESSION['formData'])) {
			...
			$ct = $this->fetchScalarTest('select count(0) from %s where order_status & 2 = 2 and (total - authorization_amount) != 0',$this->m_content);
			if ($ct == 0) {
    ...
    function recalcOrder($order_id, $update = true) {
		...
		//
		$order = array();
		$order['header'] = $this->fetchSingleTest('select * from orders where id = %d',$order_id);
		$order['products'] = $this->fetchAllTest('select * from order_lines where order_id = %d and deleted = 0',$order_id);
		//$order['taxes'] = $this->fetchAllTest('select * from order_taxes ot where ot.order_id = %d and ot.line_id in (select o.line_id from order_lines o where o.order_id = ot.order_id and o.deleted = 0',$order_id));
		if (!$address = $this->fetchSingleTest('select * from addresses where ownerid = %d and ownertype = "order" and addresstype = %d',$order_id, ADDRESS_DELIVERY))
			$address = array('id'=>0,'province_id'=>0,'country_id'=>0);
		$order['addresses']['shipping'] = $address;
		if (!$address = $this->fetchSingleTest('select * from addresses where ownerid = %d and ownertype = "order" and tax_address=0',$order_id))
			$address = array('id'=>0,'province_id'=>0,'country_id'=>0);
		$order['addresses']['billing'] = $address;

		if (!$address = $this->fetchSingleTest('select * from addresses where ownerid = %d and ownertype = "order" and addresstype = %d',$order_id, ADDRESS_PICKUP))
			$address = array('id'=>0,'province_id'=>0,'country_id'=>0);
    ...
    function formatOrder($data) {
		$data['itemCount'] = $this->fetchScalarTest('select sum(quantity) from order_lines where order_id = %d and deleted = 0',$data['id']);
    ...
    function getPayments($o_id) {
		...
		$recs = $this->fetchAllTest("select * from order_authorization where order_id = %d order by authorization_date", $o_id);
		$rows = array();
		foreach($recs as $k=>$v) {
			$inner->addData($v);
			$rows[] = $inner->show();
    ...
    function getRecurring($o_id = null) {
		...
		$count = $this->fetchScalarTest('select count(0) from order_billing where original_id = %s order by billing_date',$o_id);
		if (array_key_exists('pagenum',$_REQUEST)) 
			$pageNum = $_REQUEST['pagenum'];
		else
			$pageNum = 1;	// no 0 based calcs
		...
		$outer->addData($this->fetchSingleTest("select * from orders where id = %d", $o_id));
		if ($byAjax) {
    ...
    function showRecurring() {
		...
		$rec = $this->fetchSingleTest('select * from order_billing where id = %d',$r_id);
		$form = new Forms();
    ...
    function getNames() {
		$query = $_REQUEST['s'];
		$member = $this->fetchScalarTest("select member_id from orders where id = %d",$_REQUEST['m']);
		$select = new select();
    ...
    function getPickup($o_id) {
		...
		if ($p = $this->fetchSingleTest("select c.*, d.vehicle_id from custom_delivery c left join drivers d on d.id = c.driver_id where c.service_type='P' and c.order_id = %d",$o_id)) {
    ...
    function getDelivery($o_id) {
		...
		if ($p = $this->fetchSingleTest("select c.*, d.vehicle_id from custom_delivery c left join drivers d on d.id = c.driver_id where c.service_type='D' and c.order_id = %d",$o_id)) {
			$form->setData("order", $this->fetchSingleTest("select * from orders o where o.id = %d", $o_id));
    ...
    function getCommissions($d_id) {
		...
		$p = $this->fetchAllTest("select cdc.*, p.name from custom_delivery_commissions cdc, product p where cdc.delivery_id = %d and p.id = cdc.product_id order by p.custom_commission desc, name",$d_id);
		$recs = array();
		foreach($p as $k=>$v) {
			$form->addData($v);
			$recs[] = $form->show();
    ...
    function getAllocations($o_id) {
		...
		$recs = $this->fetchAllTest("select p.reference_number, p.reference_date, d.* from order_payment p, order_payment_detail d where d.order_id = %d and p.id = d.payment_id order by p.id", $o_id);
		$data = array();
		foreach($recs as $key=>$rec) {
			$outer->addData($rec);
			$data[] = $outer->show();
    ...
    function oneTime() {
		...
		$perPage = $this->m_perrow;
		if (array_key_exists('pager',$_REQUEST)) $perPage = $_REQUEST['pager'];
		$count = $this->fetchScalarTest('select count(o.id) from orders o, members m where m.id = o.member_id and m.custom_on_account = 0 and o.total != o.authorization_amount');
		$pagination = $this->pagination($count, $perPage, $pageNum, 
			array('prev'=>$this->M_DIR.'forms/paginationPrev.html','next'=>$this->M_DIR.'forms/paginationNext.html',
			'pages'=>$this->M_DIR.'forms/paginationPage.html', 'wrapper'=>$this->M_DIR.'forms/paginationWrapper.html'),
			array('url'=>'/modit/ajax/onAccount/orders','destination'=>'middleContent'));
		$start = ($pageNum-1)*$perPage;
		$recs = $this->fetchAllTest("select o.*, m.firstname, m.lastname from orders o, members m where m.id = o.member_id and m.custom_on_account = 0 and o.total != o.authorization_amount limit %d,%d",$start,$perPage);
		$data = array();
		$status = $this->fetchAllTest('select * from code_lookups where type="orderStatus" order by sort, code');
		foreach($recs as $key=>$rec) {
    ...
    function onAccount() {
		...
		$perPage = $this->m_perrow;
		if (array_key_exists('pager',$_REQUEST)) $perPage = $_REQUEST['pager'];
		$count = $this->fetchScalarTest('select count(o.id) from orders o, members m where o.order_status_processing and o.deleted = 0 and m.id = o.member_id and m.custom_on_account = 1 and o.total != o.authorization_amount');
		$pagination = $this->pagination($count, $perPage, $pageNum, 
			array('prev'=>$this->M_DIR.'forms/paginationPrev.html','next'=>$this->M_DIR.'forms/paginationNext.html',
			'pages'=>$this->M_DIR.'forms/paginationPage.html', 'wrapper'=>$this->M_DIR.'forms/paginationWrapper.html'),
			array('url'=>'/modit/ajax/onAccount/orders','destination'=>'middleContent'));
		$start = ($pageNum-1)*$perPage;
		$recs = $this->fetchAllTest("select o.*, m.firstname, m.lastname from orders o, members m where o.order_status_processing and o.deleted = 0 and m.id = o.member_id and m.custom_on_account = 1 and o.total != o.authorization_amount order by o.id desc limit %d,%d",$start,$perPage);
		$data = array();
		$status = $this->fetchAllTest('select * from code_lookups where type="orderStatus" order by sort, code');
		foreach($recs as $key=>$rec) {
			$rec["owing"] = $rec["total"] - $rec["authorization_amount"];
    ...
    function recalcFuel() {
		...
		$r = $this->fetchSingleTest("select p.custom_minimum_charge, mf.custom_fuel_override as g_rate, m.custom_fuel_override as m_rate, p.id as f_id
...
where o.id = %d and mbf.member_id = o.member_id and mf.id = mbf.folder_id and p.id = mf.custom_fuel and m.id = o.member_id", $o_id);
		$p = $this->fetchScalarTest("select sum(value) as value from order_lines ol, product p 
		...
		$fc = round($p * ($rt/100),2);
		if ($line = $this->fetchSingleTest("select * from order_lines where order_id = %d and product_id = %d and deleted = 0", $o_id, $r["f_id"])) {
			$line["price"] = $fc;
    ...
    function cloneAddress() {
		...
		$form->addData($this->fetchSingleTest("select * from addresses where id = %d", array_key_exists("a_id",$_REQUEST ? $_REQUEST["a_id"] : 0)));
		return $this->ajaxReturn(array("status"=>true, "html"=>$form->show()));
	}
    ...
    function weightDims($data = array() ) {
		...
		if (count($data) == 0) {
			...
			$data = $this->fetchSingleTest("select * from order_lines where id = %d", $data["id"]);
			$ajax = 1;
		}
		$ol = $this->fetchSingleTest("select * from order_lines where id = %d", $data["id"]);
		$outer = new Forms();
		$outer->init($this->getTemplate(__FUNCTION__));
		$flds = $outer->buildForm($this->getFields(__FUNCTION__));
		$inner = new Forms();
		...
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__,$_POST)) {
			...
			if (array_key_exists("delete", $_POST)) {
				if (count($_POST["delete"])) {
					$ct = $this->fetchScalarTest("select count(old.id) from order_lines_dimensions old, order_lines ol where ol.id = %d and old.order_id = ol.order_id and old.line_id = ol.line_id", $data["id"]);
			...
			$qty = 0;
			$wt = 0;
			$old = $this->fetchSingleTest("select sum(quantity) as qty, sum(weight) as wt from order_lines_dimensions where order_id = %d group by order_id", $data["order_id"]);
			...
			if ($valid)  {
				$wt = round($wt,2);
				$n = $this->fetchScalarTest("select notes from orders where id = %d", $data["order_id"]);
		...
		$recs = $this->fetchAllTest("select old.* from order_lines_dimensions old, order_lines ol where ol.id = %d and old.order_id = ol.order_id and old.line_id = ol.line_id order by id", $data["id"]);
		$data["pType"] = $this->fetchScalarTest("select p.name from product p, order_lines ol where ol.id = %d and p.id = ol.product_id", $data["id"]);
		$data["weightUnits"] = $this->fetchScalarTest("select code from code_lookups cl, order_lines ol, orders o where ol.id = %d and o.id = ol.order_id and cl.id = o.custom_weight_code", $data["id"]);
		$data["dimUnits"] = $this->fetchScalarTest("select code from code_lookups cl, order_lines ol, orders o where ol.id = %d and o.id = ol.order_id and cl.id = o.custom_dimension_code", $data["id"]);
		$outer->addData($data);
		$rows = array();
		foreach($recs as $k=>$v) {
			$inner->addData($v);
    ...
    function recalcFedex() {
		...
		$svc = $this->fetchSingleTest("select *, p.code from order_lines ol, product p where ol.order_id = %d and ol.custom_package = 'S' and p.id = ol.product_id and ol.deleted = 0", $o_id);
		$grp = $this->fetchSingleTest("select * from members_by_folder where member_id = %d order by id limit 1", $order["member_id"]);
		$g_svc = $this->fetchSingleTest("select * from custom_member_product_options where member_id = %d and product_id = %d and isgroup = 1", $grp["folder_id"], $svc["product_id"]);
		$m_svc = $this->fetchSingleTest("select * from custom_member_product_options where member_id = %d and product_id = %d and isgroup = 0", $order["member_id"], $svc["product_id"]);
		$quote = array(
			...
			"t_id" => 11,
			"s_type" => 0,
			"wt"=>$this->fetchSingleTest("select * from code_lookups where id = %d", $order["custom_weight_code"]),
			"sz"=>$this->fetchSingleTest("select * from code_lookups where id = %d", $order["custom_dimension_code"]),
		);
		if (!($p = $this->fetchAllTest("select * from order_lines where custom_package = 'A' and product_id = %d and order_id = %d and deleted = 0", FEDEX_RECALC, $o_id))) {
			$p = $this->fetchAllTest("select * from order_lines where custom_package='P' and order_id = %d and deleted = 0", $o_id);
		}
		foreach($p as $k=>$v) {
			$r = $this->fetchAllTest("select * from order_lines_dimensions where order_id = %d and line_id = %d order by id", $v["order_id"], $v["line_id"]);
			$dims = array();
			foreach($r as $sk=>$sv) {
				$dims[$sk+1] = array(
...
/*
		if ($p = $this->fetchSingleTest("select old.* from order_lines_dimensions old, order_lines ol where ol.product_id = %d and ol.order_id = %d and ol.deleted = 0 and old.order_id = ol.order_id and old.line_id = ol.line_id", FEDEX_RECALC, $o_id)) {
			//
...
*/
		...
		$cart["addresses"]["shipping"] = $this->fetchSingleTest("select * from addresses where ownertype='order' and ownerid = %d and addresstype = %d", $o_id, ADDRESS_DELIVERY);
		$cart["addresses"]["pickup"] = $this->fetchSingleTest("select * from addresses where ownertype='order' and ownerid = %d and addresstype = %d", $o_id, ADDRESS_PICKUP);
		...
		foreach($cart["custom"]["ourRates"] as $k=>$v) {
			if ($v["product_id"] == $svc["product_id"]) {
				...
				if (abs($diff) > .01) {
					$rec = $this->fetchSingleTest("select * from order_lines where order_id = %d and product_id = %d and deleted = 0", $o_id, FEDEX_RECALC);
					if (is_array($rec)) {
						$rec["price"] = $diff;
						$rec["value"] = $diff;
						...
						$prod = array(
							"order_id"=>$o_id,
							"line_id"=>1+(int)$this->fetchScalarTest("select max(line_id) from order_lines where order_id = %d group by order_id", $o_id),
    ...
    function onDemand() {
		...
		$org = array_merge(array(0),$this->fetchScalarAllTest("select distinct member_id from orders where order_status & %d and deleted = 0 and !(order_status & %d)", $outer->getData("order_status"), STATUS_CANCELLED));
		$p_org = $this->fetchScalarAllTest("select custom_parent_org from members where id in (%s)", implode(", ",$org));
		...
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__,$_POST)) {
			...
			if ($outer->validate()) {
				...
				$count = $this->fetchScalarTest('select count(o.id) %s', $sql);
...
//					$sql = sprintf('select o.*, m.company from orders o, members m, order_lines ol, product p where m.id = o.member_id and order_status = %1$d and m.deleted = 0 and o.deleted = 0 and m.enabled = 1 and (member_id = %2$d or m.custom_parent_org = %2$d) and ol.order_id = o.id and ol.custom_package = "S" and p.id = ol.product_id order by o.recurring_sequence_pickup LIMIT %3$d,%4$d', $outer->getData("order_status"), $outer->getData("member_id"), $start, $perPage);
				...
				$rows = array();
				foreach($recs as $k=>$v) {
					$v["service"] = $this->fetchSingleTest("select p.* from product p, order_lines ol where ol.order_id = %d and ol.custom_package = 'S' and p.id = ol.product_id", $v["id"]);
					$v["pickup"] = $this->fetchSingleTest("select a.* from addresses a where a.ownerid = %d and a.ownertype = 'order' and a.addressType = %d", $v["id"], ADDRESS_PICKUP);
					$v["delivery"] = $this->fetchSingleTest("select a.* from addresses a where a.ownerid = %d and a.ownertype = 'order' and a.addressType = %d", $v["id"], ADDRESS_DELIVERY);
					$inner->addData($v);
					$rows[] = $inner->show();
				}
				$outer->setData("rows", implode("",$rows));
				...
				if (array_key_exists("process",$_POST) && count($_POST["process"]) > 0 && $outer->getData("do_it") == 1) {
					...
					foreach($_POST["process"] as $k=>$v) {
						if ($v == 1) {
							if ($newOrder = $this->dupeOrder($k, $outer)) {
								$processed[] = sprintf('%1$s => <a href="/modit/orders/showOrder?o_id=%2$d" target="_new">%2$d</a>', $this->fetchScalarTest("select m.company from members m, orders o where o.id = %d and m.id = o.member_id", $k), $newOrder);
								$toResort[$k] = $newOrder;
							}
							else
					...
					if (count($processed) > 0) {
						...
						foreach($toResort as $k=>$v) {
							$actions = $this->fetchAllTest("select cd.*, o.recurring_sequence_pickup, o.recurring_sequence_delivery from custom_delivery cd, orders o where cd.order_id = %d and o.id = %d", $v, $k);
    ...
    private function dupeOrder($oldNumber, $form) {
		...
		$src_order = $this->fetchSingleTest("select * from orders where id = %d", $oldNumber);
		$user = $this->fetchSingleTest("select * from members where id = %d", $src_order["member_id"]);
		...
		$src_service = $this->fetchSingleTest("select p.*, ol.line_id from product p, order_lines ol where ol.order_id = %d and p.id = ol.product_id and ol.custom_package = 'S' and ol.deleted = 0", $src_order["id"]);
		$src_packages = $this->fetchAllTest("select ol.* from order_lines ol where ol.order_id = %d and ol.custom_package = 'P' and ol.deleted = 0", $src_order["id"]);
		$src_contracted = $this->fetchSingleTest("select * from order_lines where order_id = %d and product_id = %d", $src_order["id"], CONTRACTED_RATE);
		$this->logMessage(__FUNCTION__, sprintf("src_packages [%s]", print_r($src_packages,true)), 1);
		$src_extras = $this->fetchScalarAllTest("select ol.product_id from order_lines ol, product p where ol.order_id = %d and ol.custom_package = 'A' and p.id = ol.product_id and p.custom_special_requirement and ol.deleted = 0", $src_order["id"]);
		...
		foreach($src_packages as $k=>$v) {
			...
			$src_product[$k]["dimensions"] = $this->fetchAllTest("select quantity,weight,height,width,depth from order_lines_dimensions od where od.order_id = %d and od.line_id = %d", $src_order["id"], $v["line_id"]);
		...
		$pickup = $this->fetchSingleTest("select * from addresses where ownertype='order' and ownerid=%d and addresstype=%d", $src_order["id"], ADDRESS_PICKUP);
		$delivery = $this->fetchSingleTest("select * from addresses where ownertype='order' and ownerid=%d and addresstype=%d", $src_order["id"], ADDRESS_DELIVERY);
		...
		$group = $this->fetchSingleTest("select * from members_by_folder where member_id = %d", $src_order["member_id"]);
		$g_product = $this->fetchSingleTest("select * from custom_member_product_options where member_id = %d and isgroup = 1 and product_id = %d", $group["folder_id"], $src_service["id"]);
		$m_product = $this->fetchSingleTest("select * from custom_member_product_options where member_id = %d and isgroup = 0 and product_id = %d", $group["id"], $src_service["id"]);
		if (!is_array($g_product)) {
			...
			$_SESSION["cart"]["header"]["pickupInstructions"] = $this->fetchScalarTest("select instructions from custom_delivery where order_id=%d and service_type='P'",$oldNumber);
			$_SESSION["cart"]["header"]["deliveryInstructions"] = $this->fetchScalarTest("select instructions from custom_delivery where order_id=%d and service_type='D'",$oldNumber);
			$_SESSION["cart"]["header"]["custom_weight_code"] = $src_order["custom_weight_code"];
			$_SESSION["cart"]["header"]["custom_dimension_code"] = $src_order["custom_dimension_code"];
			...
			$_POST = $_REQUEST;
			$allowedZones = $this->fetchScalarAllTest("select zone_id from zones_by_folder z, members m where m.id = %d and z.folder_id = m.custom_zones", $c->getUserInfo("id",true));
			...
			if ($src_service["is_fedex"]==1) {
				$quote["wt"] = $this->fetchSingleTest("select * from code_lookups where id = %d",$weight_cd);
				$quote["sz"] = $this->fetchSingleTest("select * from code_lookups where id = %d",$dimension_cd);
			...
			if ($valid) {
				...
				$quote["pickupInstructions"] = $this->fetchScalarTest("select instructions from custom_delivery where order_id = %d and service_type='P'",$oldNumber);
				$quote["deliveryInstructions"] = $this->fetchScalarTest("select instructions from custom_delivery where order_id = %d and service_type='P'",$oldNumber);

			//
			//	auto-assign if the recurring drivers are set
			//
				$quote["custom_recurring_pu_driver"] = $src_order["custom_recurring_pu_driver"] > 0 ? $this->fetchScalarTest("select d.id from drivers d, members m where m.id = d.member_id and m.deleted = 0 and m.enabled = 1 and d.deleted = 0 and d.enabled = 1 and d.id = %d",$src_order["custom_recurring_pu_driver"]) : 0;
				if ($form->getData("pu_driver") > 0)
					$quote["custom_recurring_pu_driver"] = $this->fetchScalarTest("select d.id from drivers d, members m where m.id = d.member_id and m.deleted = 0 and m.enabled = 1 and d.deleted = 0 and d.enabled = 1 and d.id = %d",$form->getData("pu_driver"));
				$quote["custom_recurring_del_driver"] = $src_order["custom_recurring_del_driver"] > 0 ? $this->fetchScalarTest("select d.id from drivers d, members m where m.id = d.member_id and m.deleted = 0 and m.enabled = 1 and d.deleted = 0 and d.enabled = 1 and d.id = %d",$src_order["custom_recurring_del_driver"]) : 0;
				if ($form->getData("del_driver") > 0)
					$quote["custom_recurring_del_driver"] = $this->fetchScalarTest("select d.id from drivers d, members m where m.id = d.member_id and m.deleted = 0 and m.enabled = 1 and d.deleted = 0 and d.enabled = 1 and d.id = %d",$form->getData("del_driver"));
    ...
    function importOrders() {
		...
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__, $_POST)) {
			...
			$rows = array();
			$recs = $this->fetchAllTest("select oi.*, p.name as fee_code, m.company from order_import oi left join product p on p.id = oi.handling_fee, members m where %s", implode(" and ", $srch));
			foreach($recs as $k=>$v) {
				$inner->addData($v);
				$rows[] = $inner->show();
			}
    ...
    function addImport() {
		...
		$rows = array();
		$recs = $this->fetchAllTest("select * from order_import_messages where import_id = %d and status = 1", $i_id);
		foreach($recs as $k=>$v) {
			$rows[] = $v["message"];
		}
		if (count($rows) > 0)
		...
		$rows = array();
		$recs = $this->fetchAllTest("select * from order_import_messages where import_id = %d and status = 2", $i_id);
		foreach($recs as $k=>$v) {
			$rows[] = $v["message"];
		}
		if (count($rows) > 0)
    ...
    function getImportAttachment() {
		$i_id = array_key_exists("i_id", $_REQUEST) ? $_REQUEST["i_id"] : 0;
		if ($rec = $this->fetchSingleTest("select * from order_import where id = %d", $i_id)) {
			ob_end_clean();
