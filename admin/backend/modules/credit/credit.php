...
*/
    ...
    function salesReports() {
		...
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__,$_POST)) {
			...
			if ($valid) {
				...
				if (array_key_exists('pager',$_REQUEST)) $perPage = $_REQUEST['pager'];
				$count = $this->fetchScalarTest('select count(0) from orders o where %s',implode(' and ',$where));
				$outer->addTag('count',$count);
				if (array_key_exists('product_id',$_REQUEST) && count($_REQUEST['product_id']) > 0 && implode(",",$_REQUEST["product_id"]) != "") {					
					$t = $this->fetchSingleTest("select sum(ol.value) as sumValue, sum(o.total) as totalValue, sum(o.shipping) as sumShipping from orders o, order_lines ol where %s and ol.product_id in (%s) and ol.deleted = 0 and ol.order_id = o.id group by ol.product_id, ol.options_id",implode(' and ',$where),implode(",",$_REQUEST['product_id']));
					$totalShipping = $t['sumShipping'];
					$totalGoods = $t['sumValue'];
					$totalTotal = $t['totalValue'];
				}
				else {					
					if ($t = $this->fetchSingleTest("select sum(value) as sumValue, sum(shipping) as sumShipping, sum(total) as sumTotal from orders o where %s",implode(' and ',$where))) {
						$totalShipping = $t['sumShipping'];
						$totalGoods = $t['sumValue'];
    ...
    function QB () {
		...
		if ($this->checkArray("administrator:qb:accessToken",$_SESSION)) {
			...
			$flds["qb_invoice_id"]["value"] = $this->fetchScalarTest("select qb_invoice_id from qb_export order by id desc limit 1") + 1;
		}
...
*/
				...
				$recs = $this->fetchAllTest("select qb.*, m.company, invoice_amount+tax_amount as total from qb_export qb, members m where m.id = qb.member_id and %s order by qb_invoice_id desc", implode(" and ", array_values($srch)));
				$rows = array();
				$inner = new Forms();
				$inner->init($this->getTemplate(__FUNCTION__."Row"));
				$flds = $inner->buildForm($this->getFields(__FUNCTION__."Row"));
