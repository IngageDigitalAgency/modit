...
class credit extends Backend {
	...
	public function __construct() {
		...
		$this->setFields(array(
			...
			'QB'=>array(
				...
				'pager'=>array('type'=>'select','required'=>false,'value'=>$this->m_perrow,'lookup'=>'paging','id'=>'pager'),
				'billing_type'=>array('type'=>'select','required'=>false,'options'=>array('B'=>'Bill Later','P'=>'Prepaid','C'=>'Non-Delivery'),'value'=>'B')
			),
			'QBConnect'=>array(
			),
			'QBRow'=>array(
    ...
    function addContent($fromMain = false) {
		...
		$lines = $this->fetchAllTest('select o.*, p.code, p.name, c.name as coupon_name from order_lines o left join coupons c on c.id = o.coupon_id, product p where o.order_id = %d and p.id = o.product_id and o.deleted = 0 order by line_id',$data['id']);
		$details = array();
		$dtlForm = new Forms();
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($status) {
				if ($_POST['tempEdit'] == 1) {
					...
					$tmp['taxes'] = $this->fetchAllTest('select * from order_taxes ot where ot.order_id = %d and ot.line_id in (select o.line_id from order_lines o where o.order_id = ot.order_id and o.deleted = 0)',$data['id']);
				...
				if ($status = $stmt->execute()) {
					if ($data['id'] == 0) {
						$id = $this->insertId();
						$addresses = $this->fetchAllTest('select * from addresses where ownertype="member" and ownerid = %d and deleted = 0',$form->getData('member_id'));
    ...
    function moduleStatus($fromMain = 0) {
		if (array_key_exists('formData',$_SESSION) && array_key_exists('creditSearchForm', $_SESSION['formData'])) {
			...
			$ct = $this->fetchScalarTest('select count(0) from %s where order_status & %d = %d',$this->m_content,STATUS_CREDIT_HOLD,STATUS_CREDIT_HOLD);
			$ct = $this->fetchScalarTest('select count(0) from %s where order_status & %d = %d',$this->m_content,STATUS_EXPIRING,STATUS_EXPIRING);
			if ($ct == 0) {
    ...
    function dailyLog($fromMain = false) {
		...
		if (array_key_exists('pager',$_REQUEST)) $perPage = $_REQUEST['pager'];
		$count = $this->fetchScalarTest('select count(0) from order_processing where %s',implode(' and ',$where));
		$pageNum = max(1,min($pageNum, (floor(($count-1)/$perPage)+1)));
...
/*
		...
		if (array_key_exists('pager',$_REQUEST)) $perPage = $_REQUEST['pager'];
*/
		$count = $this->fetchScalarTest('select count(0) from order_processing_details where processing_id = %d',$r_id);
		$pageNum = max(1,min($pageNum, (floor(($count-1)/$perPage)+1)));
		$outer->setData('pagenum', $pageNum);
		$pagination = $this->pagination($count, $perPage, $pageNum,
    ...
    function showPageProperties($fromMain = false) {
		$o_id = array_key_exists('o_id',$_REQUEST) ? $_REQUEST['o_id'] : 0;
		$data = $this->fetchSingleTest('select * from orders where id = %d', $o_id);
		$outer = new Forms();
    ...
    function getBilled($o_id = null) {
		...
		$perPage = $this->m_perrow;
		$start = ($pageNum-1)*$perPage;
		$count = $this->fetchScalarTest('select count(b.id) from order_billing b where b.billed = 1 and b.original_id = %d order by order_id',$o_id);
		$pagination = $this->pagination($count, $perPage, $pageNum, 
			array(
		...
		$other = $this->fetchAllTest('select b.* from order_billing b where b.billed = 1 and b.original_id = %d order by order_id desc limit %d, %d',$o_id, $start, $this->m_perrow);
		$recurring = array();
    ...
    function getRemaining($o_id = null) {
		...
		$perPage = $this->m_perrow;
		if (array_key_exists('pager',$_REQUEST)) $perPage = $_REQUEST['pager'];
		$count = $this->fetchScalarTest('select count(id) from order_billing where billed = 0 and original_id = %d', $o_id);
		$pagination = $this->pagination($count, $perPage, $pageNum, 
			array('prev'=>$this->M_DIR.'forms/altPaginationPrev.html','next'=>$this->M_DIR.'forms/altPaginationNext.html',
			'pages'=>$this->M_DIR.'forms/altPaginationPage.html', 'wrapper'=>$this->M_DIR.'forms/altPaginationWrapper.html'),
			array('url'=>'/modit/ajax/showFolderContent/credit','destination'=>'middleContent'));
		$start = ($pageNum-1)*$perPage;
		$other = $this->fetchAllTest('select * from order_billing where original_id = %d and billed = 0 order by billing_date limit %d,%d',$o_id,$start,$perPage);
		foreach($other as $key=>$order) {
			$inner->reset();
			$inner->addData($order);
			$recurring[] = $inner->show();
    ...
    function editRecurring() {
		$r_id = array_key_exists('r_id',$_REQUEST) ? $_REQUEST['r_id'] : 0;
		$data = $this->fetchSingleTest('select * from order_billing where id = %d', $r_id);
		$outer = new Forms();
				...
				if ($data['billing_date'] != $new) {
					...
					if ($valid) {
						...
						if (array_key_exists('adjustment', $_POST) && $_POST['adjustment'] == 1) {
							$next = $this->fetchAllTest('select * from order_billing where original_id = %d and period_number > %d and billed = 0', $data['original_id'], $data['period_number']);
    ...
    function addSchedule() {
		...
		if (!$data = $this->fetchSingleTest('select * from order_processing where id = %d', $s_id))
			$data = array('id'=>0);
		$outer = new Forms();
		$outer->init($this->getTemplate(__FUNCTION__));
		$outer->addData($data);
		...
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__,$_POST)) {
			...
			if ($valid) {
				...
				if ($ct = $this->fetchScalarTest('select count(0) from order_processing where bill_date >= "%s" and bill_date <= "%s"', $outer->getData('start_date'), $outer->getData('end_date'))) {
			...
			if ($valid) {
				...
				if ($valid) {
					...
					$last = $this->fetchScalarTest('select max(end_date) from order_processing where started = "0000-00-00 00:00:00"');
					if ($last == "") 
						$last = date("Y-m-d");
					else
    ...
    function exchange() {
		...
		if (array_key_exists('pager',$_REQUEST)) $perPage = $_REQUEST['pager'];
		$count = $this->fetchScalarTest('select count(0) from exchange_rate e, code_lookups l where %s',implode(' and ',$where));
		if (array_key_exists('pagenum',$_REQUEST)) 
			$pageNum = $_REQUEST['pagenum'];
		else
			$pageNum = 1;	// no 0 based calcs
    ...
    function addCurrency() {
		...
		if (!$data = $this->fetchSingleTest('select * from exchange_rate where id = %d', $e_id))
			$data = array('id'=>0);
		$outer = new Forms();
		$outer->init($this->getTemplate(__FUNCTION__));
		$outer->addData($data);
    ...
    function salesReports() {
		...
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__,$_POST)) {
			...
			if ($valid) {
				...
				if (array_key_exists('pager',$_REQUEST)) $perPage = $_REQUEST['pager'];
				$count = $this->fetchScalarTest(sprintf('select count(0) from orders o where %s',implode(' and ',$where)));
				$outer->addTag('count',$count);
				if (array_key_exists('product_id',$_REQUEST) && count($_REQUEST['product_id']) > 0 && implode(",",$_REQUEST["product_id"]) != "") {					
					$t = $this->fetchSingleTest(sprintf("select sum(ol.value) as sumValue, sum(o.total) as totalValue, sum(o.shipping) as sumShipping from orders o, order_lines ol where %s and ol.product_id in (%s) and ol.deleted = 0 and ol.order_id = o.id group by ol.product_id, ol.options_id",implode(' and ',$where),implode(",",$_REQUEST['product_id'])));
					$totalShipping = $t['sumShipping'];
					$totalGoods = $t['sumValue'];
					$totalTotal = $t['totalValue'];
				}
				else {					
					if ($t = $this->fetchSingleTest(sprintf("select sum(value) as sumValue, sum(shipping) as sumShipping, sum(total) as sumTotal from orders o where %s",implode(' and ',$where)))) {
						$totalShipping = $t['sumShipping'];
						$totalGoods = $t['sumValue'];
    ...
    function exportSales($where) {
		...
		foreach($orders as $key=>$order) {
			...
			if ($rec = $this->fetchSingleTest("select * from addresses where ownertype='order' and ownerid = %d and tax_address=1",$order['id']))
				$order['shippingAddress'] = Address::formatData($rec);
			if ($rec = $this->fetchSingleTest("select * from addresses where ownertype='order' and ownerid = %d and tax_address=0",$order['id']))
				$order['billingAddress'] = Address::formatData($rec);
			$taxes = $this->fetchAllTest("select ot.*, t.* from order_taxes ot, taxes t where t.id = ot.tax_id and ot.order_id = %d and line_id = 0",$order["id"]);
			$order["line_taxes"] = $taxes;
			$order['member'] = $this->fetchSingleTest("select * from members where id = %d",$order['member_id']);
			$inner->addData($order);
			$rows[] = $inner->show();
		}
    ...
    function exportDetails($where) {
		...
		foreach($orders as $key=>$order) {
			...
			$order['member'] = $this->fetchSingleTest("select * from members where id = %d",$order['member_id']);
			if (array_key_exists('product_id',$_REQUEST) && count($_REQUEST['product_id']) > 0) {
				$lines = $this->fetchAllTest("select ol.*, p.code, p.name, po.teaser as opt_teaser, pr.teaser as recur_teaser from order_lines ol left join product_options po on po.id = ol.options_id left join product_recurring pr on pr.id = ol.recurring_period, product p where ol.order_id = %d and ol.product_id in (%s) and p.id = ol.product_id",$order["id"],implode(",",$_REQUEST['product_id']));				
			}
			else {
				$lines = $this->fetchAllTest("select ol.*, p.code, p.name, po.teaser as opt_teaser, pr.teaser as recur_teaser from order_lines ol left join product_options po on po.id = ol.options_id left join product_recurring pr on pr.id = ol.recurring_period, product p where ol.order_id = %d and p.id = ol.product_id",$order["id"]);
			}
			$taxes = $this->fetchAllTest("select ot.*, t.* from order_taxes ot, taxes t where t.id = ot.tax_id and ot.order_id = %d and line_id = 0",$order["id"]);
			$order["line_taxes"] = $taxes;
    ...
    function recurringOrders() {
		...
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__,$_POST)) {
			...
			if ($valid) {
				...
				if (array_key_exists('pager',$_REQUEST)) $perPage = $_REQUEST['pager'];
				$count = $this->fetchScalarTest('select count(0) from orders o, order_billing b where %s',implode(' and ',$where));
				$outer->addTag('count',$count);
				if (array_key_exists('product_id',$_REQUEST) && $_REQUEST['product_id'] > 0) {					
					$t = $this->fetchSingleTest("select sum(ol.value) as sumValue, sum(o.total) as totalValue, sum(o.shipping) as sumShipping from orders o, order_lines ol, order_billing b where %s and ol.product_id = %d and ol.deleted = 0 and ol.order_id = o.id group by ol.product_id, ol.options_id",implode(' and ',$where),$_REQUEST['product_id']);
					$totalShipping = $t['sumShipping'];
					$totalGoods = $t['sumValue'];
					$totalTotal = $t['totalValue'];
				}
				else {					
					if ($t = $this->fetchSingleTest("select sum(value) as sumValue, sum(shipping) as sumShipping, sum(total) as sumTotal from orders o, order_billing b where %s",implode(' and ',$where))) {
						$totalShipping = $t['sumShipping'];
						$totalGoods = $t['sumValue'];
						$totalTotal = $t['sumTotal'];
					}
		...
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__,$_POST)) {
				...
				if ($valid) {
					...
					foreach($_REQUEST["pay"] as $k=>$paying) {
						if ($paying == 1) {
							...
							$b = $this->fetchSingleTest("select total, authorization_amount from orders where id = %d", $k);
			...
			//if ($valid) {
				...
				foreach($_POST as $key=>$value) {
					...
					$recs = $this->fetchAllTest("select o.*, m.company, m.firstname, m.lastname from orders o, members m where m.id = o.member_id and total - authorization_amount > 0.01 and %s",implode(" and ",$srch));
					$inner = new Forms();
					$inner->init($this->getTemplate(__FUNCTION__."Row"));
					$flds = $inner->buildForm($this->getFields(__FUNCTION__."Row"));
					$result = array();
    ...
    function getMembers() {
		$query = $_REQUEST['s'];
		$member = $this->fetchScalarTest("select id from members where id = %d",$_REQUEST['m']);
		$select = new select();
    ...
    function QB () {
		...
		if ($this->checkArray("administrator:qb:accessToken",$_SESSION)) {
			...
			$flds["qb_invoice_id"]["value"] = $this->fetchScalarTest(sprintf("select qb_invoice_id from qb_export order by id desc limit 1")) + 1;
		}
...
*/
				...
				$ct = $this->fetchScalarTest($sql);				
				$recs = $this->fetchAllTest(sprintf("select qb.*, m.company, invoice_amount+tax_amount as total from qb_export qb, members m where m.id = qb.member_id and %s order by qb_invoice_id desc", implode(" and ", array_values($srch))));
				$rows = array();
				$inner = new Forms();
				$inner->init($this->getTemplate(__FUNCTION__."Row"));
				$flds = $inner->buildForm($this->getFields(__FUNCTION__."Row"));
    ...
    function qbExportCheck() {
		...
		$ct = $this->fetchScalarAllTest("select distinct invoice_date from qb_export where consolidate_id < 0 group by consolidate_id");
	...
	select * from (
		...
		having count(ot.order_id) > 1) xxx)"));
			foreach($_POST["process"] as $k=>$member_id) {
				if (!$errFound) {
					$company = $this->fetchSingleTest("select * from members where id = %d", $member_id);
//
...
*/
					...
					if ($p = $this->fetchSingleTest("select * from members where id = %d and custom_consolidate_invoices > 0", $company["custom_parent_org"])) {
						//
...
*/
    ...
    function qbInvoiceDetails() {
		...
		$flds = $inner->buildForm($this->getFields(__FUNCTION__."Row"));
		$rows = array();
		if ($hdr = $this->fetchSingleTest("select * from qb_export where id =%d", array_key_exists("i_id",$_REQUEST ? $_REQUEST["i_id"] : 0))) {
			$form->addData($hdr);
			...
			$count = $this->fetchScalarTest("select count(0) from qb_export_dtl qb, orders o, order_lines ol, product p where qb.qb_export_id =%d and o.id = qb.order_id and ol.order_id = o.id and p.id = ol.product_id and ol.custom_package ='S'", $hdr["id"]);
			$pageNum = max(1,min($pageNum, (floor(($count-1)/$perPage)+1)));
			$pagination = $this->pagination($count, $perPage, $pageNum,
							array('prev'=>$this->M_DIR.'forms/altPaginationPrev.html','next'=>$this->M_DIR.'forms/altPaginationNext.html',
				'pages'=>$this->M_DIR.'forms/altPaginationPage.html', 'wrapper'=>$this->M_DIR.'forms/altPaginationWrapper.html'),
			...
			$recs = $this->fetchAllTest("select qb.*, o.order_date, o.total, p.name from qb_export_dtl qb, orders o, order_lines ol, product p where qb.qb_export_id =%d and o.id = qb.order_id and ol.order_id = o.id and p.id = ol.product_id and ol.custom_package ='S' order by qb.id limit %d,%d", $hdr["id"], ($pageNum-1)*$perPage, $perPage);			
			foreach($recs as $k=>$v) {
				$inner->addData($v);
				$rows[] = $inner->show();
			}
    ...
    function unpaidBillNow() {
		...
		if (array_key_exists(__FUNCTION__,$_REQUEST)) {
			...
			if ($outer->validate()) {
				...
				$recs = $this->fetchAllTest("select o.*, o.total - o.authorization_amount as owing, m.company, m.firstname, m.lastname, d.actual_date, a.authorization_amount as authorized_amount, coalesce(p.authorization_amount,0) as paid from orders o left join order_authorization p on p.order_id = o.id and p.authorization_type='D' and p.authorization_success=1, members m, custom_delivery d, order_authorization a where %s", implode(" and ",array_values($srch)));				
				$rows = array();
				foreach($recs as $k=>$v) {
    ...
    function authHistory() {
		...
		$outer->addData($_REQUEST);
		$outer->validate();
		$recs = $this->fetchAllTest("select * from order_authorization where order_id = %d order by id", $outer->getData("o_id"));
		$rows = array();
		foreach($recs as $k=>$v) {
			$inner->addData($v);
			$rows[] = $inner->show();
		...
		if ($outer->validate()) {
			if (array_key_exists("process",$_REQUEST) && is_array($_REQUEST["process"])) {
				...
				foreach($_REQUEST["process"] as $k=>$v) {
					if ($order = $this->fetchSingleTest("select o.total, o.authorization_amount as paid, o.total - o.authorization_amount as balance, o.nags, o.order_status, a.* from orders o, order_authorization a where o.id = %d and a.order_id = o.id and a.authorization_type='A' and a.authorization_success = 1", $k)) {
						$query["TRXTYPE"] = "D";
						$query["TENDER"] = "C";
    ...
    function removeSchedule() {
		...
		if (array_key_exists(__FUNCTION__,$_REQUEST) && $_REQUEST[__FUNCTION__] > 0) {
			...
			if (($s_id = $outer->getData("s_id")) > 0) {
				if ($rec = $this->fetchSingleTest("select * from order_processing where id = %d", $s_id)) {
		...
		if (array_key_exists(__FUNCTION__,$_REQUEST)) {
			...
			if ($valid = $outer->validate()) {
				...
				if (count($srch) > 1) {
					...
					if ($perPage <= 0) $perPage = $this->m_perrow;
					$count = $this->fetchScalarTest("select count(0) from qb_export i, members m where %s", implode(" and ", $srch));
					$pageNum = max(1,min($pageNum, (floor(($count-1)/$perPage)+1)));
					$pagination = $this->pagination($count, $perPage, $pageNum);
					$start = ($pageNum-1)*$perPage;
					$sortby = $outer->getData('sortby');
					$sortorder = $outer->getData('sortorder');
					$recs = $this->fetchAllTest("%s order by %s %s limit %d,%d", $sql, $sortby, $sortorder, $start, $perPage);
					$outer->setData("count",$count);
					$rows = array();
					$ct = 0;
					$tot = 0;
					foreach($recs as $k=>$v) {
						$v["order_count"] = $this->fetchScalarTest("select count(0) from qb_export_dtl where qb_export_id = %d", $v["id"]);
						$v["total"] = $this->fetchScalarTest("select sum(o.total) from orders o, qb_export_dtl qb where qb.qb_export_id = %d and o.id = qb.order_id", $v["id"]);
						$ct += $v["order_count"];
						$tot += $v["total"];
						$inner->addData($v);
						$rows[] = $inner->show();
    ...
    function getInvoice() {
		...
		$outer->addData($this->fetchSingleTest("select *, member_id as parent_id from qb_export where id = %d", $i_id));
		$recs = $this->fetchAllTest("select d.*, i.member_id as p_id from qb_export_dtl d, qb_export i where i.id = %d and d.qb_export_id = i.id order by order_id", $i_id);
		if (array_key_exists(__FUNCTION__,$_REQUEST)) {
			$m_id = $outer->getData("parent_id");
			$this->beginTransaction();
			$status = true;
			if (array_key_exists("delete",$_REQUEST) && $_REQUEST["delete"]==1) {
				$orders = $this->fetchScalarAllTest("select order_id from qb_export_dtl where qb_export_id = %d", $i_id);
				foreach($orders as $k=>$v) {					
					$status &= $this->execute(sprintf("update orders set custom_qb_order = 0, authorization_amount = %f where id = %d", $this->fetchScalarTest("select sum(authorization_amount) from order_authorization where order_id = %d and authorization_type='D' and authorization_success=1", $v),$v));
				}
				...
				if ($m_id != $_REQUEST["parent_id"]) {
					...
					$outer->addData($this->fetchSingleTest("select *, member_id as parent_id from qb_export where id = %d", $i_id));
					if ($status) $outer->addFormSuccess(sprintf("Changed invoice and orders to %s",$this->fetchScalarTest("select company from members where id = %d", $_REQUEST["parent_id"])));
				}
				else {
					foreach($_REQUEST["member_id"] as $k=>$v) {
						if ($v != $m_id) {
							...
							//
							$rec = $this->fetchSingleTest("select * from qb_export_dtl where id = %d",$k);
							$status &= $this->execute(sprintf("update orders set member_id=%d, custom_qb_order=0, authorization_amount = %f where id = %d", $v, 
								$this->fetchScalarTest("select sum(authorization_amount) from order_authorization where order_id = %d and authorization_type='D' and authorization_success=1", $rec["order_id"])), $rec["order_id"]);
					...
					if (array_key_exists("remove",$_REQUEST)) {
						foreach($_REQUEST["remove"] as $k=>$v) {
							if ($v == 1) {
								...
								$status &= $this->execute(sprintf("update orders set custom_qb_order = 0, authorization_amount = %f where id = %d", $this->fetchScalarTest("select sum(authorization_amount) from order_authorization where order_id = %d and authorization_type='D' and authorization_success=1", $rec["order_id"])),$rec["order_id"]);
    ...
    function emailInvoice($db_id, $consolidated = 0) {
		$inv = $this->fetchSingleTest("select * from qb_export where id = %d", $db_id);
		if ($consolidated) {
			$member = $this->fetchSingleTest("select * from members where id = %d", $inv["consolidate_id"]);
			$url = sprintf("https://%s/consolidated-invoice?d_id=%s&r_id=%d", HOSTNAME, $inv["invoice_date"], $inv["rand_id"] );
		}
		else {
			$member = $this->fetchSingleTest("select * from members where id = %d", $inv["member_id"]);
		...
		$inv_rec = $this->fetchSingleTest("select *, invoice_amount+tax_amount as invoice_total  from qb_export where id = %d",$db_id);
		$inv_rec["user"] = $member;
    ...
    function consolidatedInvoices() {
		...
		if (array_key_exists(__FUNCTION__,$_REQUEST)) {
			...
			if ($outer->validate()) {
				...
				$recs = $this->fetchAllTest("select min(i.id) as id, i.rand_id, invoice_date, i.qb_invoice_id, m.company, consolidate_id, m.custom_qb_id from qb_export i, members m where i.invoice_date = '%s' and i.consolidate_id < 0 and m.id = -i.consolidate_id group by i.consolidate_id order by min(i.id)", $dt);
				$rows = array();
				foreach($recs as $k=>$v) {
					$o = $this->fetchAllTest("select sum(o.net) as value, custom_qb_order, m.company, t.custom_qb_id as qb_tax_id, t.name, sum(o.taxes) as taxes
