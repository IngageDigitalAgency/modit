...
*/
	...
	function QB () {
		...
		if ((!$this->checkArray("administrator:qb:accessToken",$_SESSION)) || date(DATE_ATOM) > date(DATE_ATOM,strtotime($_SESSION["administrator"]["qb"]["expiry"]))) {
			...
			$_SESSION["administrator"]["qb"] = array("state"=>sprintf("security_token=%s",rand(10000,100000)));
...
*/
					...
					$recs = $this->fetchAll($sql);
					$hdr = array("created"=>date(DATE_ATOM),"member_id"=>$company["id"],"order_count"=>count($recs),"invoice_date"=>$form->getData("cutoff_date"),"rand_id"=>rand(1,100000));
