...
class templates extends Backend {
    ...
    function showTemplate($id = null, $fromMain = false) {
		if ($id == null || $id == 0) {
			if (array_key_exists('t_id',$_REQUEST) && $_REQUEST['t_id'] > 0) {
				...
				$id = $this->fetchScalarTest('select template_id from templates where deleted = 0 order by id desc limit 1');
		}
		$form = new Forms();
		...
		//if ($rec = $this->fetchSingleTest('select * from templates t where deleted = 0 and t.template_id = %d and t.version = (select max(version from templates where template_id = t.template_id)',$id)))
		if ($rec = $this->fetchSingleTest('select * from templates t where t.id = %d',$id)) {
			$form->addData($rec);
    ...
    function showPageProperties($fromMain = false) {
		$result = array();
		$return = 'true';
		if (!(array_key_exists('id',$_REQUEST) && $data = $this->fetchSingleTest('select * from %s where deleted = 0 and id = %d',$this->m_content,$_REQUEST['id'])))
    ...
    function getTemplateInfo($fromMain = false) {
		if (array_key_exists('t_id',$_REQUEST)) {
			if ($data = $this->fetchSingleTest('select * from %s where deleted = 0 and id = %d', $this->m_content, $_REQUEST['t_id'])) {
				$form = new Forms();
				...
				$pages = $this->fetchAllTest('select * from content where id in (select content_id from pages where deleted = 0 and template_id = %d)',$data['template_id']);
				$subf = new Forms();
    ...
    function getFolderInfo($fromMain = false) {
		...
		if (array_key_exists('t_id',$_REQUEST) && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$_REQUEST['t_id'])) {
			$data['notes'] = nl2br($data['notes']);
			$form->addData($data);
		}
		if ($this->isAjax())
    ...
    function showPageContent($fromMain = false) {
		...
		$form = new Forms();
		if ($t_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d', $this->m_tree, $t_id)) {
			...
			$sql = sprintf('select count(distinct(n.template_id)) from %s n where n.template_id in (select f.template_id from %s f where f.folder_id = %d)', $this->m_content, $this->m_junction, $_REQUEST['t_id']);			
			$count = $this->fetchScalarTest($sql);
			//$count = $this->fetchScalarTest('select count(distinct(n.template_id)) from %s n where n.template_id in (select f.template_id from %s f where f.folder_id = %d)', $this->m_content, $this->m_junction, $_REQUEST['t_id']);
			...
			foreach($templates as $article) {
				//$versions = $this->fetchAllTest('select id,version from %s where template_id = %d order by version desc',$this->m_content,$article['template_id']);
    ...
    function addContent($fromMain = false) {
		...
		if (!(array_key_exists('t_id',$_REQUEST) && $_REQUEST['t_id'] > 0 && $data = $this->fetchSingleTest('select * from %s where deleted = 0 and id = %d', $this->m_content, $_REQUEST['t_id']))) {
			$data = array('id'=>0,'published'=>false,'template_id'=>$this->fetchScalarTest('select max(template_id) from templates')+1);
		}
		$data['destFolders'] = $this->fetchScalarTest('select folder_id from templates_by_folder where template_id = %d',$data['template_id']);
		$form->addData($data);
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($form->validate()) {
				...
				$adding = true;
				if ($data['id'] == 0)
					$data['template_id'] = $this->fetchScalarTest('select max(template_id) from templates')+1;
				$flds['created'] = date('c');
				$flds['version'] = $this->fetchScalarTest('select max(version) from templates where template_id = %d',$data['template_id'])+1;
    ...
    function templateList() {
		...
		$recs = $this->fetchAllTest('select t.* from templates t where t.deleted = 0 and t.id = (select max(id) from templates t1 where t1.template_id=t.template_id)');
		if (array_key_exists('t_id',$_REQUEST) && $_REQUEST['t_id'] > 0) {
    ...
    function copyTemplate() {
		if (array_key_exists('t_id',$_REQUEST) && ($data = $this->fetchSingleTest('select * from templates where id = %d',$_REQUEST['t_id']))) {
			$form = new Forms();
			...
			if (count($_POST) > 0 && array_key_exists('copyTemplate',$_POST)) {
				...
				if ($form->validate()) {
					$this->beginTransaction();
					$maxId = $this->fetchScalarTest('select max(template_id) from templates') + 1;
					$newRec = array();
					...
					if ($status = ($stmt->execute())) {
						$newId = $stmt->insertId();
						$modules = $this->fetchAllTest('select * from modules_by_page where page_type = "T" and page_id = %d',$data['id']);
						foreach($modules as $module) {
							$module['page_id'] = $newId;
							unset($module['id']);
							$stmt = $this->prepare(sprintf('insert into modules_by_page(%s) values(?%s)',implode(',',array_keys($module)),str_repeat(',?',count($module)-1)));
						...
						//$folder = $this->fetchSingleTest('select * from templates_by_folder where template_id = %d',$data['template_id']);
    ...
    function deleteTemplate() {
		$status = 'false';
		if (array_key_exists('t_id',$_REQUEST) && ($data = $this->fetchSingleTest('select * from templates where id = %d',$_REQUEST['t_id']))) {
			//$template = $this->fetchScalarTest('select template_id from templates where id = %d',$_REQUEST['t_id']);
			$ct = $this->fetchScalarAllTest('select title from content where id in (select content_id from pages where deleted = 0 and template_id = %d)',$_REQUEST['t_id']);
