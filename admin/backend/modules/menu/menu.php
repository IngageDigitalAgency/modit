			...
			switch($data['type']) {
				...
				case 'folder':
					...
					foreach($menu as $item) {
						...
						if ($item['type'] == 'page') {
							$options = $this->fetchAllTest(sprintf('select id, concat(version,", ",date_format(created,"%%c-%%b-%%Y %%T")) as versionDate from pages where content_id = %d order by id desc',$item['id']));
				...
				if (count($srch) > 0) {
					...
					foreach($recs as $article) {
						...
						case 'page':
							...
							$options = $this->fetchAllTest(sprintf('select id, concat(version,", ",date_format(created,"%%c-%%b-%%Y %%T")) as versionDate from pages where content_id = %d order by id desc',$article['id']));
							foreach($options as $key=>$opt) {
