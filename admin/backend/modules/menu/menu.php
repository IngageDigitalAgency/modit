...
class menu extends Backend {
    ...
    function showPageProperties($fromMain = false) {
		...
		if (!(array_key_exists('id',$_REQUEST) && $data = $this->fetchSingleTest('select * from content where id = %d',$_REQUEST['id'])))
			$data = array('enabled'=>0,'id'=>0,'type'=>$type,'published'=>0,'image'=>'','rollover_image'=>'');
		else {
			$type = $data['type'];
			$data['p_id'] = $this->fetchScalarTest('select id from content where level = %d and left_id < %d and right_id > %d limit 1', $data['level']-1, $data['left_id'],$data['right_id']);
		}
		//
		...
		switch(strtolower($type)) {
			...
			default:
				...
				$data['imagesel_a'] = $data['image'];
				$data['imagesel_b'] = $data['rollover_image'];
				$data['template_id'] = $this->fetchScalarTest('select template_id from pages where deleted = 0 and content_id = %d order by id desc limit 1',$data['id']);
		...
		if (count($_POST) > 0 && array_key_exists('showPageProperties',$_POST)) {
			...
			if ($valid && array_key_exists('seo_url',$_POST) && strlen($_POST['seo_url']) > 0) {
				...
				$other = $this->fetchScalarAllTest($sql);
				if (count($other) > 0) {
			...
			if ($valid) {
				...
				if ($data['id'] == 0) {
					...
					if ($data['type'] == 'page') {
						...
						//
						$modules = $this->fetchAllTest('select * from modules_by_page where page_type = "T" and page_id = (select max(id) from templates t where t.template_id = %d)',$form->getData('template_id'));
    ...
    function getPageInfo($fromMain = false) {
		...
		if (array_key_exists('c_id',$_REQUEST)) {
			if ($data = $this->fetchSingleTest('select * from content where id = %d',$_REQUEST['c_id'])) {
				...
				switch (strtolower($data['type'])) {
					...
					case 'page':
					default:						
						$altData = $this->fetchSingleTest('select p1.*, t.title as templatename, c.notes from pages p1, templates t, content c where c.id = p1.content_id and t.template_id = p1.template_id and p1.content_id = %d and p1.version = (select max(p2.version) from pages p2 where p2.content_id = p1.content_id)',$data['id']);
    ...
    function showPageContent($fromMain = false) {
		...
		$form = new Forms();
		if ($c_id > 0 && $data = $this->fetchSingleTest('select * from content where id = %d',$c_id)) {
			$altData = array();
			switch($data['type']) {
				case 'internallink':
					...
					$link = $this->fetchSingleTest('select * from content where id = %d',$data['internal_link']);
				...
				case 'folder':
					...
					$menu = $this->fetchAllTest('select c.*, l.value as prettyName from content c left join code_lookups l on l.code = c.type and l.type="contentTypes" where c.level = %d and c.left_id > %d and c.right_id < %d order by c.left_id',$data['level']+1, $data['left_id'], $data['right_id']);
					$result = array();
					foreach($menu as $item) {
						...
						if ($item['type'] == 'page') {							
							$options = $this->fetchAllTest('select id, concat(version,", ",date_format(created,"%%c-%%b-%%Y %%T")) as versionDate from pages where content_id = %d order by id desc',$item['id']);
							foreach($options as $key=>$opt) {
								$row->getField('versions')->addOption($opt['id'],$opt['versionDate']);
							}
							$row->addTag('editlayout',sprintf('<a onclick="loadContent(%d)" href="#"><em class="icon-edit"></em>&nbsp;Layout</a>',$item['id']),false);
						}
						else {
				...
				default:
					if ($p_id == 0) {
						$p_id = $this->fetchScalarTest('select id from pages where content_id = %d and deleted = 0 order by version desc limit 1',$c_id);
						$_REQUEST['p_id'] = $p_id;
					}
					...
					$altData = $this->fetchSingleTest($sql);
    ...
    function deleteContent($fromMain = false) {
		...
		if (array_key_exists('p_id',$_REQUEST)) {
			$p_id = $_REQUEST['p_id'];
			$rec = $this->fetchSingleTest('select * from content where id = %d',$p_id);
			$status = true;
			switch($rec['type']) {
				case 'page':
					$ct = $this->fetchScalarTest('select count(0) from content where internal_link = %d',$p_id);
					if ($ct > 0) $this->addError('Links still exist to this page');
					$status = $ct == 0;
					break;
				case 'folder':
					$ct = $this->fetchScalarTest('select count(0) from content where left_id > %d and right_id < %d and level > %d',$rec['left_id'],$rec['right_id'],$rec['level']);
				...
				if (count($srch) > 0) {
					...
					foreach($recs as $article) {
						...
						case 'page':
							...
							$options = $this->fetchAllTest('select id, concat(version,", ",date_format(created,"%%c-%%b-%%Y %%T")) as versionDate from pages where content_id = %d order by id desc',$article['id']);
							foreach($options as $key=>$opt) {
								$frm->getField('versions')->addOption($opt['id'],$opt['versionDate']);
							}
							$frm->addTag('editlayout',sprintf('<a href="#" onclick="loadContent(%d,%d)"><em class="icon-edit"></em>&nbsp;Layout</a>',$article['id'],$options[0]['id']),false);
							break;
						case 'folder':
