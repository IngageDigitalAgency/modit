...
class setup extends Backend {
    ...
    function showSearchForm($fromMain = false) {
		...
		$count = $this->fetchScalarTest('select count(n.id) from %s n', $this->m_content);
		...
		foreach($recs as $article) {
			...
			$article['ent_enabled'] = $this->fetchScalarTest('select count(0) from cross_functionality where enabled = 1 and (related_from = %d or related_to = %d)',$article['id'],$article['id']);
			$article['ent_disabled'] = $this->fetchScalarTest('select count(0) from cross_functionality where enabled = 0 and (related_from = %d or related_to = %d)',$article['id'],$article['id']);
    ...
    function myLoadRelations($passed_id = null) {
		...
		$recs = $this->fetchAllTest('select cf.*, m1.title as rFrom, m2.title as rTo from cross_functionality cf, modules m1, modules m2 where (cf.related_from = %d or cf.related_to = %d) and m1.id = cf.related_from and m2.id = cf.related_to',$r_id,$r_id);
		$relations = new Forms();
    ...
    function editRelation() {
		...
		$r = $this->fetchSingleTest('select cf.id, m1.title as related_from, m2.title as related_to, cf.enabled from %s cf, modules m1, modules m2 where cf.id = %d and m1.id = cf.related_from and m2.id = cf.related_to',$this->m_relations,$r_id);
		$r['p_id'] = $p_id;
		$form->addData($r);
