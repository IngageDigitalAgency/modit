		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($valid) {
				if (array_key_exists('username',$_POST) && $_POST['username'] != '') {
					...
					//
					$ct = $this->fetchScalar(sprintf('select count(0) from %s where deleted = 0 and username = "%s" and id != %d',$this->m_content, $_POST['username'], $data['id']));
					if ($ct > 0) {
			...
			//
/*
			$ct = $this->fetchScalar(sprintf("select count(0) from members where email = '%s' and deleted = 0 and id != %d", $form->getData("email"), $form->getData("id")));
			if ($ct > 0) {
				$valid = false;
...
*/
    ...
    function loadAddresses($passed_id = null, $byAjax = true) {
		...
		$count = $this->fetchScalar(sprintf('select count(a.id) from addresses a, code_lookups c where  %s',implode(" and ",$srch)));
		if (array_key_exists('pagenum',$_REQUEST)) 
			$pageNum = $_REQUEST['pagenum'];
		else
			$pageNum = 1;	// no 0 based calcs
		...
		$addresses = $this->fetchAll(sprintf('select a.*, c.value as addressType from addresses a, code_lookups c where  %s order by %s limit %d, %d',implode(" and ",$srch), $sortBy, $start, $perPage));
		$addressList = array();
    ...
    function editProduct() {
		...
		if ($g == 1) {
			if (!($rec = $this->fetchSingle(sprintf("select cp.*,p.name from custom_member_product_options cp, product p where cp.id = %d and cp.member_id = %d and p.id = cp.product_id and cp.isgroup = %d",$p_id,$m,$g)))) {
		...
		if ($rec["product_id"] > 0) {
			$p = $this->fetchSingle(sprintf("select * from product where id = %d",$rec["product_id"]));
				//
				//	get group level overrides
				//
			if ($grp = $this->fetchSingle(sprintf("select cp.* from custom_member_product_options cp, members_by_folder mf where mf.id = %d and cp.member_id = mf.folder_id and cp.product_id = %d",$m,$p["id"]))) {
    ...
    function editFedEx() {
		...
		if ($g == 1) {
			...
			if (!($rec = $this->fetchSingle(sprintf("select cp.*,p.name from custom_member_product_options cp, product p, product_by_folder pf where cp.id = %d and cp.member_id = %d and p.id = cp.product_id and cp.isgroup = %d and p.id = pf.product_id and pf.folder_id = %d",$p_id,$m,$g, FEDEX_PRODUCTS)))) {
				$rec = array("id"=>0,"member_id"=>$m,"isgroup"=>$g,"product_id"=>$p_id,"zone_surcharge"=>0,"downtown_surcharge"=>0,"minimum_charge"=>0,"km_mincharge"=>0,"km_maxcharge"=>0,"km_charge"=>0);
...
/*
		if ($rec["product_id"] > 0) {
			...
			if ($g == 0) {
				...
				//	get group level overrides
				//
				$grp = $this->fetchSingle(sprintf("select cp.* from custom_member_product_options cp, members_by_folder mf where mf.id = %d and cp.member_id = mf.folder_id and cp.product_id = %d",$m,$p["id"]));
...
*/
    ...
    function getProductOverride() {
		...
		if ($p = $this->fetchSingle(sprintf("select * from product where id = %d",$_REQUEST["p_id"]))) {
			$override = 0;
			if ($_REQUEST["isgroup"] == 0) {
				//
				//	Get the group override first
				//
				$grp = $this->fetchSingle(sprintf("select cp.* from custom_member_product_options cp, members_by_folder mf where mf.id = %d and cp.member_id = mf.folder_id and cp.product_id = %d",$_REQUEST["m_id"],$_REQUEST["p_id"]));
    ...
    function getLogins($id) {
		...
		$p = $this->fetchAll(sprintf("select * from members_login where member_id = %d",$id));
		$recs = array();
    ...
    function showPayments() {
		...
		$count = $this->fetchScalar(sprintf("select count(0) from order_payment where member_id = %d", $m_id));
		$pagination = $this->pagination($count, $perPage, $pageNum, 
				array('prev'=>$this->M_DIR.'forms/paginationPrev.html','next'=>$this->M_DIR.'forms/paginationNext.html',
						'pages'=>$this->M_DIR.'forms/paginationPage.html', 'wrapper'=>$this->M_DIR.'forms/paginationWrapper.html',
    ...
    function paymentDetails() {
		...
		$inner->init($this->getTemplate(__FUNCTION__."Row"));
		$iflds = $inner->buildForm($this->getFields(__FUNCTION__."Row"));
		$outer->addData($this->fetchSingle(sprintf("select p.* from order_payment p where id = %d and member_id = %d", $p_id, $m_id)));
		$recs = $this->fetchAll(sprintf("select d.* from order_payment_detail d, order_payment p where p.member_id = %d and p.id = %d and d.payment_id = p.id order by order_id", $m_id, $p_id));
		$data = array();
		foreach($recs as $key=>$rec) {
			$inner->addData($rec);
			$data[] = $inner->show();
    ...
    function removeProduct() {
		...
		$outer->init($this->getTemplate(__FUNCTION__));
		$status = false;
		if ($rec = $this->fetchSingle(sprintf("select * from custom_member_product_options where id = %d and member_id = %d and isgroup = %d", $p_id, $m_id, $g_id ))) {
    ...
    function listPerPiece($id = null, $fromMain = 0) {
		...
		$recs = $this->fetchAll(sprintf("select c.*, p.name, p.code from member_package_charges c, product p where c.member_id = %d and p.id = c.product_id order by p.name", $id));
		$rows = array();
		foreach($recs as $k=>$v) {
			$outer->addData($v);
			$rows[] = $outer->show();
    ...
    function editPerPiece() {
		...
		$p_id = array_key_exists("p_id",$_REQUEST) ? $_REQUEST["p_id"] : 0;
		$m_id = array_key_exists("m_id",$_REQUEST) ? $_REQUEST["m_id"] : 0;
		if (!$data = $this->fetchSingle(sprintf("select * from member_package_charges where id = %d and member_id = %d", $p_id, $m_id)))
    ...
    function kmRates($p_id = 0) {
		...
		$recs = $this->fetchAll(sprintf("select * from custom_product_km_ranges where member_product_option_id = %d order by km_max", $p_id));
		$rows = array();
		foreach($recs as $k=>$v) {
			$inner->addData($v);
			$rows[] = $inner->show();
    ...
    function kmRatesEdit() {
		...
		if ($rec = $this->fetchSingle(sprintf("select * from custom_product_km_ranges where id = %d",$km_id))) {
			$outer->addData($rec);
		}
    ...
    function exportAddresses() {
		...
		$addresses = $this->fetchAll(sprintf("select a.*, p.province_code, c.code as type from addresses a, provinces p, code_lookups c where a.ownertype='member' and a.ownerid = %d and p.id = a.province_id and c.id = a.addresstype order by c.code, a.company", $o_id));
		if (count($addresses) ==  0) {
    ...
    function buildContacts($id = null ) {
		...
		$recs = array();
		$data = $this->fetchAll(sprintf("select c.id as p_key, c.member_id, c.notes, a.* from contacts c, addresses a where c.deleted = 0 and c.member_id = %d and a.ownerid = c.id and ownertype='contact' order by id", $id));
		foreach($data as $k=>$v) {
			$inner->addData($v);
			$recs[] = $inner->show();
		}
    ...
    function editContact() {
		...
		if ($data = $this->fetchSingle(sprintf("select c.id as c_id, c.member_id, c.notes, a.* from contacts c, addresses a where c.id = %d and a.ownerid = c.id and ownertype='contact' order by id", $id)))
			$outer->addData($data);
		if ($id < 0) {
			$outer->setData("member_id", -$id);
			$id = -$id;
    ...
    function productByPC($p_id = 0) {
		...
		$inner->init($this->getTemplate(__FUNCTION__."Row"));
		$flds = $inner->buildForm($this->getFields(__FUNCTION__."Row"));
		$recs = $this->fetchAll(sprintf("select * from custom_product_by_pc where member_product_option_id = %d order by from_postal_code", $p_id));
		$rows = array();
		foreach($recs as $k=>$v) {
			$inner->addData($v);
			$rows[] = $inner->show();
    ...
    function productByPCEdit() {
		...
		if ($rec = $this->fetchSingle(sprintf("select * from custom_product_by_pc where id = %d",$pc_id))) {
			$outer->addData($rec);
		}
		if (array_key_exists(__FUNCTION__,$_REQUEST) && $_REQUEST[__FUNCTION__]==1) {
			$outer->addData($_REQUEST);
    ...
    function regroup() {
		...
		$flds = $outer->buildForm($this->getFields(__FUNCTION__));
		$m_id = array_key_exists('m_id',$_REQUEST) ? $_REQUEST["m_id"] : 0;
		if ($rec = $this->fetchSingle(sprintf("select * from members where id = %d",$m_id))) {
			$outer->addData($rec);
			$outer->setData("m_id", $m_id);
		}
		if (array_key_exists(__FUNCTION__,$_REQUEST) && $_REQUEST[__FUNCTION__]==1) {
    ...
    function kmRateOverride($p_id = 0, $j_id = 0) {
		...
		$recs = array();
		if ($p_id == 0 && array_key_exists("opt_id",$_REQUEST)) {
			$recs = $this->fetchAll(sprintf('select kmo.*, km.km_max, km.price, km.out_of_zone, km.id as opt_id
			...
			if ($grp_prod = $this->fetchSingle(sprintf("select grp.* from custom_member_product_options cpo, members_by_folder mbf, custom_member_product_options grp where cpo.id = %d and mbf.id = cpo.member_id and grp.product_id = cpo.product_id and grp.isgroup = 1 and grp.member_id = mbf.folder_id", $p_id)))
				$recs = $this->fetchAll(sprintf("select kmo.*, km.km_max, km.price, km.out_of_zone, km.id as opt_id, %d as j_id from custom_product_km_ranges km left join custom_product_km_range_overrides kmo on kmo.km_opt_id = km.id and kmo.junction_id = %d where km.member_product_option_id = %d order by km_max", $j_id, $j_id, $grp_prod["id"]));
		}
		$rows = array();
		foreach($recs as $k=>$v) {
    ...
    function editKMOverride() {
		...
		if ($rec = $this->fetchSingle(sprintf("select * from custom_product_km_range_overrides where id = %d and junction_id = %d",$km_id, $j_id))) {
			$outer->addData($rec);
		}
		$opt = $this->fetchSingle(sprintf("select * from custom_product_km_ranges where id = %d", $opt_id));
    ...
    function deleteKMOverride() {
		...
		$html = "<div>Oops</div>";
		if ($rec = $this->fetchSingle(sprintf("select * from custom_product_km_range_overrides where id = %d", $id))) {
			if ($grp = $this->fetchSingle(sprintf("select cpo.* from custom_member_product_options cpo, custom_product_km_ranges km, custom_product_km_range_overrides kmo where kmo.id = %d and km.id = kmo.km_opt_id and cpo.id = km.member_product_option_id", $id))) {
				$mbr = $this->fetchSingle(sprintf("select * from custom_member_product_options cpo where member_id = %d and product_id = %d and isgroup = 0", $rec["junction_id"], $grp["product_id"]));
    ...
    function initKMOverrides() {
		...
		$j = $this->fetchSingle(sprintf("select * from members_by_folder where id = %d", $j_id));
			...
			if ($outer->validate()) {
				...
				foreach($_POST["process"] as $k=>$v) {
					...
					if (abs($outer->getData("in_zone")[$k]) != 0 || abs($outer->getData("out_of_zone")[$k]) != 0) {
						$p_ins["product_id"] = $this->fetchScalar(sprintf("select product_id from custom_member_product_options where id = %d", $k));
						//if (!$this->fetchSingle(sprintf("select * from custom_member_product_options where member_id = %d and isgroup = 0 and product_id = %d", $j_id, $p_ins["product_id"]))) {
						//	$prd->bindParams(array_merge(array("sss"), array_values($p_ins)));
						//	$valid &= $prd->execute();
						//}
						$outer->addFormSuccess(sprintf("Processed %s", $this->fetchScalar(sprintf("select name from product p, custom_member_product_options cpo where cpo.id = %d and p.id = cpo.product_id", $k))));
						$km = $this->fetchAll(sprintf("select km.* from custom_product_km_ranges km where member_product_option_id = %d", $k));
		...
		$recs = $this->fetchAll(sprintf('select p.name, cpo.* from product p, custom_member_product_options cpo where cpo.member_id = %d and isgroup = 1 and p.id = cpo.product_id and is_fedex = 0 and cpo.product_id not in (
    ...
    function groupKMOverrides($j_id = 0) {
		...
		if (($p_id = $outer->getData("opt_id")) > 0) {
			$recs = $this->fetchAll(sprintf("select ovr.*, kr.km_max, kr.price, kr.out_of_zone from custom_product_km_ranges kr left join custom_product_km_range_overrides ovr on ovr.km_opt_id = kr.id where kr.member_product_option_id = %d and ovr.junction_id = %d order by km_max", $p_id, $j_id));
			$rows = array();
			foreach($recs as $k=>$v) {
    ...
    function groupKMDeleteOverride() {
		...
		if ($rec = $this->fetchSingle(sprintf("select * from custom_product_km_range_overrides where id = %d", $id))) {
			if ($grp = $this->fetchSingle(sprintf("select cpo.* from custom_member_product_options cpo, custom_product_km_ranges km, custom_product_km_range_overrides kmo where kmo.id = %d and km.id = kmo.km_opt_id and cpo.id = km.member_product_option_id", $id))) {
				//$mbr = $this->fetchSingle(sprintf("select * from custom_member_product_options cpo where member_id = %d and product_id = %d and isgroup = 1", $rec["junction_id"], $grp["product_id"]));
				$this->execute(sprintf("delete from custom_product_km_range_overrides where id = %d", $id));
