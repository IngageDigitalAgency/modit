...
class members extends Backend {
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$p_id)) {
			...
			$sql = sprintf('select count(n.id) from %s n where n.id in (select f.member_id from %s f where f.folder_id = %d)', $this->m_content, $this->m_junction, $_REQUEST['p_id']);
...
*/
			if ($valid) {
				...
				if ($stmt->execute()) {
					...
					$new = $this->fetchScalarAllTest('select id from %s where id in (%s) and id not in (select folder_id from %s where member_id = %d and folder_id in (%s))',
    ...
    function loadAddresses($passed_id = null, $byAjax = true) {
		...
		$addresses = $this->fetchAllTest('select a.*, c.value as addressType from addresses a, code_lookups c where  %s order by %s limit %d, %d',implode(" and ",$srch), $sortBy, $start, $perPage);
		$addressList = array();
    ...
    function showOrders() {
		...
		$start = ($pageNum-1)*$perPage;
		$orders = $this->fetchAllTest("%s and %s order by id desc limit %d,%d",$where,implode(" and ",$and,$start,$perPage));
		$form->addTag("pagination",$pagination,false);
		$return = array();
		$inner = new Forms();
