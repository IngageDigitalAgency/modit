...
/**
 * Blog feature in the backend section
 */
class blog extends Backend {
    ...
    function showPageProperties($fromMain = false) {
		...
		//
		$frmFlds['destEvents']['options'] = $this->fetchOptionsTest('select id as code,name as value from events where id in (select event_id from event_dates where event_date >= curdate()) and deleted = 0 and enabled = 1 and published = 1 order by start_date');
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from %s n where n.id in (select f.blog_id from %s f where f.folder_id = %d)', $this->m_content, $this->m_junction, $_REQUEST['p_id']);
			...
			foreach($blog as $blog) {
				...
				$frm->addData($blog);
				$frm->addTag('accepted',$this->fetchScalarTest('select count(0) from %s where blog_id = %d and approved = 1', $this->m_comments, $blog['id']));
				$frm->addTag('unaccepted',$this->fetchScalarTest('select count(0) from %s where blog_id = %d and approved = 0', $this->m_comments, $blog['id']));
				$blogs[] = $frm->show();
			}
    ...
    function addContent($fromMain = false) {
		...
		if (count($_REQUEST) > 0 && array_key_exists('destFolders',$_REQUEST) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest('select folder_id from %s where blog_id = %d', $this->m_junction, $data['id']);
		...
		if ((count($_REQUEST) > 0 && array_key_exists('destSearch',$_REQUEST)) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest('select folder_id from blog_by_search_group where blog_id = %d', $data['id']);
		...
		$frmFields['relatedDestProducts']['options'] = $this->fetchOptionsTest('select id as code,code as value from product where deleted = 0 and enabled = 1 and published = 1 and id in (%s)order by name',implode(',',array_merge(array(0),$data['relatedDestProducts'])));
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($valid) {
				...
				if ($stmt->execute()) {
					...
					$new = $this->fetchScalarAllTest('select id from %s where id in (%s) and id not in (select folder_id from %s where blog_id = %d and folder_id in (%s))',
					...
					$new = $this->fetchScalarAllTest('select id from search_groups where id in (%s) and id not in (select folder_id from blog_by_search_group where blog_id = %d and folder_id in (%s))',
    ...
    function moveArticle() {
		...
		if ($_REQUEST['type'] == 'tree') {
			...
			$curr = $this->fetchScalarTest('select blog_id from %s where id = %d',$this->m_junction,$src);
			if ($curr > 0 && $folder = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$dest)) {
				...
				if (array_key_exists('move',$_REQUEST)) {
					...
					if ($status = $this->execute(sprintf('delete from %s where id = %d', $this->m_junction, $src))) {
						if (!$this->fetchSingleTest('select * from %s where blog_id = %d and folder_id = %d',$this->m_junction,$curr,$dest)) {
							$obj = new preparedStatement(sprintf('insert into %s(blog_id,folder_id) values(?,?)',$this->m_junction));
			...
			if ($form->validate()) {
				...
				switch($type) {
					case 'cancel':
						...
						break;
					case 'all':
						//$img = $this->fetchScalarTest('select blog_id from %s where id = %d',$this->m_junction,$_REQUEST['j_id']);
					...
					case 'one':
						...
						$ct = $this->fetchScalarTest('select count(0) from %s where blog_id = %d',$this->m_junction,$_REQUEST['b_id']);
						if ($ct == 0)
							$this->execute(sprintf('update %s set deleted = 1 where id = %d',$this->m_content,$_REQUEST['b_id']));
						break;
					default:
    ...
    function loadComments() {
		if (array_key_exists('b_id',$_REQUEST) && $_REQUEST['b_id'] > 0) {
			if (!($blog = $this->fetchSingleTest('select * from %s where id = %d',$this->m_content,$_REQUEST['b_id'])))
				return $this->ajaxReturn(array('status'=>'false'));
		}
		else 
			return $this->ajaxReturn(array('status'=>'false'));
		...
		if ($pageNum <= 0) $pageNum = 1;
		$perPage = $this->m_perrow;
		$count = $this->fetchScalarTest('select count(b.id) from %s b where b.blog_id = %d', $this->m_comments, $_REQUEST['b_id']);
		$pagination = $this->pagination($count, $perPage, $pageNum,
		...
		$comments = $this->fetchAllTest('select b.*, concat(m.lastname,", ",m.firstname," ",email) as author from %s b left join members m on m.id = b.author_id where blog_id = %d order by %s %s limit %d,%d', $this->m_comments, $blog['id'], $sortby, $sortorder, $start, $perPage);
		$output = array();
		$c = new Forms();
    ...
    function editComment($fromMain = false) {
		...
		if (!(array_key_exists('c_id',$_REQUEST) && $_REQUEST['c_id'] > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d', $this->m_comments, $_REQUEST['c_id']))) {
			return $this->ajaxReturn(array('status'=>false));
		}
    ...
    function getVideo($b_id = 0) {
		...
		if (!$data = $this->fetchSingleTest('select * from videos where owner_id = %d and owner_type = "blog"', $b_id))
			$data = array('id'=>0);
    ...
    function saveVideo($id) {
		if (strlen($_POST['video']['url']) == 0 && strlen($_POST['video']['embed_code']) == 0) {
			if ($_POST['video']['v_id'] > 0)
				$this->execute(sprintf('delete from video where owner_type="blog" and owner_id = %d'));
			return true;
		}
		$form = new Forms();
		...
		if ($data = $this->fetchSingleTest('select * from videos where owner_type="blog" and owner_id = %d', $form->getData('v_id'))) {
    ...
    function getNames() {
		$query = $_REQUEST['s'];
		$member = $this->fetchScalarTest("select author_id from blog_comments where id = %d",$_REQUEST['m']);
		$select = new select();
    ...
    function getAuthor() {
		$query = $_REQUEST['s'];
		$member = $this->fetchScalarTest("select author from blog where id = %d",$_REQUEST['m']);
		$select = new select();
