...
class blog extends Backend {
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from '.$this->m_content.' n where n.id in (select f.blog_id from '.$this->m_junction.' f where f.folder_id = %d)',  $_REQUEST['p_id']);
    ...
    function addContent($fromMain = false) {
		...
		if ((count($_REQUEST) > 0 && array_key_exists('destSearch',$_REQUEST)) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {				
				$tmp = $this->fetchScalarAllTest(sprintf('select folder_id from blog_by_search_group where blog_id = %d', $data['id']));
		...
		$frmFields['destCoupons']['options'] = $this->fetchOptionsTest(sprintf('select id as code,concat(code," - ",name) as value from coupons where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by code',implode(',',array_merge(array(0),$data['destCoupons']))));
		...
		$frmFields['destStores']['options'] = $this->fetchOptionsTest(sprintf('select id as code,name as value from stores where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by name',implode(',',array_merge(array(0),$data['destStores']))));
		...
		$frmFields['relatedDestProducts']['options'] = $this->fetchOptionsTest(sprintf('select id as code,code as value from product where deleted = 0 and enabled = 1 and published = 1 and id in (%s)order by name',implode(',',array_merge(array(0),$data['relatedDestProducts']))));
