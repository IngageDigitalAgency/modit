...
/**
 * Newsletter feature for backend
 */
class newsletter extends Backend {
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from %s n where n.id in (select f.newsletter_id from %s f where f.folder_id = %d)', $this->m_content, $this->m_junction, $_REQUEST['p_id']);
			...
			$newsletters = $this->fetchAllTest($sql);
			...
			foreach($newsletters as $article) {
				...
				$article['sends'] = $this->fetchScalarTest('select count(0) from newsletter_batch where newsletter_id = %d',$article['id']);
				$frm->addData($article);
				$articles[] = $frm->show();
			}
    ...
    function addContent($fromMain = false) {
		...
		if (count($_REQUEST) > 0 && array_key_exists('destFolders',$_REQUEST) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest('select folder_id from %s where newsletter_id = %d', $this->m_junction, $data['id']);
		...
		if (count($_REQUEST) > 0 && array_key_exists('destSubscribers',$_REQUEST) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest('select subscribers_id from %s where newsletter_id = %d', $this->m_subscribers, $data['id']);
				$ids = array_merge($ids,$tmp);
			}
			if (count($ids) > 0) {
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($form->validate()) {
				...
				if ($stmt->execute()) {
					...
					$new = $this->fetchScalarAllTest('select id from %s where id in (%s) and id not in (select folder_id from %s where newsletter_id = %d and folder_id in (%s))',
					...
					$new = $this->fetchScalarAllTest('select id from subscriber_folders where id in (%s) and id not in (select subscribers_id from %s where newsletter_id = %d and subscribers_id in (%s))',
    ...
    function moveArticle() {
		...
		if ($_REQUEST['type'] == 'tree') {
			...
			if ($folder = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$dest)) {
				$curr = $this->fetchScalarTest('select newsletter_id from %s where id = %d',$this->m_junction,$src);
				$status = true;
				if (array_key_exists('move',$_REQUEST)) {
					...
					if ($status = $this->execute(sprintf('delete from %s where id = %d', $this->m_junction, $src))) {
						if (!$this->fetchSingleTest('select * from %s where newsletter_id = %d and folder_id = %d',$this->m_junction,$curr,$dest)) {
							$obj = new preparedStatement(sprintf('insert into %s(newsletter_id,folder_id) values(?,?)',$this->m_junction));
    ...
    function deleteArticle() {
			...
			if ($form->validate()) {
				...
				switch($type) {
					case 'cancel':
						...
						break;
					case 'all':
						//$img = $this->fetchScalarTest('select newsletter_id from %s where id = %d',$this->m_junction,$_REQUEST['j_id']);
					...
					case 'one':
						...
						$ct = $this->fetchScalarTest('select count(0) from %s where newsletter_id = %d',$this->m_junction,$_REQUEST['a_id']);
						if ($ct == 0)
							$this->execute(sprintf('update %s set deleted = 1 where id = %d',$this->m_content,$_REQUEST['a_id']));
						break;
					default:
		...
		if (array_key_exists('j_id',$_REQUEST)) {
			$id = $_REQUEST['j_id'];
			$curr = $this->fetchScalarTest('select newsletter_id from %s where id = %d',$this->m_junction,$id);
			...
			if (($remining = $this->fetchScalarTest('select count(0) from %s where newsletter_id = %d',$this->m_junction,$curr)) == 0) {
    ...
    function getStats($p_id = null) {
		...
		$recs = $this->fetchAllTest('select * from newsletter_batch where newsletter_id = %d order by created desc',$id);
		$return = array();
		$form = new Forms();
		$form->init($this->getTemplate('stats'));
		$flds = $form->buildForm($this->getFields('stats'));
		$form->addTag('id',$id);
		$inner = new Forms();
		...
		foreach($recs as $send) {
			...
			$inner->reset();
			$total = (int)$this->fetchScalarTest('select count(batch_id) from newsletter_batch_subscriber where batch_id = %d group by batch_id',$send['id']);
			$inner->addTag('subscribers',sprintf('%d',$total));
			$sent = (int)$this->fetchScalarTest('select count(batch_id) from newsletter_batch_subscriber where batch_id = %d and sent != "0000-00-00 00:00:00" group by batch_id',$send['id']);
			$inner->addTag('remaining',sprintf('%d',$total - $sent));
			$reads = (int)$this->fetchScalarTest('select count(batch_id) from newsletter_batch_subscriber where batch_id = %d and views != 0 group by batch_id',$send['id']);
			$inner->addTag('opens',sprintf('%d',$reads));
			$inner->addData($send);
			$unsubs = (int)$this->fetchScalarTest('select count(batch_id) from newsletter_batch_subscriber where batch_id = %d and unsubscribe != 0 group by batch_id',$send['id']);
    ...
    function submit($testing = false) {
		...
		$status = false;
		if ($rec = $this->fetchSingleTest('select * from newsletter_batch where newsletter_id = %d and started = "0000-00-00 00:00:00" and aborted = "0000-00-00 00:00:00" and completed = "0000-00-00 00:00:00"',$id)) {
			$this->addError('An unstarted batch exists');
		}
		elseif ($rec = $this->fetchSingleTest('select * from newsletter_batch where newsletter_id = %d and started != "0000-00-00 00:00:00" and completed = "0000-00-00 00:00:00" and aborted = "0000-00-00 00:00:00"',$id)) {
    ...
    function start() {
		...
		$result = false;
		$html = 'Invalid id';
		if ($rec = $this->fetchSingleTest('select * from newsletter_batch where id = %d',$id)) {
