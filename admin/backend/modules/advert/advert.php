...
/**
 * Advertisement class for backend
 */
class advert extends Backend {
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from %s n where n.id in (select f.advert_id from %s f where f.folder_id = %d)', $this->m_content, $this->m_junction, $_REQUEST['p_id']);
    ...
    function addContent($fromMain = false) {
		...
		if (count($_REQUEST) > 0 && array_key_exists('destFolders',$_REQUEST) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest('select folder_id from %s where advert_id = %d', $this->m_junction, $data['id']);
		...
		if ((count($_REQUEST) > 0 && array_key_exists('destSearch',$_REQUEST)) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest('select folder_id from advert_by_search_group where event_id = %d', $data['id']);
		...
		$frmFields['relatedDestProducts']['options'] = $this->fetchOptionsTest('select id as code,name as value from product where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by name',implode(',',array_merge(array(0),$data['relatedDestProducts'])));
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($valid) {
				...
				if ($stmt->execute()) {
					...
					$new = $this->fetchScalarAllTest('select id from %s where id in (%s) and id not in (select folder_id from %s where advert_id = %d and folder_id in (%s))',
					...
					$new = $this->fetchScalarAllTest('select id from search_groups where id in (%s) and id not in (select folder_id from advert_by_search_group where event_id = %d and folder_id in (%s))',
    ...
    function moveArticle() {
		...
		if ($_REQUEST['type'] == 'tree') {
			...
			if ($folder = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$dest)) {
				$curr = $this->fetchScalarTest('select advert_id from %s where id = %d',$this->m_junction,$src);
				$status = true;
				if (array_key_exists('move',$_REQUEST)) {
					...
					if ($status = $this->execute(sprintf('delete from %s where id = %d', $this->m_junction, $src))) {
						if (!$this->fetchSingleTest('select * from %s where advert_id = %d and folder_id = %d',$this->m_junction,$curr,$dest)) {
							$obj = new preparedStatement(sprintf('insert into %s(advert_id,folder_id) values(?,?)',$this->m_junction));
    ...
    function deleteArticle() {
			...
			if ($form->validate()) {
				...
				switch($type) {
					case 'cancel':
						...
						break;
					case 'all':
						//$img = $this->fetchScalarTest('select advert_id from %s where id = %d',$this->m_junction,$_REQUEST['j_id']);
					...
					case 'one':
						...
						$ct = $this->fetchScalarTest('select count(0) from %s where advert_id = %d',$this->m_junction,$_REQUEST['a_id']);
						if ($ct == 0)
							$this->execute(sprintf('update %s set deleted = 1 where id = %d',$this->m_content,$_REQUEST['a_id']));
						break;
					default:
		...
		if (array_key_exists('j_id',$_REQUEST)) {
			$id = $_REQUEST['j_id'];
			$curr = $this->fetchScalarTest('select advert_id from %s where id = %d',$this->m_junction,$id);
			...
			if (($remining = $this->fetchScalarTest('select count(0) from %s where advert_id = %d',$this->m_junction,$curr)) == 0) {
    ...
    function getVideo($b_id = 0, $fromPost) {
		...
		if ($fromPost)
			...
			if (!$data = $this->fetchSingleTest('select * from videos where owner_id = %d and owner_type = "advert"', $b_id))
				$data = array('id'=>0);
    ...
    function saveVideo($id) {
		...
		if ($data = $this->fetchSingleTest('select * from videos where owner_type="advert" and owner_id = %d', $form->getData('v_id'))) {
