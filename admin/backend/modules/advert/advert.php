...
class advert extends Backend {
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from '.$this->m_content.' n where n.id in (select f.advert_id from '.$this->m_junction.' f where f.folder_id = %d)', $_REQUEST['p_id']);
