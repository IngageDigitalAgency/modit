...
class search extends Backend {
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from %s n, %s t where t.id = %d and n.folder_id = t.id', $this->m_content, $this->m_tree, $_REQUEST['p_id']);
		...
		if (count($_POST) > 0 && array_key_exists('showSearchForm',$_POST)) {
			...
			if ($form->validate()) {
				...
				if (count($srch) > 0) {
					...
					$perPage = $this->m_perrow;
					if (array_key_exists('pager',$_REQUEST)) $perPage = $_REQUEST['pager'];
					$count = $this->fetchScalarTest('select count(n.id) from %s n, %s t where 1=1 and %s and n.folder_id = t.id', $this->m_content, $this->m_tree, implode(' and ',$srch));
