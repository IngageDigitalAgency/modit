...
class coupons extends Backend {
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d', $this->m_tree, $p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from '.$this->m_content.' n where n.deleted = 0 and n.id in (select f.coupon_id from %s f where f.folder_id = %d)',  $this->m_junction, $_REQUEST['p_id']);
