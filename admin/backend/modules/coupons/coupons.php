...
class coupons extends Backend {
    ...
    function showPageProperties($fromMain = false) {
		...
		$frmFlds['destStores']['options'] = $this->fetchOptionsTest('select id as code, name as value from stores where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by code',implode(',',array_merge(array(0),$data['destStores'])));
		...
		$frmFlds['destEvents']['options'] = $this->fetchOptionsTest('select id as code, name as value from events where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by start_date',implode(',',array_merge(array(0),$data['destEvents'])));
		...
		$frmFlds['destAds']['options'] = $this->fetchOptionsTest('select id as code, title as value from advert where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by title',implode(',',array_merge(array(0),$data['destAds'])));
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d', $this->m_tree, $p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from %s n where n.deleted = 0 and n.id in (select f.coupon_id from %s f where f.folder_id = %d)', $this->m_content, $this->m_junction, $_REQUEST['p_id']);
    ...
    function addContent($fromMain = false) {
		...
		if (count($_REQUEST) > 0 && array_key_exists('destFolders',$_REQUEST) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest('select folder_id from %s where coupon_id = %d',$this->m_junction,$data['id']);
				$ids = array_merge($ids,$tmp);
			}
			if (count($ids) > 0) {
				$data['destFolders'] = $ids;
				//$ids = implode(',',$ids);
				//$source = $this->fetchAllTest('select * from %s where id in (%s order by left_id', $this->m_tree, $ids));
		...
		$frmFields['destStores']['options'] = $this->fetchOptionsTest('select id as code, name as value from stores where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by code',implode(',',array_merge(array(0),$data['destStores'])));
		...
		$frmFields['destEvents']['options'] = $this->fetchOptionsTest('select id as code, name as value from events where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by start_date',implode(',',array_merge(array(0),$data['destEvents'])));
		...
		$frmFields['destBlogs']['options'] = $this->fetchOptionsTest('select id as code, title as value from blog where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by title',implode(',',array_merge(array(0),$data['destBlogs'])));
		...
		$frmFields['destAds']['options'] = $this->fetchOptionsTest('select id as code, title as value from advert where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by title',implode(',',array_merge(array(0),$data['destAds'])));
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($id == 0) {
				...
				//
				$ct = $this->fetchScalarTest('select count(0) from coupons where code = "%s"',$this->escape_string($form->getData('code')));
				if ($ct > 0) {
    ...
    function moveCoupon() {
		...
		if ($_REQUEST['type'] == 'tree') {
			...
			$curr = $this->fetchScalarTest('select coupon_id from %s where id = %d',$this->m_junction,$src);
			if ($curr > 0 && $folder = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$dest)) {
				...
				if (array_key_exists('move',$_REQUEST)) {
					...
					if ($status = $this->execute(sprintf('delete from %s where id = %d', $this->m_junction, $src))) {
						if (!$this->fetchSingleTest('select * from %s where coupon_id = %d and folder_id = %d',$this->m_junction,$curr,$dest)) {
							$obj = new preparedStatement(sprintf('insert into %s(coupon_id,folder_id) values(?,?)',$this->m_junction));
    ...
    function resequence($folder) {
		...
		$coupons = $this->fetchAllTest('select * from %s where folder_id = %d order by sequence',$this->m_junction,$folder);
		$seq = 10;
		foreach($coupons as $coupon) {
			...
			if ($form->validate()) {
				...
				switch($type) {
					case 'cancel':
						...
						break;
					case 'all':
						//$img = $this->fetchScalarTest('select coupon_id from %s where id = %d',$this->m_junction,$_REQUEST['c_id']);
					...
					case 'one':
						...
						$ct = $this->fetchScalarTest('select count(0) from %s where coupon_id = %d',$this->m_junction,$_REQUEST['c_id']);
						if ($ct == 0)
							$this->execute(sprintf('update %s set deleted = 1 where id = %d',$this->m_content,$_REQUEST['c_id']));
						break;
					default:
