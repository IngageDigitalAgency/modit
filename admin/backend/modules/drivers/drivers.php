...
/**
 * Customize some TCPDF default configurations
 */
...
/**
 * Drivers details for backend section
 */
class drivers extends Backend {
    ...
    function showContentTree() {
		$d = $this->fetchAllTest("select d.*, m.company, m.firstname, m.lastname from drivers d, members m where m.id = d.member_id and d.deleted = 0 and d.enabled = 1 and m.enabled = 1 and m.deleted = 0 order by company, lastname, firstname");
		$outer = new Forms();
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from %s n where n.id in (select f.zone_id from %s f where f.folder_id = %d)', $this->m_content, $this->m_junction, $_REQUEST['p_id']);
		...
		if (count($_POST) > 0 && array_key_exists('showSearchForm',$_POST)) {
			...
			if ($form->validate()) {
				...
				if (count($srch) > 0) {
					...
					$form->setData('pager',$perPage);
					$count = $this->fetchScalarTest('select count(d.id) from %s d, members m left join addresses a on a.ownerid = m.id and a.ownertype="member" and a.deleted = 0 and a.addresstype = 392 and a.address_book = 1 where m.id = d.member_id and %s', $this->m_content, implode(' and ',$srch));
					...
					foreach($recs as $article) {
						$frm->reset();
						if ($address = $this->fetchSingleTest("select * from addresses where ownertype='member' and ownerid = %d",$article["id"]))
    ...
    function addContent($fromMain = false) {
		...
		if (!(array_key_exists('a_id',$_REQUEST) && $_REQUEST['a_id'] > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d', $this->m_content, $_REQUEST['a_id']))) {
		...
		if (count($_REQUEST) > 0 && array_key_exists('destFolders',$_REQUEST) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest('select folder_id from %s where zone_id = %d', $this->m_junction, $data['id']);
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($form->validate()) {
				...
				if ($stmt->execute()) {
					...
					$new = $this->fetchScalarAllTest('select id from %s where id in (%s) and id not in (select folder_id from %s where zone_id = %d and folder_id in (%s))',
    ...
    function editDriver() {
		...
		if (!$d = $this->fetchSingleTest("select d.*, m.company, m.firstname, m.lastname from drivers d, members m where d.id = %d and m.id = d.member_id",$d_id)) {
    ...
    function showSchedule() {
		...
		$driver = $this->fetchSingleTest("select d.*, m.company, m.firstname, m.lastname from drivers d, members m where d.id = %d and m.id = d.member_id",$_REQUEST["d_id"]);
		...
		foreach($recs as $key=>$value) {
			$inner->reset();
			$address = $this->fetchSingleTest("select * from addresses where ownerid = %d and ownertype='order' and addresstype = %d",$value["order_id"],$value["service_type"]=="D"?ADDRESS_DELIVERY:ADDRESS_PICKUP);
    ...
    function editPackage() {
		$p_id = array_key_exists("p_id",$_REQUEST) ? $_REQUEST["p_id"] : 0;
		$package = $this->fetchSingleTest("select * from custom_delivery where id = %d",$p_id);
		$package["address"] = $this->fetchSingleTest("select * from addresses where ownerid = %d and ownertype = 'order'",$package["order_id"],$package["service_type"] == "P" ? ADDRESS_PICKUP : ADDRESS_DELIVERY);
		$outer = new Forms();
		$outer->init($this->getTemplate(__FUNCTION__));
    ...
    function getFSA_dnu($fromMain = false) {
		$d_id = array_key_exists("d_id",$_REQUEST) ? $_REQUEST["d_id"] : 0;
		$driver = $this->fetchSingleTest("select * from drivers where id = %d",$d_id);
		$outer = new Forms();
		$outer->init($this->getTemplate(__FUNCTION__));
		$inner = new Forms();
		$inner->init($this->getTemplate(__FUNCTION__."Row"));
		$flds = $inner->buildForm($this->getFields(__FUNCTION__));
		$rows = $this->fetchAllTest("select d.*, f.fsa, z.title from driver_fsa d, fsa f, zone_fsa zf, zones z where d.driver_id = %d and f.id = d.fsa_id and zf.fsa_id = f.id and z.id = zf.zone_id order by f.fsa",$d_id);
		$result = array();
		...
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__,$_POST)) {
			...
			if ($valid) {
				...
				foreach($_REQUEST["fsa_id"] as $key=>$value) {
					...
					//
					$v_type = $this->fetchSingleTest("select v.* from drivers d, vehicles v where d.id = %d and v.id = d.vehicle_id",$d_id);
					$tmp_valid = true;
					if ($v_type["downtown_only"]) {
						$f_type = $this->fetchSingleTest("select * from fsa where id = %d",$value);
						if (!$f_type["downtown"]) {
							$tmp_valid = false;
							$valid = false;
    ...
    function zones() {
		...
		$recs = $this->fetchAllTest("select z.*, m.company, m.lastname, m.firstname, concat(m.company, '-', m.lastname,' ',m.firstname) as dname, v.name as vehicle from delivery_zones z, drivers d, members m, vehicles v where d.id = z.driver_id and m.id = d.member_id and v.id = d.vehicle_id order by z.name asc");
		foreach($recs as $key=>$value) {
			$inner->addData($value);
			$result[] = $inner->show();
		}
    ...
    function editZone() {
		...
		if (!$zone = $this->fetchSingleTest("select z.*, v.name as vehicle_name from delivery_zones z, vehicles v where z.id = %d and v.id = z.vehicle_id",$z_id)) {
			$zone = array("id"=>0,"vehicle_id"=>0);
		}
		else $flds["vehicle_id"]["type"] = "hidden";
		$zone["fsa"] = $this->fetchScalarAllTest("select fsa_id from delivery_zones_fsa where delivery_zone_id = %d",$z_id);
		...
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__,$_POST)) {
			...
			if ($valid = $outer->validate()) {
				...
				$outer->setData("z_id",$z_id);
				$exists = $this->fetchScalarAllTest("select fsa_id from delivery_zones_fsa where delivery_zone_id = %d",$z_id);
				...
				$vehicle_type = $this->fetchScalarTest("select vehicle_id from drivers d where d.id = %d",$_POST["driver_id"]);
				...
				foreach($toAdd as $key=>$value) {
					...
					if (!$tmp) $outer->addFormError(sprintf("FSA %s is probably already assigned to the same vehicle type",$this->fetchScalarTest("select fsa from fsa where id = %d",$value)));
					$valid &= $tmp;
				}
    ...
    function zoneFSA() {
		...
		$z_id = array_key_exists("z_id",$_REQUEST) ? $_REQUEST["z_id"] : 0;
		$fsa = new select();
		$exists = $this->fetchOptionsTest("select f.id, f.fsa from fsa f, delivery_zones_fsa zf where zf.delivery_zone_id = %d",$z_id );
		...
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__,$_POST)) {
			...
			if (array_key_exists("driver_id",$_REQUEST) && $_REQUEST["driver_id"] > 0) {
				...
				if ($outer->getData("mark_as_paid") == 1) {
					...
					implode(" and ",$srch));
					$recs = $this->fetchScalarAllTest($sql);
    ...
    public function editPayment() {
		...
		if (array_key_exists(__FUNCTION__,$_POST) && $_POST[__FUNCTION__] == 1) {
			...
			if ($outer->validate()) {
				...
				$old = $this->fetchSingleTest("select * from orders where id = %d",$data["order_id"]);
				...
				$stmt->bindParams(array_merge(array(str_repeat("s",count($upd))),array_values($upd)));
				$stmt->execute();
				$reciprocal = $this->fetchScalarTest("select id from custom_delivery where order_id=%d and id != %d", $data["order_id"], $p_id);
...
/*
				if (array_key_exists("adjust",$_POST) && $_POST["adjust"] == 1) {
					$_POST["p_id"] = $this->fetchScalarTest(sprintf("select id from custom_delivery where order_id=%d and id != %d", 
...
*/
    ...
    function mapIt() {
		...
		foreach($packages as $key=>$value) {
			$address = $this->fetchSingleTest("select * from addresses where id = %d",$value["a_id"]);
    ...
    function showPayments() {
		...
		$driver = $this->fetchSingleTest("select m.* from members m, drivers d where d.id = %d and m.id = d.member_id", $_REQUEST["driver_id"]);
    ...
    function deductions() {
		...
		$drivers = $this->fetchAllTest("select d.*, m.company, m.firstname, m.lastname from drivers d, members m where m.id = d.member_id and d.enabled = 1 and d.deleted = 0 order by m.company, m.lastname, m.firstname");
		$recs = array();
    ...
    function getZones($d_id) {
		...
		$outer->init($this->getTemplate(__FUNCTION__));
		$flds = $outer->buildForm($this->getFields(__FUNCTION__));
		$zones = $this->fetchAllTest("select * from delivery_zones where driver_id = %d order by name", $d_id);
		$recs = array();
    ...
    function editCommission() {
		...
		$outer->init($this->getTemplate(__FUNCTION__));
		$flds = $outer->buildForm($this->getFields(__FUNCTION__));
		$rec = $this->fetchSingleTest("select cdc.*, p.name from custom_delivery_commissions cdc, product p, custom_delivery cd where cdc.id = %d and cd.id = cdc.delivery_id and p.id = cdc.product_id", $_REQUEST["c_id"]);
		$outer->addData($rec);
    ...
    function zoneDelete() {
		...
		$z_id = array_key_exists("z_id", $_REQUEST) ? $_REQUEST["z_id"] : 0;
		$status = false;
		if ($rec = $this->fetchSingleTest("select * from delivery_zones where id = %d", $z_id)) {
			$ct = $this->fetchScalarTest("select count(0) from delivery_zones_fsa where delivery_zone_id = %d", $z_id);
			if ($ct == 0) {
...
order by %s", implode(" and ", $srch), $sortby);
			...
			foreach($recs as $key=>$rec) {
				...
				$rec["pu_address"] = Address::formatData($this->fetchSingleTest("select * from addresses where ownertype='order' and ownerid=%d and addresstype =%d", $rec["order_id"], ADDRESS_PICKUP ));
				$rec["del_address"] = Address::formatData($this->fetchSingleTest("select * from addresses where ownertype='order' and ownerid=%d and addresstype =%d", $rec["order_id"], ADDRESS_DELIVERY ));
				if ($this->fetchScalarTest("select count(cd.id) from custom_delivery cd where cd.order_id = %d and driver_id = %d and DATE(actual_date) between '%s' and '%s'", $rec["order_id"], $rec["driver_id"], date("Y-m-d", strtotime($outer->getData("start_date"))), date("Y-m-d", strtotime($outer->getData("end_date")))) > 1) $rec["service_type"] = "B";
				$inner->addData($rec);
				$list[] = $inner->show();
		...
		try {
			ob_clean();
			if ($asFile) {
				$driver = $this->fetchSingleTest("select m.* from members m, drivers d where m.id = d.member_id and d.id = %d", $outer->getData("driver_id"));
    ...
    function sendPDF() {
		...
		$flds = $outer->buildForm($this->getFields(__FUNCTION__));
		$outer->addData($_REQUEST);
		$outer->setData("driver", $this->fetchSingleTest("select m.*, a.email as email_to from members m left join addresses a on a.ownertype='member' and a.ownerid = m.id and a.addresstype = %d and a.addressbook_id > 0, drivers d where m.id = d.member_id and d.id = %d", ADDRESS_COMPANY, $outer->getData("driver_id")));
		$outer->validate();
		$mailer = new MyMailer();
