...
class custom extends Backend {
    ...
    public function memberFolderDisplay($data = array('id'=>0)) {
		$p = $this->fetchAllTest("select p.*,po.product_id as opt_id from custom_member_product_options po, product p where po.member_id = %d and p.id = po.product_id and po.isgroup = 1 and p.enabled = 1 and p.published = 1 order by p.name",$data["id"]);
		$recs = array();
		$f = new Forms();
    ...
    public function memberDisplay(&$data = array('id'=>0,'custom_parent_org'=>0)) {
		$p = $this->fetchAllTest("select p.*,po.product_id as opt_id from custom_member_product_options po, product p where po.member_id = %d and p.id = po.product_id and po.isgroup = 0 and p.enabled = 1 and p.published = 1 order by p.name",$data["id"]);
		$recs = array();
		$f = new Forms();
		$f->init($this->M_DIR."forms/memberProduct.html");
		$flds = array(
		...
		$data["optional_required_fields"] = $this->fetchScalarAllTest("select optional_id from member_optional_fields where member_id = %d", $data["id"]);
		return $return;
	}
			...
			if ($this->checkArray("addresses:shipping:postalcode",$cart) && $this->checkArray("addresses:pickup:postalcode",$cart) && strpos(MSH_NO_FUEL,sprintf("|%s|", substr($cart["addresses"]["shipping"]["postalcode"],0,3))) !== FALSE &&
				...
				foreach($cart["products"] as $k=>$v) {
					if ($v["product_id"] == $fuel["id"]) {
						...
						unset($tmp["line_id"]);
						unset($tmp["id"]);
						$tbr = $this->fetchSingleTest("select * from order_lines limit 1");
...
/*
		if ($p = $this->fetchSingleTest("select * from product_pricing where product_id = %d order by min_quantity limit 1",$f)) {
			$price = $p["price"];
			$override = ($p["price"] + $o);
		}
		else {
...
*/
		if ($p = $this->fetchSingleTest("select * from product where id = %d",$f)) {
...
*/
    ...
    public function getMemberWeight() {
		...
		if ($m == 0) {
			...
			if ($f = $this->fetchSingleTest("select f.* from members_folders f, members_by_folder mf where mf.member_id = %d and f.id = mf.folder_id order by id limit 1",$m)) {
				if ($p = $this->fetchSingleTest("select * from product_pricing where product_id = %d order by min_quantity limit 1",$f['custom_weight'])) {
					$price = $p["price"];
					//
    ...
    public function getMemberKm() {
		...
		if ($m == 0) {
			...
			$override = "Could not get group pricing";
			if ($f = $this->fetchSingleTest("select f.* from members_folders f, members_by_folder mf where mf.member_id = %d and f.id = mf.folder_id order by id limit 1",$m)) {
				//if ($p = $this->fetchSingleTest("select * from product_pricing where product_id = %d order by min_quantity limit 1",$f['custom_km'])) {
				if ($p = $this->fetchSingleTest("select * from product where id = %d",$f['custom_km'])) {
					$price = $p["custom_minimum_charge"];
    ...
    public function getMemberFuel() {
		...
		if ($m == 0) {
			...
			$override = "Could not get group pricing";
			if ($f = $this->fetchSingleTest("select f.* from members_folders f, members_by_folder mf where mf.member_id = %d and f.id = mf.folder_id order by id limit 1",$m)) {
				//if ($p = $this->fetchSingleTest("select * from product_pricing where product_id = %d order by min_quantity limit 1",$f['custom_fuel'])) {
				if ($p = $this->fetchSingleTest("select * from product where id = %d",$f['custom_fuel'])) {
					$price = $p["custom_minimum_charge"];
    ...
    public function getMemberInsurance() {
		...
		if ($m == 0) {
			...
			$override = "Could not get group pricing";
			if ($f = $this->fetchSingleTest("select f.* from members_folders f, members_by_folder mf where mf.member_id = %d and f.id = mf.folder_id order by id limit 1",$m)) {
				//if ($p = $this->fetchSingleTest("select * from product_pricing where product_id = %d order by min_quantity limit 1",INSURANCE)) {
				if ($p = $this->fetchSingleTest("select * from product where id = %d",INSURANCE)) {
					$price = $p["custom_minimum_charge"];
    ...
    function enhancedProductList($parms, $obj) {
		...
		$folders = $this->fetchAllTest('select * from product_folders where enabled = 1 order by left_id');
		$opts = array();
		$ct = 0;
		...
		foreach($folders as $k=>$fldr) {
			...
			$products = $this->fetchAllTest("select p.* from product p, product_by_folder pf where pf.folder_id = %d and p.id = pf.product_id order by pf.sequence", $fldr["id"]);
    ...
    function optgroup() {
		...
		$grps = $this->fetchAllTest("select * from product_folders order by left_id");
    ...
    function subOptGroup($fldr) {
		...
		$prods = $this->fetchAllTest("select * from product p, product_by_folder pf where pf.folder_id = %d and p.id = pf.product_id", $fldr["id"]);
