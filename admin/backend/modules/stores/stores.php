...
class stores extends Backend {
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d', $this->m_tree, $p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from '.$this->m_content.' n where n.id in (select f.store_id from '.$this->m_junction.' f where f.folder_id = %d)', $_REQUEST['p_id']);
    ...
    function addContent($fromMain = false) {
		...
		if (!(array_key_exists('s_id',$_REQUEST) && $_REQUEST['s_id'] > 0 && $data = $this->fetchSingleTest(sprintf('select * from %s where id = %d', $this->m_content, $_REQUEST['s_id'])))) {
			$data = array('id'=>0,'published'=>false,'image1'=>'','image2'=>'','image3'=>'','image4'=>'','mapmarker'=>'');
			$addresses = array();
		} else {
			$addresses = $this->fetchAllTest(sprintf('select a.*, c.value from addresses a, code_lookups c where c.code = a.addresstype and c.type = "storeAddressTypes" and a.ownertype = "store" and a.ownerid = %d and a.deleted = 0',$_REQUEST['s_id']));
		}
		if ((count($_REQUEST) > 0 && array_key_exists('destSearch',$_REQUEST)) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest(sprintf('select folder_id from store_by_search_group where store_id = %d', $data['id']));
		...
		if (count($_REQUEST) > 0 && array_key_exists('destFolders',$_REQUEST) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest(sprintf('select folder_id from %s where store_id = %d',$this->m_junction,$data['id']));
		...
		$data["amenities"] = $this->fetchScalarAllTest(sprintf("select option_id from store_options where store_id = %d",$data["id"]));
		...
		$frmFields['destCoupons']['options'] = $this->fetchOptionsTest(sprintf('select id as code,code as value from coupons where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by code',implode(',',array_merge(array(0),$data['destCoupons']))));
		...
		$frmFields['relatedDestProducts']['options'] = $this->fetchOptionsTest(sprintf('select id as code,concat(code," - ",name) as value from product where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by code',implode(',',array_merge(array(0),$data['relatedDestProducts']))));
		...
		$frmFields['destEvents']['options'] = $this->fetchOptionsTest(sprintf('select id as code,name as value from events where id in (select event_id from event_dates where deleted = 0 and enabled = 1 and event_date >= curdate()) and id in (%s) order by start_date',implode(',',array_merge(array(0),$data['destEvents']))));
		//	access levels
		//
		$level = $this->getAccessLevel();
		switch($level) {
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($form->validate()) {
				...
				if ($stmt->execute()) {
					...
					//	insert new folders
					//
					$new = $this->fetchScalarAllTest(sprintf('select id from %s where id in (%s) and id not in (select folder_id from %s where store_id = %d and folder_id in (%s))', $this->m_tree, implode(',',$destFolders),$this->m_junction,$id,implode(',',$destFolders)));
					$status = true;
					...
					$new = $this->fetchScalarAllTest(sprintf('select id from search_groups where id in (%s) and id not in (select folder_id from store_by_search_group where store_id = %d and folder_id in (%s))',
					...
					if (array_key_exists("amenities",$_REQUEST)) {
						...
						$exists = "|".implode("|",$this->fetchScalarAllTest(sprintf("select option_id from store_options where store_id = %d",$id)))."|";
