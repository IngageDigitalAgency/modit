...
class stores extends Backend {
    ...
    function showPageProperties($fromMain = false) {
		...
		$frmFlds['destCoupons']['options'] = $this->fetchOptionsTest('select id as code,code as value from coupons where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by code',implode(',',array_merge(array(0),$data['destCoupons'])));
		...
		$frmFlds['relatedDestProducts']['options'] = $this->fetchOptionsTest('select id as code,concat(code," - ",name) as value from product where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by code',implode(',',array_merge(array(0),$data['relatedDestProducts'])));
		...
		$frmFlds['destEvents']['options'] = $this->fetchOptionsTest('select id as code,name as value from events where id in (select event_id from event_dates where event_date >= curdate()) and deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by start_date',implode(',',array_merge(array(0),$data['destEvents'])));
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d', $this->m_tree, $p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from %s n where n.id in (select f.store_id from %s f where f.folder_id = %d)', $this->m_content, $this->m_junction, $_REQUEST['p_id']);
			...
			//
			$coupons = $this->fetchAllTest('select * from coupons c, relations r where c.deleted = 0 and r.owner_id = %d and r.owner_type = "storefolder" and c.id = r.related_id and r.related_type="coupon"',$data['id']);
			$output = array();
			...
			$coupons = $this->fetchAllTest('select * from coupon_folders c, relations r where r.owner_id = %d and r.owner_type = "storefolder" and c.id = r.related_id and r.related_type="couponfolder"',$data['id']);
			$output = array();
			foreach($coupons as $coupon) {
				$frm = new Forms();
			...
			//
			$products = $this->fetchAllTest('select * from product c, relations r where c.deleted = 0 and r.owner_id = %d and r.owner_type = "storefolder" and c.id = r.related_id and r.related_type="product"',$data['id']);
			$output = array();
			...
			$products = $this->fetchAllTest('select * from product_folders c, relations r where r.owner_id = %d and r.owner_type = "storefolder" and c.id = r.related_id and r.related_type="productfolder"',$data['id']);
			$output = array();
			foreach($products as $product) {
				$frm = new Forms();
			...
			//			
			$events = $this->fetchAllTest('select * from events c, relations r where c.deleted = 0 and r.owner_id = %d and r.owner_type = "storefolder" and c.id = r.related_id and r.related_type="event" and c.id in (select event_id from event_dates where event_date >= curdate())',$data['id']);
			$output = array();
			...
			$events = $this->fetchAllTest('select * from members_folders c, relations r where r.owner_id = %d and r.owner_type = "storefolder" and c.id = r.related_id and r.related_type="eventfolder"',$data['id']);
			$output = array();
			foreach($events as $event) {
				$frm = new Forms();
    ...
    function addContent($fromMain = false) {
		...
		if (!(array_key_exists('s_id',$_REQUEST) && $_REQUEST['s_id'] > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d', $this->m_content, $_REQUEST['s_id']))) {
			$data = array('id'=>0,'published'=>false,'image1'=>'','image2'=>'','image3'=>'','image4'=>'','mapmarker'=>'');
			$addresses = array();
		} else {
			$addresses = $this->fetchAllTest('select a.*, c.value from addresses a, code_lookups c where c.code = a.addresstype and c.type = "storeAddressTypes" and a.ownertype = "store" and a.ownerid = %d and a.deleted = 0',$_REQUEST['s_id']);
		}
		if ((count($_REQUEST) > 0 && array_key_exists('destSearch',$_REQUEST)) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest('select folder_id from store_by_search_group where store_id = %d', $data['id']);
		...
		if (count($_REQUEST) > 0 && array_key_exists('destFolders',$_REQUEST) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest('select folder_id from %s where store_id = %d',$this->m_junction,$data['id']);
		...
		$data["amenities"] = $this->fetchScalarAllTest("select option_id from store_options where store_id = %d",$data["id"]);
		...
		$frmFields['destCoupons']['options'] = $this->fetchOptionsTest('select id as code,code as value from coupons where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by code',implode(',',array_merge(array(0),$data['destCoupons'])));
		...
		$frmFields['relatedDestProducts']['options'] = $this->fetchOptionsTest('select id as code,concat(code," - ",name) as value from product where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by code',implode(',',array_merge(array(0),$data['relatedDestProducts'])));
		...
		$frmFields['destEvents']['options'] = $this->fetchOptionsTest('select id as code,name as value from events where id in (select event_id from event_dates where deleted = 0 and enabled = 1 and event_date >= curdate()) and id in (%s) order by start_date',implode(',',array_merge(array(0),$data['destEvents'])));
		//	access levels
		//
		$level = $this->getAccessLevel();
		switch($level) {
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($form->validate()) {
				...
				if ($stmt->execute()) {
					...
					//	insert new folders
					//
					$new = $this->fetchScalarAllTest('select id from %s where id in (%s) and id not in (select folder_id from %s where store_id = %d and folder_id in (%s))', $this->m_tree, implode(',',$destFolders),$this->m_junction,$id,implode(',',$destFolders);
					$status = true;
					...
					$new = $this->fetchScalarAllTest('select id from search_groups where id in (%s) and id not in (select folder_id from store_by_search_group where store_id = %d and folder_id in (%s))',
					...
					if (array_key_exists("amenities",$_REQUEST)) {
						...
						$exists = "|".implode("|",$this->fetchScalarAllTest("select option_id from store_options where store_id = %d",$id))."|";
    ...
    function moveStore() {
		...
		if ($_REQUEST['type'] == 'tree') {
			...
			$curr = $this->fetchScalarTest('select store_id from %s where id = %d',$this->m_junction,$src);
			if ($curr > 0 && $folder = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$dest)) {
				$status = true;
				if (array_key_exists('move',$_REQUEST)) {
					...
					if ($status = $this->execute(sprintf('delete from %s where id = %d', $this->m_junction, $src))) {
						if (!$this->fetchSingleTest('select * from %s where store_id = %d and folder_id = %d',$this->m_junction,$curr,$dest)) {
							$obj = new preparedStatement(sprintf('insert into %s(store_id,folder_id) values(?,?)',$this->m_junction));
    ...
    function resequence($folder) {
		...
		$stores = $this->fetchAllTest('select * from %s where folder_id = %d order by sequence',$this->m_junction,$folder);
		$seq = 10;
		foreach($stores as $store) {
			...
			if ($form->validate()) {
				...
				switch($type) {
					case 'cancel':
						...
						break;
					case 'all':
						//$img = $this->fetchScalarTest('select store_id from %s where id = %d',$this->m_junction,$_REQUEST['j_id']);
					...
					case 'one':
						...
						$ct = $this->fetchScalarTest('select count(0) from %s where store_id = %d',$this->m_junction,$_REQUEST['s_id']);
						if ($ct == 0)
							$this->execute(sprintf('update %s set deleted = 1 where id = %d',$this->m_content,$_REQUEST['s_id']));
						break;
					default:
    ...
    function editAddress() {
		...
		$a_id = $_REQUEST['a_id'];
		$o_id = $_REQUEST['o_id'];
		if (!($data = $this->fetchSingleTest('select * from addresses where id = %d and ownertype = "store" and ownerid = %d',$a_id,$o_id))) {
			$data = array('id'=>0,'ownertype'=>'store','ownerid'=>$o_id);
			$addresses = array();
		}
		else 
			$addresses = $this->fetchAllTest('select a.*, c.value from addresses a, code_lookups c where c.code = a.addresstype and c.type = "storeAddressTypes" and a.ownertype = "store" and a.ownerid = %d and a.deleted = 0',$_REQUEST['o_id']);
		$form = new Forms();
		...
		if (count($_POST) > 0 && array_key_exists('editAddress',$_POST)) {
			...
			if ($status = $form->validate()) {
				...
				$this->beginTransaction();
				$store = $this->fetchSingleTest("select * from stores where id = %d",$_POST["o_id"]);
				if ($status &= $stmt->execute()) {
					if (array_key_exists("geocode",$_POST) && $_POST["geocode"] == 1) {
						$url = urlencode(sprintf("%s, %s, %s %s",$flds['line1'],$flds['city'],
							$this->fetchScalarTest('select province_code from provinces where id = %d',$flds['province_id']),$flds['postalcode']));
		...
		$addresses = $this->fetchAllTest('select * from addresses where ownertype = "store" and ownerid = %d and deleted = 0',$o_id);
		$addresses = $this->fetchAllTest('select a.*, c.value from addresses a, code_lookups c where c.code = a.addresstype and c.type = "storeAddressTypes" and a.ownertype = "store" and a.ownerid = %d and a.deleted = 0',$_REQUEST['o_id']);
		$addr = $form->getField('addresses');
		$addr->addOption('','-select an address-');
		foreach($addresses as $rec) {
    ...
    function deleteAddress() {
		if (array_key_exists('a_id',$_REQUEST) && array_key_exists('o_id',$_REQUEST)) {
			...
			if ($data = $this->fetchSingleTest('select * from addresses where ownertype = "store" and id = %d and ownerid = %d',$_REQUEST['a_id'],$_REQUEST['o_id'])) {
    ...
    function loadAddresses($passed_id = null) {
		if ($passed_id == null)
			...
			$o_id = $passed_id;
		$addresses = $this->fetchAllTest('select a.*, c.value as addressType from addresses a, code_lookups c where ownertype = "store" and ownerid = %d and deleted = 0 and c.id = a.addressType',$o_id);
    ...
    function moduleStatus($fromMain = 0) {
		if (array_key_exists('formData',$_SESSION) && array_key_exists('storeSearchForm', $_SESSION['formData'])) {
			...
			$ct = $this->fetchScalarTest('select count(0) from %s where deleted = 0 and published = 0',$this->m_content);
			if ($ct == 0) {
