...
class config extends Backend {
    ...
    function showPageContent($fromMain = false) {
		...
		$types = $this->fetchScalarAllTest('select distinct groupName from config where type="config" order by groupName');
		$li = array();
		$groups = new Forms();
		...
		foreach($types as $key=>$type) {
			...
			$items = $this->fetchAllTest('select * from config where groupName = "%s"',$type);
			$itemList = array();
			foreach($items as $subkey=>$item) {
				$itemList[] = sprintf('<tr><td>%s</td><td>%s</td><td><a href="#" onclick="editArticle(%d)"><em class="icon-edit"></em></a></td><td><a href="#"><em class="icon-trash"></em></a></td></tr>',$item['pretty_name'],$item['description'],$item['id']);
			}
					...
					foreach($recs as $article) {
						...
						switch($article['field_type']) {
							...
							case 'paired_list':
								...
								break;
							case 'address':
								$tmp = $this->fetchSingleTest('select * from addresses where id = %d',$article['value']);
    ...
    function addContent($fromMain = false) {
		...
		switch($data['field_type']) {
			...
			case 'image':
				break;
			case 'address':
				$tmp = $this->fetchSingleTest('select * from addresses where id = %d',$data['value']);
				//unset($data['name']);
				...
				switch($data['field_type']) {
					case 'address':
						...
						if ((int)($_POST['value']) > 0)
							...
							$flds['ownerid'] = $id;
							$flds['addresstype'] = $this->fetchScalarTest('select id from code_lookups where type="storeAddressTypes" and code = "%s"', str_replace('-','',$data['name']));
    ...
    function deleteArticle() {
		if (array_key_exists('j_id',$_REQUEST)) {
			$id = $_REQUEST['j_id'];
			$curr = $this->fetchScalarTest('select article_id from %s where id = %d',$this->m_junction,$id);
			...
			if (($remining = $this->fetchScalarTest('select count(0) from %s where article_id = %d',$this->m_junction,$curr)) == 0) {
    ...
    function taxList() {
		...
		$taxes = $this->fetchAllTest('select t.*,c.country,p.province from taxes t, countries c, provinces p where p.id = t.province_id and c.id = p.country_id and t.deleted = 0 order by c.country, p.province');
		$inner = new Forms();
    ...
    function editTax() {
		...
		if (!($data = $this->fetchSingleTest('select t.*, p.country_id from taxes t, provinces p where t.id = %d and p.id = t.province_id',$_REQUEST['t_id'])))
			$data = array('id'=>0);
		$outer->addData($data);
    ...
    function formList() {
		...
		$forms = $this->fetchAllTest('select h.*, m.title from htmlForms h, modules m where m.id = h.class order by m.title, h.type');
		$inner = new Forms();
    ...
    function editForm() {
		...
		if (!($data = $this->fetchSingleTest('select h.* from htmlForms h where h.id = %d',$_REQUEST['f_id'])))
			$data = array('id'=>0);
		$outer->addData($data);
