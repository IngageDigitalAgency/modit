		...
		if (count($_POST) > 0 && array_key_exists('showSearchForm',$_POST)) {
			...
			if ($form->validate()) {
				...
				if (count($srch) > 0) {
					...
					$count = $this->fetchScalarTest(sprintf('select count(n.id) from %s n where 1=1 and %s', $this->m_content, implode(' and ',$srch)));
...
order by 2",$data['id'],$data['id']);
    ...
    function buildTemplates($data = null) {
		...
		//if ($data['id'] <= 0) return "";
		$templates = $this->fetchAllTest('select st.*, t.module_name, m.title from sub_templates st, fetemplates t, modules m where fetemplate_id = %d and t.id = st.submodule_id and m.id = t.module_id',$data['id']);
		$form = new Forms();
    ...
    function addSubtemplate() {
		...
		if (array_key_exists('s_id',$_REQUEST) && $_REQUEST['s_id'] > 0) {
			$data = $this->fetchSingleTest('select * from sub_templates where id = %d',$_REQUEST['s_id']);
		}
    ...
    function addPlacement() {
		...
		if (array_key_exists('s_id',$_REQUEST) && $_REQUEST['s_id'] > 0) {
			$data = $this->fetchSingleTest('select * from fetemplate_placement where id = %d',$_REQUEST['s_id']);
		}
		else $data = array('id'=>0,'fetemplate_id'=>$_REQUEST['fetemplate_id']);
		$form->addData($data);
     ...
     */
    function editHtml() {
		$class = $this->fetchSingleTest("select * from modules where id = %d",$_REQUEST["class"]);
