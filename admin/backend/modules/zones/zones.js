...
function addItem() {
	clearMessages();
	$.ajax({
		url: '/modit/ajax/showPageProperties/zones',
		data: {'id': 0},
...
function getInfo(lnk) {
	...
	var id = lnk.split("_");
	$.ajax({
		url: '/modit/ajax/getFolderInfo/zones',
		data: {p_id: id[1]},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = JSON.parse(data);
...
function loadContent(cId) {
	$.ajax({
		url: '/modit/ajax/showPageContent/zones',
		data: {p_id : cId},
		method: 'post',
		dataType: "html",
		success: function(data,textStatus,jqXHR) {
...
function deleteContent(cId) {
	if (confirm("Are you sure you want to delete this item?")) {
		$.ajax({
			url: '/modit/ajax/deleteContent/zones',
			data: {'p_id':cId},
			success: function(data,textStatus,jqXHR) {
				try {
					obj = JSON.parse(data);
...
function loadSearchForm(id) {
	$.ajax({
		url: '/modit/ajax/showSearchForm/zones',
		data: {'p_id':id},
		success: function(data,textStatus,jqXHR) {
			try {
				obj = JSON.parse(data);
...
//
var editArticle = function(a_id,j_id) {
	$.ajax({
		url: '/modit/ajax/addContent/zones',
		data: {'a_id': a_id, 'j_id': j_id },
		success: function(data,textStatus,jqXHR) {
			try {
				obj = JSON.parse(data);
				removeTinyMCE($('#popup textarea'))
			...
			$("#dialog-confirm" ).dialog({
				...
				buttons: {
					"Copy the ad": function() {
						$.ajax({
							url: '/modit/ajax/moveArticle/zones',
...
function deleteArticle(a_id,j_id) {
	$.ajax({
		url: '/modit/ajax/deleteArticle/zones',
...
function initSearch() {
	if ($('#search-tabs').length > 0) {
		$('#articleList #pager').change(function() {
			formCheck("searchForm","/modit/ajax/showSearchForm/zones","middleContent");
		});
...
function reSort(src,dest) {
	...
	data = $.ajax({
		url: '/modit/ajax/moveArticle/zones',
		data: {'src':src[1],'dest':dest[1],'type':'zones'},
		async:false
	});
	try {
		obj = JSON.parse(data.responseText);
		if (obj.status == 'true') {
			formCheck("showFolderContent","/modit/ajax/showPageContent/zones","middleContent");
		}
		if (obj.messages)
			showError(obj.messages);
		if (obj.code && obj.code.length > 0)
		...
		$( '#articleList tbody').sortable({
			...
			update: function( event, ui) {
				if(!dropped) {
					...
					zonesDrop(idx[1],curLoc,null,'news');
				}
				dropped = false;
			}
		});
...
function makeDroppable() {
	$('#contentTree ol.ui-sortable li .wrapper').droppable({
		...
		drop: function( event, ui ) {
			dropped = true;
			zonesDrop(this,event,ui,'tree');
		}
	});
}

function reloadFSA(id,j_id) {
	$.ajax({
		url: "/modit/ajax/loadFSA/zones",
		data: {z_id:id,j_id:j_id},
		success: function(obj,status,xhr) {
			try {
				obj = JSON.parse(obj);
				$("#tabs-4").html(obj.html);
				closeAltPopup();
				var objcode = "<script type='text/javascript'>"+obj.code+"</script>"; $('body').append(objcode);
			}
			catch(err) {
...
function addZoneRate(j_id,zz_id) {
	$.ajax({
		url: "/modit/ajax/addInterzone/zones",
		data: {"j_id":j_id,"zz_id":zz_id},
		success: function(obj,status,xhr) {
			try {
				obj = JSON.parse(obj);
...
function loadInterzone(j_id) {
	$.ajax({
		url: "/modit/ajax/loadInterzone/zones",
		data: {"j_id":j_id,"z_id":1},
		success: function(obj,status,xhr) {
			try {
				obj = JSON.parse(obj);
...
addMinMax = function() {
	$.ajax({
		url: "/modit/ajax/addMinMax/zones",
		success: function(obj,status,xhr) {
			try {
				obj = JSON.parse(obj);
				showPopup(obj.html);
...
removeCap = function(id) {
	if (confirm("Are you sure?")) {
		alert('removed');
	}
}

cloneContent = function(id) {
	$.ajax({
		url: "/modit/ajax/cloneZone/zones",
		data: {g_id: id },
		success: function( obj, status, xhr) {
			try {
				obj = JSON.parse(obj);
...
addFSA = function(z_id,j_id,fsa_id) {
	$.ajax({
		url: "/modit/ajax/addFSA/zones",
		data: {zone_id:z_id,j_id:j_id,id:fsa_id},
		success: function(obj,status,xhr) {
			try {
				obj = JSON.parse(obj);
				showAltPopup(obj.html);
...
removeFSA = function(z_id,fsa_id,cd) {
	if (confirm("Confirm remove FSA ".concat(cd," from this zone"))) {
		$.ajax({
			url: "/modit/ajax/removeFSA/zones",
			data: {z_id:z_id,fsa_id:fsa_id},
			success: function(obj,status,xhr) {
				try {
					h = JSON.parse(obj);
					$("#tabs-4").html(h.html);
...
newFSA = function(id) {
	$.ajax({
		url: "/modit/ajax/editFSA/zones",
		data: {fsa_id:id},
		success: function( obj, status, xhr ) {
			try {
				o = JSON.parse(obj);
				showPopup(o.html);
				var objcode = "<script type='text/javascript'>"+o.code+"</script>"; $('body').append(objcode);
			}
			catch(err) {
			}
		}
