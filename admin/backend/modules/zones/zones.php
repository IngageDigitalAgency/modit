...
class zones extends Backend {
    ...
    function moveArticle() {
		...
		if ($_REQUEST['type'] == 'tree') {
			...
			if ($folder = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$dest)) {
				$curr = $this->fetchScalarTest('select zone_id from %s where id = %d',$this->m_junction,$src);
				$status = true;
				if (array_key_exists('move',$_REQUEST)) {
					...
					if ($status = $this->execute(sprintf('delete from %s where id = %d', $this->m_junction, $src))) {
						if (!$this->fetchSingleTest('select * from %s where zone_id = %d and folder_id = %d',$this->m_junction,$curr,$dest)) {
							$obj = new preparedStatement(sprintf('insert into %s(zone_id,folder_id) values(?,?)',$this->m_junction));
    ...
    function deleteArticle() {
		...
		$ct = $this->fetchScalarTest("select count(0) from zone_fsa where zone_id = %d", $form->getData("a_id"));
		if ($ct > 0) {
			...
			if ($form->validate()) {
				...
				switch($type) {
					case 'cancel':
						...
						break;
					case 'all':
						//$img = $this->fetchScalarTest('select zone_id from %s where id = %d',$this->m_junction,$_REQUEST['j_id']);
					...
					case 'one':
						...
						$ct = $this->fetchScalarTest('select count(0) from %s where zone_id = %d',$this->m_junction,$_REQUEST['a_id']);
						if ($ct == 0)
							$this->execute(sprintf('update %s set deleted = 1 where id = %d',$this->m_content,$_REQUEST['a_id']));
						break;
					default:
		...
		if (array_key_exists('j_id',$_REQUEST)) {
			$id = $_REQUEST['j_id'];
			$curr = $this->fetchScalarTest('select zones_id from %s where id = %d',$this->m_junction,$id);
			...
			if (($remining = $this->fetchScalarTest('select count(0 from %s where zone_id = %d',$this->m_junction,$curr))) == 0) {
    ...
    function moduleStatus($fromMain = 0) {
		if (array_key_exists('formData',$_SESSION) && array_key_exists('zonesSearchForm', $_SESSION['formData'])) {
			...
			$ct = $this->fetchScalarTest('select count(0) from %s where enabled = 0',$this->m_content);
			if ($ct == 0) {
				$_POST = array('showSearchForm'=>1,'deleted'=>0,'sortby'=>'created','sortorder'=>'desc');
    ...
    function loadFSA($data = array()) {
		...
		if (array_key_exists('pager',$_REQUEST)) 
			$perPage = $_REQUEST['pager'];
		$count = $this->fetchScalarTest("select count(0) from zone_fsa where zone_id = %d",$data["id"]);
		$pagination = $this->pagination($count, $perPage, $pageNum, 
		...
		$fsa = $this->fetchAllTest("select z.*, f.fsa from zone_fsa z, fsa f where z.zone_id = %d and f.id = z.fsa_id order by f.fsa limit %d,%d",$data["id"],($pageNum-1)*$perPage,$perPage);
		$rows = array();
		$line = new Forms();
    ...
    function addFSA() {
		...
		//
		$zones = $this->fetchScalarAllTest("select z2.zone_id from zones_by_folder z1, zones_by_folder z2 where z1.id = %d and z2.folder_id = z1.folder_id", $j_id);
		if ($id == 0) {
			...
			$form->addData($this->fetchSingleTest("select * from zone_fsa where id = %d", $id));
		}
/*
		if (!$fsa = $this->fetchSingleTest("select * from zone_fsa where id = %d",$fsa)) {
...
/*
				...
				$ct = $this->fetchSingleTest("select * from zone_fsa where fsa_id = %d",$rec["fsa_id"]);
				if ($rec["id"] == 0) {
					if (is_array($ct) > 0) {
...
",$_REQUEST["j_id"],$_REQUEST["j_id"]);
		//$costs = $this->fetchAllTest("SELECT zz.* FROM zone_to_zone zz, zones_by_folder zf WHERE (zz.zone_from = zf.id or zz.zone_to = zf.id) and zf.zone_id = %d",$data["id"]);
		$rows = array();
		$line = new Forms();
    ...
    function addInterzone() {
		...
		if (!$jn = $this->fetchSingleTest("select zf.*, z.title from zones_by_folder zf, zones z where zf.id = %d and z.id = zf.zone_id",$j_id))
			$jn = array("id"=>0,"folder_id"=>0,"zone_id"=>0);
		$exists = $this->fetchScalarAllTest("
...
select distinct zone_id from zones_by_folder zf, zone_to_zone zz where zz.zone_from = %d and zf.id = zz.zone_to",$jn["id"],$jn["id"]);
		...
		if (!$data = $this->fetchSingleTest("select * from zone_to_zone where id = %d",$zz_id)) {
			$data = array('id'=>0,'cost'=>0.00);
		} else {
			if ($data["zone_from"] == $j_id)
				$dest = $this->fetchSingleTest("select * from zones z, zones_by_folder zf where zf.id = %d and z.id = zf.zone_id",$data["zone_to"]);
			else
				$dest = $this->fetchSingleTest("select * from zones z, zones_by_folder zf where zf.id = %d and z.id = zf.zone_id",$data["zone_from"]);
			$data["dest"] = $dest;
		}
		...
		if (array_key_exists("addInterzone",$_REQUEST)) {
			...
			if ($valid = $form->validate()) {
				...
				if ($data["id"] == 0) {
					...
					$values["zone_to"] = $this->fetchScalarTest("select id from zones_by_folder where folder_id = %d and zone_id = %d",$jn["folder_id"],$_REQUEST["zone_add"]);
...
and %s",$where);
		...
		$recs = $this->fetchAllTest("%s order by %s %s limit %d,%d",$sql,$_POST["sortby"],$_POST["sortorder"],($pageNum-1)*$perPage,$perPage);
    ...
    function getZones() {
		...
		if (array_key_exists("r",$_REQUEST) && $_REQUEST["r"] == 0)
			$options = $this->fetchOptionsTest("select '','-select a zone-' union (select zf.id, z.title from zones z, zones_by_folder zf where zf.folder_id = %d and z.id = zf.zone_id order by z.title)",$g_id);
		else
			$options = $this->fetchOptionsTest("select zf.id, z.title from zones z, zones_by_folder zf where zf.folder_id = %d and z.id = zf.zone_id order by z.title",$g_id);
		$s = new select();
    ...
    function addMinMax() {
		...
		if ($c_id > 0) {
			$rec = $this->fetchSingleTest("select c.*, c.id as c_id, zf.title as groups, p.code as product_id, z1.title as zone_from, z2.title as zone_to 
...
where c.id = %d and zz.id = c.zone_to_zone_id and zf1.id = zz.zone_from and z1.id = zf1.zone_id and zf2.id = zz.zone_to and z2.id = zf2.zone_id and p.id = c.product_id and zf.id = zf1.folder_id",$c_id);
		...
		if (count($_POST) > 0 && array_key_exists("addMinMax",$_POST)) {
			...
			if ($valid) {
				if ($c_id == 0) {					
					$zz_id = $this->fetchScalarTest("select id from zone_to_zone where (zone_from = %d and zone_to = %d) or (zone_from = %d and zone_to = %d)",$_POST["zone_from"],$_POST["zone_to"],$_POST["zone_to"],$_POST["zone_from"]);
					if ($zz_id > 0) {
			...
			if ($valid && $c_id == 0) {
				if ($this->fetchSingleTest("select * from zone_to_zone_caps where zone_to_zone_id = %d and product_id = %d",$zz_id, $_POST["product_id"])) {
					$valid = false;
...
order by 2",$_POST["zone_from"],$_POST["groups"],$_POST["zone_from"],$_POST["groups"]);
    ...
    function getZoneDest() {
		...
		$options = $this->fetchOptionsTest("select zf2.id, z2.title from zones z, zones_by_folder zf, zone_to_zone zz, zones_by_folder zf2, zones z2
where zf.id = %d and z.id = zf.zone_id and zf.folder_id = %d and zz.zone_from = zf.id
and zf2.id = zz.zone_to and z2.id = zf2.zone_id
union
select zf2.id, z2.title from zones z, zones_by_folder zf, zone_to_zone zz, zones_by_folder zf2, zones z2
    ...
    function cloneZone() {
		...
		if (count($_POST) > 0 && array_key_exists(__FUNCTION__,$_POST)) {
			...
			if ($valid) {
				...
				$this->beginTransaction();
				$zbf = $this->fetchAllTest("select * from zones_by_folder where folder_id = %d",$old_zone);
				$zf_ids = array();
				$status = true;
				...
				$zz = $this->fetchAllTest("select zz.* from zone_to_zone zz, zones_by_folder zf where zf.folder_id = %d and zz.zone_from = zf.zone_id union select zz.* from zone_to_zone zz, zones_by_folder zf where zf.folder_id = %d and zz.zone_to = zf.zone_id",$old_zone,$old_zone);
				$zz_ids = array();
				...
				$zc = $this->fetchAllTest("select zc.* from zone_to_zone_caps zc, zone_to_zone zz, zones_by_folder zbf, zone_folders zf
where zz.id = zc.zone_to_zone_id and zbf.id = zz.zone_from and zf.id = zbf.folder_id and zf.id = %d", $old_zone);
    ...
    function editFSA() {
		...
		if ($data = $this->fetchSingleTest("select * from fsa where id = %d", $id)) {
			$outer->addData($data);
		}
		if (array_key_exists(__FUNCTION__,$_POST)) {
			...
			if ($valid) {
				if ($outer->getData("fsa_id") == 0) {
					if ($this->fetchSingleTest("select * from fsa where fsa = '%s'", $outer->getData("fsa"))) {
						$valid = false;
		...
		$zones = $this->fetchAllTest("select z.*, zf.title as folder_name, zbf.id as j_id from zones z, zone_folders zf, zones_by_folder zbf, zone_fsa zfsa 
    ...
    function fsa($fromMain = false) {
		...
		$count = $this->fetchScalarTest('select count(0) from fsa');
		$pagination = $this->pagination($count, $perPage, $pageNum);
		$start = ($pageNum-1)*$perPage;
		$sortby = 'fsa';
		$sortorder = 'asc';
