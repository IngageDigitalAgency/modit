...
class dispatch extends Backend {
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from '.$this->m_content.' n where n.id in (select f.product_id from %s f where f.folder_id = %d) and n.deleted = 0', $this->m_junction, $_REQUEST['p_id']);
		...
		if (count($_POST) > 0 && array_key_exists('showSearchForm',$_POST)) {
			...
			if ($form->validate()) {
				...
				if (count($srch) > 0) {
					...
					$form->setData('pager',$perPage);
					$count = $this->fetchScalarTest('select count(n.id) from '.$this->m_content.' n left join drivers d on n.driver_id = d.id, order_lines ol, product p, orders o, addresses a where %s', implode(' and ',array_values($srch)));
...
where %s and o.deleted = 0 and o.id = n.order_id and a.addresstype = IF(n.service_type="P",%d,%d)  and ol.order_id = o.id and ol.custom_package = "S" and p.id = ol.product_id order by %s %s limit %d,%d',
					...
					foreach($recs as $article) {
						...
						$frm->init($this->getTemplate('articleList'),array());
						$tmp = $frm->buildForm($this->getFields('articleList'));
						$article["services"] = implode(", ", $this->fetchScalarAllTest("select p.name from product p, order_lines ol where ol.order_id = %d and ol.custom_package = 'A' and ol.deleted = 0 and p.id = ol.product_id and p.custom_special_requirement = 1", $article["order_id"]));
						$frm->addData($article);
						$articles[] = $frm->show();
					}
					$form->addTag('articles',implode('',$articles),false);
    ...
    function moveArticle() {
		...
		if ($_REQUEST['type'] == 'tree') {
			...
			if ($driver = $this->fetchSingleTest('select * from drivers where id = %d',$dest)) {
				//
				...
				//
				$curr = $this->fetchSingleTest("select * from custom_delivery where id = %d", $src);
				...
				$o_id = $this->fetchScalarTest("select order_id from custom_delivery where id = %d",$src);
				$c = new Common();
			...
			$src = $this->fetchSingleTest('select * from %s where id = %d',$this->m_content,$src);
    ...
    function resequence($driver, $dt) {
		...
		$articles = $this->fetchAllTest('select cd.* from %s cd, orders o where driver_id = %d and date(scheduled_date) <= "%s" and completed = 0 and o.id = cd.order_id and o.order_status & %d = %d  order by driver_sequence',$this->m_content,$driver, max(date("Y-m-d"),$dt), STATUS_PROCESSING, STATUS_PROCESSING);
		$seq = 10;
    ...
    function getDrivers() {
		...
		$drivers = $this->fetchAllTest("select d.*, m.company, m.firstname, m.lastname from members m, drivers d where d.enabled = 1 and d.deleted = 0 and m.id = d.member_id order by m.company, lastname, firstname");
		$result = array();
    ...
    function getStatus() {
		...
		$packages = $this->fetchAllTest("select count(cd.id) as cnt, service_type, completed as delivered from custom_delivery cd left join drivers d on d.id = cd.driver_id where date(scheduled_date) <= '%s' and actual_date = '0000-00-00 00:00:00' and third_party=0 group by service_type, delivered",date("Y-m-d"));
		//$packages = $this->fetchAllTest("select count(cd.id) as cnt, service_type, completed as delivered from custom_delivery cd where date(scheduled_date) <= '%s' and actual_date = '0000-00-00 00:00:00' group by service_type, delivered",date("Y-m-d"));
		$result = array();
		...
		$packages = $this->fetchAllTest("select count(cd.id) as cnt, service_type, IF(driver_id=0,0,1) as assigned from custom_delivery cd left join drivers d on d.id = cd.driver_id where date(scheduled_date) <= '%s' and actual_date = '0000-00-00 00:00:00' and third_party = 0 group by service_type, assigned",date("Y-m-d"));
		//$packages = $this->fetchAllTest("select count(cd.id) as cnt, service_type, IF(driver_id=0,0,1) as assigned from custom_delivery cd where date(scheduled_date) <= '%s' and actual_date = '0000-00-00 00:00:00' group by service_type, assigned",date("Y-m-d"));
		$result = array();
    ...
    function getDriver() {
			...
			if ($resort) {
				if ($outer->getData("resortTo") < 0) {
					...
					$this->execute(sprintf("update custom_delivery set driver_sequence = %d+1 where id = %d", 
						$this->fetchScalarTest("select driver_sequence from custom_delivery where id = %d", $outer->getData("resortTo")),
						$outer->getData("resortId")));
				}
				$recs = $this->fetchAllTest("select * from custom_delivery where driver_id = %d and scheduled_date <= '%s' and completed = 0 order by driver_sequence",
					$outer->getData("driver_id"), $outer->getData("scheduled_date"));				
				$recs = $this->fetchAllTest(sprintf("select c.id from custom_delivery c, orders o 
		...
		$packages = $this->fetchAllTest(sprintf("select c.*, curdate() as td, date(scheduled_date) as check_date, a.line1, a.line2, a.city, a.postalcode, a.phone1, a.phone2, p1.code deliveryCode, p1.name as deliveryName from custom_delivery c, addresses a, product p1, order_lines ol, orders o where %s and a.ownertype='order' and a.ownerid = c.order_id and a.addresstype = IF(c.service_type='P',%d,%d) and p1.id = ol.product_id and ol.custom_package = 'S' and ol.order_id = c.order_id and o.id = ol.order_id and order_status & %d = %d order by %s %s",$where,ADDRESS_PICKUP,ADDRESS_DELIVERY, STATUS_PROCESSING, STATUS_PROCESSING, $_POST["sortby"], $_POST["sortorder"]));

		$d_id = $_POST["driver_id"];		
		$driver = $this->fetchSingleTest(sprintf("select d.id as driver_id, m.*, a.line1, a.line2, a.city, a.phone1, a.phone2, a.fax, a.email from drivers d, members m left join addresses a on a.ownertype = 'member' and a.ownerid = m.id where d.id = %d and m.id = d.member_id limit 1",$d_id));
		...
		$rt = $this->fetchSingleTest("select * from custom_delivery_route where driver_id = %d and delivery_date = '%s'",$rt_d_id,$rt_dt);
    ...
    function mapIt() {
		...
		$driver = $this->fetchSingleTest("select m.*, a.* from drivers d, members m left join addresses a on a.ownertype = 'member' and a.ownerid = m.id where d.id = %d and m.id = d.member_id limit 1",$d_id);
		$outer = new Forms();
		$outer->init($this->getTemplate("mapIt"));
		$outer->addData($driver);
		$outer->validate();
		$flds = $this->getFields("mapIt");
		if ($route = $this->fetchSingleTest("select * from custom_delivery_route where driver_id = %d and delivery_date = '%s'", $d_id, $dt)) {
			...
			foreach($addr as  $key=>$value) {
				if ($value > 0) {					
					if ($address = $this->fetchSingleTest("select a.*, DATEDIFF(cd.scheduled_date,'%s') as daysToDeliver, p.custom_same_day from addresses a, custom_delivery cd, product p, order_lines ol where cd.id = %d and a.ownerid = cd.order_id and a.ownertype='order' and addresstype=if(cd.service_type='P',%d,%d) and ol.order_id = a.ownerid and ol.custom_package='S' and p.id = ol.product_id",$dt, $value, ADDRESS_PICKUP, ADDRESS_DELIVERY)) {
						...
						$o_id = $this->fetchScalarTest("select order_id from custom_delivery where id = %d",$value);
    ...
    function editPackage() {
		$p_id = array_key_exists("p_id",$_REQUEST) ? $_REQUEST["p_id"] : 0;
		$package = $this->fetchSingleTest("select cd.*, o.custom_signature_required from custom_delivery cd, orders o where cd.id = %d and o.id = cd.order_id",$p_id);
		$package["address"] = $this->fetchSingleTest("select * from addresses where ownerid = %d and ownertype = 'order' and addresstype=%d",$package["order_id"],$package["service_type"] == "P" ? ADDRESS_PICKUP : ADDRESS_DELIVERY);
		if ($package["service_type"] == "P") {
			$tmp = Address::formatData($this->fetchSingleTest("select * from addresses where ownerid = %d and ownertype = 'order' and addresstype=%d",$package["order_id"], ADDRESS_DELIVERY));
			$package["delivery"] = array();
			foreach($tmp as $key=>$value) {
				$package["delivery"]["d_".$key] = $value;
			}
		}
		$package["mgmt"] = $this->fetchSingleTest("select * from members m, orders o where o.id = %d and m.id = o.login_id", $package["order_id"] );
		$package["account"] = $this->fetchSingleTest("select * from members m, orders o where o.id = %d and m.id = o.member_id", $package["order_id"] );
		$package["order"] = $this->fetchSingleTest("select * from orders where id = %d", $package["order_id"] );
		$outer = new Forms();
    ...
    function assignSameday() {
		...
		$curr = $this->fetchSingleTest("select * from custom_delivery where order_id = %d and service_type='P'", $o_id);
    ...
    function optimizeGoogle($request = null) {
		...
		$recs = $this->fetchAllTest("select * from custom_delivery c where %s order by driver_sequence", implode(" and ",$srch));
		if (count($recs) > 23) {
		...
		foreach($recs as $key=>$wp) {
			$address = $this->fetchSingleTest("select * from addresses where ownerid = %d and ownertype='order' and addresstype=%d",$wp["order_id"],$wp["service_type"]=="P"?ADDRESS_PICKUP:ADDRESS_DELIVERY);
    ...
    function calendar() {
		...
		while ($sd < $ed) {
			...
			if ($events = $this->fetchSingleTest($sql))
    ...
    function editDate() {
		...
		if (!($rec = $this->fetchSingleTest("select * from office_schedule where id = %d",$id))) {
...
*/
    ...
    function optimize($request = null) {
		...
		$recs = $this->fetchAllTest(sprintf("select c.* from custom_delivery c, orders o where %s order by driver_sequence", implode(" and ",$srch)));
		$routes = array();
		$inner = new Forms();
		...
		$errors = array();
		foreach($recs as $key=>$wp) {			
			$address = $this->fetchSingleTest(sprintf("select * from addresses where ownerid = %d and ownertype='order' and addresstype=%d",$wp["order_id"],$wp["service_type"]=="P"?ADDRESS_PICKUP:ADDRESS_DELIVERY));
    ...
    function editAck() {
		...
		if ($data = $this->fetchSingleTest("select cda.*, cd.order_id, cd.driver_id from custom_delivery_acknowledgement cda, custom_delivery cd where cda.id = %d and cd.id = cda.delivery_id", $a_id)) {
			$pu = $this->fetchSingleTest("select * from addresses where ownerid = %d and ownertype = 'order' and addresstype=12", $data["order_id"]);
			$data["pickup"] = Address::formatData($pu);
			$del= $this->fetchSingleTest("select * from addresses where ownerid = %d and ownertype = 'order' and addresstype=13", $data["order_id"]);
			$data["delivery"] = Address::formatData($del);
			$driver = $this->fetchSingleTest("select * from members m, drivers d where d.id = %d and m.id = d.member_id", $data["driver_id"]);
			$data["driver"] = $driver;
			$outer->addData($data);
		}
		if (array_key_exists(__FUNCTION__,$_POST)) {
    ...
    function getMessages($p_id) {
		...
		$msgs = $this->fetchAllTest("select cda.*, m.company, m.firstname, m.lastname, u.fname, u.lname from custom_delivery_acknowledgement cda left join members m on m.id = cda.author_id left join users u on u.id = cda.author_id where delivery_id = %d order by created desc", $p_id);
		$recs = array();
		foreach($msgs as $k=>$v) {
			$inner->addData($v);
			$recs[] = $inner->show();
...
where o.deleted = 0 and p.id = ol.product_id and ol.order_id = cd.order_id and ol.custom_package = 'S' and o.id = ol.order_id and %s order by driver_id, order_id", $where );
			...
			foreach($records as $key=>$value) {
				...
				$value["ct"] = $ct;
				$value["address"] = Address::formatData($this->fetchSingleTest("select * from addresses where ownerid = %d and addresstype=%d and ownertype='order'",$value["order_id"], ADDRESS_DELIVERY ));
				$value["product"] = $this->fetchSingleTest("select p.* from product p, order_lines ol where ol.order_id = %d and p.id = ol.product_id and ol.custom_package='S'", $value["order_id"]);
				$tmp = $this->fetchAllTest("select p.name, sum(od.quantity) as qty, sum(od.weight) as wt, cd.code as UOM from product p, order_lines ol, order_lines_dimensions od, orders o, code_lookups cd where ol.order_id = %d and p.id = ol.product_id and ol.custom_package='P' and od.order_id = ol.order_id and od.line_id = ol.line_id and o.id = ol.order_id and cd.id = o.custom_weight_code group by p.id", $value["order_id"]);
				$pkgs = array();
				...
				$tmp = $this->fetchAllTest("select p.* from product p, order_lines ol where ol.order_id = %d and p.id = ol.product_id and ol.custom_package='A' and p.custom_special_requirement = 1", $value["order_id"]);
				$pkgs = array();
				foreach($tmp as $sk=>$sv) {
					$pkgs[] = $sv["name"];
				}
