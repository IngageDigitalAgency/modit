		...
		if (count($_POST) > 0 && array_key_exists('showSearchForm',$_POST)) {
			...
			if ($form->validate()) {
				...
				if (count($srch) > 0) {
					...
					$form->setData('pager',$perPage);
					$count = $this->fetchScalarTest('select count(n.id) from %s n left join drivers d on n.driver_id = d.id, order_lines ol, product p, orders o, addresses a where %s', $this->m_content, implode(' and ',array_values($srch)));
    ...
    function getDriver() {
		...
		if (array_key_exists("resort",$_POST) && $_POST["resort"] == 1) {
			...
			if ($resort) {
				...
				$recs = $this->fetchAllTest("select c.id from custom_delivery c, orders o 
