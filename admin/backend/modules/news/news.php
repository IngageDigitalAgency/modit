...
class news extends Backend {
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from '.$this->m_content.' n where n.id in (select f.article_id from '.$this->m_junction.' f where f.folder_id = %d) and n.deleted = 0',  $_REQUEST['p_id']);
    ...
    function addContent($fromMain = false) {
		...
		$frmFields['destEvents']['options'] = $this->fetchOptionsTest(sprintf('select id as code,name as value from events where id in (select event_id from event_dates where event_date >= curdate()) and deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by start_date',implode(',',array_merge(array(0),$data['destEvents']))));
		...
		$frmFields['relatedDestGallery']['options'] = $this->fetchOptionsTest(sprintf('select id as code, title as value from gallery_images where enabled = 1 and published = 1 and id in (%s) order by code',implode(',',array_merge(array(0),$data['relatedDestGallery']))));
