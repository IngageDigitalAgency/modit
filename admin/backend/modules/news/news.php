		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($valid) {
				...
				if ($status = $stmt->execute()) {
					...
					$sql = sprintf('select id from %s where id in (%s) and id not in (select folder_id from %s where article_id = %d and folder_id in (%s))',
					...
					$sql = sprintf('select id from %s where id in (%s) and id not in (select folder_id from news_by_search_group where article_id = %d and folder_id in (%s))',
