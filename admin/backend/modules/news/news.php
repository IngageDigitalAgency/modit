...
class news extends Backend {
    ...
    function showPageProperties($fromMain = false) {
		...
		$frmFlds['destCoupons']['options'] = $this->fetchOptionsTest($sql);
		...
		$frmFlds['destEvents']['options'] = $this->fetchOptionsTest('select id as code,name as value from events where id in (%s) and id in (select event_id from event_dates where event_date >= curdate()) and deleted = 0 and enabled = 1 and published = 1 order by start_date',implode(',',array_merge(array(0),$data['destEvents'])));
		...
		$frmFlds['relatedDestProducts']['options'] = $this->fetchOptionsTest('select id as code,code as value from product where deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by code',implode(',',array_merge(array(0),$data['relatedDestProducts'])));
		...
		$frmFlds['relatedDestGallery']['options'] = $this->fetchOptionsTest('select id as code, title as value from gallery_images where enabled = 1 and published = 1 and id in (%s) order by code',implode(',',array_merge(array(0),$data['relatedDestGallery'])));
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from %s n where n.id in (select f.article_id from %s f where f.folder_id = %d) and n.deleted = 0', $this->m_content, $this->m_junction, $_REQUEST['p_id']);
			...
			//
			$coupons = $this->fetchAllTest('select * from coupons c, relations r where c.deleted = 0 and r.owner_id = %d and r.owner_type = "newsfolder" and c.id = r.related_id and r.related_type="coupon"',$data['id']);
			$output = array();
			...
			$coupons = $this->fetchAllTest('select * from coupon_folders c, relations r where r.owner_id = %d and r.owner_type = "newsfolder" and c.id = r.related_id and r.related_type="couponfolder"',$data['id']);
			$output = array();
			foreach($coupons as $coupon) {
				$frm = new Forms();
			...
			//			
			$events = $this->fetchAllTest('select * from events c, relations r where c.deleted = 0 and r.owner_id = %d and r.owner_type = "newsfolder" and c.id = r.related_id and r.related_type="event" and c.id in (select event_id from event_dates where event_date >= curdate())',$data['id']);
			$output = array();
			...
			$events = $this->fetchAllTest('select * from members_folders c, relations r where r.owner_id = %d and r.owner_type = "newsfolder" and c.id = r.related_id and r.related_type="eventfolder"',$data['id']);
			$output = array();
			foreach($events as $event) {
				$frm = new Forms();
    ...
    function addContent($fromMain = false) {
		...
		if (count($_REQUEST) > 0 && array_key_exists('destFolders',$_REQUEST) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest('select folder_id from %s where article_id = %d', $this->m_junction, $data['id']);
				$ids = array_merge($ids,$tmp);
				$tmp = $this->fetchScalarAllTest('select folder_id from news_by_search_group where article_id = %d', $data['id']);
		...
		$frmFields['destEvents']['options'] = $this->fetchOptionsTest('select id as code,name as value from events where id in (select event_id from event_dates where event_date >= curdate()) and deleted = 0 and enabled = 1 and published = 1 and id in (%s) order by start_date',implode(',',array_merge(array(0),$data['destEvents'])));
		...
		$frmFields['relatedDestGallery']['options'] = $this->fetchOptionsTest('select id as code, title as value from gallery_images where enabled = 1 and published = 1 and id in (%s) order by code',implode(',',array_merge(array(0),$data['relatedDestGallery'])));
		...
		if ((count($_REQUEST) > 0 && array_key_exists('news_type',$_REQUEST)) || $data['id'] > 0) {
			...
			if (array_key_exists('news_type',$_REQUEST)) {
				...
				$tmp = $this->fetchScalarAllTest('select news_type from news_types where news_id = %d', $data['id']);
				$ids = array_merge($ids,$tmp);
			}
			$data['news_type'] = $ids;
		}
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($valid) {
				...
				if ($status = $stmt->execute()) {
					...
					$new = $this->fetchScalarAllTest('select id from %s where id in (%s) and id not in (select folder_id from %s where article_id = %d and folder_id in (%s))',
					...
					$new = $this->fetchScalarAllTest('select id from %s where id in (%s) and id not in (select folder_id from news_by_search_group where article_id = %d and folder_id in (%s))',
					...
					if (array_key_exists('news_type',$data) && count($data['news_type']) > 0) {
						...
						$currIds = $this->fetchScalarAllTest($sql);
		...
		if ($_REQUEST['type'] == 'tree') {
			...
			if ($curr > 0 && $folder = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$dest)) {
				...
				if (array_key_exists('move',$_REQUEST)) {
					...
					//
					$curr = $this->fetchScalarTest('select article_id from %s where id = %d',$this->m_junction,$src);
					...
					if ($status = $this->execute(sprintf('delete from %s where id = %d', $this->m_junction, $src))) {
						if (!$this->fetchSingleTest('select * from %s where article_id = %d and folder_id = %d',$this->m_junction,$curr,$dest)) {
							$obj = new preparedStatement(sprintf('insert into %s(article_id,folder_id) values(?,?)',$this->m_junction));
			...
			if ($form->validate()) {
				...
				switch($type) {
					case 'cancel':
						...
						break;
					case 'all':
						//$img = $this->fetchScalarTest('select article_id from %s where id = %d',$this->m_junction,$_REQUEST['j_id']);
					...
					case 'one':
						...
						$ct = $this->fetchScalarTest('select count(0) from %s where article_id = %d',$this->m_junction,$_REQUEST['a_id']);
						if ($ct == 0)
							$this->execute(sprintf('update %s set deleted = 1 where id = %d',$this->m_content,$_REQUEST['a_id']));
						break;
					default:
    ...
    function moduleStatus($fromMain = 0) {
		if (array_key_exists('formData',$_SESSION) && array_key_exists('newsSearchForm', $_SESSION['formData'])) {
			...
			$ct = $this->fetchScalarTest('select count(0) from %s where published = 0',$this->m_content);
			if ($ct == 0) {
    ...
    function getVideo($b_id = 0, $fromPost) {
		...
		if ($fromPost)
			...
			if (!$data = $this->fetchSingleTest('select * from videos where owner_id = %d and owner_type = "news"', $b_id))
				$data = array('id'=>0);
    ...
    function saveVideo($id) {
		...
		if ($data = $this->fetchSingleTest('select * from videos where owner_type="news" and owner_id = %d', $form->getData('v_id'))) {
