...
/**
 * Manages users for backend section
 */
class users extends Backend {
    ...
    function showPageContent($fromMain = false) {
		...
		if ($p_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$p_id)) {
			...
			$count = $this->fetchScalarTest('select count(n.id) from '.$this->m_content.' n where n.id in (select f.user_id from '.$this->m_junction.' f where f.role_id = %d)', $_REQUEST['p_id']);
    ...
    function addContent($fromMain = false) {
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($id == 0) {
				...
				//
				$ct = $this->fetchScalarTest('select count(0) from users where email = "%s" and deleted = 0',$_POST['email']);
				if ($ct > 0) {
					$status = false;
			...
			if ($status) {
				...
				if ($status = $status && $stmt->execute()) {
					...
					if (array_key_exists('enableAllModules',$_POST) && $_POST['enableAllModules'] == 1) {
						...
						$modules = $this->fetchAllTest('select * from modules where backend = 1 and enabled = 1 and admin = 0');
						...
						if (array_key_exists('ids',$roles)) {
							foreach($roles['ids'] as $key=>$value) {
								if ($value == 0) {
									if ($this->fetchSingleTest('select * from users_by_module where user_id = %d and module_id = %d',$id,$roles['module_id'][$key])) {
										$status = false;
    ...
    function deleteArticle() {
		if (array_key_exists('j_id',$_REQUEST)) {
			...
			if ($level >= 4) {
                $this->noAccessError();
                return $this->ajaxReturn(array('status'=>'false','messages'=> 'You do not have access to this function'));
            }
			$id = $_REQUEST['j_id'];
     ...
     */
    function edit() {
		if (array_key_exists('u_id',$_REQUEST) && $data = $this->fetchSingleTest('select * from %s where id = %d',$this->m_content,$_REQUEST['u_id'])) {
			$f = new Forms();
    ...
    function showRole($r_id = null) {
		if (is_null($r_id))
			...
			$tmp = $r_id;
		if (!($data = $this->fetchSingleTest('select *, id as deleteRole from users_by_module where id = %d',$tmp)))
