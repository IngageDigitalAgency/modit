...
class polls extends Backend {
    ...
    function getFolderInfo($fromMain = false) {
		if (array_key_exists('f_id',$_REQUEST)) {
			if ($data = $this->fetchSingleTest('select * from %s where id = %d', $this->m_tree, $_REQUEST['f_id'])) {
				$form = new Forms();
    ...
    function showPageContent($fromMain = false) {
		...
		$form = new Forms();
		if ($f_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d', $this->m_tree, $f_id)) {
			...
			if ($this->m_perrow > 0) {
				...
				$count = $this->fetchScalarTest('select count(n.id) from '.$this->m_content.' n where n.id in (select f.poll_id from '.$this->m_junction.' f where f.folder_id = %d)',  $f_id);
			...
			foreach($polls as $poll) {
				...
				$stats = $this->fetchAllTest('select completed, count(completed) as ct from %s where poll_id = %d group by completed',$this->m_responses,$poll['id']);
			...
			if ($form->validate()) {
				...
				if (count($srch) > 0) {
					if (array_key_exists('pagenum',$_REQUEST))
						...
						$pageNum = 1;	// no 0 based calcs
					$perPage = $this->m_perrow;
					$sql = sprintf('select count(n.id) from '.$this->m_content.' n where 1=1 and %s', implode(' and ',$srch));
    ...
    function addContent($fromMain = false) {
		...
		if (count($_REQUEST) > 0 && array_key_exists('destFolders',$_REQUEST) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest('select folder_id from %s where poll_id = %d',$this->m_junction,$data['id']);
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($form->validate()) {
				...
				if ($stmt->execute()) {
					...
					//	insert new folders
					//
					$sql = sprintf('select id from %s where id in (%s) and id not in (select folder_id from %s where poll_id = %d and folder_id in (%s))', $this->m_tree, implode(',',$destFolders),$this->m_junction,$id,implode(',',$destFolders));
    ...
    function editQuestion() {
		...
		if (!(array_key_exists('q_id',$_REQUEST) && $_REQUEST['q_id'] > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d and poll_id = %d', $this->m_details, $_REQUEST['q_id'], $_REQUEST['p_id']))) {
		...
		if (count($_POST) > 0 && array_key_exists('editQuestion',$_POST)) {
			...
			if ($form->validate()) {
				...
				if ($id > 0) {
					...
					if (($ct = $this->fetchScalarTest('select count(0) from %s where poll_id = %d and question_id = %d',$this->m_details,$form->getData('poll_id'),$form->getData('question_id'))) > 0) {
    ...
    function moduleStatus($fromMain = 0) {
		$ct = $this->fetchScalarTest('select count(0) from %s where enabled = 0 or published = 0 order by title',$this->m_content);
		if ($ct > 0) {
    ...
    function getQuestions($p_parm = null) {
		...
		$q = $this->fetchAllTest('select pq.*,q.name from %s pq, questions q where pq.poll_id = %d and q.id = pq.question_id and pq.deleted = 0 order by pq.sequence',$this->m_details,$p_id);
		$return = array();
		$form = new Forms();
    ...
    function deleteContent($fromMain = false) {
		...
		if (array_key_exists('p_id',$_REQUEST)) {
			$p_id = $_REQUEST['p_id'];
			$ct = $this->fetchScalarTest('select count(0) from %s where folder_id = %d',$this->m_junction,$p_id);
			if ($ct == 0) {
				$mptt = new mptt($this->m_tree);
    ...
    function moveArticle() {
		...
		if ($_REQUEST['type'] == 'tree') {
			...
			$curr = $src;	//$this->fetchScalarTest('select article_id from %s where id = %d',$this->m_junction,$src);
			if ($curr > 0 && $folder = $this->fetchSingleTest('select * from %s where id = %d',$this->m_tree,$dest)) {
				...
				if (array_key_exists('move',$_REQUEST)) {
					...
					//
					$curr = $this->fetchScalarTest('select poll_id from %s where id = %d',$this->m_junction,$src);
					...
					if ($status = $this->execute(sprintf('delete from %s where id = %d', $this->m_junction, $src))) {
						if (!$this->fetchSingleTest('select * from %s where poll_id = %d and folder_id = %d',$this->m_junction,$curr,$dest)) {
							$obj = new preparedStatement(sprintf('insert into %s(poll_id,folder_id) values(?,?)',$this->m_junction));
    ...
    function loadQuestions() {
		$poll = $_REQUEST['p_id'];
		$questions = $this->fetchAllTest('select * from questions q, question_by_folder f where f.folder_id = %d and q.id = f.question_id and q.id not in (select question_id from poll_questions where poll_id = %d and deleted = 0) and deleted = 0 order by name',$_REQUEST['f_id'],$poll);
		$outer = new Forms();
    ...
    function dropQuestion() {
		...
		$form = new Forms();
		$status = true;
		if (!$poll = $this->fetchSingleTest('select * from %s where id = %d',$this->m_content,$p_id)) {
			$status = false;
			$this->addError('Could not locate the selected poll');
		}
		else {
			if (!$question = $this->fetchSingleTest('select * from %s where id = %d',$this->m_questions,$q_id)) {
				$status = false;
				$this->addError('Could not locate the selected question');
			}
			else {
				if ($rec = $this->fetchSingleTest('select * from %s where question_id = %d and poll_id = %d',$this->m_details,$q_id,$p_id)) {
					if ($rec['deleted'] == 0) {
						$status = false;
    ...
    function resequenceQuestions($poll) {
		$curr = 10;
		$recs = $this->fetchAllTest('select * from %s where poll_id = %d order by sequence',$this->m_details,$poll);
    ...
    function resortQuestions() {
		...
		$src = $this->fetchSingleTest('select * from %s where id = %d',$this->m_details,$src);
    ...
    function removeQuestion() {
		...
		$form->init($this->getTemplate('editQuestionSuccess'));
		$form->addTag('poll_id',$p_id);
		if ($rec = $this->fetchSingleTest('select * from %s where id = %d and poll_id = %d',$this->m_details,$q_id,$p_id)) {
			if ($ct = $this->fetchScalarTest('select count(0) from %s where poll_id = %d',$this->m_responses,$p_id) > 0) {
				//
    ...
    function copyPoll() {
		...
		if (count($_POST) > 0 && array_key_exists('copyPoll',$_POST)) {
			$form->addData($_POST);
			if ($form->validate()) {
				$poll = $this->fetchSingleTest('select * from %s where id = %d',$this->m_content,$p_id);
				$this->beginTransaction();
				unset($poll['id']);
				...
				if ($valid) {
					$questions = $this->fetchAllTest('select * from %s where poll_id = %d and deleted = 0 order by sequence',$this->m_details,$p_id);
...
qa.id = pra.answer_id
group by qa.id',$q_id);
		$question = $this->fetchSingleTest('select * from questions q, poll_questions pq where pq.id = %d and q.id = pq.question_id',$q_id);
		$stats = $this->fetchAllTest($sql);
		$result = array();
		$ticks = array();
		$series = array();
		$total = 0;
...
pra.answer_id = 0 and
length(other) > 0',$q_id);
		if ($ct = $this->fetchScalarTest($sql) > 0) {
...
length(other) > 0
group by other',$q_id);
		if ($stats = $this->fetchAllTest($sql)) {
			foreach($stats as $key=>$stat) {
				$text = str_replace("\r\n"," ",$stat['other']);
				$text = str_replace("\n"," ",$text);
    ...
    function pollCSV() {
		$p_id = $_REQUEST['p_id'];
		ob_end_clean();
		$poll = $this->fetchSingleTest('select * from poll where id = %d',$p_id);
		header('Content-type: text/csv');
		header(sprintf('Content-Disposition: attachment; filename="%s.csv"',preg_replace('#[^a-z0-9_]#i', '-', strtolower($poll['title']))));
		echo 'question;response;count;other text'.PHP_EOL;
		$questions = $this->fetchAllTest('select q.*, pq.id as q_id from questions q, poll_questions pq 
...
order by pq.sequence',$p_id);
		...
		foreach($questions as $key=>$question) {
			...
			$rows[] = $inner->show();
			$total = $this->fetchScalarTest('SELECT count(0) as ct
FROM `poll_questions` pq, poll_responses pr, poll_response_answers pra, question_answers qa
WHERE pq.poll_id = %d and 
pq.question_id = %d and
pr.poll_id = pq.poll_id and 
...
pra.answer_id = 0
group by pq.poll_id',$p_id,$question['id']);
			$answers = $this->fetchAllTest('select qa.* from question_answers qa, poll_questions pq where pq.poll_id = %d and pq.question_id = %d and qa.question_id = pq.question_id and pq.deleted = 0 order by qa.sequence',$p_id,$question['id']);
			$p_total = 0;
			$q_total = 0;
	...
	group by qa.id',$p_id,$question['id'],$answer['id']);
				$stats = $this->fetchAllTest($sql);
				if (count($stats) == 0) $stats = $this->fetchAllTest('select text,0 as ct from question_answers qa where id = %d',$answer['id']);
				$tmp = 0;
				foreach($stats as $key=>$stat) {
					$inner->reset();
    ...
    function showOther() {
		$q_id = $_REQUEST['q_id'];
		$answers = $this->fetchAllTest('select * from poll_response_answers pa, poll_responses pr, poll_questions pq
    ...
    function addComparison() {
		...
		if ($poll = $this->fetchSingleTest('select * from %s where deleted = 0 and id = %d',$this->m_content,$p_id)) {
			$found = 0;
		...
		foreach($_SESSION['poll_comparator'] as $key=>$value) {
			$inner->reset();
			$rec = $this->fetchSingleTest('select * from %s where id = %d',$this->m_content,$value);
			$inner->addData($rec);
			$results[] = $inner->show();
		}
    ...
    function comparePolls() {
		...
		$polls = $this->fetchAllTest('select * from %s where deleted = 0 and id in (%s) order by instr("%s,",concat(id,","))',$this->m_content,implode(',',$poll_ids),implode(',',$poll_ids));
		$outer = new Forms();
		...
		$questions = $this->fetchAllTest('SELECT pq.question_id FROM %s pq WHERE pq.poll_id in (1,2,3) GROUP BY pq.question_id order by min(sequence)',$this->m_details,implode(',',$poll_ids));
		$tmp = new Forms();
		$tmp->init($this->getTemplate('comparePollsQuestion'));
		$rows = array();
		foreach($questions as $key=>$question) {
			$q = $this->fetchSingleTest('select * from %s where id = %d',$this->m_questions,$question['question_id']);
			$tmp->addData($q);
			$result = new Forms();
			$result->init($this->getTemplate('comparePollsQuestionResults'));
			$maxRows = 0;
			$answers = $this->fetchAllTest('select * from question_answers qa where question_id = %d and deleted = 0',$q['id']);
			$stats = array();
			$rows[] = $tmp->show();
			$statForm = new Forms();
			$totals = array();
...
group by pr.poll_id,pra.question_id', $poll_id, $question['question_id']);
    ...
    function responseDetails() {
		...
		$responses = $this->fetchAllTest($sql);
		$result = array();
    ...
    function responseDetailsView() {
		$r_id = $_REQUEST['r_id'];
		if (!$response = $this->fetchSingleTest('select * from %s where id = %d',$this->m_responses,$r_id)) {
			$this->addError("We couldn't locate that response");
			return $this->ajaxReturn(array('status'=>false));
		}
		$poll = $this->fetchSingleTest('select * from %s where id = %d',$this->m_content,$response['poll_id']);
		$outer = new Forms();
		$outer->init($this->getTemplate('viewResponse'));
		$questions = $this->fetchAllTest('select q.* from questions q, poll_questions pq where pq.poll_id = %d and q.id = pq.question_id and q.deleted = 0 order by pq.sequence',$response['poll_id']);
		$result = array();
		$q = new Forms();
		...
		$demo = array();
		if ($demographics = $this->fetchSingleTest('select * from addresses where ownertype="poll" and ownerid = %d',$r_id)) {
    ...
    function responseCSV($sql) {
		$poll = $this->fetchSingleTest('select * from %s where id = %d',$this->m_content,$_POST['p_id']);
		$records = $this->fetchAllTest($sql);
		$questions = $this->fetchAllTest('select q.* from poll_questions pq, questions q where q.id = pq.question_id and pq.poll_id = %d order by pq.sequence',$poll['id']);
		ob_end_clean();
		header('Content-type: text/csv');
		...
		foreach($records as $key=>$response) {
			...
			if (!$addr = $this->fetchSingleTest('select * from addresses where ownertype="poll" and ownerid = %d',$response['id']))
				$addr = array();
			foreach($demo as $key=>$field) {
				$tmp[] = array_key_exists($demoFields[$field]['field'],$addr) ? $addr[$demoFields[$field]['field']] : '';
			}
			foreach($questions as $key=>$question) {
				$answers = $this->fetchAllTest('select pra.*, qa.text from poll_response_answers pra left join question_answers qa on qa.id = pra.answer_id where pra.response_id = %d and pra.question_id = %d',$response['id'],$question['id']);
				$a = array();
				foreach($answers as $key=>$answer) {
