...
class question extends Backend {
    ...
    function showPageContent($fromMain = false) {
		...
		if ($f_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d', $this->m_tree, $f_id)) {
			...
			if ($this->m_perrow > 0) {
				...
				$count = $this->fetchScalarTest('select count(n.id) from '.$this->m_content.' n where n.id in (select f.question_id from %s f where f.folder_id = %d)', $this->m_junction, $f_id);
			...
			foreach($images as $image) {
				...
				$frm->addData($image);
				$tmp = $this->fetchScalarTest('select count(0) from poll_questions where question_id = %d and deleted = 0',$image['id']);
				$frm->addTag('usage',$tmp);
				$tmp = $this->fetchScalarTest('select count(0) as ct from (select distinct response_id from %s where question_id = %d) as x',$this->m_responses,$image['id']);
    ...
    function addContent($fromMain = false) {
		...
		if (count($_REQUEST) > 0 && array_key_exists('destFolders',$_REQUEST) || $data['id'] > 0) {
			...
			if ($data['id'] > 0) {
				$tmp = $this->fetchScalarAllTest('select folder_id from %s where question_id = %d',$this->m_junction,$data['id']);
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($form->validate()) {
				...
				if ($stmt->execute()) {
					...
					//	insert new folders
					//
					$sql = sprintf('select id from %s where id in (%s) and id not in (select folder_id from %s where question_id = %d and folder_id in (%s))', $this->m_tree, implode(',',$destFolders),$this->m_junction,$id,implode(',',$destFolders));
    ...
    function deleteArticle() {
		...
		$used = $this->fetchScalarTest('select count(0) from poll_questions where question_id = %d and deleted = 0',$_REQUEST['a_id']);
		$deleted = $this->fetchScalarTest('select count(0) from poll_questions where question_id = %d and deleted = 1',$_REQUEST['a_id']);
		...
		if ($used > 0) {
			...
			$recs = $this->fetchAllTest('select p.* from poll p, poll_questions pq where pq.question_id = %d and pq.deleted = 0 and p.id = pq.poll_id',$_REQUEST['a_id']);
			...
			if ($form->validate()) {
				...
				switch($type) {
					...
					case 'one':
						...
						$ct = $this->fetchScalarTest('select count(0) from %s where question_id = %d',$this->m_junction,$_REQUEST['a_id']);
						if ($ct == 0) {
							if ($used > 0 || $deleted > 0)
								$this->execute(sprintf('update %s set deleted = 1 where id = %d',$this->m_content,$_REQUEST['a_id']));
							else
    ...
    function getResponses($id_parm = null) {
		...
		$answers = $this->fetchAllTest('select * from %s where question_id = %d order by sequence',$this->m_details,$id);
		$form = new Forms();
		...
		foreach($answers as $key=>$rec) {
			...
			$rec['image'] = $img->show();
			if ($ct = $this->fetchScalarTest('select count(0) from %s where answer_id = %d',$this->m_responses,$rec['id']) > 0) {
				$rec['answered'] = 'check';
				$rec['responses'] = $ct;
			}
			else {
    ...
    function editResponse() {
		...
		if (!(array_key_exists('r_id',$_REQUEST) && $_REQUEST['r_id'] > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d and question_id = %d', $this->m_details, $_REQUEST['r_id'], $_REQUEST['q_id']))) {
    ...
    function getUsage($id) {
		...
		$usage = $this->fetchAllTest('select * from poll where id in (select poll_id from poll_questions where question_id = %d) order by title',$id);
		$return = array();
    ...
    function removeResponse() {
		...
		if ($rec = $this->fetchSingleTest('select * from %s where id = %d and question_id = %d',$this->m_details,$r_id,$q_id)) {
			if ($ct = $this->fetchScalarTest('select count(0) from %s where answer_id = %d',$this->m_responses,$r_id) > 0) {
				//
    ...
    function resequenceResponses($question) {
		$curr = 10;
		$recs = $this->fetchAllTest('select * from %s where question_id = %d order by sequence',$this->m_details,$question);
