    ...
    function showPageContent($fromMain = false) {
		...
		if ($f_id > 0 && $data = $this->fetchSingleTest('select * from %s where id = %d', $this->m_tree, $f_id)) {
			...
			if ($this->m_perrow > 0) {
				...
				$count = $this->fetchScalarTest('select count(n.id) from %s n where n.id in (select f.question_id from %s f where f.folder_id = %d)', $this->m_content, $this->m_junction, $f_id);
		...
		if (count($_POST) > 0 && array_key_exists('addContent',$_POST)) {
			...
			if ($form->validate()) {
				...
				if ($stmt->execute()) {
					...
					//	insert new folders
					//
					$new = $this->fetchScalarAllTest('select id from %s where id in (%s) and id not in (select folder_id from %s where question_id = %d and folder_id in (%s))', $this->m_tree, implode(',',$destFolders),$this->m_junction,$id,implode(',',$destFolders));
					$status = true;
