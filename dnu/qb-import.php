	...
	if (!array_key_exists("ShipAddr",$v)) {
		...
		if (!(
			...
			$shipTo = array(
				...
				"province_id"=>$c->fetchScalar(sprintf("select id from provinces where province_code = '%s'", $prov)),
				"country_id"=>$c->fetchScalar(sprintf("select c.id from countries c, provinces p where p.province_code = '%s' and c.id = p.country_id", $prov)),
