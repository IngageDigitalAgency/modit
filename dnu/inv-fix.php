...
$o = $c->fetchAllTest("select o.id, o.total, o.taxes, sum(ol.taxes) as l_taxes, sum(ol.value+ol.taxes) as value, abs(o.total - sum(ol.value+ol.taxes)) as diff from orders o, order_lines ol where ol.order_id = o.id and ol.deleted = 0 group by o.id having abs(o.total - sum(ol.value+ol.taxes)) > 0 or abs(o.taxes - sum(ol.taxes)) > 0");
foreach($o as $k=>$v) {
