...
/**
 * Prepare custom mailer using PHPMailer
 */
...
$c->setDebug(2);
$recs = $c->fetchAllTest("select *,invoice_amount+tax_amount as invoice_total from qb_export where invoice_date='2018-07-20'");
foreach($recs as $k=>$v) {
	$member = $c->fetchSingleTest("select * from members where id = %d", $v["member_id"]);
	$c->m_module = array();
...
if (DEV==0)
	...
	$html = $c->fetchSingleTest($sql);
	$body->setHTML($html['html']);
	$v["user"] = $_SESSION["user"]["info"];
	$v["address"] = Address::formatData($c->fetchSingleTest("select * from addresses where ownertype='member' and ownerid = %d", $v["member_id"]));
	$body->addData($v);
...
if (DEV==0) {
	...
	$mailer->FromName = "ian@development.ingagedigital.com";
	...
	if (DEV==0) {
		...
		$mailer->addAddress("ian@development.ingagedigital.com","Ian MacArthur");
		//$mailer->addAddress("vpileggi@kjvcourier.com","Victor Pileggi");
	}
